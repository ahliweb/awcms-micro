 ...
 *    the enforced `NNN_awcms_micro_<area>_<desc>.sql` numbering).
 ...
 *
 * Unlike `docs/awcms-micro/github/` (which stamps a literal snapshot
 ...
 * (same convention `docs/awcms-micro/api-reference.md` documents), and
...
export const REPO_INVENTORY_PATH = "docs/awcms-micro/repo-inventory.md";
...
export const RLS_EXEMPT_TABLES: Readonly<Record<string, string>> = {
  ...
  // `idn_admin_regions` and `reference_data` modules. AWCMS-Micro ports
  // neither module (ADR-0025), so those entries are removed: an exemption for
  // a table no migration creates is not a harmless leftover — the generator
  // renders it as "(not found in sql/ — stale entry, review)" in the
  // committed inventory, which is noise that trains readers to ignore a
  // marker meant to catch a REAL missing-RLS mistake.
};
...
async function buildRawRepoInventoryMarkdown(
  rootDir = process.cwd(),
  // Defaults to the real, effective registry (`listModules()` — the reviewed
  // base registry; ADR-0036 removed the derived-application pathway).
  ...
  const lines: string[] = [];
  lines.push("# AWCMS-Micro Repository Inventory (generated)");
  lines.push("");
  lines.push(
  ...
  lines.push(
    "**Freshness.** This document has no embedded generation timestamp on " +
      ...
      "is tracked separately in [`docs/awcms-micro/github/`](github/README.md) " +
  ...
  lines.push("");
  lines.push(
    "Tracked separately at [`docs/awcms-micro/github/`](github/README.md) — " +

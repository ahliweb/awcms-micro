 ...
 * full-online R2-only media registry (`awcms_micro_news_media_objects`,
 * Issue #633, `docs/awcms-micro/news-portal/r2-backup-lifecycle.md`).
 *
 ...
 * native `Bun.S3Client`) and to `awcms_micro_news_media_objects` metadata —
 * it never reads/writes a local file, and never stores object bytes
 * anywhere (this table has no binary column and never will,
 * `sql/041_awcms_micro_news_media_object_registry_schema.sql`'s own header).
 *
...
async function main() {
  ...
  // Issue #683 (epic #679): `awcms_micro_worker` role — see migration 046's
  // grant addition (`awcms_micro_news_media_objects`).
    ...
    const result = await runJob(
      {
        ...
        description:
          "Reconciles awcms_micro_news_media_objects metadata against the real R2 bucket contents; cleans up expired pending uploads and grace-period-expired orphans in bounded batches.",
        ...
        handler: async (ctx) => {
          ...
          if (totals.orphanInDb > 0) {
            console.log(
              `news-media:reconcile — ${totals.orphanInDb} object(s) reported as orphan-in-DB (DB row expects an R2 object that is missing) — REPORT ONLY, no automatic remediation; see docs/awcms-micro/news-portal/r2-backup-lifecycle.md's operator SOP.`
            );
          }

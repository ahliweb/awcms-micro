 ...
 * `docs/awcms-micro/module-composition-inventory.json` (ADR-0036 removed the
 * derived-application pathway, so there is no application registry to compose).
 *
 ...
 * check`).
 *
 * Deliberately separate from `docs/awcms-micro/repo-inventory.md`
 * (Markdown, for humans, one section per repository-level concern):
 * this file is JSON, specifically scoped to the module registry, intended to
 * be diffed/parsed by CI or release tooling — not prose.
 *

 ...
 *  2. Conditional: if AWCMS_MICRO_SYNC_ENABLED === "true", then
 *     AWCMS_MICRO_SYNC_HMAC_SECRET must be set and not left at the
...
export function checkSyncConfig(
  ...
  const name = "AWCMS_MICRO_SYNC_HMAC_SECRET (conditional on sync enabled)";
  ...
  if (env.AWCMS_MICRO_SYNC_ENABLED !== "true") {
    return {
      ...
      status: "pass",
      detail:
        'AWCMS_MICRO_SYNC_ENABLED is not "true" — sync HMAC secret not required.'
    };
  }
  ...
  if (result.status === "fail") {
    return {
      ...
      status: "fail",
      detail:
        "AWCMS_MICRO_SYNC_ENABLED=true but AWCMS_MICRO_SYNC_HMAC_SECRET is unset or still the documented placeholder."
    };
  }
 ...
 * (`awcms_micro_auth_providers`, migration 036), not deployment-level env
 ...
 * awcms-micro-social-publishing/SKILL.md` §643 Keputusan kunci #3 — every

...
/**
 * Deteksi regresi penamaan identifier: `awcms-micro_x` / `AWCMS-Micro_X`
 * (yang benar `awcms_micro_x` / `AWCMS_MICRO_X`).
 * @param {string} file
 * @param {string[]} lines
 * @returns {Problem[]}
 */
export function checkNaming(file, lines) {
  ...
  lines.forEach((line, i) => {
    if (/awcms-micro_[a-z]/.test(line) || /AWCMS-Micro_[A-Z]/.test(line)) {
      problems.push({
        file,
        line: i + 1,
        message: "penamaan rusak (gunakan awcms_micro_ / AWCMS_MICRO_)"
      });
    }
  });
  return problems;

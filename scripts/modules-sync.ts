...
async function main() {
  ...
  // capability provider, an incompatible deployment-profile claim, or a
  // navigation path conflict. This explicit pre-check gives a fast, clean
  // CLI failure before a DB connection is even opened; `syncModuleDescriptors`
  // itself ALSO refuses to write on the same condition (defense in depth for
  // every other real call site — the live API endpoint and the four internal
  // "sync first" callers — see that function's own file header for the full
  // reasoning), so this is deliberately redundant with, not a replacement
  // for, that guard.
    ...
    const result = await runJob(
      {
        name: "modules:sync",
        description:
          "Syncs the trusted code module registry (listModules()) into awcms_micro_modules.",
        handler: async (ctx) => {

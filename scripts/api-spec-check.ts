...
export const ALLOWED_PUBLIC_OPERATIONS: readonly `${string} ${string}`[] = [
  ...
  // (ADR-0025), so the entry is removed: an unauthenticated-by-design allow-list
  // is the last place a stale entry should be tolerated, and this gate says so
  // itself by failing on any entry the spec no longer documents as public.
];
...
export function checkAsyncApi(spec: unknown, file: string): Problem[] {
  ...
  } else if (!channels["awcms-micro.sync.push.requested"]) {
    problems.push({
      file,
 ...
 * (`openapi/awcms-micro-public-api.openapi.yaml`) is now GENERATED from
 * source fragments (`openapi/awcms-micro-public-api.src.yaml` +
    ...
    return [
      {
        file,
        message:
          "Bundled OpenAPI file is stale relative to openapi/awcms-micro-public-api.src.yaml and openapi/modules/*.yaml — run `bun run openapi:bundle` and commit the result."
      }
    ];
  }

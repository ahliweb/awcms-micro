...
const MIGRATION_FILE_PATTERN = /^\d{3}_awcms_micro_[a-z0-9_]+\.sql$/;
  ...
  if (invalidFile) {
    throw new Error(
      `Invalid migration file name: ${invalidFile}. ` +
        "Use NNN_awcms_micro_<area>_<description>.sql."
    );
  }
...
async function runMigrations(
  ...
  try {
    const appliedRows = await sql<AppliedMigration[]>`
      ...
      FROM awcms_micro_schema_migrations
    ...
    for (const migration of migrations) {
      ...
      await sql.begin(async (tx) => {
        ...
        await tx`
          INSERT INTO awcms_micro_schema_migrations (migration_name, checksum)

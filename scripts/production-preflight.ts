 ...
 * `awcms-micro-production-preflight`), reworked by Issue #684 (epic #679,
 ...
 *     exists — see `docs/awcms-micro/production-preflight-runbook.md`).
...
async function checkDatabaseConnectivity(): Promise<StageResult> {
  ...
  try {
    ...
    const rows =
      await sql`SELECT to_regclass('public.awcms_micro_schema_migrations') AS reg`;
...
> {
  ...
  try {
    ...
    const appliedRows = tableCheck[0]?.reg
      ? ((await sql`
          SELECT migration_name, checksum FROM awcms_micro_schema_migrations
...
export function authorizeApply(
  ...
  if (!options.backupVerified) {
    return {
      ok: false,
      reason:
        "--apply-migrations requires --backup-verified (confirm a recent, restorable backup exists first — see docs/awcms-micro/production-preflight-runbook.md)."
    };
  }

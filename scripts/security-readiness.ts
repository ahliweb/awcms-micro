 ...
 * skill `awcms-micro-production-preflight`). Runs a fixed list of named
 ...
 *
 * - `awcms_micro_schema_migrations` (sql/001) — migration bookkeeping.
 * - `awcms_micro_modules` (sql/001) — global module registry, no `tenant_id`.
 * - `awcms_micro_tenants` (sql/002) — the tenant table itself; each row IS a
 *   tenant, it doesn't belong to one.
 * - `awcms_micro_permissions` (sql/005) — global `module.activity.action`
 *   permission catalog, shared by all tenants.
 * - `awcms_micro_setup_state` (sql/006) — global singleton setup lock that
 *   exists before any tenant does.
 * - `awcms_micro_module_dependencies`, `awcms_micro_module_navigation`,
 *   `awcms_micro_module_jobs`, `awcms_micro_module_health_checks` (sql/025) —
...
export async function checkRlsEnabled(): Promise<SecurityCheckResult> {
  ...
  try {
    ...
    >`
      ...
      FROM pg_class
      WHERE relname LIKE 'awcms_micro_%' AND relkind = 'r'
      ORDER BY relname
    `;
    ...
    if (rows.length === 0) {
      return {
        ...
        status: "fail",
        evidence:
          "No awcms_micro_% tables found in pg_class — has `bun run db:migrate` been run against this database?"
      };
    }
  ...
  try {
    ...
    if (role.rolsuper || role.rolbypassrls) {
      return {
        ...
        severity,
        status: "fail",
        evidence: `The app connects as "${role.rolname}" which is ${role.rolsuper ? "a SUPERUSER" : "BYPASSRLS"} — it bypasses RLS entirely, so tenant isolation is not enforced at the database. Connect as a least-privilege role (e.g. awcms_micro_app).`
      };
    }
 ...
 * `sql/045_awcms_micro_db_role_separation.sql`'s header, restricted to the
 ...
 * PRIVILEGES` convenience is meant to auto-grant `awcms_micro_app` on every
...
const ALLOWED_GLOBAL_TABLE_GRANTS: Record<string, Record<string, string[]>> = {
  awcms_micro_permissions: {
    awcms_micro_app: ["SELECT"],
    awcms_micro_setup: ["SELECT"]
  },
  awcms_micro_schema_migrations: {
    awcms_micro_app: ["SELECT"]
  },
  awcms_micro_setup_state: {
    // INSERT/UPDATE (not just SELECT) stay on awcms_micro_app: the setup
    ...
    awcms_micro_app: ["SELECT", "INSERT", "UPDATE"],
    awcms_micro_setup: ["SELECT", "INSERT", "UPDATE"]
  },
  awcms_micro_tenants: {
    // Same fallback reasoning as awcms_micro_setup_state above — INSERT is
    // kept alongside the pre-existing UPDATE (PATCH /api/v1/settings).
    awcms_micro_app: ["SELECT", "INSERT", "UPDATE"],
    awcms_micro_worker: ["SELECT"],
    ...
    awcms_micro_setup: ["INSERT", "SELECT"]
  },
  awcms_micro_modules: {
    awcms_micro_app: ["SELECT", "INSERT", "UPDATE", "DELETE"]
  },
  awcms_micro_module_dependencies: {
    awcms_micro_app: ["SELECT", "INSERT", "UPDATE", "DELETE"]
  },
  ...
  // default-privileges grant `awcms_micro_app` would otherwise inherit, so
  ...
  // Issue #750 (epic #738, ADR-0021) — full DML for awcms_micro_app, same
  ...
  // Zero access for awcms_micro_worker — no background/unattended job
 ...
 * assumption) for `awcms_micro_app`/`awcms_micro_worker`/`awcms_micro_setup` on
 * every `awcms_micro_%` table, keeps only the global (non-RLS) ones (see
...
export async function checkRuntimeRoleGlobalTableGrants(): Promise<SecurityCheckResult> {
  ...
  try {
    ...
    >`
      ...
      WHERE c.relname LIKE 'awcms_micro_%' AND c.relkind = 'r'
        AND a.rolname IN ('awcms_micro_app', 'awcms_micro_worker', 'awcms_micro_setup')
    ...
    if (unexpected.length > 0) {
      return {
        ...
        severity,
        status: "fail",
        evidence: `Unexpected grant(s) on global (non-RLS) table(s) — see sql/045_awcms_micro_db_role_separation.sql's header for the approved matrix: ${unexpected.join("; ")}.`
      };
    }
    ...
    return {
      ...
      severity,
      status: "pass",
      evidence: `Checked ${globalRows.length} grant(s) across ${RLS_FREE_TABLES.size} global table(s) for awcms_micro_app/awcms_micro_worker/awcms_micro_setup — all match the approved allowlist (sql/045_awcms_micro_db_role_separation.sql).`
    };
  } catch (error) {
    return {
      name,
...
export async function checkAuditLogTableReachable(): Promise<SecurityCheckResult> {
  ...
  try {
    ...
    const rows = await sql<{ to_regclass: string | null }[]>`
      SELECT to_regclass('awcms_micro_audit_events') AS to_regclass
    `;
    ...
    if (value) {
      return {
        ...
        severity,
        status: "pass",
        evidence: `to_regclass('awcms_micro_audit_events') = ${value}.`
      };
    }
    ...
    return {
      ...
      status: "fail",
      evidence:
        "to_regclass('awcms_micro_audit_events') returned null — the audit table does not exist."
    };
  } catch (error) {
    return {
      name,
...
export async function checkSoftDeletePermissionsSeededAndAudited(
  ...
  try {
    ...
    const rows = await sql<{ action: string }[]>`
      SELECT action FROM awcms_micro_permissions
    ...
    problems.push(
      `could not query awcms_micro_permissions: ${errorMessage(error)}`
    );
  }
...
export function checkSyncHmacSecretNotDefault(
  ...
  if (env.AWCMS_MICRO_SYNC_ENABLED !== "true") {
    return {
      name,
      severity: "info",
      status: "pass",
      evidence: `AWCMS_MICRO_SYNC_ENABLED is not "true" — sync is disabled by design, so its HMAC secret is not a live risk (not checked).`
    };
  }

  const secret = env.AWCMS_MICRO_SYNC_HMAC_SECRET;
  ...
  if (!secret || secret === SYNC_SECRET_PLACEHOLDER) {
    return {
      ...
      severity: "warning",
      status: "fail",
      evidence: `AWCMS_MICRO_SYNC_ENABLED=true but AWCMS_MICRO_SYNC_HMAC_SECRET is unset or still the documented placeholder ("${SYNC_SECRET_PLACEHOLDER}").`
    };
  }
  ...
  return {
    ...
    status: "pass",
    evidence:
      "AWCMS_MICRO_SYNC_ENABLED=true and AWCMS_MICRO_SYNC_HMAC_SECRET has been changed from its documented placeholder."
  };
}
 ...
 *
 * Iterates tenants from `awcms_micro_tenants` (RLS-free, migration 002,
 ...
 * window (which governs how long `awcms_micro_visitor_sessions.ip_address`
 ...
 * `docs/awcms-micro/18_configuration_env_reference.md`): no raw-user-agent
 ...
 * `awcms_micro_visitor_key` cookie used to have a hardcoded ~2-year
...
export async function checkErrorsDontLeakStackTraces(
  ...
  try {
    ...
    // of the required X-AWCMS-Micro-* auth headers, which should produce a
 ...
 * `docs/awcms-micro/news-portal/r2-security-checklist.md` §7 after #632-634
 * landed): `awcms_micro_news_media_objects` rows stuck in `pending_upload`
 ...
 * above — iterates `awcms_micro_tenants` and opens one short transaction per
 ...
 * `awcms_micro_social_accounts` rows for distinct `provider_key` values and
    ...
    for (const tenant of tenants) {
      ...
      try {
        await sql.begin(async (tx) => {
          ...
          const rows = (await tx`
            SELECT DISTINCT provider_key FROM awcms_micro_social_accounts
      ...
      try {
        await sql.begin(async (tx) => {
          ...
          const rows = (await tx`
            ...
            FROM awcms_micro_social_accounts
            WHERE connection_status = 'connected'
...
export const OUT_OF_SCOPE_ITEMS: OutOfScopeItem[] = [
  ...
  },
  {
    ...
    reason:
      "Requires a real backup/restore run against a provisioned environment using `deploy/backup/backup-postgres.sh` / `deploy/backup/restore-postgres.sh` (Issue 12.2; see skill `awcms-micro-production-preflight` §Backup & restore). Manual — run the scripts and verify a restored row count."
  },
  {

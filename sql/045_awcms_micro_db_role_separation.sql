...
-- blanket-DML `awcms_micro_app` role (migration 013) with least-privilege,
-- purpose-specific roles.
--
-- EVIDENCE (audit at repo revision 4b6ccfc): migration 013 grants `awcms_micro_app`
...
-- all: `awcms_micro_permissions`, `awcms_micro_schema_migrations`,
-- `awcms_micro_setup_state`, `awcms_micro_tenants`, `awcms_micro_modules`,
-- `awcms_micro_module_dependencies`, `awcms_micro_module_navigation`,
-- `awcms_micro_module_jobs`, `awcms_micro_module_health_checks`. For those,
...
-- 2. `awcms_micro_app` ("web runtime") — kept (renaming would break every
...
--    `awcms_micro_permissions`/`awcms_micro_schema_migrations` (never
...
--    `awcms_micro_tenants`/`awcms_micro_setup_state`, DELETE is revoked
...
--    to the `awcms_micro_app` connection when `SETUP_DATABASE_URL` isn't
--    set, so `POST /api/v1/setup/initialize` runs those exact statements
--    AS `awcms_micro_app` on any deployment that doesn't configure the
--    dedicated `awcms_micro_setup` role. Revoking them here would silently
...
--    path). The practical consequence: `awcms_micro_app`'s isolation from
--    `awcms_micro_tenants`/`awcms_micro_setup_state` is real only once an
...
--    kept on the module-registry tables (`awcms_micro_modules` + 3
...
-- 3. `awcms_micro_worker` ("background worker", NEW) — the 7 unattended
...
--    stays on `awcms_micro_app`, not this role — it is the same actor/
--    capability as the web endpoint, not an unattended background job.
-- 4. `awcms_micro_setup` ("bootstrap/setup", NEW) — the ONE-TIME
...
--    RLS-protected exactly as for `awcms_micro_app` (same policy, any role
...
--    of that lock, not instead of it: a stolen `awcms_micro_app` credential
...
-- `DATABASE_URL` (the `awcms_micro_app` connection) when
...
-- uneven across the 9 global tables: `awcms_micro_permissions`/
-- `awcms_micro_schema_migrations` are narrowed unconditionally (nothing
-- ever writes them at runtime, dedicated role or not), but
-- `awcms_micro_tenants`/`awcms_micro_setup_state` are only FULLY isolated
-- from `awcms_micro_app` once `SETUP_DATABASE_URL` is actually configured
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'awcms_micro_worker') THEN
    CREATE ROLE awcms_micro_worker NOLOGIN;
  END IF;
END
$$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'awcms_micro_setup') THEN
    CREATE ROLE awcms_micro_setup NOLOGIN;
  END IF;
END
$$;

-- Same fail-closed default tenant GUC as `awcms_micro_app` (migration 013) —
...
ALTER ROLE awcms_micro_worker SET app.current_tenant_id = '00000000-0000-0000-0000-000000000000';
ALTER ROLE awcms_micro_setup SET app.current_tenant_id = '00000000-0000-0000-0000-000000000000';

GRANT USAGE ON SCHEMA public TO awcms_micro_worker;
GRANT USAGE ON SCHEMA public TO awcms_micro_setup;

-- 2. Narrow `awcms_micro_app` on the 9 global (non-RLS) tables. Every
...
-- the awcms_micro_setup fallback path needs them (see header note #2).
REVOKE DELETE ON awcms_micro_setup_state FROM awcms_micro_app;
REVOKE DELETE ON awcms_micro_tenants FROM awcms_micro_app;
-- `awcms_micro_modules`/`_module_dependencies`/`_module_navigation`/
...
-- 3. `awcms_micro_worker` — exactly the tables each of the 7 unattended
-- scripts touches (verified per-script against the application code each
-- calls). No grant at all on any of the 9 global tables.
GRANT SELECT ON awcms_micro_tenants TO awcms_micro_worker;
GRANT SELECT, DELETE ON awcms_micro_visit_events TO awcms_micro_worker;
-- UPDATE is required, not just SELECT/DELETE: purgeVisitorAnalyticsData's
-- raw-detail-clear step (retention-purge.ts) does
-- `UPDATE awcms_micro_visitor_sessions SET ip_address = NULL, ... RETURNING id`
...
GRANT SELECT, UPDATE, DELETE ON awcms_micro_visitor_sessions TO awcms_micro_worker;
GRANT SELECT, INSERT, UPDATE, DELETE ON awcms_micro_visitor_daily_rollups TO awcms_micro_worker;
GRANT SELECT, INSERT, DELETE ON awcms_micro_audit_events TO awcms_micro_worker;
GRANT SELECT, UPDATE ON awcms_micro_object_sync_queue TO awcms_micro_worker;
GRANT SELECT, UPDATE ON awcms_micro_email_messages TO awcms_micro_worker;
GRANT SELECT, INSERT ON awcms_micro_email_delivery_attempts TO awcms_micro_worker;
GRANT SELECT ON awcms_micro_email_suppression_list TO awcms_micro_worker;
GRANT SELECT, UPDATE ON awcms_micro_blog_posts TO awcms_micro_worker;
-- UPDATE is required, not just SELECT/DELETE: expireOverdueFormDrafts
-- (form-draft-purge.ts, step 1) does
-- `UPDATE awcms_micro_form_drafts SET status = 'expired' ... RETURNING id`
-- before purgeExpiredFormDrafts' later DELETE step — same class of bug as
-- awcms_micro_visitor_sessions above, caught by the same PR #703 review pass.
GRANT SELECT, UPDATE, DELETE ON awcms_micro_form_drafts TO awcms_micro_worker;

-- 4. `awcms_micro_setup` — exactly what `bootstrapPlatformTenant` writes.
...
GRANT SELECT, INSERT, UPDATE ON awcms_micro_setup_state TO awcms_micro_setup;
GRANT SELECT, INSERT ON awcms_micro_tenants TO awcms_micro_setup;
GRANT SELECT ON awcms_micro_permissions TO awcms_micro_setup;
GRANT INSERT ON awcms_micro_tenant_settings TO awcms_micro_setup;
GRANT SELECT, INSERT ON awcms_micro_offices TO awcms_micro_setup;
GRANT SELECT, INSERT ON awcms_micro_profiles TO awcms_micro_setup;
GRANT SELECT, INSERT ON awcms_micro_identities TO awcms_micro_setup;
GRANT SELECT, INSERT ON awcms_micro_tenant_users TO awcms_micro_setup;
GRANT SELECT, INSERT ON awcms_micro_roles TO awcms_micro_setup;
GRANT INSERT ON awcms_micro_role_permissions TO awcms_micro_setup;
GRANT INSERT ON awcms_micro_access_assignments TO awcms_micro_setup;

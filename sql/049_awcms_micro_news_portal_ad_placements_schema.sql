...
-- principle (Keputusan kunci #8, `.claude/skills/awcms-micro-news-portal/
...
-- for the identical dilemma with `awcms_micro_blog_ads`'s `image_url`. Every
...
-- on `awcms_micro_news_media_objects` itself, migration 041) -- a deliberate
...
CREATE TABLE IF NOT EXISTS awcms_micro_news_portal_ad_placements (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  placement_key text NOT NULL,
  name text NOT NULL,
  media_object_id uuid NOT NULL REFERENCES awcms_micro_news_media_objects (id),
  link_url text,
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_news_portal_ad_placements_placement_key_check
  ...
  CONSTRAINT awcms_micro_news_portal_ad_placements_rotation_mode_check
    CHECK (rotation_mode IN ('latest', 'priority', 'random_safe', 'weighted')),
  CONSTRAINT awcms_micro_news_portal_ad_placements_priority_check
    CHECK (priority >= 0),
  CONSTRAINT awcms_micro_news_portal_ad_placements_schedule_check
...
CREATE INDEX IF NOT EXISTS awcms_micro_news_portal_ad_placements_tenant_key_idx
  ON awcms_micro_news_portal_ad_placements (tenant_id, placement_key)
  WHERE deleted_at IS NULL;

-- FK lookup index (doc 10 convention: every foreign key gets an index).
CREATE INDEX IF NOT EXISTS awcms_micro_news_portal_ad_placements_media_object_id_idx
  ON awcms_micro_news_portal_ad_placements (media_object_id);

ALTER TABLE awcms_micro_news_portal_ad_placements ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_news_portal_ad_placements FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_news_portal_ad_placements_tenant_isolation
  ON awcms_micro_news_portal_ad_placements

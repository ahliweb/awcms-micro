...
-- bootstrap gap flagged in `sql/031_awcms_micro_tenant_domain_schema.sql`'s
-- header comment (lines ~77-95): the public host resolver must discover
-- `tenant_id` from a hostname in `awcms_micro_tenant_domains` BEFORE any
...
-- least-privilege `awcms_micro_app` role always returns zero rows without
...
--     `awcms-micro` in `docker-compose.yml`), which is created by the
--     official postgres image as an actual Postgres SUPERUSER
--     (`SELECT rolsuper FROM pg_roles WHERE rolname = 'awcms-micro'` ->
...
--          only to `awcms_micro_app` — no other non-superuser role can even
--          invoke this bypass. `awcms_micro_app` still cannot query
--          `awcms_micro_tenant_domains` directly without `withTenant(...)`;
...
-- `awcms_micro_tenants` to confirm the tenant itself was `active`. That
...
-- `awcms_micro_tenants` into this same function and returning the tenant's
...
-- `awcms_micro_tenants` is already RLS-free by design (ADR-0003/migration
-- 013) and freely `SELECT`-able by `awcms_micro_app` with zero bypass
...
CREATE OR REPLACE FUNCTION awcms_micro_resolve_tenant_domain_lookup(
...
COMMENT ON FUNCTION awcms_micro_resolve_tenant_domain_lookup(text) IS
  'Issue #559: narrow SECURITY DEFINER bootstrap read for hostname -> tenant lookup before tenant context exists. Joins the (already RLS-free) awcms_micro_tenants row in the same call so the TypeScript resolver needs exactly one round trip regardless of outcome (avoids a timing side-channel between "unmapped host" and "mapped but inactive tenant"). Returns only tenant_id/domain_status/is_primary/route_mode/tenant_status/tenant_code/tenant_name/default_locale for non-deleted domain rows matching a normalized hostname. Never returns verification_token_hash, verification_record_value, or raw hostname. EXECUTE restricted to awcms_micro_app.';

...
-- #638's (not yet built) R2-only ad image validation — `awcms_micro_blog_ads`
...
-- `awcms_micro_blog_pages` was also considered and dropped — there is no
...
CREATE TABLE IF NOT EXISTS awcms_micro_news_portal_homepage_sections (
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_news_portal_homepage_sections_type_check
    CHECK (section_type IN (
      'headline', 'latest_posts', 'featured_posts', 'editor_picks',
      'category_grid', 'gallery_block'
    )),
  CONSTRAINT awcms_micro_news_portal_homepage_sections_schedule_check
    CHECK (starts_at IS NULL OR ends_at IS NULL OR ends_at > starts_at)
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_news_portal_homepage_sections_tenant_key_dedup
  ON awcms_micro_news_portal_homepage_sections (tenant_id, section_key)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS awcms_micro_news_portal_homepage_sections_tenant_order_idx
  ON awcms_micro_news_portal_homepage_sections (tenant_id, sort_order)
  WHERE deleted_at IS NULL;

ALTER TABLE awcms_micro_news_portal_homepage_sections ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_news_portal_homepage_sections FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_news_portal_homepage_sections_tenant_isolation
  ON awcms_micro_news_portal_homepage_sections
...
-- (`sql/030_awcms_micro_blog_content_presentation_permissions.sql`) — this

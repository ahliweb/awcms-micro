...
-- `awcms_micro_email_messages` row already IS one recipient's delivery unit
-- (one row = one send attempt to one address), the same "one row per
-- delivery unit" shape `awcms_micro_object_sync_queue` already uses. A bulk
...
-- least-privilege `awcms_micro_app`'s default DML grants already cover new
-- tables automatically).
--
-- Sensitive-data handling (doc 04 §Alur perlindungan data sensitif, skill
-- `awcms-micro-sensitive-data`) reused verbatim, not reinvented: recipient
...
-- `awcms_micro_audit_events`/`AUDIT_LOG_RETENTION_DAYS` (doc 18) — tracked
-- for Issue #499.
CREATE TABLE IF NOT EXISTS awcms_micro_email_templates (
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_email_templates_template_key_format_check
    CHECK (template_key ~ '^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)+$'),
  CONSTRAINT awcms_micro_email_templates_has_body_check
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_email_templates_tenant_key_idx
  ON awcms_micro_email_templates (tenant_id, template_key)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS awcms_micro_email_templates_tenant_idx
  ON awcms_micro_email_templates (tenant_id)
  WHERE deleted_at IS NULL;

ALTER TABLE awcms_micro_email_templates ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_email_templates FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_email_templates_tenant_isolation
  ON awcms_micro_email_templates
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

CREATE TABLE IF NOT EXISTS awcms_micro_email_messages (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  correlation_id text,
  ...
  updated_at timestamptz NOT NULL DEFAULT now(),
  sent_at timestamptz,
  CONSTRAINT awcms_micro_email_messages_category_format_check
    CHECK (category ~ '^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)+$'),
  CONSTRAINT awcms_micro_email_messages_priority_check
    CHECK (priority IN ('low', 'normal', 'high')),
  CONSTRAINT awcms_micro_email_messages_status_check
    CHECK (status IN (
...
-- awcms_micro_object_sync_queue_retry_idx, migration 009/017): one tenant at
...
CREATE INDEX IF NOT EXISTS awcms_micro_email_messages_dispatch_idx
  ON awcms_micro_email_messages (tenant_id, status, next_attempt_at);

-- Admin/diagnostics list view (Issue #499): filter by tenant, optionally by
-- category, newest first.
CREATE INDEX IF NOT EXISTS awcms_micro_email_messages_tenant_category_idx
  ON awcms_micro_email_messages (tenant_id, category, created_at DESC);
...
-- itself.
CREATE INDEX IF NOT EXISTS awcms_micro_email_messages_recipient_hash_idx
  ON awcms_micro_email_messages (tenant_id, to_address_hash);

ALTER TABLE awcms_micro_email_messages ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_email_messages FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_email_messages_tenant_isolation
  ON awcms_micro_email_messages
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

CREATE TABLE IF NOT EXISTS awcms_micro_email_delivery_attempts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  message_id uuid NOT NULL REFERENCES awcms_micro_email_messages (id),
  ...
  CONSTRAINT awcms_micro_email_delivery_attempts_outcome_check
    CHECK (outcome IN ('success', 'failure')),
  CONSTRAINT awcms_micro_email_delivery_attempts_attempt_no_check
    CHECK (attempt_no > 0),
  CONSTRAINT awcms_micro_email_delivery_attempts_unique_attempt
...
CREATE INDEX IF NOT EXISTS awcms_micro_email_delivery_attempts_tenant_idx
  ON awcms_micro_email_delivery_attempts (tenant_id, attempted_at DESC);

ALTER TABLE awcms_micro_email_delivery_attempts ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_email_delivery_attempts FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_email_delivery_attempts_tenant_isolation
  ON awcms_micro_email_delivery_attempts
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE TABLE IF NOT EXISTS awcms_micro_email_suppression_list (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  ...
  created_by uuid,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_email_suppression_list_reason_check
    CHECK (reason IN ('bounced', 'complained', 'manual', 'unsubscribed'))
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_email_suppression_list_tenant_hash_idx
  ON awcms_micro_email_suppression_list (tenant_id, recipient_hash);

ALTER TABLE awcms_micro_email_suppression_list ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_email_suppression_list FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_email_suppression_list_tenant_isolation
  ON awcms_micro_email_suppression_list
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

...
-- enqueues a row into `awcms_micro_email_messages` (`sql/020`'s own comment
-- already anticipated this: "the reset token itself is hashed at rest in
-- its own auth table (Issue #496), never persisted here").
--
-- `awcms_micro_password_reset_tokens` mirrors `awcms_micro_sessions`'s shape
...
-- `awcms_micro_sessions` itself predates 013 and was force'd retroactively
-- there).
CREATE TABLE IF NOT EXISTS awcms_micro_password_reset_tokens (
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_password_reset_tokens_hash_key
  ON awcms_micro_password_reset_tokens (token_hash);

-- Supersede-prior-tokens path at request time: WHERE tenant_id = ? AND
-- identity_id = ? AND used_at IS NULL.
CREATE INDEX IF NOT EXISTS awcms_micro_password_reset_tokens_identity_idx
  ON awcms_micro_password_reset_tokens (tenant_id, identity_id)
  WHERE used_at IS NULL;

ALTER TABLE awcms_micro_password_reset_tokens ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_password_reset_tokens FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_password_reset_tokens_tenant_isolation
  ON awcms_micro_password_reset_tokens

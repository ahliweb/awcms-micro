...
--    `awcms_micro_object_sync_queue_retry_idx (tenant_id, status,
...
--    `created_at` alone — the existing `awcms_micro_sync_conflicts_tenant_status_idx
...
CREATE INDEX IF NOT EXISTS awcms_micro_object_sync_queue_tenant_created_idx
  ON awcms_micro_object_sync_queue (tenant_id, created_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_object_sync_queue_tenant_status_created_idx
  ON awcms_micro_object_sync_queue (tenant_id, status, created_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_object_sync_queue_tenant_node_created_idx
  ON awcms_micro_object_sync_queue (tenant_id, node_id, created_at);

CREATE INDEX IF NOT EXISTS awcms_micro_sync_conflicts_tenant_created_idx
  ON awcms_micro_sync_conflicts (tenant_id, created_at DESC);

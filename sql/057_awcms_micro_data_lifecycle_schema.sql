...
--
-- 1. `awcms_micro_data_lifecycle_legal_holds` — the one genuine runtime/
...
-- 2. `awcms_micro_data_lifecycle_cursors` — bounded-job pause/resume state
--    per (tenant, descriptor, phase) — a genuinely mutable execution-state
--    fact, not a descriptor fact.
-- 3. `awcms_micro_data_lifecycle_archive_manifests` — archive artifact
...
-- 4. `awcms_micro_data_lifecycle_runs` — dry-run/archive/purge execution
...
CREATE TABLE IF NOT EXISTS awcms_micro_data_lifecycle_legal_holds (
  ...
  CONSTRAINT awcms_micro_data_lifecycle_legal_holds_status_check
    CHECK (status IN ('active', 'released')),
  CONSTRAINT awcms_micro_data_lifecycle_legal_holds_release_consistency_check
    CHECK (
  ...
  CONSTRAINT awcms_micro_data_lifecycle_legal_holds_ends_after_starts_check
    CHECK (ends_at IS NULL OR ends_at > starts_at),
  CONSTRAINT awcms_micro_data_lifecycle_legal_holds_descriptor_key_format_check
...
CREATE INDEX IF NOT EXISTS awcms_micro_data_lifecycle_legal_holds_active_idx
  ON awcms_micro_data_lifecycle_legal_holds (tenant_id, descriptor_key)
  WHERE status = 'active';

CREATE INDEX IF NOT EXISTS awcms_micro_data_lifecycle_legal_holds_tenant_status_idx
  ON awcms_micro_data_lifecycle_legal_holds (tenant_id, status, created_at DESC);

ALTER TABLE awcms_micro_data_lifecycle_legal_holds ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_data_lifecycle_legal_holds FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_data_lifecycle_legal_holds_tenant_isolation
  ON awcms_micro_data_lifecycle_legal_holds
...
CREATE TABLE IF NOT EXISTS awcms_micro_data_lifecycle_cursors (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_data_lifecycle_cursors_phase_check
    CHECK (phase IN ('archive', 'purge')),
  CONSTRAINT awcms_micro_data_lifecycle_cursors_status_check
    CHECK (status IN ('idle', 'in_progress', 'completed', 'error')),
  CONSTRAINT awcms_micro_data_lifecycle_cursors_descriptor_key_format_check
    CHECK (descriptor_key ~ '^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$')
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_data_lifecycle_cursors_identity_key
  ON awcms_micro_data_lifecycle_cursors (tenant_id, descriptor_key, phase);

ALTER TABLE awcms_micro_data_lifecycle_cursors ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_data_lifecycle_cursors FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_data_lifecycle_cursors_tenant_isolation
  ON awcms_micro_data_lifecycle_cursors
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
-- "reference, not secret" convention `awcms_micro_social_accounts.token_
-- reference` already established.
CREATE TABLE IF NOT EXISTS awcms_micro_data_lifecycle_archive_manifests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  descriptor_key text NOT NULL,
  ...
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_port_check
    CHECK (archive_port IN ('local_offline', 'external_object_storage')),
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_format_check
    CHECK (format IN ('jsonl', 'csv')),
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_status_check
    CHECK (status IN ('written', 'verified', 'restored', 'deleted')),
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_row_count_check
    CHECK (row_count >= 0),
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_checksum_algorithm_check
    CHECK (checksum_algorithm = 'sha256'),
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_checksum_hex_check
    CHECK (checksum_hex ~ '^[0-9a-f]{64}$'),
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_range_check
    CHECK (cursor_range_start IS NULL OR cursor_range_end IS NULL OR cursor_range_end >= cursor_range_start),
  CONSTRAINT awcms_micro_data_lifecycle_archive_manifests_descriptor_key_format_check
    CHECK (descriptor_key ~ '^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$')
);

-- Planner's "already archived through cursor X?" lookup — MAX(cursor_range_end)
-- per (tenant, descriptor) among non-deleted manifests.
CREATE INDEX IF NOT EXISTS awcms_micro_data_lifecycle_archive_manifests_coverage_idx
  ON awcms_micro_data_lifecycle_archive_manifests (tenant_id, descriptor_key, cursor_range_end DESC)
  WHERE status <> 'deleted';

CREATE INDEX IF NOT EXISTS awcms_micro_data_lifecycle_archive_manifests_status_idx
  ON awcms_micro_data_lifecycle_archive_manifests (tenant_id, status);

ALTER TABLE awcms_micro_data_lifecycle_archive_manifests ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_data_lifecycle_archive_manifests FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_data_lifecycle_archive_manifests_tenant_isolation
  ON awcms_micro_data_lifecycle_archive_manifests
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE TABLE IF NOT EXISTS awcms_micro_data_lifecycle_runs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  descriptor_key text NOT NULL,
  ...
  triggered_by uuid,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_data_lifecycle_runs_run_type_check
    CHECK (run_type IN ('dry_run', 'archive', 'purge')),
  CONSTRAINT awcms_micro_data_lifecycle_runs_status_check
    CHECK (status IN ('completed', 'partial', 'failed')),
  CONSTRAINT awcms_micro_data_lifecycle_runs_counts_check
    CHECK (
      eligible_count >= 0 AND held_count >= 0 AND archived_count >= 0 AND
      purgeable_count >= 0 AND purged_count >= 0 AND blocked_count >= 0 AND
      error_count >= 0
    ),
  CONSTRAINT awcms_micro_data_lifecycle_runs_finished_after_started_check
    CHECK (finished_at >= started_at),
  CONSTRAINT awcms_micro_data_lifecycle_runs_descriptor_key_format_check
    CHECK (descriptor_key ~ '^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$')
);

CREATE INDEX IF NOT EXISTS awcms_micro_data_lifecycle_runs_descriptor_idx
  ON awcms_micro_data_lifecycle_runs (tenant_id, descriptor_key, created_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_data_lifecycle_runs_type_idx
  ON awcms_micro_data_lifecycle_runs (tenant_id, run_type, created_at DESC);

ALTER TABLE awcms_micro_data_lifecycle_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_data_lifecycle_runs FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_data_lifecycle_runs_tenant_isolation
  ON awcms_micro_data_lifecycle_runs
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- `awcms_micro_worker` (Issue #683, migration 045) grants — exactly the
...
-- own per-script grant precedent. `awcms_micro_app` needs no explicit grant
...
GRANT SELECT ON awcms_micro_data_lifecycle_legal_holds TO awcms_micro_worker;
GRANT SELECT, INSERT, UPDATE ON awcms_micro_data_lifecycle_cursors TO awcms_micro_worker;
GRANT SELECT, INSERT, UPDATE ON awcms_micro_data_lifecycle_archive_manifests TO awcms_micro_worker;
GRANT SELECT, INSERT, DELETE ON awcms_micro_data_lifecycle_runs TO awcms_micro_worker;

...
-- `013_awcms_micro_enforce_rls_least_privilege.sql` (PR #777 review
...
--
-- No new `awcms_micro_app` grants needed: every table here is
...
-- `awcms_micro_worker` grants either — duplicate-candidate generation runs
-- on-demand inside an ordinary tenant-scoped request (`analyze` guard),
-- not as a scheduled worker job, so it never runs as `awcms_micro_worker`.
...
ALTER TABLE awcms_micro_profile_identifiers
...
ALTER TABLE awcms_micro_profile_identifiers
  ADD CONSTRAINT awcms_micro_profile_identifiers_provenance_check
    CHECK (provenance IN (
      'self_reported', 'verified_by_staff', 'imported', 'system_generated'
    ));

ALTER TABLE awcms_micro_profile_identifiers
  ADD CONSTRAINT awcms_micro_profile_identifiers_validity_window_check
...
ALTER TABLE awcms_micro_profile_channels
  ADD COLUMN IF NOT EXISTS valid_from timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS valid_until timestamptz,
  ADD COLUMN IF NOT EXISTS verified_at timestamptz,
  ADD COLUMN IF NOT EXISTS verified_by uuid;
...
ALTER TABLE awcms_micro_profile_channels
  ADD CONSTRAINT awcms_micro_profile_channels_validity_window_check
    CHECK (valid_until IS NULL OR valid_until > valid_from);
...
ALTER TABLE awcms_micro_profile_addresses
  ADD COLUMN IF NOT EXISTS valid_from timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS valid_until timestamptz;

ALTER TABLE awcms_micro_profile_addresses
  ADD CONSTRAINT awcms_micro_profile_addresses_validity_window_check
    CHECK (valid_until IS NULL OR valid_until > valid_from);
...
CREATE TABLE IF NOT EXISTS awcms_micro_profile_relationships (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  from_profile_id uuid NOT NULL REFERENCES awcms_micro_profiles (id),
  to_profile_id uuid NOT NULL REFERENCES awcms_micro_profiles (id),
  ...
  end_reason text,
  CONSTRAINT awcms_micro_profile_relationships_status_check
    CHECK (status IN ('active', 'ended')),
  CONSTRAINT awcms_micro_profile_relationships_distinct_parties_check
    CHECK (from_profile_id <> to_profile_id),
  CONSTRAINT awcms_micro_profile_relationships_validity_window_check
    CHECK (valid_until IS NULL OR valid_until > valid_from),
  CONSTRAINT awcms_micro_profile_relationships_type_shape_check
    CHECK (relationship_type ~ '^[a-z][a-z0-9_]{1,63}$')
);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_relationships_tenant_idx
  ON awcms_micro_profile_relationships (tenant_id);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_relationships_tenant_from_idx
  ON awcms_micro_profile_relationships (tenant_id, from_profile_id);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_relationships_tenant_to_idx
  ON awcms_micro_profile_relationships (tenant_id, to_profile_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_profile_relationships_active_key
  ON awcms_micro_profile_relationships (
    tenant_id, from_profile_id, to_profile_id, relationship_type
  )
  WHERE status = 'active';

ALTER TABLE awcms_micro_profile_relationships ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_profile_relationships FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_profile_relationships_tenant_isolation
  ON awcms_micro_profile_relationships
...
CREATE TABLE IF NOT EXISTS awcms_micro_profile_duplicate_candidates (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  profile_id_a uuid NOT NULL REFERENCES awcms_micro_profiles (id),
  profile_id_b uuid NOT NULL REFERENCES awcms_micro_profiles (id),
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_profile_duplicate_candidates_basis_check
    CHECK (match_basis IN (
      'deterministic_identifier', 'heuristic_name_similarity', 'heuristic_combined'
    )),
  CONSTRAINT awcms_micro_profile_duplicate_candidates_status_check
    CHECK (status IN ('pending', 'confirmed_duplicate', 'not_duplicate')),
  CONSTRAINT awcms_micro_profile_duplicate_candidates_score_range_check
  ...
  CONSTRAINT awcms_micro_profile_duplicate_candidates_ordered_pair_check
    CHECK (profile_id_a < profile_id_b)
);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_duplicate_candidates_tenant_idx
  ON awcms_micro_profile_duplicate_candidates (tenant_id);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_duplicate_candidates_tenant_status_idx
  ON awcms_micro_profile_duplicate_candidates (tenant_id, status);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_duplicate_candidates_profile_a_idx
  ON awcms_micro_profile_duplicate_candidates (profile_id_a);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_duplicate_candidates_profile_b_idx
  ON awcms_micro_profile_duplicate_candidates (profile_id_b);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_profile_duplicate_candidates_pair_key
  ON awcms_micro_profile_duplicate_candidates (tenant_id, profile_id_a, profile_id_b);

ALTER TABLE awcms_micro_profile_duplicate_candidates ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_profile_duplicate_candidates FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_profile_duplicate_candidates_tenant_isolation
  ON awcms_micro_profile_duplicate_candidates
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
ALTER TABLE awcms_micro_profile_merge_requests
  ...
  ADD COLUMN IF NOT EXISTS duplicate_candidate_id uuid
    REFERENCES awcms_micro_profile_duplicate_candidates (id),
  ADD COLUMN IF NOT EXISTS executed_at timestamptz,
  ADD COLUMN IF NOT EXISTS executed_by uuid;

CREATE INDEX IF NOT EXISTS awcms_micro_profile_merge_requests_duplicate_candidate_idx
  ON awcms_micro_profile_merge_requests (duplicate_candidate_id);

-- ---------------------------------------------------------------------
-- 8. Immutable merge history — append-only, distinct from the mutable
--    `awcms_micro_profile_merge_requests` status row. No application code
...
CREATE TABLE IF NOT EXISTS awcms_micro_profile_merge_history (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  merge_request_id uuid NOT NULL REFERENCES awcms_micro_profile_merge_requests (id),
  survivor_profile_id uuid NOT NULL REFERENCES awcms_micro_profiles (id),
  loser_profile_id uuid NOT NULL REFERENCES awcms_micro_profiles (id),
  ...
  recovery_notes text,
  CONSTRAINT awcms_micro_profile_merge_history_distinct_parties_check
    CHECK (survivor_profile_id <> loser_profile_id)
);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_merge_history_tenant_idx
  ON awcms_micro_profile_merge_history (tenant_id);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_merge_history_merge_request_idx
  ON awcms_micro_profile_merge_history (merge_request_id);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_merge_history_survivor_idx
  ON awcms_micro_profile_merge_history (tenant_id, survivor_profile_id);

CREATE INDEX IF NOT EXISTS awcms_micro_profile_merge_history_loser_idx
  ON awcms_micro_profile_merge_history (tenant_id, loser_profile_id);

ALTER TABLE awcms_micro_profile_merge_history ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_profile_merge_history FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_profile_merge_history_tenant_isolation
  ON awcms_micro_profile_merge_history
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

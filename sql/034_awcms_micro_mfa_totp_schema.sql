...
-- enable the feature.
--
-- `awcms_micro_identity_mfa_factors` — one row per identity per factor type.
...
-- row, not deleted, for audit/history — mirrors why `awcms_micro_identities`
...
CREATE TABLE IF NOT EXISTS awcms_micro_identity_mfa_factors (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  identity_id uuid NOT NULL REFERENCES awcms_micro_identities (id),
  ...
  CONSTRAINT awcms_micro_identity_mfa_factors_factor_type_check
    CHECK (factor_type IN ('totp')),
  CONSTRAINT awcms_micro_identity_mfa_factors_status_check
    CHECK (status IN ('pending', 'active', 'disabled'))
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_identity_mfa_factors_active_key
  ON awcms_micro_identity_mfa_factors (tenant_id, identity_id, factor_type)
  WHERE status <> 'disabled';

ALTER TABLE awcms_micro_identity_mfa_factors ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_identity_mfa_factors FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_identity_mfa_factors_tenant_isolation
  ON awcms_micro_identity_mfa_factors
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- `awcms_micro_identity_mfa_recovery_codes` — single-use backup codes shown
...
CREATE TABLE IF NOT EXISTS awcms_micro_identity_mfa_recovery_codes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  identity_id uuid NOT NULL REFERENCES awcms_micro_identities (id),
  factor_id uuid NOT NULL REFERENCES awcms_micro_identity_mfa_factors (id) ON DELETE CASCADE,
  code_hash text NOT NULL,
  used_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_identity_mfa_recovery_codes_hash_key
  ON awcms_micro_identity_mfa_recovery_codes (code_hash);

CREATE INDEX IF NOT EXISTS awcms_micro_identity_mfa_recovery_codes_factor_idx
  ON awcms_micro_identity_mfa_recovery_codes (tenant_id, factor_id)
  WHERE used_at IS NULL;

ALTER TABLE awcms_micro_identity_mfa_recovery_codes ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_identity_mfa_recovery_codes FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_identity_mfa_recovery_codes_tenant_isolation
  ON awcms_micro_identity_mfa_recovery_codes
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- `awcms_micro_mfa_challenges` — the ephemeral bridge between "password
...
CREATE TABLE IF NOT EXISTS awcms_micro_mfa_challenges (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  identity_id uuid NOT NULL REFERENCES awcms_micro_identities (id),
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_mfa_challenges_failed_attempts_check
    CHECK (failed_attempts >= 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_mfa_challenges_hash_key
  ON awcms_micro_mfa_challenges (challenge_token_hash);

CREATE INDEX IF NOT EXISTS awcms_micro_mfa_challenges_identity_idx
  ON awcms_micro_mfa_challenges (tenant_id, identity_id)
  WHERE consumed_at IS NULL;

ALTER TABLE awcms_micro_mfa_challenges ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_mfa_challenges FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_mfa_challenges_tenant_isolation
  ON awcms_micro_mfa_challenges
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

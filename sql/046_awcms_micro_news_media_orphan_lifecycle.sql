...
-- wave). Adds `orphaned_at` to `awcms_micro_news_media_objects` so the new
...
-- `awcms_micro_news_media_objects_owner_consistency_check` idiom (migration
...
ALTER TABLE awcms_micro_news_media_objects
  ADD COLUMN IF NOT EXISTS orphaned_at timestamptz;

ALTER TABLE awcms_micro_news_media_objects
  ADD CONSTRAINT awcms_micro_news_media_objects_orphaned_at_consistency_check
  CHECK (
...
-- worker grant in migration 045 already follows. `awcms_micro_audit_events`
-- (INSERT, for this job's own audit events) is already granted (migration
-- 045) — not repeated here.
GRANT SELECT, UPDATE, DELETE ON awcms_micro_news_media_objects TO awcms_micro_worker;
...
-- existing `idx_awcms_micro_news_media_objects_tenant_status` partial index
...
-- reasoning `idx_awcms_micro_news_media_objects_tenant_status` itself already

...
-- (Issue #479/#480, docs/awcms-micro/examples/wizard-form-pattern.md
...
-- reconciled here to match `awcms_micro_workflow_instances.resource_id` and
-- `awcms_micro_audit_events.resource_id` (both `text`), so a draft can
...
-- since the least-privilege `awcms_micro_app` role and its default DML grants
...
-- needed here.
CREATE TABLE IF NOT EXISTS awcms_micro_form_drafts (
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_form_drafts_status_check
    CHECK (status IN ('draft', 'submitted', 'abandoned', 'expired')),
  CONSTRAINT awcms_micro_form_drafts_module_key_format_check
    CHECK (module_key ~ '^[a-z][a-z0-9_]{1,63}$'),
  CONSTRAINT awcms_micro_form_drafts_wizard_key_format_check
...
CREATE INDEX IF NOT EXISTS awcms_micro_form_drafts_tenant_wizard_idx
  ON awcms_micro_form_drafts (tenant_id, module_key, wizard_key, status);

CREATE INDEX IF NOT EXISTS awcms_micro_form_drafts_tenant_idx
  ON awcms_micro_form_drafts (tenant_id);

-- Retention job's query shape: WHERE tenant_id = ? AND status = 'draft' AND
-- expires_at < now() — see application/form-draft-purge.ts.
CREATE INDEX IF NOT EXISTS awcms_micro_form_drafts_tenant_expiry_idx
  ON awcms_micro_form_drafts (tenant_id, status, expires_at)
  WHERE expires_at IS NOT NULL;

ALTER TABLE awcms_micro_form_drafts ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_form_drafts FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_form_drafts_tenant_isolation
  ON awcms_micro_form_drafts
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- Permission model is generic infra, not gated per producing module_key —
-- same reasoning as awcms_micro_idempotency_keys (migration 012): RLS

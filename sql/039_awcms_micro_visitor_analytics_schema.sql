...
-- for visitor presence (`awcms_micro_visitor_sessions`), individual page
-- view/API events (`awcms_micro_visit_events`), and pre-aggregated daily
-- statistics (`awcms_micro_visitor_daily_rollups`). Schema-only issue —
...
-- log-like analytics rows, same shape as `awcms_micro_audit_events`
...
-- migration 013 (see `sql/031_awcms_micro_tenant_domain_schema.sql` for
-- the closest precedent). No explicit `GRANT` needed for
-- `awcms_micro_app` — migration 013's `ALTER DEFAULT PRIVILEGES` already
-- covers every table the owning role creates from here on.

CREATE TABLE IF NOT EXISTS awcms_micro_visitor_sessions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  visitor_key_hash text NOT NULL,
  identity_id uuid REFERENCES awcms_micro_identities (id),
  ...
  CONSTRAINT awcms_micro_visitor_sessions_area_check
    CHECK (area IN ('admin', 'public', 'api', 'auth', 'setup', 'unknown')),
  CONSTRAINT awcms_micro_visitor_sessions_device_type_check
    CHECK (device_type IS NULL
      OR device_type IN ('desktop', 'mobile', 'tablet', 'bot', 'unknown'))
);

CREATE INDEX IF NOT EXISTS awcms_micro_visitor_sessions_tenant_last_seen_idx
  ON awcms_micro_visitor_sessions (tenant_id, last_seen_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_visitor_sessions_tenant_area_last_seen_idx
  ON awcms_micro_visitor_sessions (tenant_id, area, last_seen_at DESC);

ALTER TABLE awcms_micro_visitor_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_visitor_sessions FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_visitor_sessions_tenant_isolation
  ON awcms_micro_visitor_sessions
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

CREATE TABLE IF NOT EXISTS awcms_micro_visit_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  visitor_session_id uuid REFERENCES awcms_micro_visitor_sessions (id),
  identity_id uuid REFERENCES awcms_micro_identities (id),
  ...
  correlation_id text,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_visit_events_area_check
    CHECK (area IN ('admin', 'public', 'api', 'auth', 'setup', 'unknown')),
  CONSTRAINT awcms_micro_visit_events_human_status_check
    CHECK (human_status IN ('human', 'bot', 'unknown')),
  CONSTRAINT awcms_micro_visit_events_status_code_check
    CHECK (status_code IS NULL OR (status_code >= 100 AND status_code <= 599))
);

CREATE INDEX IF NOT EXISTS awcms_micro_visit_events_tenant_occurred_idx
  ON awcms_micro_visit_events (tenant_id, occurred_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_visit_events_tenant_area_occurred_idx
  ON awcms_micro_visit_events (tenant_id, area, occurred_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_visit_events_tenant_human_status_occurred_idx
  ON awcms_micro_visit_events (tenant_id, human_status, occurred_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_visit_events_session_occurred_idx
  ON awcms_micro_visit_events (visitor_session_id, occurred_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_visit_events_identity_occurred_idx
  ON awcms_micro_visit_events (identity_id, occurred_at DESC);

ALTER TABLE awcms_micro_visit_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_visit_events FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_visit_events_tenant_isolation
  ON awcms_micro_visit_events
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE TABLE IF NOT EXISTS awcms_micro_visitor_daily_rollups (
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  date date NOT NULL,
  area text NOT NULL,
  ...
  CONSTRAINT awcms_micro_visitor_daily_rollups_area_check
    CHECK (area IN ('admin', 'public', 'api', 'auth', 'setup', 'unknown'))
);

ALTER TABLE awcms_micro_visitor_daily_rollups ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_visitor_daily_rollups FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_visitor_daily_rollups_tenant_isolation
  ON awcms_micro_visitor_daily_rollups
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

...
ALTER TABLE awcms_micro_blog_posts DROP COLUMN IF EXISTS search_vector;
...
ALTER TABLE awcms_micro_blog_pages DROP COLUMN IF EXISTS search_vector;

ALTER TABLE awcms_micro_blog_pages
  ADD COLUMN search_vector tsvector
  GENERATED ALWAYS AS (
    setweight(to_tsvector('simple', coalesce(title, '')), 'A') ||
    setweight(to_tsvector('simple', coalesce(excerpt, '')), 'B') ||

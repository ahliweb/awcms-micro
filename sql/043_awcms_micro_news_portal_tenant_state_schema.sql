...
--   1. `awcms_micro_tenant_modules.enabled` (`fetchTenantModuleEntry`) is
...
--   2. `awcms_micro_module_settings` (the generic per-tenant module settings
...
CREATE TABLE IF NOT EXISTS awcms_micro_news_portal_tenant_state (
  tenant_id uuid PRIMARY KEY REFERENCES awcms_micro_tenants (id),
  full_online_r2_mode_applied_at timestamptz NOT NULL,
  updated_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE awcms_micro_news_portal_tenant_state ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_news_portal_tenant_state FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_news_portal_tenant_state_tenant_isolation
  ON awcms_micro_news_portal_tenant_state

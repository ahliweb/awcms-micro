...
--
-- AWCMS-Micro already has three real outbox/queue precedents this migration
-- deliberately follows the shape of (not replaces): `awcms_micro_object_sync_
-- queue` + `dispatchObjectSyncQueue` (Issue #436), `awcms_micro_email_messages`
-- + `dispatchEmailQueue` (Issue #493-#495), and `awcms_micro_social_publish_
...
-- Six tables:
--
-- 1. `awcms_micro_domain_events` — the outbox itself. Append-only (never
...
-- 2. `awcms_micro_domain_event_deliveries` — one row per (event, registered
...
-- 3. `awcms_micro_domain_event_consumer_effects` — a generic, reusable
...
--    documentation.
-- 4. `awcms_micro_domain_event_consumer_state` — per (tenant, consumer)
...
-- 5. `awcms_micro_domain_event_replays` — append-only audit trail of every
--    replay action (who, when, reason, which delivery). Separate from the
--    generic `awcms_micro_audit_events` table (which ALSO gets a row per
...
-- 6. `awcms_micro_domain_event_activity_daily` — a small denormalized
...
CREATE TABLE IF NOT EXISTS awcms_micro_domain_events (
  ...
  -- name in this repo uses a hyphenated `awcms-micro` namespace prefix
  -- (e.g. `awcms-micro.domain-event-runtime.sample.recorded`); matches
  -- `domain/envelope.ts`'s `EVENT_TYPE_PATTERN` exactly.
  CONSTRAINT awcms_micro_domain_events_event_type_format_check
    CHECK (event_type ~ '^[a-z0-9][a-z0-9_-]*(\.[a-z0-9_-]+)+$'),
  CONSTRAINT awcms_micro_domain_events_event_version_format_check
    CHECK (event_version ~ '^[0-9]+\.[0-9]+$'),
  CONSTRAINT awcms_micro_domain_events_aggregate_type_length_check
    CHECK (char_length(aggregate_type) BETWEEN 1 AND 100),
  CONSTRAINT awcms_micro_domain_events_order_key_length_check
    CHECK (char_length(order_key) BETWEEN 1 AND 300),
  CONSTRAINT awcms_micro_domain_events_producer_module_length_check
  ...
  CONSTRAINT awcms_micro_domain_events_payload_size_check
    CHECK (octet_length(payload::text) <= 65536)
);

CREATE INDEX IF NOT EXISTS awcms_micro_domain_events_sequence_idx
  ON awcms_micro_domain_events (event_sequence);
CREATE INDEX IF NOT EXISTS awcms_micro_domain_events_tenant_type_idx
  ON awcms_micro_domain_events (tenant_id, event_type, recorded_at DESC);
CREATE INDEX IF NOT EXISTS awcms_micro_domain_events_tenant_aggregate_idx
  ON awcms_micro_domain_events (tenant_id, aggregate_type, aggregate_id);
CREATE INDEX IF NOT EXISTS awcms_micro_domain_events_tenant_recorded_idx
  ON awcms_micro_domain_events (tenant_id, recorded_at DESC);

ALTER TABLE awcms_micro_domain_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_domain_events FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_domain_events_tenant_isolation
  ON awcms_micro_domain_events
...
CREATE TABLE IF NOT EXISTS awcms_micro_domain_event_deliveries (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  event_id uuid NOT NULL REFERENCES awcms_micro_domain_events (id),
  ...
  replay_of_delivery_id uuid REFERENCES awcms_micro_domain_event_deliveries (id),
  correlation_id text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_domain_event_deliveries_status_check
    CHECK (status IN ('pending', 'delivered', 'dead_letter', 'skipped')),
  CONSTRAINT awcms_micro_domain_event_deliveries_attempt_count_check
    CHECK (attempt_count >= 0 AND max_attempts >= 1),
  CONSTRAINT awcms_micro_domain_event_deliveries_consumer_name_length_check
...
-- original row.
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_domain_event_deliveries_identity_key
  ON awcms_micro_domain_event_deliveries (tenant_id, event_id, consumer_name)
...
CREATE INDEX IF NOT EXISTS awcms_micro_domain_event_deliveries_dispatch_idx
  ON awcms_micro_domain_event_deliveries (tenant_id, consumer_name, order_key, event_sequence)
  WHERE status = 'pending';

CREATE INDEX IF NOT EXISTS awcms_micro_domain_event_deliveries_event_id_idx
  ON awcms_micro_domain_event_deliveries (event_id);
CREATE INDEX IF NOT EXISTS awcms_micro_domain_event_deliveries_tenant_status_idx
  ON awcms_micro_domain_event_deliveries (tenant_id, status);
CREATE INDEX IF NOT EXISTS awcms_micro_domain_event_deliveries_tenant_consumer_idx
  ON awcms_micro_domain_event_deliveries (tenant_id, consumer_name, status);
CREATE INDEX IF NOT EXISTS awcms_micro_domain_event_deliveries_replay_of_idx
  ON awcms_micro_domain_event_deliveries (replay_of_delivery_id)
  WHERE replay_of_delivery_id IS NOT NULL;

ALTER TABLE awcms_micro_domain_event_deliveries ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_domain_event_deliveries FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_domain_event_deliveries_tenant_isolation
  ON awcms_micro_domain_event_deliveries
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- Generic per-consumer side-effect idempotency marker (see file header
-- point 3). `event_id` intentionally has NO foreign key to
-- `awcms_micro_domain_events` — the marker must remain valid even after a
...
-- retention decisions.
CREATE TABLE IF NOT EXISTS awcms_micro_domain_event_consumer_effects (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  consumer_name text NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_domain_event_consumer_effects_identity_key
  ON awcms_micro_domain_event_consumer_effects (tenant_id, consumer_name, event_id);

ALTER TABLE awcms_micro_domain_event_consumer_effects ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_domain_event_consumer_effects FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_domain_event_consumer_effects_tenant_isolation
  ON awcms_micro_domain_event_consumer_effects
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- Per (tenant, consumer) pause/resume switch (see file header point 4).
CREATE TABLE IF NOT EXISTS awcms_micro_domain_event_consumer_state (
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  consumer_name text NOT NULL,
...
ALTER TABLE awcms_micro_domain_event_consumer_state ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_domain_event_consumer_state FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_domain_event_consumer_state_tenant_isolation
  ON awcms_micro_domain_event_consumer_state
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- Append-only replay audit trail (see file header point 5) — never
-- UPDATEd/DELETEd by application code.
CREATE TABLE IF NOT EXISTS awcms_micro_domain_event_replays (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  original_delivery_id uuid NOT NULL REFERENCES awcms_micro_domain_event_deliveries (id),
  replay_delivery_id uuid NOT NULL REFERENCES awcms_micro_domain_event_deliveries (id),
  requested_by uuid NOT NULL,
  reason text NOT NULL,
  correlation_id text,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_domain_event_replays_reason_length_check
    CHECK (char_length(reason) BETWEEN 1 AND 500)
);

CREATE INDEX IF NOT EXISTS awcms_micro_domain_event_replays_original_idx
  ON awcms_micro_domain_event_replays (tenant_id, original_delivery_id);

ALTER TABLE awcms_micro_domain_event_replays ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_domain_event_replays FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_domain_event_replays_tenant_isolation
  ON awcms_micro_domain_event_replays
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE TABLE IF NOT EXISTS awcms_micro_domain_event_activity_daily (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  ...
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_domain_event_activity_daily_count_check
    CHECK (event_count >= 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_domain_event_activity_daily_identity_key
  ON awcms_micro_domain_event_activity_daily (tenant_id, activity_date, event_type);

ALTER TABLE awcms_micro_domain_event_activity_daily ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_domain_event_activity_daily FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_domain_event_activity_daily_tenant_isolation
  ON awcms_micro_domain_event_activity_daily
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- `awcms_micro_worker` least-privilege role (migration 045) — extends its
...
-- PRIVILEGES` only grants the ordinary runtime `awcms_micro_app` role
-- automatically (see that migration's own header) — the narrower
-- `awcms_micro_worker` role always needs an explicit per-table grant here,
...
--   - `awcms_micro_domain_events`: SELECT only — the dispatcher JOINs this
...
--     transactions, which run as `awcms_micro_app`, not the worker).
--   - `awcms_micro_domain_event_deliveries`: SELECT (head-of-line
...
--     `appendDomainEvent`/`replayDomainEventDelivery`, both `awcms_micro_app`
--     call sites: the producer's transaction and the replay API route,
--     respectively).
--   - `awcms_micro_domain_event_consumer_effects`: SELECT + INSERT —
...
--   - `awcms_micro_domain_event_consumer_state`: SELECT only — the
...
--     the admin API routes, which run as `awcms_micro_app`.
--   - `awcms_micro_domain_event_activity_daily`: SELECT + INSERT + UPDATE —
...
--   - `awcms_micro_domain_event_replays` is deliberately NOT granted here —
--     the dispatcher never reads or writes it; only the replay API route
--     (`awcms_micro_app`) touches this table.
--   - `awcms_micro_audit_events` (INSERT, for the audit-projector reference
...
--     here. `awcms_micro_tenants` (SELECT, for `fetchActiveTenants`'s
--     tenant-iteration query) is likewise already granted by migration 045.
GRANT SELECT ON awcms_micro_domain_events TO awcms_micro_worker;
GRANT SELECT, UPDATE ON awcms_micro_domain_event_deliveries TO awcms_micro_worker;
GRANT SELECT, INSERT ON awcms_micro_domain_event_consumer_effects TO awcms_micro_worker;
GRANT SELECT ON awcms_micro_domain_event_consumer_state TO awcms_micro_worker;
GRANT SELECT, INSERT, UPDATE ON awcms_micro_domain_event_activity_daily TO awcms_micro_worker;

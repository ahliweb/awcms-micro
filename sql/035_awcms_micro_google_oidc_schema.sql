...
-- the feature.
--
-- `awcms_micro_identity_provider_accounts` — links an identity to an external
...
CREATE TABLE IF NOT EXISTS awcms_micro_identity_provider_accounts (
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_identity_provider_accounts_subject_key
  ON awcms_micro_identity_provider_accounts (tenant_id, provider, provider_subject);
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_identity_provider_accounts_identity_key
  ON awcms_micro_identity_provider_accounts (tenant_id, identity_id, provider);

ALTER TABLE awcms_micro_identity_provider_accounts ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_identity_provider_accounts FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_identity_provider_accounts_tenant_isolation
  ON awcms_micro_identity_provider_accounts
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- `awcms_micro_oidc_auth_requests` — the ephemeral bridge across the OAuth
...
-- `awcms_micro_mfa_challenges` (Issue #589). `state_hash` is the CSRF/replay
...
CREATE TABLE IF NOT EXISTS awcms_micro_oidc_auth_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  provider text NOT NULL,
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_oidc_auth_requests_purpose_check
    CHECK (purpose IN ('login', 'link')),
  CONSTRAINT awcms_micro_oidc_auth_requests_link_has_identity_check
    CHECK (purpose <> 'link' OR identity_id IS NOT NULL)
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_oidc_auth_requests_hash_key
  ON awcms_micro_oidc_auth_requests (state_hash);

ALTER TABLE awcms_micro_oidc_auth_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_oidc_auth_requests FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_oidc_auth_requests_tenant_isolation
  ON awcms_micro_oidc_auth_requests
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

...
-- five (`awcms_micro_social_accounts`, `..._social_publish_rules`,
-- `..._social_publish_jobs`, `..._social_publish_attempts`,
-- `..._social_publish_templates`). The sixth,
-- `awcms_micro_social_publishing_settings`, exists to satisfy the issue's own
...
-- security-relevant flag. See `.claude/skills/awcms-micro-social-publishing/
...
CREATE TABLE IF NOT EXISTS awcms_micro_social_accounts (
  ...
  CONSTRAINT awcms_micro_social_accounts_provider_key_format_check
    CHECK (provider_key ~ '^[a-z][a-z0-9_]{1,49}$'),
  CONSTRAINT awcms_micro_social_accounts_account_type_check
    CHECK (provider_account_type IN ('page', 'profile', 'channel', 'group', 'organization')),
  CONSTRAINT awcms_micro_social_accounts_connection_status_check
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_social_accounts_identity_key
  ON awcms_micro_social_accounts (tenant_id, provider_key, provider_account_id);

CREATE INDEX IF NOT EXISTS awcms_micro_social_accounts_tenant_status_idx
  ON awcms_micro_social_accounts (tenant_id, connection_status);

ALTER TABLE awcms_micro_social_accounts ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_social_accounts FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_social_accounts_tenant_isolation
  ON awcms_micro_social_accounts
...
CREATE TABLE IF NOT EXISTS awcms_micro_social_publish_rules (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  social_account_id uuid NOT NULL REFERENCES awcms_micro_social_accounts (id),
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_social_publish_rules_trigger_event_check
    CHECK (trigger_event IN ('post_published', 'scheduled_published', 'manual_editor_action'))
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_social_publish_rules_identity_key
  ON awcms_micro_social_publish_rules (tenant_id, social_account_id, trigger_event)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS awcms_micro_social_publish_rules_account_id_idx
  ON awcms_micro_social_publish_rules (social_account_id);

ALTER TABLE awcms_micro_social_publish_rules ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_social_publish_rules FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_social_publish_rules_tenant_isolation
  ON awcms_micro_social_publish_rules
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
-- reasoning `awcms_micro_news_media_objects.owner_resource_id` uses, chosen
...
CREATE TABLE IF NOT EXISTS awcms_micro_social_publish_templates (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  provider_key text,
  name text NOT NULL,
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_social_publish_templates_provider_key_format_check
    CHECK (provider_key IS NULL OR provider_key ~ '^[a-z][a-z0-9_]{1,49}$'),
  CONSTRAINT awcms_micro_social_publish_templates_name_length_check
    CHECK (char_length(name) <= 200),
  CONSTRAINT awcms_micro_social_publish_templates_caption_length_check
    CHECK (char_length(caption_template) <= 2000)
);

CREATE INDEX IF NOT EXISTS awcms_micro_social_publish_templates_tenant_idx
  ON awcms_micro_social_publish_templates (tenant_id)
  WHERE deleted_at IS NULL;

ALTER TABLE awcms_micro_social_publish_templates ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_social_publish_templates FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_social_publish_templates_tenant_isolation
  ON awcms_micro_social_publish_templates
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- The outbox itself. `article_id` is a REAL foreign key into
-- `awcms_micro_blog_posts` (unlike the polymorphic, FK-less
...
-- back to `awcms_micro_blog_posts` — required by the outbox pattern (ADR-0006):
...
-- `awcms_micro_object_sync_queue`'s `requires_upload` flag was added for.
--
...
CREATE TABLE IF NOT EXISTS awcms_micro_social_publish_jobs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  social_account_id uuid NOT NULL REFERENCES awcms_micro_social_accounts (id),
  rule_id uuid REFERENCES awcms_micro_social_publish_rules (id),
  article_id uuid NOT NULL REFERENCES awcms_micro_blog_posts (id),
  provider_key text NOT NULL,
  trigger_event text NOT NULL,
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_social_publish_jobs_provider_key_format_check
    CHECK (provider_key ~ '^[a-z][a-z0-9_]{1,49}$'),
  CONSTRAINT awcms_micro_social_publish_jobs_trigger_event_check
    CHECK (trigger_event IN ('post_published', 'scheduled_published', 'manual_editor_action')),
  CONSTRAINT awcms_micro_social_publish_jobs_action_check
    CHECK (action IN ('publish')),
  CONSTRAINT awcms_micro_social_publish_jobs_status_check
    CHECK (status IN (
      'pending', 'requires_approval', 'approved', 'scheduled', 'publishing',
      'published', 'failed', 'cancelled', 'skipped', 'rate_limited', 'needs_reauth'
    )),
  CONSTRAINT awcms_micro_social_publish_jobs_attempt_count_check
    CHECK (attempt_count >= 0 AND max_attempts >= 1)
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_social_publish_jobs_idempotency_key
  ON awcms_micro_social_publish_jobs (tenant_id, idempotency_key);

-- Dispatcher CLAIM query predicate (`status`, `next_attempt_at`), same
-- convention `awcms_micro_object_sync_queue_retry_idx` uses.
CREATE INDEX IF NOT EXISTS awcms_micro_social_publish_jobs_dispatch_idx
  ON awcms_micro_social_publish_jobs (tenant_id, status, next_attempt_at);

CREATE INDEX IF NOT EXISTS awcms_micro_social_publish_jobs_article_id_idx
  ON awcms_micro_social_publish_jobs (article_id);

CREATE INDEX IF NOT EXISTS awcms_micro_social_publish_jobs_social_account_id_idx
  ON awcms_micro_social_publish_jobs (social_account_id);

CREATE INDEX IF NOT EXISTS awcms_micro_social_publish_jobs_rule_id_idx
  ON awcms_micro_social_publish_jobs (rule_id);

ALTER TABLE awcms_micro_social_publish_jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_social_publish_jobs FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_social_publish_jobs_tenant_isolation
  ON awcms_micro_social_publish_jobs
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE TABLE IF NOT EXISTS awcms_micro_social_publish_attempts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  job_id uuid NOT NULL REFERENCES awcms_micro_social_publish_jobs (id),
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_social_publish_attempts_outcome_check
    CHECK (outcome IN ('success', 'failed', 'rate_limited', 'needs_reauth', 'skipped')),
  CONSTRAINT awcms_micro_social_publish_attempts_attempt_number_check
    CHECK (attempt_number >= 1)
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_social_publish_attempts_job_attempt_key
  ON awcms_micro_social_publish_attempts (job_id, attempt_number);

ALTER TABLE awcms_micro_social_publish_attempts ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_social_publish_attempts FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_social_publish_attempts_tenant_isolation
  ON awcms_micro_social_publish_attempts
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE TABLE IF NOT EXISTS awcms_micro_social_publishing_settings (
...
ALTER TABLE awcms_micro_social_publishing_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_social_publishing_settings FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_social_publishing_settings_tenant_isolation
  ON awcms_micro_social_publishing_settings
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

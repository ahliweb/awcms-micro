...
-- see `.claude/skills/awcms-micro-news-portal/SKILL.md` §641: "Tidak terkait
...
--
-- 1. `awcms_micro_blog_posts.auto_internal_tag_links_disabled` — per-post
...
--    `visibility`).
--
-- 2. `awcms_micro_blog_internal_tag_link_settings` — per-tenant policy
...
--    per-tenant shape as `awcms_micro_blog_theme_settings` (migration 029)
--    — NOT folded into `awcms_micro_blog_settings.settings` jsonb the way
...
-- application layer against `awcms_micro_blog_terms` (must exist, same
...
ALTER TABLE awcms_micro_blog_internal_tag_link_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_blog_internal_tag_link_settings FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_blog_internal_tag_link_settings_tenant_isolation
  ON awcms_micro_blog_internal_tag_link_settings

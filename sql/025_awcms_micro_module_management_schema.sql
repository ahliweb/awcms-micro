...
--
-- `awcms_micro_modules` already exists (migration 001) as a dormant table —
...
-- a dedicated `awcms_micro_module_lifecycle_events` table (both were in the
-- original issue draft) — module lifecycle/config actions are recorded
-- through the existing generic `awcms_micro_audit_events` table
...
ALTER TABLE awcms_micro_modules
...
ALTER TABLE awcms_micro_modules
  ADD CONSTRAINT awcms_micro_modules_lifecycle_status_check
    CHECK (lifecycle_status IN ('active', 'experimental', 'deprecated', 'maintenance', 'disabled'));

ALTER TABLE awcms_micro_modules
  ADD CONSTRAINT awcms_micro_modules_module_type_check
...
CREATE TABLE IF NOT EXISTS awcms_micro_tenant_modules (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  module_key text NOT NULL REFERENCES awcms_micro_modules (module_key),
  ...
  CONSTRAINT awcms_micro_tenant_modules_tenant_module_key UNIQUE (tenant_id, module_key)
);

ALTER TABLE awcms_micro_tenant_modules ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_tenant_modules FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_tenant_modules_tenant_isolation
  ON awcms_micro_tenant_modules
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

CREATE INDEX IF NOT EXISTS awcms_micro_tenant_modules_tenant_idx
  ON awcms_micro_tenant_modules (tenant_id, module_key);
...
-- meaning for every tenant (same justification as `awcms_micro_modules`
-- itself being RLS-free since migration 001).
CREATE TABLE IF NOT EXISTS awcms_micro_module_dependencies (
  module_key text NOT NULL REFERENCES awcms_micro_modules (module_key),
  depends_on_module_key text NOT NULL REFERENCES awcms_micro_modules (module_key),
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (module_key, depends_on_module_key),
  CONSTRAINT awcms_micro_module_dependencies_no_self_dependency
...
CREATE TABLE IF NOT EXISTS awcms_micro_module_settings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  module_key text NOT NULL REFERENCES awcms_micro_modules (module_key),
  ...
  updated_at timestamptz NOT NULL DEFAULT now(),
  updated_by uuid,
  CONSTRAINT awcms_micro_module_settings_tenant_module_key UNIQUE (tenant_id, module_key)
);

ALTER TABLE awcms_micro_module_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_module_settings FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_module_settings_tenant_isolation
  ON awcms_micro_module_settings
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE TABLE IF NOT EXISTS awcms_micro_module_navigation (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  module_key text NOT NULL REFERENCES awcms_micro_modules (module_key),
...
CREATE TABLE IF NOT EXISTS awcms_micro_module_jobs (
  ...
  synced_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_module_jobs_module_command_key UNIQUE (module_key, command)
);
...
CREATE TABLE IF NOT EXISTS awcms_micro_module_health_checks (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  module_key text NOT NULL REFERENCES awcms_micro_modules (module_key),
  status text NOT NULL,
  message text,
  checked_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT awcms_micro_module_health_checks_status_check
    CHECK (status IN ('healthy', 'degraded', 'failed', 'unknown'))
);

CREATE INDEX IF NOT EXISTS awcms_micro_module_health_checks_module_idx
  ON awcms_micro_module_health_checks (module_key, checked_at DESC);

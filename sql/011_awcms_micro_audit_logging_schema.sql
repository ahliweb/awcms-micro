...
--
-- Adds the generic, cross-module audit trail table (`awcms_micro_audit_events`)
-- referenced by doc 10 §Audit helper and skill `awcms-micro-audit-log`. This is
-- separate from and complements the existing narrower audit tables
-- (`awcms_micro_profile_audit_logs` from migration 003, scoped to profile
-- lifecycle only, and `awcms_micro_abac_decision_logs` from migration 005,
...
-- `docs/awcms-micro/03_srs_detail_per_modul.md` and skill `awcms-micro-audit-log`.
--
...
CREATE TABLE IF NOT EXISTS awcms_micro_audit_events (
  ...
  CONSTRAINT awcms_micro_audit_events_severity_check
    CHECK (severity IN ('info', 'warning', 'critical'))
);

CREATE INDEX IF NOT EXISTS awcms_micro_audit_events_tenant_created_idx
  ON awcms_micro_audit_events (tenant_id, created_at DESC);

CREATE INDEX IF NOT EXISTS awcms_micro_audit_events_tenant_resource_idx
  ON awcms_micro_audit_events (tenant_id, resource_type, resource_id);

ALTER TABLE awcms_micro_audit_events ENABLE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_audit_events_tenant_isolation
  ON awcms_micro_audit_events

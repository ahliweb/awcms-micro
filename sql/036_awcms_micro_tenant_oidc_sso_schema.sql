...
-- `awcms_micro_identity_provider_accounts` or `awcms_micro_oidc_auth_requests`
...
--
-- `awcms_micro_auth_providers` — one tenant-configured OIDC identity
-- provider (Okta, Azure AD, Keycloak, etc.). `provider_key` is the stable
-- slug used everywhere else this issue touches `awcms_micro_identity_provider_accounts.provider`
-- /`awcms_micro_oidc_auth_requests.provider`, and in the `/api/v1/auth/sso/{providerKey}/...`
...
-- list-shaped columns, see `awcms_micro_module_settings.settings` and doc 04
...
CREATE TABLE IF NOT EXISTS awcms_micro_auth_providers (
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_auth_providers_provider_type_check
    CHECK (provider_type IN ('oidc')),
  CONSTRAINT awcms_micro_auth_providers_provider_key_format_check
    CHECK (provider_key ~ '^[a-z0-9][a-z0-9_-]*$'),
  CONSTRAINT awcms_micro_auth_providers_secret_source_check
    CHECK (
...
-- `awcms_micro_identity_provider_accounts.provider`/
-- `awcms_micro_oidc_auth_requests.provider` columns — must be unique per
-- tenant among non-deleted providers (an archived provider's slug may be
-- reused, same convention as `awcms_micro_tenant_domains`' hostname dedup).
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_auth_providers_key_active
  ON awcms_micro_auth_providers (tenant_id, provider_key)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_awcms_micro_auth_providers_tenant
  ON awcms_micro_auth_providers (tenant_id);
CREATE INDEX IF NOT EXISTS idx_awcms_micro_auth_providers_tenant_created
  ON awcms_micro_auth_providers (tenant_id, created_at DESC)
  WHERE deleted_at IS NULL;

ALTER TABLE awcms_micro_auth_providers ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_auth_providers FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_auth_providers_tenant_isolation
  ON awcms_micro_auth_providers
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

-- `awcms_micro_tenant_auth_policies` — one row per tenant (unique index
-- below), same "one row per tenant, upsert, no `id` in the URL" shape as
-- `awcms_micro_blog_settings` (migration 029). `break_glass_identity_ids`
...
CREATE TABLE IF NOT EXISTS awcms_micro_tenant_auth_policies (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES awcms_micro_tenants (id),
  ...
  updated_at timestamptz NOT NULL DEFAULT now(),
  updated_by uuid,
  CONSTRAINT awcms_micro_tenant_auth_policies_login_method_check
    CHECK (password_login_enabled OR sso_enabled)
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_tenant_auth_policies_tenant_key
  ON awcms_micro_tenant_auth_policies (tenant_id);

ALTER TABLE awcms_micro_tenant_auth_policies ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_tenant_auth_policies FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_tenant_auth_policies_tenant_isolation
  ON awcms_micro_tenant_auth_policies
  USING (tenant_id = current_setting('app.current_tenant_id')::uuid);

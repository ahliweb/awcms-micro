...
-- (`docs/awcms-micro/12_generator_prompt.md`'s epic target model, see
-- `.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md`).
--
-- Numbering note: the issue's own §Scope only names this one file
-- (`sql/031_awcms_micro_tenant_domain_schema.sql`), but this migration
...
-- `sql/032_awcms_micro_tenant_domain_permissions.sql`.
--
-- Column design notes:
...
-- `awcms_micro_app` role's fail-closed default, migration 013) returns zero
...
-- `awcms_micro_tenants`, which migration 013 deliberately left RLS-free
...
--
-- No explicit `GRANT` needed for `awcms_micro_app` on the new table below:
...
CREATE TABLE IF NOT EXISTS awcms_micro_tenant_domains (
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_tenant_domains_domain_type_check
    CHECK (domain_type IN ('subdomain', 'custom_domain')),
  CONSTRAINT awcms_micro_tenant_domains_route_mode_check
    CHECK (route_mode IN ('canonical', 'legacy_blog')),
  CONSTRAINT awcms_micro_tenant_domains_status_check
    CHECK (status IN ('pending_verification', 'active', 'suspended', 'failed')),
  CONSTRAINT awcms_micro_tenant_domains_verification_method_check
    CHECK (verification_method IS NULL
      OR verification_method IN ('dns_txt', 'dns_cname', 'file', 'manual')),
  CONSTRAINT awcms_micro_tenant_domains_hostname_not_blank_check
    CHECK (btrim(hostname) <> ''),
  CONSTRAINT awcms_micro_tenant_domains_normalized_hostname_matches_check
...
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_tenant_domains_normalized_hostname_dedup
  ON awcms_micro_tenant_domains (normalized_hostname)
  WHERE deleted_at IS NULL;

-- At most one active primary domain per tenant.
CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_tenant_domains_primary_dedup
  ON awcms_micro_tenant_domains (tenant_id)
  WHERE is_primary = true AND deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS awcms_micro_tenant_domains_tenant_idx
  ON awcms_micro_tenant_domains (tenant_id);

CREATE INDEX IF NOT EXISTS awcms_micro_tenant_domains_tenant_status_idx
  ON awcms_micro_tenant_domains (tenant_id, status)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS awcms_micro_tenant_domains_tenant_deleted_idx
  ON awcms_micro_tenant_domains (tenant_id, deleted_at);

ALTER TABLE awcms_micro_tenant_domains ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_tenant_domains FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_tenant_domains_tenant_isolation
  ON awcms_micro_tenant_domains

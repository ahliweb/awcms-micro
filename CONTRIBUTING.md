# Berkontribusi ke AWCMS-Micro

Terima kasih sudah tertarik berkontribusi. Dokumen ini menjelaskan cara berkontribusi ke AWCMS-Micro secara aman, konsisten, dan sesuai standar proyek.
...
3. **Konsisten** — ikuti coding standard (`docs/awcms-micro/10_template_kode_coding_standard.md`) dan konvensi commit di bawah.
...
1. **Mulai dari issue.** Cek [`docs/awcms-micro/06_github_issues_detail.md`](docs/awcms-micro/06_github_issues_detail.md) dan snapshot di [`docs/awcms-micro/github/`](docs/awcms-micro/github/README.md). Bila belum ada issue, buat lebih dulu memakai template.
...
Prasyarat: **Bun** (versi terkunci di `package.json` `packageManager`/`engines`). Repositori ini **Bun-only** — jangan menambahkan `node`/`npm`/`npx`/`pnpm`/`yarn` atau tooling yang memaksa runtime Node.js (lihat `AGENTS.md` aturan 14 dan `docs/awcms-micro/10_template_kode_coding_standard.md`).
...
bun run performance:query-plan:check  # bila mengubah query kritikal (RLS/pagination/search/outbox-claim/retention-purge/reporting) — lihat docs/awcms-micro/performance-suite.md
```
...
- **Scopes base:** `foundation`, `db`, `api`, `auth`, `access`, `profile`, `tenant`, `sync`, `ui`, `logging`, `pooling`, `reporting`, `security`, `docs`. Aplikasi turunan menambah scope domainnya sendiri.
...
- Reviewer memakai kriteria di [`docs/awcms-micro/09_roadmap_repository_commit.md`](docs/awcms-micro/09_roadmap_repository_commit.md) dan skill `awcms-micro-pr-review`.
- Modul sensitif (auth, access, profile, sync) memerlukan review keamanan tambahan (`awcms-micro-security-review`).

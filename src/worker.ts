...
(async function(){
try{
var m=await f(A+"/public/metadata");
var s=await f(A+"/public/summary");
var fl=await f(A+"/public/filters");
ge("meta-desc").textContent=m.description||m.title;
ge("meta-update").textContent=m.latestUpdateAt?"Diperbarui "+d(m.latestUpdateAt):"";
ge("footer-contact").textContent=m.officialContact?"Kontak: "+m.officialContact:"";
ge("kpi-total").textContent=n(s.kpis.totalEntities);
ge("kpi-verified").textContent=n(s.kpis.verifiedEntities);
ge("kpi-villages").textContent=n(s.kpis.activeVillages);
ge("kpi-update").textContent=d(s.kpis.latestUpdateAt);
ge("caveat-text").textContent=s.caveat||"";
ge("caveat-scope").textContent=m.dataScopeNote||"";
var c=ge("chart-types");
if(fl.objectTypes&&fl.objectTypes.length){c.innerHTML=fl.objectTypes.map(function(t){return'<div class="bar"><span class="barlabel">'+t.name+'</span><div class="bartrack"><div class="barfill" style="width:100%"></div></div><span class="barval">—</span></div>'}).join("")}else{c.innerHTML="Belum ada data kategori"}
}catch(e){ge("kpi-grid").innerHTML='Gagal memuat data. Coba lagi nanti.';console.error(e)}
})();
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      // Public /sikesra page — aggregate-safe dashboard
      if (path === "/sikesra" || path === "/sikesra/") {
        return new Response(SIKESRA_PUBLIC_HTML, {

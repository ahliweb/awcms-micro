 ...
 * `awcms_micro_setup` role, not the ordinary `awcms_micro_app` web-runtime
 ...
 * own narrower-in-a-different-way role (write access to `awcms_micro_tenants`/
 * `awcms_micro_setup_state`/the bootstrap tables, nothing `awcms_micro_app`
 ...
 * setup-once lock (`awcms_micro_setup_state`'s singleton claim) were ever
 * bypassed by a future bug. See `sql/045_awcms_micro_db_role_separation.sql`'s

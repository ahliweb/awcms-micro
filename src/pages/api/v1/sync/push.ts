...
export const POST: APIRoute = async ({ request }) => {
  ...
  const nodeCode = request.headers.get("x-awcms-micro-node-id");
  ...
  if (!nodeCode) {
    return fail(
      400,
      "VALIDATION_ERROR",
      "X-AWCMS-Micro-Node-ID header is required."
    );
  }
  ...
  const authResult = verifySyncHeaders(
    request.headers.get("x-awcms-micro-timestamp"),
    request.headers.get("x-awcms-micro-signature"),
    rawBody
  );
  ...
  return withTenant(
    ...
    async (tx) => {
      ...
      SELECT event_count, conflicted_count FROM awcms_micro_sync_push_batches
    ...
    `;
      ...
      // N+1 audit (skill `awcms-micro-performance` §Hindari N+1). Keyed by
      // `aggregateType:aggregateId` (not aggregate_id alone) since the
      // uniqueness guarantee on `awcms_micro_sync_aggregate_versions` is the
      ...
      // to `awcms_micro_sync_inbox`/`awcms_micro_sync_conflicts`/
      // `awcms_micro_sync_aggregate_versions` still happens per event (those
      ...
      if (aggregateIds.length > 0) {
        const versionRows = (await tx`
          ...
          FROM awcms_micro_sync_aggregate_versions
      ...
      for (const event of events) {
        if (event.aggregateId === undefined) {
          await tx`
          INSERT INTO awcms_micro_sync_inbox
        ...
        if (evaluation.conflict) {
          await tx`
          INSERT INTO awcms_micro_sync_conflicts
      ...
      `;
        ...
        await tx`
        INSERT INTO awcms_micro_sync_aggregate_versions (tenant_id, aggregate_type, aggregate_id, current_version)
      ...
      await tx`
      INSERT INTO awcms_micro_sync_push_batches (tenant_id, node_id, batch_id, event_count, conflicted_count)
      VALUES (${tenantId}, ${node.id}, ${batchId}, ${events.length}, ${conflictedCount})
    `;

      await tx`
      UPDATE awcms_micro_sync_nodes SET last_pushed_at = now() WHERE id = ${node.id}
    `;

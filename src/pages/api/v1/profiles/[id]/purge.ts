 ...
 * awcms_micro_profiles`. Requires the profile to already be soft-deleted
 * (400 otherwise). Several other tables reference `profile_id`/
 * `source_profile_id`/`target_profile_id` with no `ON DELETE CASCADE`
 * (`awcms_micro_identities`, `awcms_micro_profile_identifiers`,
 * `awcms_micro_profile_channels`, `awcms_micro_profile_addresses`,
 * `awcms_micro_profile_entity_links`, `awcms_micro_profile_merge_requests` —
...
export const POST: APIRoute = async ({ request, params, locals }) => {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    const profileRows = await tx`
      SELECT id, deleted_at FROM awcms_micro_profiles
    ...
    try {
      await tx.savepoint(async (sp) => {
        await sp`
          DELETE FROM awcms_micro_profiles
          WHERE tenant_id = ${tenantId} AND id = ${profileId}
        `;
      });
    } catch (error) {

...
export const PATCH: APIRoute = async ({ request, cookies }) => {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    // awcms_micro_tenants is intentionally RLS-free (it IS the tenant root —
    ...
    if (
      ...
      const tenantRows = await tx`
        SELECT id FROM awcms_micro_tenants WHERE id = ${tenantId}
      `;
    ...
    if (input.timezone !== undefined) {
      await tx`
        UPDATE awcms_micro_tenant_settings

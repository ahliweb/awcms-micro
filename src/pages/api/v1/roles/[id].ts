...
export const PATCH: APIRoute = async ({ request, cookies, params }) => {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    const roleRows = (await tx`
      SELECT id, role_code, is_system FROM awcms_micro_roles
    ...
    if (input.permissionIds !== undefined) {
      ...
      if (input.permissionIds.length > 0) {
        ...
        // Issue #435 N+1 audit (skill `awcms-micro-performance` §Hindari N+1).
        await tx`
...
export const DELETE: APIRoute = async ({ request, cookies, params }) => {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    const assignments = await tx`
      SELECT 1 FROM awcms_micro_access_assignments
      WHERE tenant_id = ${tenantId} AND role_id = ${roleId}
      LIMIT 1
    `;

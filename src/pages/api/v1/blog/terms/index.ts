  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    if (input.parentId) {
      const parentRows = await tx`
        SELECT taxonomy_type FROM awcms_micro_blog_terms
    ...
    try {
      ...
      if (message.includes("awcms_micro_blog_terms_slug_dedup")) {
        return fail(
          409,
          "SLUG_CONFLICT",
      ...
      if (message.includes("awcms_micro_blog_terms_tag_no_parent_check")) {

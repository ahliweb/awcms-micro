 ...
 * `awcms_micro_idempotency_keys` wrapper around this function, not from
 * this function being independently idempotent). `reason` is validated as
 * non-empty by the DB CHECK constraint on `awcms_micro_domain_event_replays`
...
export async function replayDomainEventDelivery(
  ...
  const insertedRows = (await tx`
    INSERT INTO awcms_micro_domain_event_deliveries
  ...
  await tx`
    INSERT INTO awcms_micro_domain_event_replays

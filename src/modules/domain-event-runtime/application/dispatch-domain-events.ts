 ...
 * why `awcms_micro_domain_event_deliveries.status` (migration 056) has no
...
async function isConsumerPaused(
  ...
  return withTenant(
    ...
    async (tx) => {
      const rows = (await tx`
        SELECT is_paused FROM awcms_micro_domain_event_consumer_state
    ...
    async (tx) => {
      const rows = (await tx`
        WITH head AS (
          ...
          FROM awcms_micro_domain_event_deliveries
    ...
    const outcome = await withTenant(
      ...
      async (tx) => {
        const rows = (await tx`
          ...
          FROM awcms_micro_domain_event_deliveries d
          JOIN awcms_micro_domain_events e ON e.id = d.event_id

 ...
 * #742). Field names mirror the generic envelope skill `awcms-micro-new-event`
 * documents and the AsyncAPI `DomainEventEnvelope` schema
 * (`asyncapi/awcms-micro-domain-events.asyncapi.yaml`) already used by every
 ...
 * naming, skill `awcms-micro-new-event`). Each segment allows hyphens too
 * (not just the trailing segments) — every real event name in this repo
 * uses a hyphenated `awcms-micro` namespace prefix (e.g.
 * `awcms-micro.social-publishing.job.published`,
 * `awcms-micro.domain-event-runtime.sample.recorded`), so a pattern that
...
/** Matches the DB CHECK constraint in `sql/056_awcms_micro_domain_event_runtime_schema.sql` — kept in sync deliberately (the DB constraint is the hard backstop, this is the friendly pre-check that lets a caller get a clear application error instead of a raw Postgres constraint violation). */

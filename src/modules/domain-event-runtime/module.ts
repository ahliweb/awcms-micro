...
export const domainEventRuntimeModule = defineModule({
  ...
  events: {
    ...
    publishes: ["awcms-micro.domain-event-runtime.sample.recorded"],
    subscribes: ["awcms-micro.domain-event-runtime.sample.recorded"]
  },
  ...
  jobs: [
    {
      ...
      purpose:
        "Claim/execute/finalize due awcms_micro_domain_event_deliveries rows for every active tenant and every registered consumer, applying per-order-key ordering, exponential backoff, and dead-letter transitions. A no-op tick when there is no due backlog.",

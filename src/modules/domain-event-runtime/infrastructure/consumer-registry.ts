 ...
 * `awcms_micro_domain_event_activity_daily`, a small denormalized rollup —
...
export const activityRollupProjectorConsumer: DomainEventConsumerDefinition = {
  ...
  description:
    "Reference reporting/read-model projection consumer — maintains a per-tenant/day/event-type activity rollup (awcms_micro_domain_event_activity_daily) for operational dashboards (Issue #742).",
  eventTypes: [SAMPLE_RECORDED_EVENT_TYPE],
  eventVersions: [SAMPLE_RECORDED_EVENT_VERSION],
  handler: async (tx, event, ctx) => {
    await applyConsumerEffectOnce(
      ...
      async () => {
        ...
        await tx`
            INSERT INTO awcms_micro_domain_event_activity_daily
            ...
            DO UPDATE SET
              event_count = awcms_micro_domain_event_activity_daily.event_count + 1,
              updated_at = now()
          `;
      }
    );
...
/**
 * Upstream AWCMS-Mini registers a fourth entry here — `integration_hub`'s
 * outbound-fanout consumer (Issue #754). AWCMS-Micro does not port
 * `integration_hub` (ADR-0025), so that entry and its event types are gone
 * rather than left importing a module no registry entry provides. Nothing
 * else in this file depended on it: the dispatcher iterates
 * `DOMAIN_EVENT_CONSUMERS` generically, so removing a consumer is a pure
 * subtraction. A derived application that adds a webhook-fanout module
 * registers its consumer through its own registry contribution.
 */
 ...
 * `awcms_micro_reporting_event_activity_summary` projection metric via
 ...
 * `awcms_micro_domain_events` via a plain SQL table name, never a
...
const EVENT_ACTIVITY_PROJECTOR_DESCRIPTION =
  "reporting module consumer — projects a sample.recorded domain event into awcms_micro_reporting_projection_metrics' reporting.event_activity_summary/sample_recorded_count counter (Issue #753).";

...
in the path. See `.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md`
...
Issue #557: the `awcms_micro_tenant_domains` schema (see §Tables) and its
...
`awcms_micro_modules` via `bun run modules:sync`, and declares the six
...
## Tables (migration `031_awcms_micro_tenant_domain_schema.sql`, Issue #557)

`awcms_micro_tenant_domains` — one row per hostname/subdomain mapped to a
...
Constraints: `awcms_micro_tenant_domains_normalized_hostname_dedup`
(globally unique across tenants, `WHERE deleted_at IS NULL` — one hostname
belongs to exactly one tenant); `awcms_micro_tenant_domains_primary_dedup`
...
returns zero rows. `awcms_micro_tenants` solves the equivalent bootstrap
...
DEFINER` function, `awcms_micro_resolve_tenant_domain_lookup` (migration
`033_awcms_micro_tenant_domain_lookup_function.sql`) — see
`.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md` §Resolver for
the full mechanism/security writeup. `FORCE ROW LEVEL SECURITY` was **not**
dropped from this table to build it.

## Permission seed (migration `032_awcms_micro_tenant_domain_permissions.sql`, Issue #557)
...
`awcms_micro_resolve_tenant_domain_lookup`, the migration-033 `SECURITY
...
(`awcms_micro_setup_state.tenant_id`, also RLS-free by design), and the
...
`awcms_micro_tenant_domains` bootstrap function is never even reached by a
...
`.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md` §Resolver.
...
that issue — it still only owns the `awcms_micro_tenant_domains` schema,
...
`awcms_micro_tenant_domains` — the first application code that ever writes
...
access path — skill `awcms-micro-tenant-domain-routing` §Aturan lintas-issue
...
**Duplicate hostname handling**: `awcms_micro_tenant_domains_normalized_hostname_dedup`
...
route never queries `awcms_micro_tenant_domains` across tenants to decide
...
`awcms_micro_tenant_domains_primary_dedup` (migration 031's partial unique
...
`awcms-micro-tenant-domain-routing`'s §Aturan lintas-issue #8, mirroring
...
- `TENANT_DOMAIN_CLOUDFLARE_API_TOKEN` — required only when selected. A
  ...
  `awcms_micro_tenant_domains`, `awcms_micro_module_settings`, or any other

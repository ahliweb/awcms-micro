 ...
 * `awcms_micro_tenant_domains` (migration 031). Every query here runs inside
 ...
 * function from migration 033 (`awcms_micro_resolve_tenant_domain_lookup`),
 * which is reserved for the anonymous, pre-tenant-context public resolver
 * (Issue #559) per the epic's own binding rule (skill
 * `awcms-micro-tenant-domain-routing` §Aturan lintas-issue #10).
 *
 ...
 * `awcms_micro_tenant_domains_normalized_hostname_dedup` to a generic 409,
...
export async function updateTenantDomain(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_tenant_domains
 ...
 * (`awcms_micro_tenant_domains_primary_dedup`, `tenant_id WHERE is_primary
 ...
 * (`awcms_micro_tenant_domains_primary_dedup`) at commit time. That's caught
...
export async function setPrimaryTenantDomain(
  ...
  try {
    ...
    if (message.includes("awcms_micro_tenant_domains_primary_dedup")) {

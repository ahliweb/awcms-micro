...
Implementasi Issue 6.1 (`docs/awcms-micro/06_github_issues_detail.md` §Issue 6.1 — Add Sync Outbox and Inbox), Issue 6.2 (§Issue 6.2 — Add Sync Conflict Tracking and Resolution), dan Issue 6.3 (§Issue 6.3 — Add R2 Object Sync Queue).

## Scope — Issue 6.1 (Outbox/Inbox)

- `awcms_micro_sync_nodes` — registrasi node sync per tenant (`node_code` unik per tenant), status active/inactive, checkpoint (`last_pull_sequence`), `last_pushed_at`/`last_pulled_at`.
- `awcms_micro_sync_outbox` — event lokal yang tersedia untuk di-pull node lain, `sequence` (identity, monoton) jadi cursor checkpoint.
- `awcms_micro_sync_inbox` — event yang diterima dari node lain via push, disimpan status `received` (belum ada modul domain untuk benar-benar "menerapkan" event-nya pada base ini).
- `awcms_micro_sync_push_batches` — ledger idempotency per `(tenant_id, node_id, batch_id)`; push dengan `batch_id` yang sama diulang dianggap sukses tanpa memroses ulang event.
- Endpoint `POST /api/v1/sync/push`, `POST /api/v1/sync/pull`, `GET /api/v1/sync/status`.

Skema ada di `sql/007_awcms_micro_sync_storage_outbox_inbox_schema.sql`.

## Scope — Issue 6.2 (Conflict Tracking)

- `awcms_micro_sync_aggregate_versions` — versi terakhir yang diketahui server untuk tiap `(aggregate_type, aggregate_id)`, dipakai evaluator konflik optimistic-concurrency generik (tidak butuh pengetahuan domain apa pun tentang aggregate-nya).
- `awcms_micro_sync_conflicts` — catatan konflik **immutable** (fakta inti — node, batch, aggregate, tipe konflik, payload — tidak pernah diubah setelah dibuat; hanya kolom resolusi yang diisi **sekali** saat resolve). Dua tipe konflik generik:
...
Skema ada di `sql/008_awcms_micro_sync_storage_conflict_schema.sql`.
...
- `requires_upload` diisi dari env var `AWCMS_MICRO_R2_ENABLED` (di-rename dari `R2_ENABLED`; nama lama masih dibaca sebagai fallback selama migrasi) saat enqueue: `AWCMS_MICRO_R2_ENABLED=true` → objek **memang butuh** di-upload ke object storage; `AWCMS_MICRO_R2_ENABLED=false` (default) → objek tetap diantre (dicatat sebagai fakta lokal/checksum) tapi tidak menandai kebutuhan upload nyata. Saat Issue 6.3 ini diimplementasikan, enqueue sendiri tidak memicu pemanggilan R2/Cloudflare SDK apa pun — `AWCMS_MICRO_R2_ENABLED` di sini hanya flag data, bukan trigger jaringan (ADR-0006: provider eksternal opsional, tidak boleh jadi dependency alur kritikal). **Diperbarui Issue #436**: pemanggilan R2 nyata sekarang ada, tapi tetap bukan di jalur enqueue ini — dispatcher terpisah (§Dispatcher di bawah) yang memanggilnya, di luar transaksi, sesuai ADR-0006.
...
Skema ada di `sql/009_awcms_micro_object_sync_queue_schema.sql`.

## Domain logic

`domain/sync-hmac.ts` (pure, murni) — `computeSyncSignature` (`HMAC-SHA256("<timestamp>.<body>")`, sesuai skill `awcms-micro-sync-hmac`/doc 10 §Sync HMAC standard), `verifySyncSignature` (timing-safe compare via `node:crypto.timingSafeEqual`), `isTimestampWithinSkew` (anti-replay, default maksimum skew 300 detik — `AWCMS_MICRO_SYNC_MAX_SKEW_SEC`).
...
Endpoint sync **tidak** memakai bearer token/session — ini komunikasi node-ke-server (machine-to-machine), bukan user login. Autentikasi memakai HMAC (`X-AWCMS-Micro-Node-ID`, `X-AWCMS-Micro-Timestamp`, `X-AWCMS-Micro-Signature`) dengan secret tunggal dari environment (`AWCMS_MICRO_SYNC_HMAC_SECRET`) — bukan secret per-node, sesuai `.env.example` (satu secret untuk seluruh deployment). Header `X-AWCMS-Micro-Tenant-ID` tetap wajib untuk isolasi tenant.

Endpoint menolak (403) jika `AWCMS_MICRO_SYNC_ENABLED` bukan `true` — mengaktifkan env var yang sudah ada di `.env.example` sejak Issue 0.1 namun belum pernah benar-benar dibaca kode.
...
- **Audit event resolusi konflik** (menutup gap "Belum tersedia" di bawah yang sudah usang): `POST /sync/conflicts/{id}/resolve` kini juga memanggil `recordAuditEvent` (`resourceType: "sync_conflict"`, `action: "approve"`) — sebelumnya jejak resolusi hanya melekat di baris `sync_conflicts` itu sendiri, tidak pernah masuk `awcms_micro_audit_events` walau tabel itu sudah ada sejak Issue 10.1.
...
Menutup gap "dispatcher upload R2 nyata" di atas. **Fungsi internal, bukan endpoint HTTP publik** — hanya worker terpercaya yang boleh menandai `awcms_micro_object_sync_queue` sebagai `sent`/`failed`, bukan node sync mana pun (sama seperti sebelumnya, hanya sekarang ada implementasinya).
...
- Penerapan otomatis event `awcms_micro_sync_inbox` ke tabel domain (base tidak punya modul domain untuk diterapkan — event tetap `received`, aplikasi turunan yang memprosesnya).

 ...
 * `awcms_micro_sync_nodes`, rather than sorting/limiting the joined result.
 * Measured during Issue #435's performance audit (EXPLAIN ANALYZE against a
 * 200k-row seeded queue, migration 017's new
 * `awcms_micro_object_sync_queue_tenant_created_idx` /
 ...
 * `awcms-micro-performance` §Pagination keyset) lets `GET
      ...
      ? await tx`
        ...
        FROM (
          SELECT id, node_id, object_key, status, retry_count, next_retry_at,
                 ...
                 tenant_id
          FROM awcms_micro_object_sync_queue
        ...
        JOIN awcms_micro_sync_nodes n ON n.id = q.node_id AND n.tenant_id = q.tenant_id

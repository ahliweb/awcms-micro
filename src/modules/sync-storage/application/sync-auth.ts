...
export function verifySyncHeaders(
  ...
  if (process.env.AWCMS_MICRO_SYNC_ENABLED !== "true") {
    return {
      ok: false,
      status: 403,
      code: "ACCESS_DENIED",
  ...
  const secret = process.env.AWCMS_MICRO_SYNC_HMAC_SECRET;
  ...
  const maxSkewSeconds = Number(
    process.env.AWCMS_MICRO_SYNC_MAX_SKEW_SEC ?? DEFAULT_MAX_SKEW_SECONDS
  );
...
export async function resolveOrRegisterSyncNode(
  ...
  const existing = await tx`
    SELECT id, status FROM awcms_micro_sync_nodes

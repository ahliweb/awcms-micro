 ...
 * trusted internal worker may transition `awcms_micro_object_sync_queue` rows
    ...
    async (tx) => {
      const rows = uploadBreakerOpen
        ? await tx`
            ...
            WHERE id IN (
              SELECT id FROM awcms_micro_object_sync_queue
 ...
 * `awcms_micro_object_sync_queue` entries for a single tenant. Safe to call

...
export async function createChannel(
  ...
  const identifierRows = await tx`
    SELECT id FROM awcms_micro_profile_identifiers
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_profile_channels
...
export async function listChannels(
  ...
  const rows = (await tx`
    SELECT id, profile_id, profile_identifier_id, channel_type, is_opt_in, is_default,
      verified_at, valid_from, valid_until, created_at, updated_at
    FROM awcms_micro_profile_channels
...
export async function softDeleteChannel(
  ...
  const rows = await tx`
    UPDATE awcms_micro_profile_channels

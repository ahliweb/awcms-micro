...
/** Reference-impact preview: how many `awcms_micro_profile_entity_links` rows, grouped by owning module/entity type, would be repointed from `profileId` to a survivor. Read-only — safe to call for a preview without executing anything. */
export async function computeReferenceImpactSummary(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_profile_entity_links
...
export async function fetchMergeRequestById(
  ...
  const rows = (await tx`
    SELECT id, source_profile_id, target_profile_id, status, reason,
      requires_approval, requested_by, decided_by, decided_at, executed_by, executed_at,
      field_conflict_snapshot, reference_impact_snapshot, duplicate_candidate_id, created_at, updated_at
    FROM awcms_micro_profile_merge_requests
...
export async function decideMergeRequest(
  ...
  const updatedRows = (await tx`
    UPDATE awcms_micro_profile_merge_requests
...
export async function executeMergeRequest(
  ...
  const repointedRows = await tx`
    UPDATE awcms_micro_profile_entity_links AS src
    SET profile_id = ${target.id}
    WHERE src.tenant_id = ${tenantId} AND src.profile_id = ${source.id}
      AND NOT EXISTS (
        SELECT 1 FROM awcms_micro_profile_entity_links tgt
  ...
  await tx`
    DELETE FROM awcms_micro_profile_entity_links
  ...
  await tx`
    INSERT INTO awcms_micro_profile_merge_history

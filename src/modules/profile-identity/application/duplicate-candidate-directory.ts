...
async function upsertCandidate(
  ...
  await tx`
    INSERT INTO awcms_micro_profile_duplicate_candidates
    ...
    ON CONFLICT (tenant_id, profile_id_a, profile_id_b) DO UPDATE SET
      ...
      updated_at = now()
    WHERE awcms_micro_profile_duplicate_candidates.status = 'pending'
  `;
}
 ...
 * request-scoped avoids adding a new `awcms_micro_worker` grant/role
...
export async function generateDuplicateCandidatesForProfile(
  ...
  const profileRows = (await tx`
    SELECT id, display_name FROM awcms_micro_profiles
  ...
  const deterministicMatchRows = (await tx`
    ...
    FROM awcms_micro_profile_identifiers AS mine
    JOIN awcms_micro_profile_identifiers AS other
    ...
    JOIN awcms_micro_profiles AS other_profile
...
export async function listDuplicateCandidates(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_profile_duplicate_candidates
...
export async function reviewDuplicateCandidate(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_profile_duplicate_candidates

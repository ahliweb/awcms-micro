  ...
  dataLifecycle: [
    {
      ...
      tableName: "awcms_micro_form_drafts",
      ...
      requiredIndexes: [
        {
          columns: ["tenant_id"],
          purpose:
            "awcms_micro_form_drafts_tenant_idx (migration 019) — sufficient for this descriptor's dry-run count query; the authoritative purge query path is owned by form_drafts' own awcms_micro_form_drafts_tenant_expiry_idx (tenant_id, status, expires_at)."
        }
      ],
      batchLimit: 5000,
      backupRestoreNotes:
        "Included in ordinary full-database backup/restore (docs/awcms-micro/resilience-dr-verification.md); no standalone archive artifact exists (archive.archivable is false above) — by design, draft scratch state is not archived.",

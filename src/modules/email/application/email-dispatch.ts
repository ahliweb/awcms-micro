 ...
 *    (`awcms_micro_tenants`, no per-message override yet — #496/#497 can add
 ...
 *    failure — is recorded in `awcms_micro_email_delivery_attempts`.
 *
  ...
  return withTenant(
    ...
    async (tx) => {
      const rows = await tx`
        ...
        WHERE id IN (
          SELECT id FROM awcms_micro_email_messages
...
/** `awcms_micro_tenants` has no RLS (it's the root table, not tenant-scoped data — same reasoning `object-sync-dispatch.ts` relies on) — queried directly, no `withTenant` needed. Falls back to `DEFAULT_RENDER_LOCALE` if the tenant row is somehow missing (defensive; should never happen for an active tenant). */
...
async function recordDeliveryAttempt(
  ...
  await withTenant(
    ...
    tenantId,
    (tx) => tx`
      INSERT INTO awcms_micro_email_delivery_attempts
 ...
 * `awcms_micro_email_delivery_attempts` row is written (that table's
 ...
 * `awcms_micro_email_messages` entries for a single tenant. Safe to call

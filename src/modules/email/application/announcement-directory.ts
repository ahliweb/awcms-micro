 ...
 * Reuses `awcms_micro_email_messages` (`sql/020`) exactly as the password
 ...
 * and always excludes anyone on `awcms_micro_email_suppression_list`
...
export async function resolveAnnouncementTargets(
  ...
  if (target.type === "tenant") {
    rows = (await tx`
      ...
      FROM awcms_micro_tenant_users tu
      JOIN awcms_micro_identities i
    ...
    rows = (await tx`
      ...
      JOIN awcms_micro_tenant_users tu

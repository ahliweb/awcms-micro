 ...
 * (`awcms_micro_email_suppression_list`) and its `suppression.{read,create,
...
export async function fetchSuppressedRecipientHashes(
  ...
  const rows = (await tx`
    SELECT recipient_hash FROM awcms_micro_email_suppression_list
...
export async function listSuppressions(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_email_suppression_list
    WHERE tenant_id = ${tenantId}
...
export async function createSuppression(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_email_suppression_list
...
export async function deleteSuppression(
  ...
  const rows = (await tx`
    DELETE FROM awcms_micro_email_suppression_list

...
announcements) menulis baris `awcms_micro_email_messages` (`sql/020`) di
...
`sent`/`retry_wait`/`failed` + mencatat `awcms_micro_email_delivery_attempts`.
...
`sql/021` mengubah tiga kolom body `awcms_micro_email_templates`
...
- **Rendering aman** — `domain/email-template-render.ts` (Issue #495's
  ...
  `default_locale` tenant (`awcms_micro_tenants`, tanpa override per-pesan
...
- **CRUD + restore + preview** — `application/email-template-directory.ts`
  ...
  `awcms_micro_email_messages`/antrean — murni pratinjau.
...
"tenant"}` (seluruh tenant). Setiap target di-resolve hanya identity
  **aktif** dan **tidak** ada di `awcms_micro_email_suppression_list`
...
- **Kategori baru** `system.maintenance` ditambahkan (allowlist:
  ...
  ada sejak Issue #498.
- **Event AsyncAPI** `awcms-micro.email.message.{queued,sent,failed}` —
...
(`asyncapi/awcms-micro-domain-events.asyncapi.yaml`) — contract-only, same
"structured logger is the producer" convention as
`awcms-micro.database.pool.saturated` (no live pub/sub bus in this repo).
...
`awcms_micro_audit_events` row (`recordAuditEvent`): template CRUD/restore
...
[`../../../docs/awcms-micro/production-readiness.md`](../../../docs/awcms-micro/production-readiness.md).

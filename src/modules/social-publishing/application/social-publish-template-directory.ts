...
/**
 * CRUD for `awcms_micro_social_publish_templates` (Issue #643). Gated by the
...
export async function createSocialPublishTemplate(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_social_publish_templates
...
export async function fetchSocialPublishTemplateById(
  ...
  const rows = (await tx`
    SELECT id, tenant_id, provider_key, name, caption_template, is_default,
      is_active, created_at, updated_at
    FROM awcms_micro_social_publish_templates
...
export async function updateSocialPublishTemplate(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_social_publish_templates
...
export async function fetchActiveCaptionTemplate(
  ...
  const rows = (await tx`
    SELECT caption_template FROM awcms_micro_social_publish_templates

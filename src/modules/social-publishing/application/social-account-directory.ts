...
/**
 * Read/write query module for `awcms_micro_social_accounts` (Issue #643).
 ...
 * (`awcms_micro_social_accounts_identity_key`). Reconnecting an
...
export async function connectSocialAccount(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_social_accounts
...
export async function updateSocialAccountAutoPublish(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_social_accounts

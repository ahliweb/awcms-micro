...
terhubung. Lihat `.claude/skills/awcms-micro-social-publishing/SKILL.md`
...
(`awcms_micro_social_publishing_settings.auto_publishing_enabled`, default
...
- **#647** — dokumentasi/SOP lintas-provider, butuh #643-#646 semua ada.
  **Selesai** (PR #756, merged) — 5 dokumen di
  `docs/awcms-micro/news-portal/`: `social-sharing.md`,
...
## Tables (migration `053_awcms_micro_social_publishing_schema.sql`)

Enam tabel, semuanya tenant-scoped dengan `ENABLE`+`FORCE ROW LEVEL
SECURITY` dan policy `tenant_isolation` standar:

- **`awcms_micro_social_accounts`** — satu baris per (tenant, provider,
...
- **`awcms_micro_social_publish_rules`** — satu rule per (account,
...
- **`awcms_micro_social_publish_templates`** — caption template opsional
  per tenant (referensi polymorphic dari `rules.template_id`, tanpa FK —
  sama alasan `awcms_micro_news_media_objects.owner_resource_id`, supaya
  template bisa soft-delete tanpa pelanggaran FK; render-time fallback ke
  caption default). Soft delete standar.
- **`awcms_micro_social_publish_jobs`** — outbox-nya sendiri. `article_id`
  adalah FK **nyata** ke `awcms_micro_blog_posts` (`blog_content` selalu
...
- **`awcms_micro_social_publish_attempts`** — append-only, satu baris per
...
- **`awcms_micro_social_publishing_settings`** — master switch per-tenant
  (`auto_publishing_enabled`, PK `tenant_id`).

## Permission seed (migration `053_awcms_micro_social_publishing_schema.sql`, `055_awcms_micro_social_publishing_verify_permission.sql`)
...
3. **FINALIZE** — satu transaksi pendek per baris menerapkan hasil:
   ...
   `awcms_micro_social_publish_attempts`.
...
## Domain events (`asyncapi/awcms-micro-domain-events.asyncapi.yaml`)

17 event dipublish, channel `awcms-micro.social-publishing.*`:
...
- Error dari provider disanitasi (`errorCode`/`errorMessage` aman untuk
  log/audit) sebelum masuk `awcms_micro_social_publish_attempts` atau
...
- `.claude/skills/awcms-micro-social-publishing/SKILL.md` — status penuh

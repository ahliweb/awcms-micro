...
AWCMS-Micro already has several resource-specific retention/purge jobs
...
- **Own another module's table.** Per ADR-0013 §6 ("no shared-table
  write"), `data_lifecycle` never writes to `awcms_micro_audit_events`,
  `awcms_micro_visit_events`, or any other module's table directly. It
...
- **Assert one universal legal retention period.** Every descriptor
  ...
  `docs/awcms-micro/data-lifecycle.md` §Pemetaan kepatuhan for why.
...
dataLifecycle: [
  {
    ...
    tableName: "awcms_micro_audit_events",
...
- `"delegated"` descriptors: a dry-run snapshot only, recorded to
  `awcms_micro_data_lifecycle_runs` — never mutated.
...
(`awcms_micro_data_lifecycle_archive_manifests`: location, row count,
...
Restore procedure: see `docs/awcms-micro/data-lifecycle.md` §Restore
...
## Schema (migration `057_awcms_micro_data_lifecycle_schema.sql`)

Four tenant-scoped tables (`ENABLE`+`FORCE ROW LEVEL SECURITY`) — this
module owns exactly these, never another module's table:

- **`awcms_micro_data_lifecycle_legal_holds`** — the one genuine runtime/
  tenant override this system needs.
- **`awcms_micro_data_lifecycle_cursors`** — bounded-job pause/resume
  state per (tenant, descriptor, phase).
- **`awcms_micro_data_lifecycle_archive_manifests`** — archive artifact
  evidence.
- **`awcms_micro_data_lifecycle_runs`** — dry-run/archive/purge execution
...
`awcms_micro_worker` grants are narrow and explicit (migration 057's own
tail): `SELECT` only on legal holds (the worker reads holds, never
creates/releases them — that stays an admin/API action via
`awcms_micro_app`), full DML on cursors/manifests/runs. `awcms_micro_app`
...
## Permission seed (migration `058_awcms_micro_data_lifecycle_permissions.sql`)
...
4. Update `docs/awcms-micro/data-lifecycle.md` §Pemetaan kepatuhan and

...
export const reportingModule = defineModule({
  ...
  dependencies: [
    ...
    // `awcms_micro_domain_events` directly (read-only, same posture
  ...
  reportingProjections: [
    {
      ...
      scope: "tenant",
      description:
        "All-time allow/deny/total ABAC decision counts, incrementally derived from awcms_micro_abac_decision_logs (append-only — every decision is logged once and never updated/deleted, the ideal cursor_table source). Wraps a subset of the existing live GET /api/v1/reports/access-audit view (that endpoint's separate 30-day WINDOWED counts are unaffected and remain live-aggregated — a sliding time window is not safely expressible as a monotonic increment-only cursor stream without day-bucketing, deliberately out of scope for this issue's demonstration).",
      source: {
        strategy: "cursor_table",
        streams: [
          {
            streamKey: "abac_decision_logs",
            tableName: "awcms_micro_abac_decision_logs",
      ...
      retentionClass:
        "Not separately registered with data_lifecycle in this issue — this projection's own tables (awcms_micro_reporting_projection_*) are small per-tenant aggregate counters/cursors, not a high-volume table in the sense that registry targets.",
      batchLimit: 2000
    },
    {
      ...
      source: {
        ...
        streams: [
          ...
          },
          {
            ...
            tableName: "awcms_micro_sync_nodes",
            cursorColumn: "created_at",
            metrics: [
              {
    ...
    },
    {
      ...
      scope: "tenant",
      description:
        "New (not a wrap of an existing report) event-driven demonstration projection: counts awcms-micro.domain-event-runtime.sample.recorded domain events as they are delivered, via a registered domain_event_runtime consumer (Issue #742) rather than polling — proves the event-consumer incremental-update pathway end-to-end, reusing domain_event_runtime's own shared jobs/locks/batching/idempotency/retry/pause-resume machinery instead of building a second one. Rebuild recomputes directly from awcms_micro_domain_events (the authoritative outbox table), never by re-triggering delivery — see application/projection-rebuild.ts's header comment.",
      source: {
        ...
        events: [
          ...
          {
            eventType: "awcms-micro.domain-event-runtime.sample.recorded",
            eventVersion: "1.0"
          }
        ],
        ...
        streams: [
          {
            ...
            tableName: "awcms_micro_domain_events",
            ...
            metrics: [
              {
                ...
                matchValue: "awcms-micro.domain-event-runtime.sample.recorded"
              }
            ]
          }
        ]

 ...
 * `awcms_micro_reporting_projection_metrics`. Every value is a
...
export async function getProjectionMetrics(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_reporting_projection_metrics
...
export async function applyMetricDeltas(
  ...
  for (const { metricKey, delta } of deltas) {
    ...
    await tx`
      INSERT INTO awcms_micro_reporting_projection_metrics
      ...
      ON CONFLICT (tenant_id, projection_key, metric_key) DO UPDATE SET
        metric_value = GREATEST(awcms_micro_reporting_projection_metrics.metric_value + ${delta}, 0),
        updated_at = now()
    `;
  }
}

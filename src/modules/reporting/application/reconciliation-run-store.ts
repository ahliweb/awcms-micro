 ...
 * `awcms_micro_reporting_reconciliation_runs`. Each row is a snapshot
...
export async function recordReconciliationRun(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_reporting_reconciliation_runs
...
export async function listReconciliationRuns(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_reporting_reconciliation_runs

 ...
 * `awcms_micro_reporting_scheduled_exports`. A tenant-scoped, soft-deletable
...
export async function createScheduledExport(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_reporting_scheduled_exports
...
export async function listScheduledExports(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_reporting_scheduled_exports
...
export async function listDueScheduledExports(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_reporting_scheduled_exports se
    WHERE se.tenant_id = ${tenantId} AND se.enabled = true AND se.deleted_at IS NULL
      AND NOT EXISTS (
        SELECT 1 FROM awcms_micro_reporting_export_runs er
...
export async function disableScheduledExport(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_reporting_scheduled_exports

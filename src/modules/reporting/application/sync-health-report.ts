 ...
 * read-aggregation over `awcms_micro_sync_nodes` (migration 007),
 * `awcms_micro_sync_conflicts` (migration 008), and
 * `awcms_micro_object_sync_queue` (migration 009) — no new tables.
 *
...
export async function fetchSyncHealthReport(
  ...
  const conflictRows = await tx`
    ...
    FROM awcms_micro_sync_conflicts

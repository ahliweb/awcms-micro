 ...
 * `awcms_micro_reporting_projection_state`. Raw facts only; the actual
...
export async function getProjectionState(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_reporting_projection_state
...
export async function recordProjectionSuccess(
  ...
  await tx`
    INSERT INTO awcms_micro_reporting_projection_state
...
export async function recordProjectionFailure(
  ...
  await tx`
    ...
    ON CONFLICT (tenant_id, projection_key) DO UPDATE SET
      last_attempt_at = now(),
      consecutive_failures = awcms_micro_reporting_projection_state.consecutive_failures + 1,

 ...
 * read-aggregation over `awcms_micro_abac_decision_logs` (migration 005) and
 * `awcms_micro_profile_audit_logs` (migration 003) — no new tables.
 *
...
export async function fetchAccessAuditReport(
  ...
  const profileAuditRows = await tx`
    ...
    FROM awcms_micro_profile_audit_logs
    WHERE tenant_id = ${tenantId}
  `;

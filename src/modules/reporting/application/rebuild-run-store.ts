 ...
 * `awcms_micro_reporting_rebuild_runs`. Migration 069's partial unique
...
export async function findRunningRebuild(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_reporting_rebuild_runs
...
export async function createRebuildRun(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_reporting_rebuild_runs
...
export async function addRebuildRowsProcessed(
  ...
  await tx`
    UPDATE awcms_micro_reporting_rebuild_runs

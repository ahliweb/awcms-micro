 ...
 * (Issue #753) — `awcms_micro_reporting_projection_cursors`. Shared by the
...
export async function getStreamCursor(
  ...
  const rows = (await tx`
    SELECT cursor_value
    FROM awcms_micro_reporting_projection_cursors
...
export async function upsertStreamCursor(
  ...
  await tx`
    INSERT INTO awcms_micro_reporting_projection_cursors

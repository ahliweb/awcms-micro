...
`.claude/skills/awcms-micro-visitor-analytics/SKILL.md` for the full
cross-issue context.

## Why a separate module

AWCMS-Micro already has `reporting` (management reporting views) and
...
(`src/modules/index.ts`) so it syncs into `awcms_micro_modules` via
...
Issue #618 (`sql/039_awcms_micro_visitor_analytics_schema.sql`): adds
`awcms_micro_visitor_sessions`, `awcms_micro_visit_events`, and
`awcms_micro_visitor_daily_rollups` — tenant-scoped, `ENABLE`+`FORCE ROW
...
`awcms_micro_visitor_sessions`/`awcms_micro_visit_events`.**
...
behavior. While disabled, no `awcms_micro_visitor_key` cookie is ever set
...
## Permission seed (migration `038_awcms_micro_visitor_analytics_permissions.sql`, Issue #617)
...
## Schema (migration `039_awcms_micro_visitor_analytics_schema.sql`, Issue #618)
...
`awcms_micro_audit_events`.

- **`awcms_micro_visitor_sessions`** — one row per visitor presence
...
- **`awcms_micro_visit_events`** — one row per page-view/API call
...
- **`awcms_micro_visitor_daily_rollups`** — pre-aggregated daily stats,
...
- **`human-classifier.ts`** — `classifyHumanStatus()` (tri-state
  `human`/`bot`/`unknown` for `awcms_micro_visit_events.human_status`: bot
  ...
  `is_human`/`bot_reason` for `awcms_micro_visitor_sessions`, which has no
...
The only writer of `awcms_micro_visitor_sessions`/`awcms_micro_visit_events`.
...
`collector.ts` never reads it, and neither `awcms_micro_visitor_sessions`
nor `awcms_micro_visit_events` (migration 039) has a raw-user-agent
...
- **Visitor cookie** (`domain/visitor-key-cookie.ts`, revised by the
  Issue #624 repository audit addendum): `awcms_micro_visitor_key`,
...
directly from raw `awcms_micro_visit_events`/`awcms_micro_visitor_sessions`
rows, not `awcms_micro_visitor_daily_rollups`. This was true by necessity
...
`awcms_micro_visitor_sessions.country_code`/`region`/`city`/`timezone`
and `awcms_micro_visit_events.geo` (jsonb `{countryCode, region, city,
...
`bun run analytics:rollup` aggregates `awcms_micro_visit_events` into
`awcms_micro_visitor_daily_rollups`, one row per `(tenant, date, area)`,
...
- **An area with zero events for a date gets no row** — not a zero-value
  row, matching the underlying `awcms_micro_visit_events` table itself.
...
`docs/awcms-micro/visitor-analytics.md`.
...
- `docs/awcms-micro/visitor-analytics.md` (new) — operational guide:
  ...
  (previously 2019).
- `docs/awcms-micro/18_configuration_env_reference.md` §Visitor
...
- `docs/awcms-micro/deployment-profiles.md` — §Visitor analytics section
...
- `docs/awcms-micro/20_threat_model_security_architecture.md` — §Standar
...
- `docs/awcms-micro/04_erd_data_dictionary.md` — retention table rows and

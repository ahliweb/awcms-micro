 ...
 * #617-#624). The only writer of `awcms_micro_visitor_sessions`/
 * `awcms_micro_visit_events` — `src/middleware.ts` is the sole caller,
 ...
 * `.claude/skills/awcms-micro-visitor-analytics/SKILL.md`): `identityId`
...
async function upsertVisitorSession(
  ...
  const existingRows = (await tx`
    SELECT id, last_seen_at FROM awcms_micro_visitor_sessions
...
/**
 * Writes one `awcms_micro_visit_events` row and creates/refreshes the
 * matching `awcms_micro_visitor_sessions` row. Never throws — see the

 ...
 * transaction (`withTenant`) — RLS on `awcms_micro_visitor_sessions`/
 * `awcms_micro_visit_events` is defense in depth on top of the explicit
 ...
 * `awcms_micro_visitor_daily_rollups` — the rollup job (#624) hasn't
...
export async function fetchRealtimeStats(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_visitor_sessions
...
export async function fetchSecurityView(
  ...
  const reasonRows = (await tx`
    ...
    FROM awcms_micro_visit_events e
    JOIN awcms_micro_visitor_sessions s ON s.id = e.visitor_session_id

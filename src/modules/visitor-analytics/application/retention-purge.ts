 ...
 *   1. `awcms_micro_visit_events` older than `eventRetentionDays` — hard
 *      deleted.
 *   2. `awcms_micro_visitor_sessions.ip_address`/`login_identifier_snapshot`
 ...
 *      should be gone).
 *   3. `awcms_micro_visitor_sessions` rows older than `eventRetentionDays`
 ...
 *      `awcms_micro_visit_events.visitor_session_id`'s FK (no `ON DELETE`
 ...
 *   4. `awcms_micro_visitor_daily_rollups` older than
 ...
 * (`awcms_micro_visit_events`) is this module's registered "delegated"
...
export async function purgeVisitorAnalyticsData(
  ...
  const deletedSessions = await tx`
    DELETE FROM awcms_micro_visitor_sessions s
    WHERE s.tenant_id = ${tenantId} AND s.last_seen_at < ${eventCutoff}
      AND NOT EXISTS (
        SELECT 1 FROM awcms_micro_visit_events e
  ...
  const deletedRollups = await tx`
    DELETE FROM awcms_micro_visitor_daily_rollups

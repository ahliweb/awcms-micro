...
/**
 * Retention/purge for `awcms_micro_audit_events` (Issue #447). Doc 04
 ...
 * `awcms_micro_audit_events` rows older than the retention cutoff, and —
 ...
 *
 * Age-only cutoff, no cascading delete: `awcms_micro_audit_events` (migration
  ...
  const purgedCount = await withTenant(
    ...
    async (tx) => {
      ...
      const deleted = (await tx`
        DELETE FROM awcms_micro_audit_events
        WHERE id IN (

...
Implementasi Issue 10.1 (`docs/awcms-micro/06_github_issues_detail.md` §Issue 10.1 — Add Structured Logging and Audit Trail).

## Scope

- `awcms_micro_audit_events` — tabel audit trail generik lintas modul (append-only, tenant-scoped, RLS). Berbeda dari dan **melengkapi** dua tabel audit sempit yang sudah ada sebelumnya:
  - `awcms_micro_profile_audit_logs` (migration 003) — hanya lifecycle profile.
  - `awcms_micro_abac_decision_logs` (migration 005) — hanya keputusan allow/deny ABAC.

  Tabel ini adalah sink umum untuk aksi high-risk apa pun di modul mana pun (soft delete/restore/purge, login, access assignment, price change, transaksi posted/cancel/return, stock adjustment, transfer, ekspor Coretax, resolusi konflik sync, pemanggilan tool AI, keputusan security readiness) — sesuai `docs/awcms-micro/03_srs_detail_per_modul.md` dan skill `awcms-micro-audit-log`.

  Skema ada di `sql/011_awcms_micro_audit_logging_schema.sql`. Index `(tenant_id, created_at DESC)` untuk listing terbaru dan `(tenant_id, resource_type, resource_id)` untuk lookup per-resource.
...
Untuk membuktikan pipeline audit bekerja nyata (bukan hanya diklaim), Issue ini menambahkan **tiga endpoint lifecycle tipis** pada `awcms_micro_profiles` (lihat `src/modules/profile-identity/README.md` §Lifecycle endpoints untuk detail) — bukan profile CRUD penuh, yang tetap backlog.
...
- Retensi/purge `awcms_micro_audit_events`: lihat §Scope di atas.

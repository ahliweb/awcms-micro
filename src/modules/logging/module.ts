...
export const loggingModule = defineModule({
  ...
  status: "active",
  description:
    "Cross-module audit trail (awcms_micro_audit_events), structured JSON logging, and correlation ID propagation. Complements, not replaces, domain events and per-module audit tables.",
  ...
  jobs: [
    {
      ...
      purpose:
        "Delete awcms_micro_audit_events rows past their retention cutoff for every active tenant, in bounded batches.",
  ...
  // unchanged. See `.claude/skills/awcms-micro-data-lifecycle/SKILL.md`.
  dataLifecycle: [
    {
      ...
      partition: {
        ...
        granularity: "monthly",
        rationale:
          "High insert volume, append-only, age-based purge only (no updates) — a textbook monthly range-partition candidate. Not automated by this issue (destructive migration of an existing table is explicitly out of scope) — tracked as partitioning runbook guidance for a future issue, see docs/awcms-micro/data-lifecycle.md §Partitioning policy and runbook guidance."
      },
      archive: {
        archivable: false,
        rationale:
      ...
      requiredIndexes: [
        {
          ...
          purpose:
            "awcms_micro_audit_events_tenant_created_idx (migration 011) — the same index purgeExpiredAuditEvents' own age-based DELETE already relies on."
        }
      ],
      batchLimit: 5000,
      backupRestoreNotes:
        "Included in ordinary full-database backup/restore (docs/awcms-micro/resilience-dr-verification.md); no standalone archive artifact exists yet (archive.archivable is false above).",

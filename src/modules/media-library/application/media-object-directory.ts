 ...
 * (skill `awcms-micro-audit-log`). The intermediate `pending_upload -> uploaded`
...
export async function markNewsMediaObjectUploaded(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_news_media_objects
    SET status = 'uploaded',
...
export async function purgeExpiredPendingNewsMediaObject(
  ...
  const rows = (await tx`
    DELETE FROM awcms_micro_news_media_objects
    WHERE tenant_id = ${tenantId} AND id = ${id}
...
export async function objectKeyExistsForTenant(
  ...
  const rows = (await tx`
    SELECT 1 AS exists_flag FROM awcms_micro_news_media_objects
 ...
 * `awcms_micro_blog_posts`): deleting never rewrites `status`, it only sets

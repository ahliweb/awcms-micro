 ...
 * (`awcms_micro_tenant_modules` is the only thing written here).
 */
...
async function fetchTenantModuleRows(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_tenant_modules
 ...
 * `fetchTenantModuleEntries` — no `awcms_micro_tenant_modules` row means
 * `tenantEnabled: true`.
 */
...
export async function enableTenantModule(
  ...
  // `awcms_micro_tenant_modules.module_key` has an FK to `awcms_micro_modules`
  ...
  await tx`
    INSERT INTO awcms_micro_tenant_modules
...
export async function disableTenantModule(
  ...
  // Same reasoning as `enableTenantModule` — the FK to `awcms_micro_modules`

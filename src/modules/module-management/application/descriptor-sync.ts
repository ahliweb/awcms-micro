 ...
 * upserts it into the database-backed registry (`awcms_micro_modules` +
 * `awcms_micro_module_dependencies`/`_navigation`/`_jobs`, migration 025).
 *
 ...
 * to `awcms_micro_modules` goes through: `scripts/modules-sync.ts` (CLI),
 ...
 * `awcms_micro_modules`. Build-time composition validation
 ...
 * `awcms_micro_modules.module_key` is `upsertModule`'s own `ON CONFLICT`
...
async function upsertModule(
  ...
  await sql`
    INSERT INTO awcms_micro_modules
...
async function replaceDependencies(
  ...
  await sql`
    DELETE FROM awcms_micro_module_dependencies WHERE module_key = ${descriptor.key}
  `;
...
async function replaceNavigation(
  ...
  await sql`
    DELETE FROM awcms_micro_module_navigation WHERE module_key = ${descriptor.key}
  `;

  for (const entry of descriptor.navigation ?? []) {
    await sql`
      INSERT INTO awcms_micro_module_navigation
...
async function replaceJobs(
  ...
  await sql`
    DELETE FROM awcms_micro_module_jobs WHERE module_key = ${descriptor.key}
  `;

  for (const job of descriptor.jobs ?? []) {
    await sql`
      INSERT INTO awcms_micro_module_jobs

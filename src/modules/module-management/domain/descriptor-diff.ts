 ...
 * `awcms_micro_modules` rows, then hands both to `planModuleSync` to decide
...
export type ModuleSyncPlan = {
  ...
  /** Present in `awcms_micro_modules` but no longer in `listModules()` — reported so the sync service can mark them (never delete), and so operators/tests can see drift explicitly. */

 ...
 * state from `listModules()` + `awcms_micro_tenant_modules`, hands it to
...
export const MODULE_PRESETS: readonly ModulePresetDefinition[] = [
  ...
  },
  {
    ...
    label: "News portal",
    description:
      "Online website plus the news portal and visitor analytics stack, with no R2 media requirement.",
    // Upstream AWCMS-Mini's `news_portal` preset is "online website + editorial
    // approval workflow", i.e. this same set plus `workflow`. AWCMS-Micro does
    // not port `workflow` (ADR-0025), so the preset is redefined around what
    // actually distinguishes a news portal in THIS repo — the `news_portal`
    // module and `visitor_analytics` — rather than left listing a module key
    // `applyModulePreset` could never resolve. Editorial review here is
    // blog_content's own post status/revision model, not a workflow instance.
    enabledModuleKeys: [
      "tenant_domain",
      "blog_content",
      "media_library",
  ...
  // DIFFERENT concepts — "news_portal" carries no R2/media requirement at
  // all, while "news_portal_full_online_r2" is the full-online,
  // R2-only-media editorial+news profile. Do not merge/rename either one to
  // "fix" the apparent naming overlap — a tenant using the plain
  // `news_portal` preset is not required to have any NEWS_MEDIA_R2_* config,
  // and the R2 preset's own activation is gated (see
  ...
  },
  {
    ...
    label: "SaaS (online)",
    description:
      "Multi-tenant SaaS profile: custom domain, email, reporting — no public content module.",
    enabledModuleKeys: ["tenant_domain", "email", "reporting"]
  },
  // Upstream AWCMS-Mini defines a sixth preset here, `pos_lan` — an
  // offline-capable point-of-sale profile (sync_storage + reporting +
  // workflow). AWCMS-Micro drops it (ADR-0025): a LAN-first POS terminal is
  // not a website, so it is out of this repo's scope regardless of whether
  // its module keys still resolve. A derived POS application contributes its
  // own preset alongside its own modules.
  {
    name: "minimal",
    label: "Minimal",
    description:

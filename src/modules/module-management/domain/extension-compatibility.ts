...
export function parseExtensionManifest(raw: unknown): ManifestParseResult {
  ...
  if (typeof raw.compatibleAwcmsMicroRange !== "string") {
    fail("compatibleAwcmsMicroRange", "must be a string");
  }
...
export function formatExtensionManifestIssue(
  ...
  switch (issue.type) {
    ...
    case "base_version_range_invalid":
      return `Manifest field "compatibleAwcmsMicroRange" ("${issue.range}") is not a valid SemVer range.`;
    case "base_version_range_incompatible":
      return `This AWCMS-Micro release does not satisfy the manifest's declared compatible range "${issue.declaredRange}" (actual version: ${issue.actualVersion}).`;
...
function checkBaseVersionRange(
  ...
  const result = satisfiesSemverRange(
    ...
    manifest.compatibleAwcmsMicroRange
  );
    ...
    return [
      {
        ...
        range: manifest.compatibleAwcmsMicroRange
      }
    ];
  }
  if (!result) {
    return [
      {
        type: "base_version_range_incompatible",
        declaredRange: manifest.compatibleAwcmsMicroRange,

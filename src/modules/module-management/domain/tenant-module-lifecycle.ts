 ...
 * from `listModules()` + `awcms_micro_tenant_modules` and hands it to these
 ...
 * `awcms_micro_module_dependencies` table — that table only reflects
...
/** `tenantEnabled` defaults to `true` when no `awcms_micro_tenant_modules` row exists — a module with no explicit tenant state is available by default (backward-compatible with pre-epic behavior). */

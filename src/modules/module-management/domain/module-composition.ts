 ...
 * (originally Issue #740, epic #738 `platform-evolution`, Wave 1). Validates
 * the reviewed base registry (`src/modules/index.ts`'s `listBaseModules()`)
 * into one final, validated module list. Validation is 100% compile-time
 * TypeScript — no runtime discovery, file upload, package scanning, `eval`, or
 * untrusted code loading.
 *
 * ADR-0036 removed the derived-application pathway: the `application` registry
 * parameter, the migration-namespace overlap check, the base-override /
 * reserved-module-type checks, and the `mergeModuleRegistries` seam are gone.
 * These functions now validate a single reviewed registry (the base). The
 * remaining checks are all base-load-bearing invariants that also hold when new
 * website/domain modules are added directly to `src/modules/`
 * ("template dipakai-langsung").
 *
 ...
 * is deliberately dependency-free (every module, including this one, depends ON
 * it — the contract file has always had zero imports); if this validation
 * engine lived in `_shared/` instead, it would need to import FROM
 * `module-management/domain/`, inverting that direction. `module_management` is
 * already the established owner of "registry-wide, pure, no-I/O" validation
 * (`module-dependency-graph.ts`'s own header comment, Issue #680) — this is the
 * same kind of concern, so it lives alongside it. Only `src/modules/index.ts`
 * (the composition root, which already legitimately imports every module) and
 * CLI scripts import this file.
 *
 * ## Every composition issue class (see `ModuleCompositionIssue`)
 *
 * Reused as-is from the whole-registry DAG validator (Issue #680):
 * `self_dependency`, `duplicate_dependency`, `missing_dependency`, `cycle`.
 * Plus:
 ...
 *   string in their own `capabilities.provides` — ambiguous which module a
 *   consumer's `providedBy` actually resolves to.
 * - `capability_provider_missing` — a REQUIRED (`optional` not `true`)
 *   `capabilities.consumes` entry names a `providedBy` module that either isn't
 *   in the registry at all, or IS but doesn't declare that capability in its
 *   own `provides`. An `optional: true` entry is never checked — ADR-0011's
 *   whole point is that the consumer degrades safely when it resolves to "not
 *   applicable".
 ...
 *   — claiming to run somewhere a required dependency cannot.
 * - `navigation_path_conflict` — two navigation entries (same or different
 *   modules) declare the exact same `path`.
 * - `invalid_job_descriptor` — reuses `./job-registry.ts`'s
 *   `validateJobDescriptor` against every module's `jobs` array.
 *
 * No issue type exists for permission catalogs or health descriptors beyond
 * what's covered above — `ModuleHealthContract` is two optional booleans with
 * no shape to validate, and permission entries are inherently namespaced by
 * their own module's `key` (`comparePermissions`'s
 * `${moduleKey}.${activityCode}.${action}`, `domain/permission-sync.ts`) so
 * cross-module collisions are structurally impossible. Both still flow into
 * `buildComposedModuleInventory` below so the deterministic inventory reflects
 * every module's permission and health metadata.
 */
...
/**
 * The full rule engine. Reports EVERY distinct issue across the whole registry
 * in one pass (never stops at the first), same philosophy
...
/**
 * Deterministic, machine-readable module inventory for CI/release evidence. No
 * wall-clock timestamp embedded — same "pure function of committed source"
 * freshness design `scripts/repo-inventory-generate.ts` documents for its own
 * output, enforced the same way (`scripts/module-composition-inventory-
 * check.ts`'s regenerate-and-diff gate). `modules` is sorted by `key` so two
 * runs against the same input always produce byte-identical JSON, independent

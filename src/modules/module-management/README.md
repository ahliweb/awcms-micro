...
management capability. `awcms_micro_modules` (migration 001) had existed
...
1. Reads the current `awcms_micro_modules` rows.
2. Computes a plan (`domain/descriptor-diff.ts`'s `planModuleSync` — pure,
   ...
   `unchanged`.
3. Upserts `awcms_micro_modules` for every `create`/`update` entry.
4. Fully replaces (`DELETE` then `INSERT`) each module's
   `awcms_micro_module_dependencies`/`_navigation`/`_jobs` rows from its
...
5. Any `awcms_micro_modules` row whose `module_key` is no longer in
...
- **`fetchTenantModuleEntry(tx, tenantId, moduleKey)`** — single-module
  ...
  `awcms_micro_tenant_modules` row means `tenantEnabled: true`).
...
tenant-scoped operational preferences only (`awcms_micro_module_settings`,
...
`awcms_micro_permissions` catalog rows for that module —
...
- **The "optional safe sync action" the issue mentions is deliberately not
  ...
  `awcms_micro_modules`/`_dependencies`/`_navigation`/`_jobs` from
  `listModules()`, but never touches `awcms_micro_permissions` — extending
...
`listModules()` (never `awcms_micro_module_navigation` — same reasoning as
...
- Hidden if the tenant has disabled that module
  (`awcms_micro_tenant_modules.enabled = false`).
...
— same reasoning as the navigation registry (#518): `awcms_micro_module_jobs`
...
- `news_portal` — `news-media:reconcile` (Issue #690, epic #679,
  platform-hardening — reconciles `awcms_micro_news_media_objects`
...
- `social_publishing` — `social-publishing:dispatch` (Issue #643, epic
  ...
  `awcms_micro_social_publish_jobs`; no-op unless `SOCIAL_PUBLISHING_ENABLED`
...
`docs/awcms-micro/deploy-coolify.md` §Dispatcher terjadwal (email) — not
duplicated here.
...
- `db_registry_synced` — the `awcms_micro_modules` row's `lifecycle_status`
  matches the descriptor's own `status`.
- `migrations_applied` — every `sql/*.sql` file has a matching
  `awcms_micro_schema_migrations` row. A **local, minimal re-listing** of
...
**`awcms_micro_module_health_checks` (migration 025) is written only by
...
`detail` text). That table has an FK to `awcms_micro_modules`, so `POST`
...
- **Detail**: dependency panel, tenant enable/disable action, settings
  ...
  over `awcms_micro_audit_events` scoped to the module-management actions
...
(`online_website`, `news_portal`, `news_portal_full_online_r2`, `saas_online`, `minimal`), so
...
`online_website`/`news_portal`/`saas_online`) depends on
...
**Security**: never writes `awcms_micro_tenant_modules` directly — every
change goes through the real `enableTenantModule`/`disableTenantModule`.
Never touches `awcms_micro_roles`/`awcms_micro_role_permissions`/
`awcms_micro_access_assignments` — a preset changes module _availability_
...
## Module-registry composition — `domain/module-composition.ts` (Issue #740; ADR-0036)

Validates the reviewed **base** registry as a whole. ADR-0036 removed the
derived-application pathway (the `application-registry.ts` seam, migration
namespace 900-999, `mergeModuleRegistries`, and `extension:check`) — AWCMS-Micro
is a template used directly, so a new website/domain module is added straight to
`src/modules/index.ts`. The composition engine now validates the single reviewed
registry the same way it would validate that registry with a new module in it.
...
- **`validateComposedModuleRegistry(registry)` / `composeModuleRegistry(registry)`**
  ...
  (`scripts/validate-module-composition.ts`, wired into `bun run check`), never
  embedded in module load. Each takes a `readonly ModuleDescriptor[]`. Reuses
  `domain/module-dependency-graph.ts`'s whole-registry DAG validator (Issue #680)
  and `domain/job-registry.ts`'s `validateJobDescriptor` rather than duplicating
  either. Full issue taxonomy (duplicate module key, conflicting/missing
  capability provider, incompatible deployment-profile claim, navigation path
  conflict, invalid job descriptor, plus the four reused DAG issues) is
  documented in `module-composition.ts`'s own file header. It is a strict
  superset of `bun run modules:dag:check`.
- **Every consumer of the registry calls `listModules()`** (`bun run
modules:sync`, `bun run modules:dag:check`, `bun run repo:inventory:generate`,
  and every `module-management` application service above), which returns the
  reviewed base registry — so a new module added to `src/modules/` automatically
  flows through all of them with zero code changes.
- **`bun run modules:composition:inventory:generate`/`:check`**
  (`docs/awcms-micro/module-composition-inventory.json`) — a deterministic,
  ...
  evidence, same freshness-gate pattern as `docs/awcms-micro/repo-inventory.md`.
- **`tests/fixtures/example-domain-modules/`** is a minimal, in-repo test-support
  fixture of example domain modules — consumed only by
  `tests/unit/module-composition-fixture.test.ts` to prove the composition engine
  validates domain modules; never registered in the base registry.

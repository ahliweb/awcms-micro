 ...
 * `.claude/skills/awcms-micro-module-management/SKILL.md`'s §Dependency
 * graph section for the full reasoning.
 *
 * Extracted verbatim from the route handler — same SQL, same order, same
 * transaction, same idempotency lock (`awcms_micro_setup_state`'s
...
export async function bootstrapPlatformTenant(
  ...
  const claimed = await tx`
    INSERT INTO awcms_micro_setup_state (id, locked_at)
    VALUES (true, now())
  ...
  await tx`
    INSERT INTO awcms_micro_tenant_settings (tenant_id) VALUES (${tenantId})
  `;

  const officeRows = await tx`
    INSERT INTO awcms_micro_offices (tenant_id, office_code, office_name, office_type)
  ...
  await tx`
    UPDATE awcms_micro_setup_state SET tenant_id = ${tenantId} WHERE id = true
  `;

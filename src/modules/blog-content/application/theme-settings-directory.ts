...
/**
 * `awcms_micro_blog_theme_settings` (Issue #542 §Theme Mode). Absence of a
 ...
 * `awcms_micro_blog_settings` — so this reads `awcms_micro_tenants.
...
export async function fetchBlogThemeSettings(
  ...
  const overrideRows = (await tx`
    SELECT mode FROM awcms_micro_blog_theme_settings WHERE tenant_id = ${tenantId}
  ...
  const tenantRows = (await tx`
    SELECT default_theme FROM awcms_micro_tenants WHERE id = ${tenantId}
...
/** Upsert — one row per tenant, same shape `awcms_micro_blog_settings` uses. */
export async function upsertBlogThemeSettings(
  ...
  await tx`
    INSERT INTO awcms_micro_blog_theme_settings (tenant_id, mode)

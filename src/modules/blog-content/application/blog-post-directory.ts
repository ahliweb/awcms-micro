...
/**
 * Read/write query module for `awcms_micro_blog_posts` (Issue #537 scaffolded
...
export type ListBlogPostsForAdminFilter = {
  search?: string;
  status?: BlogContentStatus;
  /** Matches posts assigned this category/tag term id (via `awcms_micro_blog_post_terms`). */
  termId?: string;
  page?: number;
  pageSize?: number;
};
 ...
 * `awcms_micro_blog_post_terms` rather than a `JOIN`, so a post with several
  ...
  const rows = (await tx`
    ...
    WHERE p.tenant_id = ${tenantId} AND p.deleted_at IS NULL
      ...
      AND (
        ${termId}::uuid IS NULL
        OR EXISTS (
          SELECT 1 FROM awcms_micro_blog_post_terms pt
...
/**
 * Hard delete. `awcms_micro_blog_post_terms` rows for this post are deleted
 * first — they are pure join metadata with no independent meaning once the
 * post is gone, unlike `awcms_micro_blog_revisions` (no FK to the post,
...
export async function purgeBlogPost(
  ...
  await tx`
    DELETE FROM awcms_micro_blog_post_terms
    WHERE tenant_id = ${tenantId} AND post_id = ${id}
  `;

  const rows = await tx`
    DELETE FROM awcms_micro_blog_posts

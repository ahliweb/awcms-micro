...
/** Read/write query module for `awcms_micro_blog_widgets` (Issue #542) — same "one directory, reads and writes" convention as `blog-taxonomy-directory.ts`. */
...
export async function createWidget(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_blog_widgets
...
export async function updateWidget(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_blog_widgets

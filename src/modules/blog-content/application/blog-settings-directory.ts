/**
 * `awcms_micro_blog_settings` read/write (Issue #543 §Settings Page) — one
 * row per tenant, same upsert convention `theme-settings-directory.ts`
 * (Issue #542) uses for `awcms_micro_blog_theme_settings`. `blogTitle`,
...
export async function upsertBlogSettings(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_blog_settings

/**
 * Public (anonymous) read queries for `awcms_micro_blog_posts` (Issue
 ...
 * `awcms_micro_blog_post_terms`, excludes soft-deleted terms (same
...
export async function fetchPublicPostTaxonomyTerms(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_blog_post_terms pt
    JOIN awcms_micro_blog_terms t
...
export async function listPublicBlogPostsByTermId(
  ...
  const rows = (await tx`
    ...
    JOIN awcms_micro_blog_post_terms pt
...
/** Reads `awcms_micro_blog_settings` (Issue #537), falling back to schema defaults when the tenant has never configured a row — same "missing row = default" convention `resolveModuleEnabled` uses for `awcms_micro_tenant_modules`. */
export async function fetchPublicBlogSettings(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_blog_settings
    WHERE tenant_id = ${tenantId}
  `) as {

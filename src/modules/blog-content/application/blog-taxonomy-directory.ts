...
/**
 * Read/write query module for `awcms_micro_blog_terms` and
 * `awcms_micro_blog_post_terms` (Issue #537 scaffolded this as a read-only
...
export async function createBlogTerm(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_blog_terms
...
export async function fetchPostTermIds(
  ...
  const rows = (await tx`
    SELECT term_id FROM awcms_micro_blog_post_terms
    WHERE tenant_id = ${tenantId} AND post_id = ${postId}
  `) as { term_id: string }[];
...
export async function countExistingTerms(
  ...
  const rows = (await tx`
    SELECT count(*)::int AS count FROM awcms_micro_blog_terms

...
/** Read/write query module for `awcms_micro_blog_ads`/`_ad_placements` (Issue #542) — same "one directory, reads and writes" convention as `blog-taxonomy-directory.ts`. */
...
export async function createAd(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_blog_ads
...
export async function syncAdPlacements(
  ...
  await tx`
    DELETE FROM awcms_micro_blog_ad_placements
  ...
  for (const placement of placements) {
    const rows = (await tx`
      INSERT INTO awcms_micro_blog_ad_placements (tenant_id, ad_id, placement_type, target_id)
...
export async function fetchAdPlacements(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_blog_ad_placements
    WHERE tenant_id = ${tenantId} AND ad_id = ${adId}
  `) as BlogAdPlacementRow[];
...
export async function listActiveAdsForPlacement(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_blog_ads a
    JOIN awcms_micro_blog_ad_placements p

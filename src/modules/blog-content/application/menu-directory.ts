...
/** Read/write query module for `awcms_micro_blog_menus`/`_menu_items` (Issue #542) — same "one directory, reads and writes" convention as `blog-taxonomy-directory.ts`. */
...
export async function createMenu(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_blog_menus (tenant_id, key, name)
...
export async function syncMenuItems(
  ...
  await tx`
    DELETE FROM awcms_micro_blog_menu_items
  ...
  for (const item of [...roots, ...children]) {
    const rows = (await tx`
      INSERT INTO awcms_micro_blog_menu_items
...
export async function fetchMenuItems(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_blog_menu_items
    WHERE tenant_id = ${tenantId} AND menu_id = ${menuId}

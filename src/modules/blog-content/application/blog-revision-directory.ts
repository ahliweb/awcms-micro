...
/**
 * Read/write query module for `awcms_micro_blog_revisions` (Issue #541).
 ...
 * `awcms_micro_workflow_decisions`/`awcms_micro_audit_events` already use.
 */
...
export async function createBlogRevision(
  ...
  const rows = (await tx`
    ...
    VALUES (
      ...
      COALESCE(
        (SELECT MAX(revision_number) FROM awcms_micro_blog_revisions
...
export async function listBlogRevisions(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_blog_revisions
    WHERE tenant_id = ${tenantId} AND resource_type = ${resourceType}
      AND resource_id = ${resourceId}

...
/** Read/write query module for `awcms_micro_blog_templates` (Issue #542) — same "one directory, reads and writes" convention as `blog-taxonomy-directory.ts`. */
...
export async function createTemplate(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_blog_templates
...
export async function fetchTemplateById(
  ...
  const rows = (await tx`
    SELECT id, tenant_id, key, name, layout_json, is_active,
      created_at, updated_at, deleted_at, deleted_by, delete_reason
    FROM awcms_micro_blog_templates
...
export async function updateTemplate(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_blog_templates

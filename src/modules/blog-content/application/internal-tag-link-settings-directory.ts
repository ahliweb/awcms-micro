/**
 * `awcms_micro_blog_internal_tag_link_settings` read/write (Issue #641) —
 * one row per tenant, same upsert convention `theme-settings-directory.ts`
 * uses for `awcms_micro_blog_theme_settings`. A tenant that never
...
export async function fetchInternalTagLinkingSettings(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_blog_internal_tag_link_settings
...
export async function upsertInternalTagLinkingSettings(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_blog_internal_tag_link_settings

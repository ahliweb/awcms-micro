 ...
 * `docs/awcms-micro/deployment-profiles.md` ever adds a profile —
...
export type ExtensionManifestApplication = {
  ...
  key: string;
  /** The derived application's OWN release SemVer — never compared against `compatibleAwcmsMicroRange` (that range is this REPOSITORY's version, not the derived app's own). */
  version: string;
  name?: string;
};
...
export type ExtensionCompatibilityManifest = {
  ...
  compatibleAwcmsMicroRange: string;

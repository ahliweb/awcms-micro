 ...
 * doc 16 §Idempotency store, skill `awcms-micro-idempotency`). Backed by the
 * generic `awcms_micro_idempotency_keys` table (migration 012), first
...
export async function findIdempotencyRecord(
  ...
  const rows = await tx`
    ...
    FROM awcms_micro_idempotency_keys

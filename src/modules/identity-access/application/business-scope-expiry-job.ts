 ...
 * assignment gets an `awcms_micro_business_scope_assignment_events` row
 ...
 * `awcms_micro_audit_events` row per expired assignment when a backlog is
  ...
  return withTenant(
    ...
    async (tx) => {
      const expiredRows = (await tx`
        UPDATE awcms_micro_business_scope_assignments
      ...
      for (const row of expiredRows) {
        await tx`
          INSERT INTO awcms_micro_business_scope_assignment_events
...
async function expireSoDConflictExceptionsPass(
  ...
  return withTenant(
    ...
    async (tx) => {
      const expiredRows = (await tx`
        UPDATE awcms_micro_sod_conflict_exceptions
        SET status = 'expired', updated_at = now()
        WHERE id IN (
          SELECT id FROM awcms_micro_sod_conflict_exceptions
  ...
  await withTenant(
    ...
    async (tx) => {
      const rows = (await tx`
        ...
        FROM awcms_micro_business_scope_assignments
        WHERE tenant_id = ${tenantId} AND status = 'active'
        GROUP BY scope_type
      `) as {
        scope_type: string;
 ...
 * both tables are `FORCE ROW LEVEL SECURITY`'d and `awcms_micro_worker`'s
...
async function countExpiredBacklogForTenant(
  ...
  return withTenant(
    ...
    async (tx) => {
      const rows = (await tx`
        SELECT
          (SELECT count(*) FROM awcms_micro_business_scope_assignments
            WHERE tenant_id = ${tenantId} AND status = 'active'
              AND effective_to IS NOT NULL AND effective_to <= ${now}) AS assignments,
          (SELECT count(*) FROM awcms_micro_sod_conflict_exceptions

 ...
 * (`awcms_micro_auth_providers`, migration 036) instead of Google's hardcoded
...
export async function createSsoOAuthRequest(
  ...
  await tx`
    INSERT INTO awcms_micro_oidc_auth_requests
...
export async function consumeSsoOAuthRequest(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_oidc_auth_requests
  ...
  const consumedRows = (await tx`
    UPDATE awcms_micro_oidc_auth_requests
    SET consumed_at = ${now}
...
export async function findIdentityByProviderSubject(
  ...
  const rows = (await tx`
    SELECT identity_id FROM awcms_micro_identity_provider_accounts
...
export async function autoLinkByEmailForProvider(
  ...
  const identityRows = (await tx`
    SELECT id, status FROM awcms_micro_identities
  ...
  const tenantUserRows = (await tx`
    SELECT status FROM awcms_micro_tenant_users
  ...
  await tx`
    INSERT INTO awcms_micro_identity_provider_accounts
...
export async function unlinkProviderAccount(
  ...
  const deletedRows = (await tx`
    DELETE FROM awcms_micro_identity_provider_accounts
...
export async function completeTenantSsoCallback(
  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    const identityRows = (await tx`
      SELECT status FROM awcms_micro_identities WHERE id = ${identityId}
    `) as { status: string }[];
    const tenantUserRows = (await tx`
      SELECT status FROM awcms_micro_tenant_users

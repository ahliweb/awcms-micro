 ...
 * real caller that enqueues into `awcms_micro_email_messages` (`sql/020`).
...
export async function requestPasswordReset(
  ...
  const identityRows = (await tx`
    ...
    JOIN awcms_micro_profiles p ON p.id = i.profile_id
  ...
  await tx`
    UPDATE awcms_micro_password_reset_tokens
    SET used_at = ${now}
  ...
  await tx`
    INSERT INTO awcms_micro_password_reset_tokens (tenant_id, identity_id, token_hash, expires_at)
...
export async function completePasswordReset(
  ...
  const tokenRows = (await tx`
    ...
    FROM awcms_micro_password_reset_tokens

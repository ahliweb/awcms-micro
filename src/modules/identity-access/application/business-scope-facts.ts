 ...
 *   real scope) and the ordinary `awcms_micro_access_assignments` →
 *   `awcms_micro_role_permissions` path (`resolveOrdinaryRbacFacts` below —
 ...
 * `awcms_micro_permissions`) — the exact same path
...
async function resolveOrdinaryRbacFacts(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_access_assignments aa
    JOIN awcms_micro_role_permissions rp ON rp.role_id = aa.role_id AND rp.tenant_id = aa.tenant_id
    JOIN awcms_micro_permissions p ON p.id = rp.permission_id
...
export async function resolveSoDAssignmentFacts(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_business_scope_assignments bsa
    JOIN awcms_micro_role_permissions rp

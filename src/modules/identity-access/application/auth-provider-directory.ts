 ...
 * epic: full-online auth hardening). `awcms_micro_auth_providers` (migration
...
export async function listAuthProviders(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_auth_providers
...
export async function createAuthProvider(
  ...
  const existingRows = await tx`
    SELECT id FROM awcms_micro_auth_providers
  ...
  const countRows = (await tx`
    SELECT count(*)::int AS count FROM awcms_micro_auth_providers
    WHERE tenant_id = ${tenantId} AND deleted_at IS NULL
  `) as { count: number }[];

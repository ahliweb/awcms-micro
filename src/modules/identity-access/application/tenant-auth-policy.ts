 ...
 * hardening) over `awcms_micro_tenant_auth_policies` (migration 036) — same
...
export async function getTenantAuthPolicy(
  ...
  const rows = (await tx`
    ...
    FROM awcms_micro_tenant_auth_policies
 ...
 * and has an `active` `awcms_micro_tenant_users` membership. Every identity

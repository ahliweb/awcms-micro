 ...
 * `awcms_micro_offices` (owned by `tenant_admin`) — a FLAT resolution (no
 * ancestors/descendants; `awcms_micro_offices.parent_office_id` exists in
 ...
 *
 * Reading `awcms_micro_offices` directly here (a `tenant_admin`-owned
 ...
 * `awcms_micro_tenants` (also owned by `tenant_admin`) directly via raw SQL

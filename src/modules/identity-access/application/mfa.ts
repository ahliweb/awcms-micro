...
export async function startTotpEnrollment(
  ...
  const activeRows = await tx`
    SELECT id FROM awcms_micro_identity_mfa_factors
    WHERE tenant_id = ${tenantId} AND identity_id = ${identityId} AND status = 'active'
  `;
  ...
  await tx`
    DELETE FROM awcms_micro_identity_mfa_factors
...
export async function disableMfa(
  ...
  await tx`
    DELETE FROM awcms_micro_identity_mfa_recovery_codes
...
export async function createMfaChallenge(
  ...
  await tx`
    INSERT INTO awcms_micro_mfa_challenges (tenant_id, identity_id, challenge_token_hash, expires_at)
...
export async function verifyMfaChallenge(
  ...
  const challengeRows = (await tx`
    ...
    FROM awcms_micro_mfa_challenges
  ...
  if (!factor) {
    ...
    await tx`
      UPDATE awcms_micro_mfa_challenges SET consumed_at = ${now} WHERE id = ${challenge!.id}
    `;
    return { ok: false, code: "MFA_CHALLENGE_INVALID" };
  }
  ...
  if (credentials.code) {
    ...
    const consumedRows = (await tx`
      UPDATE awcms_micro_identity_mfa_recovery_codes
      SET used_at = ${now}
  ...
  if (!matched) {
    await tx`
      UPDATE awcms_micro_mfa_challenges

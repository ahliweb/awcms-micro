...
export type AccessAction =
  ...
  // lifecycle history, `awcms_micro_business_scope_assignment_events`).
  ...
  // holds an `awcms_micro_profile_entity_links` reference, so a role
  ...
  // never touches `awcms_micro_reference_codes`), so a role granted

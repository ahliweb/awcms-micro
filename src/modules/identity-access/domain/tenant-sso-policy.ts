...
/** Validates the admin `POST /api/v1/identity/sso/providers` body. Exactly one of `clientSecret`/`clientSecretEnvVar` must be provided — mirrors the DB CHECK constraint on `awcms_micro_auth_providers`, checked here first so a bad request gets a clean `400`, not a raw constraint-violation error. */

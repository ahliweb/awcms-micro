...
Implementasi Issue 2.3 (`docs/awcms-micro/06_github_issues_detail.md` §Issue 2.3 — Add Identity Login and Tenant User Membership) dan Issue 2.4 (§Issue 2.4 — Add RBAC and ABAC Access Control).

## Scope — Issue 2.3 (Identity & Login)

- `awcms_micro_identities` — login identity per tenant (`login_identifier` unik per tenant), `password_hash` (Bun native `Bun.password` — argon2id), lockout (`failed_login_count`, `locked_until`).
- `awcms_micro_tenant_users` — keanggotaan identity pada tenant (status active/inactive — dicek terpisah dari status identity).
- `awcms_micro_sessions` — token sesi opaque (bukan JWT stateless): hanya `token_hash` yang disimpan, token mentah cuma dikirim sekali saat login berhasil. `expires_at`/`revoked_at` untuk logout dan kedaluwarsa.
- Endpoint `POST /api/v1/auth/login`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/me`.

Skema ada di `sql/004_awcms_micro_identity_login_schema.sql`.

## Scope — Issue 2.4 (RBAC/ABAC)

- `awcms_micro_permissions` — katalog global `module_key.activity_code.action` (bukan tenant-scoped, tanpa RLS — daftar kemampuan yang bisa diberikan, sama untuk semua tenant). Diseed 17 permission untuk modul base yang sudah ada (`tenant_admin`, `identity_access`, `profile_identity`) — **bukan** matriks 26+ permission domain retail/POS doc 17 (katalog, POS, gudang, pajak, CRM, AI dikeluarkan; itu scope aplikasi turunan).
- `awcms_micro_roles` — role per tenant (dibuat runtime, tidak diseed lewat migration — role Owner/Admin/dsb. adalah tanggung jawab Setup Wizard Issue 12.1, generik, bukan 10 role retail doc 17).
- `awcms_micro_role_permissions` — matriks role → permission per tenant.
- `awcms_micro_access_assignments` — assignment role ke tenant user.
- `awcms_micro_abac_policies` — tabel policy ABAC per tenant (schema tersedia; belum ada policy row yang diseed — evaluator base memakai aturan generik bawaan, lihat di bawah).
- `awcms_micro_abac_decision_logs` — append-only, setiap panggilan `/access/evaluate` dan setiap guard endpoint (`assignments`, `decision-logs`) mencatat keputusan.
...
Skema ada di `sql/005_awcms_micro_abac_access_control_schema.sql`.
...
Dan sekali lagi untuk `enable`/`disable` (Issue #515, epic #510) — migrasi 025 sudah menyeed `module_management.tenant_modules.{enable,disable}`, konsumen pertamanya `POST /api/v1/tenant/modules/{moduleKey}/{enable,disable}` di issue ini. Keduanya **tidak** ditambahkan ke `HIGH_RISK_ACTIONS` — toggle ketersediaan modul per-tenant bersifat reversibel dan tidak menghapus data tenant (`disable` hanya menulis baris `awcms_micro_tenant_modules`, tidak pernah menyentuh data modul lain), berbeda dari `delete`/`purge` yang ireversibel. Tetap diaudit eksplisit (`action: "tenant_module_enabled"`/`"tenant_module_disabled"`) terlepas dari klasifikasi itu.
...
- `login.ts` sekarang juga men-set dua cookie httpOnly + `SameSite=Lax` + `Path=/`: `awcms_micro_session` (raw session token) dan `awcms_micro_tenant_id`, dengan `maxAge` sama dengan `AUTH_SESSION_TTL_MIN` (menit → detik). Body JSON response (`{ token, expiresAt }`) **tidak berubah** — klien bearer-token lama tetap kompatibel.
- `logout.ts` menerima tenant/token dari header (`X-AWCMS-Micro-Tenant-ID` + `Authorization: Bearer`, perilaku lama tidak berubah) **atau**, bila header tidak ada, dari kedua cookie tersebut — fallback ini dibutuhkan karena skrip klien tidak bisa membaca cookie httpOnly untuk menyusun header `Authorization` secara manual. Setelah revoke sukses, kedua cookie dihapus (`cookies.delete`).
...
meng-enqueue baris `awcms_micro_email_messages` (kategori
...
- **Skema** — `awcms_micro_password_reset_tokens` (`sql/022`), mirip
  `awcms_micro_sessions` tapi dengan `used_at` untuk single-use (sesi tidak
...
di bawah). Lihat skill `awcms-micro-auth-online-hardening` untuk detail
lintas-issue.
...
- Login (`login.ts`): password-valid TAPI identity punya factor TOTP
  ...
  `awcms_micro_mfa_challenges` row dan balas `401 MFA_REQUIRED` berisi
...
- **Tenant id lewat `state`, bukan header**: `GET .../callback` adalah
  ...
  membawa header `X-AWCMS-Micro-Tenant-ID`. `state` yang dikirim ke
...
- **Login vs link, dua flow berbeda**: `GET .../start` (dari tombol
  ...
  `awcms_micro_oidc_auth_requests` yang tersimpan saat start/link.
...
- **Provider account ditautkan via `sub`, TIDAK PERNAH via email** —
  `awcms_micro_identity_provider_accounts` (unique per tenant+provider+subject
...
- Detail lengkap + rasional lintas-issue: skill
  `awcms-micro-auth-online-hardening` §Google OIDC login.
...
* **Menggeneralisasi Google OIDC (#590) tanpa mengubahnya**: jalur ini
  ...
  sebelumnya. SSO generik memakai ulang `awcms_micro_oidc_auth_requests`/
  `awcms_micro_identity_provider_accounts` (migration 035, sudah generik
  `provider text` sejak awal — lihat komentar migration itu) dengan
  `provider = <providerKey>` alih-alih `'google'`.
* **Skema baru (migration 036)**: `awcms_micro_auth_providers` (tenant
  ...
  `awcms_micro_tenant_auth_policies` (satu baris per tenant —
...
* **Auto-link by email**, dua lapis fail-closed: `provider.allowed_email_domains`
  ...
  semata.
* Detail lengkap + rasional lintas-issue: skill
  `awcms-micro-auth-online-hardening` §Generic tenant OIDC SSO provider.
...
`docs/awcms-micro/18_configuration_env_reference.md` §Full-online auth
security hardening, `docs/awcms-micro/deployment-profiles.md` §Full-online
...
- Detail lengkap + rasional lintas-issue (termasuk kuirk
  ...
  skill `awcms-micro-auth-online-hardening` §Admin policy UI.
...
- `awcms_micro_business_scope_assignments` — one row = one tenant_user
...
- `awcms_micro_business_scope_assignment_events` — append-only lifecycle
  history (granted/revoked/expired/renewed).
- `awcms_micro_sod_conflict_exceptions` — temporary exception/override
...
- `awcms_micro_sod_conflict_evaluations` — append-only SoD conflict
  decision log, recorded regardless of outcome (mirrors
  `awcms_micro_abac_decision_logs`'s "append-always" convention).
...
assignments, recording to `awcms_micro_sod_conflict_evaluations`
...
least-privilege `awcms_micro_worker` access in `sql/061` (`SELECT`/`UPDATE`
on assignments and exceptions, `SELECT`/`INSERT` on the assignment-events
history — no access to `awcms_micro_sod_conflict_evaluations`, which the
worker never writes).

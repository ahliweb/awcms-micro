...
export const identityAccessModule = defineModule({
  ...
  // No `capabilities.consumes` entry in AWCMS-Micro. Upstream Mini declares an
  // optional `organization_hierarchy_resolution` consumption here, provided
  // by its ERP-scope `organization_structure` module; this website-scope repo
  // does not port that module (ADR-0025), so the entry is removed rather than
  // left pointing at a provider that no registry entry can ever satisfy.
  //
  // Nothing else changes: the consumption was ALREADY `optional: true` — Core
  // must never hard-fail when an Optional module is absent (ADR-0013 §1) —
  // and it was deliberately never a `dependencies` edge. So business-scope
  // resolution simply always takes the fallback it was already designed to
  // take: identity-access's own flat adapter, wired at the route composition
  // root (`api/v1/identity/business-scope/hierarchy-port-composition.ts`).
  // See `_shared/ports/business-scope-hierarchy-port.ts` for the full "why a
  // port, not an import" rationale, which still governs any derived app that
  // registers a real hierarchy provider.

...
/**
 * Read/write query module for `awcms_micro_news_portal_homepage_sections`
...
export async function createHomepageSection(
  ...
  const rows = (await tx`
    INSERT INTO awcms_micro_news_portal_homepage_sections
...
export async function fetchHomepageSectionById(
  ...
  const rows = (await tx`
    SELECT id, tenant_id, section_key, section_type, title, config_json,
      sort_order, is_enabled, starts_at, ends_at, created_at, updated_at,
      deleted_at, deleted_by, delete_reason
    FROM awcms_micro_news_portal_homepage_sections
...
export async function updateHomepageSection(
  ...
  const rows = (await tx`
    UPDATE awcms_micro_news_portal_homepage_sections

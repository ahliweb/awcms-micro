 ...
 * scenario is not duplicated for R2 (see `docs/awcms-micro/
...
const BASE_ENV = {
  ...
  EMAIL_FROM_NAME: "AWCMS-Micro DR Drill"
  ...
  return {
    ...
    async run(ctx): Promise<ScenarioOutcome> {
      ...
      try {
        ...
        // `awcms_micro_tenants` has no RLS (root table, not tenant-scoped
        ...
        // Reviewer finding on PR #716: `awcms_micro_email_templates`/
        // `awcms_micro_email_messages` both have `FORCE ROW LEVEL SECURITY`
        // (sql/020_awcms_micro_email_schema.sql) requiring `app.current_
        ...
        // is genuinely correct against the least-privilege `awcms_micro_app`
        // role too, not just accidentally passing under CI's role.
        await withTenant(
          sql,
          tenantId,
          (tx) => tx`
            INSERT INTO awcms_micro_email_templates
        ...
        const afterFirst = (await withTenant(
          ...
          tenantId,
          (tx) =>
            tx`SELECT status FROM awcms_micro_email_messages WHERE id = ${messageId}`,
          { workClass: "maintenance" }
        )) as { status: string }[];
        ...
        await withTenant(
          ...
          tenantId,
          (tx) => tx`
            UPDATE awcms_micro_email_messages
        ...
        const deliveryAttemptCountRows = (await withTenant(
          ...
          (tx) => tx`
            ...
            FROM awcms_micro_email_delivery_attempts
        ...
        // awcms_micro_tenants row does not (see the setup comment above).
        await withTenant(
          sql,
          tenantId,
          (tx) =>
            tx`DELETE FROM awcms_micro_email_delivery_attempts WHERE tenant_id = ${tenantId}`,
          { workClass: "maintenance" }
        ).catch(() => undefined);
        await withTenant(
          sql,
          tenantId,
          (tx) =>
            tx`DELETE FROM awcms_micro_email_messages WHERE tenant_id = ${tenantId}`,
          { workClass: "maintenance" }
        ).catch(() => undefined);
        await withTenant(
          sql,
          tenantId,
          (tx) =>
            tx`DELETE FROM awcms_micro_email_templates WHERE tenant_id = ${tenantId}`,
          { workClass: "maintenance" }
        ).catch(() => undefined);
        await sql`DELETE FROM awcms_micro_tenants WHERE id = ${tenantId}`.catch(
          () => undefined
        );

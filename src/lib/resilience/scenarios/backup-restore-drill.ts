 ...
 *   `awcms_micro_dr_drill` disposable database (distinct from the sh
 *   script's own default `awcms_micro_restore_drill`, so a scheduled
 ...
 *   disposable `awcms_micro_dr_drill` database.
 */
...
const DRILL_TARGET_DB_PREFIX = "awcms_micro_dr_drill";
...
export function backupRestoreDrillScenario(): ScenarioDefinition {
  return {
    ...
    async run(ctx): Promise<ScenarioOutcome> {
      ...
      const backupDir = mkdtempSync(join(tmpdir(), "awcms-micro-dr-drill-"));
      const keyDir = mkdtempSync(join(tmpdir(), "awcms-micro-dr-drill-keys-"));

...
export const ERROR_CODE_KEYS: Record<string, string> = {
  ...
  // Ported from the awcms-mini base standard (Issue #840): no endpoint emits
  // this any more — `POST /auth/login` used to, but its `403
  // PASSWORD_LOGIN_DISABLED` let an unauthenticated caller enumerate
  // identifiers (and fingerprint break-glass identities), so that denial now
  // collapses into `AUTH_INVALID_CREDENTIALS`. The mapping and its catalog
  // entry are retained as vocabulary, NOT as a live code: do not wire a UI
  // branch to it, and do not take its presence here as licence to reintroduce
  // a distinguishable password-login-disabled response.

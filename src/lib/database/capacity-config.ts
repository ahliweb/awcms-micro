 ...
 *   `awcms_micro_app` role, `DATABASE_URL`. Also the ONLY process class that
 ...
 *   `scripts/work-class-registry-check.ts` uses), the `awcms_micro_worker`
 ...
 *   `docs/awcms-micro/database-capacity-runbook.md` for why job CONCURRENCY
 *   is exempted from the work-class gate while job CONNECTION BUDGET is
 *   still counted below.
 * - `setup` — `POST /api/v1/setup/initialize` only, the `awcms_micro_setup`

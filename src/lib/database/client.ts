 ...
 *   mode (doc 16, `docs/awcms-micro/database-pooling.md`).
 ...
 * to its own least-privilege Postgres role (`sql/045_awcms_micro_db_role_
 * separation.sql`): `app` -> `awcms_micro_app` (`DATABASE_URL`, unchanged
 * name/var for backward compatibility), `worker` -> `awcms_micro_worker`
 * (`WORKER_DATABASE_URL`), `setup` -> `awcms_micro_setup`
 ...
 * everything through the single narrowed `awcms_micro_app` role; operators

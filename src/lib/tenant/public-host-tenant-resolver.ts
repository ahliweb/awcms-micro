 ...
 * module never touches tenant content — only `awcms_micro_tenant_domains`
 * (via the `SECURITY DEFINER` bootstrap function, migration 033),
 * `awcms_micro_tenants` (RLS-free by design, ADR-0003/migration 013), and
 * `awcms_micro_setup_state` (RLS-free singleton, migration 006).
 *
...
export type PublicHostResolverConfig = {
   ...
   * chain (steps 2-4). This keeps the `awcms_micro_tenant_domains` bootstrap
   ...
   * `awcms_micro_setup_state.tenant_id` before giving up.
   *
...
/**
 * Fetches an `active` tenant by id from `awcms_micro_tenants` — RLS-free by
 ...
 * `SECURITY DEFINER` bootstrap function `awcms_micro_resolve_tenant_domain_lookup`
 ...
 * `awcms_micro_tenants` in the same call and returns the tenant's own
...
export async function resolvePublicTenantByHost(
  ...
  const rows = (await sql`
    ...
    FROM awcms_micro_resolve_tenant_domain_lookup(${normalizedHost})
...
/**
 * Step 4 of the resolution order: `awcms_micro_setup_state.tenant_id` — the
 * tenant chosen during the setup wizard. `awcms_micro_setup_state` is a
 * RLS-free singleton table (migration 006), same "shared by design"
 * category as `awcms_micro_tenants` (migration 013's comment), so this also
 ...
 * `.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md` and
 * `docs/awcms-micro/18_configuration_env_reference.md` — repeated here so
 ...
 * 4. `awcms_micro_setup_state.tenant_id`

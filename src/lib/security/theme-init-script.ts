...
export const THEME_INIT_SCRIPT_BODY = `(function () {
  var STORAGE_KEY = "awcms_micro_theme";
...
// hand discipline alone. Recomputed for AWCMS-Micro: the body's STORAGE_KEY is
// `awcms_micro_theme`, so upstream AWCMS-Mini's hash
// (`sha256-nLhd693cGQjNH4T21n0Bl5PrlirImH2NPjiy3gJp+5A=`) no longer matches
// these bytes. Carrying it over would have let CSP block the one script that
// prevents the theme flash — silently, and only in a real browser. That test
// is precisely what caught it.
export const THEME_INIT_SCRIPT_HASH =
  "sha256-zcPa+fEZmTcqhGZjtHlOV185GjaidqVtsCruqakjdsM=";

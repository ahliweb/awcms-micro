 ...
 * to this helper immediately; see `docs/awcms-micro/deployment-profiles.md`
...
/** Every batched job iterates only `active` tenants — mirrors every existing script's own `SELECT id FROM awcms_micro_tenants WHERE status = 'active'`. `awcms_micro_tenants` itself has no RLS (it is the root table, not tenant-scoped data), so this plain query is safe on any client. */

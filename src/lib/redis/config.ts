...
const INTEGER_RULES = {
  REDIS_CONNECTION_TIMEOUT_MS: { min: 100, max: 30_000 },
  REDIS_COMMAND_TIMEOUT_MS: { min: 50, max: 30_000 },
  REDIS_MAX_RETRIES: { min: 0, max: 20 },
  REDIS_CACHE_DEFAULT_TTL_SEC: { min: 1, max: 86_400 }
} as const;
...
export function loadRedisConfig(
  ...
  return {
    ...
    connectionTimeoutMs: parseBoundedInteger(
      ...
      INTEGER_RULES.REDIS_CONNECTION_TIMEOUT_MS.min,
      INTEGER_RULES.REDIS_CONNECTION_TIMEOUT_MS.max
    ),
    commandTimeoutMs: parseBoundedInteger(
      env.REDIS_COMMAND_TIMEOUT_MS,
      DEFAULTS.commandTimeoutMs,
      INTEGER_RULES.REDIS_COMMAND_TIMEOUT_MS.min,
      INTEGER_RULES.REDIS_COMMAND_TIMEOUT_MS.max
    ),
    maxRetries: parseBoundedInteger(
      env.REDIS_MAX_RETRIES,
      DEFAULTS.maxRetries,
      INTEGER_RULES.REDIS_MAX_RETRIES.min,
      INTEGER_RULES.REDIS_MAX_RETRIES.max
    ),
    cacheDefaultTtlSec: parseBoundedInteger(
      env.REDIS_CACHE_DEFAULT_TTL_SEC,
      DEFAULTS.cacheDefaultTtlSec,
      INTEGER_RULES.REDIS_CACHE_DEFAULT_TTL_SEC.min,
      INTEGER_RULES.REDIS_CACHE_DEFAULT_TTL_SEC.max
    )
  };
}
...
function validateBooleanLiteral(
  ...
  return {
    severity: "fail",
    code: "redis_boolean_invalid",
    message: `${name} must be either true or false.`
  };
}

function validateIntegerLiteral(
  name: keyof typeof INTEGER_RULES,
  value: string | undefined
): RedisValidationFinding | null {
  if (value === undefined || value.trim() === "") {
    return null;
  ...
  return {
    severity: "fail",
    code: "redis_integer_out_of_range",
    message: `${name} must be an integer between ${rule.min} and ${rule.max}.`
  };
}
...
export function validateRedisConfig(
  ...
  const booleanFinding = validateBooleanLiteral(
    "REDIS_ENABLED",
    env.REDIS_ENABLED
  );
  ...
  for (const name of Object.keys(INTEGER_RULES) as Array<
    keyof typeof INTEGER_RULES
  >) {
    const finding = validateIntegerLiteral(name, env[name]);
...
function encodeKeySegment(segment: string, label: string): string {
  ...
  if (normalized.length > 256) {
    throw new Error(`Redis key ${label} must not exceed 256 characters.`);
  }
...
/**
 * Builds a collision-resistant key with explicit tenant scope. Tenant-scoped
 * data must pass tenantId; `global` is reserved for genuinely cross-tenant,
 * non-sensitive platform state.
 */

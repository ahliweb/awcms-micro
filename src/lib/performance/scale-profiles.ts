 ...
 *   by design — see `docs/awcms-micro/performance-suite.md` §Safe subset vs.
 *   full lane).
 *
...
export type TableRowCounts = {
  /** `awcms_micro_audit_events` — high-risk-action audit trail. */
  auditEvents: number;
  /** `awcms_micro_abac_decision_logs` — ABAC allow/deny decision log. */
  abacDecisionLogs: number;
  /** `awcms_micro_visitor_sessions` — analytics session shape. */
  visitorSessions: number;
  /** `awcms_micro_sync_outbox` — outbox/delivery event shape. */
  syncOutbox: number;
  /** `awcms_micro_object_sync_queue` — sync/delivery queue shape (also the outbox-claim query-plan budget's driving table). */
  objectSyncQueue: number;
  /** `awcms_micro_idempotency_keys` — idempotency store shape. */
  idempotencyKeys: number;
  /** `awcms_micro_blog_posts` — representative tenant-scoped business/content table (also the full-text search query-plan budget's driving table). */
  blogPosts: number;
};

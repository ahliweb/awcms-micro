 ...
 * `docs/awcms-micro/work-class-registry.generated.json` already established
...
export const QUERY_PLAN_BUDGETS: QueryPlanBudget[] = [
  {
    ...
    description:
      "GET /api/v1/logs/audit's real query shape: tenant_id = $1 AND (created_at, id) < cursor ORDER BY created_at DESC, id DESC LIMIT 100 against awcms_micro_audit_events, run under RLS (app.current_tenant_id set) so the RLS predicate is genuinely part of the planned query.",
  ...
  },
  {
    ...
    category: "rls_pagination",
    description:
      "Representative ABAC decision-log read: tenant_id = $1 ORDER BY created_at DESC LIMIT 100 against awcms_micro_abac_decision_logs, under RLS.",
    forbiddenNodeTypes: ["Seq Scan"],
    requiredNodeTypesAny: ["Index Scan", "Index Only Scan", "Bitmap Heap Scan"],
    maxTotalCost: 500,
    maxExecutionTimeMs: 50,
  ...
  },
  {
    ...
    description:
      "audit-purge.ts's real batch shape: SELECT id FROM awcms_micro_audit_events WHERE tenant_id = $1 AND created_at < $2 ORDER BY created_at ASC LIMIT $3 (the inner SELECT feeding the batched DELETE).",
    forbiddenNodeTypes: ["Seq Scan"],
    requiredNodeTypesAny: ["Index Scan", "Bitmap Heap Scan"],
    maxTotalCost: 600,
    maxExecutionTimeMs: 50,
  ...
  },
  {
    ...
    category: "reporting",
    description:
      "Representative reporting aggregate: tenant-scoped GROUP BY severity, count(*) over awcms_micro_audit_events (module-usage/tenant-activity report shape).",
    forbiddenNodeTypes: ["Seq Scan"],
    requiredNodeTypesAny: ["Index Scan", "Index Only Scan", "Bitmap Heap Scan"],
    ...
    // `awcms_micro_audit_events` fixture tenants immediately before this
    ...
    // (the only privilege the least-privilege `awcms_micro_app` role this

 ...
 * `DATABASE_URL`, the `awcms_micro_app` role) every real endpoint uses —
 ...
 *
 * `awcms_micro_tenants` itself has no RLS policy (it is the global tenant
 ...
 * (doc skill `awcms-micro-performance` §Hindari N+1, precedent:
...
async function insertSyncNode(
  ...
  await tx`
    INSERT INTO awcms_micro_sync_nodes (id, tenant_id, node_code, node_name, status)
...
async function insertAuditEvents(
  ...
  for (const batch of chunk(rows, SEED_CHUNK_SIZE)) {
    await tx`
      INSERT INTO awcms_micro_audit_events
...
async function insertAbacDecisionLogs(
  ...
  for (const batch of chunk(rows, SEED_CHUNK_SIZE)) {
    await tx`
      INSERT INTO awcms_micro_abac_decision_logs
...
async function insertVisitorSessions(
  ...
  for (const batch of chunk(rows, SEED_CHUNK_SIZE)) {
    await tx`
      INSERT INTO awcms_micro_visitor_sessions
...
async function insertSyncOutbox(
  ...
  for (const batch of chunk(rows, SEED_CHUNK_SIZE)) {
    await tx`
      INSERT INTO awcms_micro_sync_outbox
...
async function insertObjectSyncQueue(
  ...
  for (const batch of chunk(rows, SEED_CHUNK_SIZE)) {
    await tx`
      INSERT INTO awcms_micro_object_sync_queue
...
async function insertIdempotencyKeys(
  ...
  for (const batch of chunk(rows, SEED_CHUNK_SIZE)) {
    await tx`
      INSERT INTO awcms_micro_idempotency_keys
 ...
 * synthetic performance-fixture tenants (`awcms_micro_tenants.tenant_code
 ...
 * test` (whose integration tests write/reset real `awcms_micro_audit_events`
 * rows throughout). With no isolation between any of these,
 * `awcms_micro_audit_events` had accumulated to roughly DOUBLE the row count
 ...
 * `awcms_micro_app` role is actually granted (`TRUNCATE` and `ANALYZE` both
...
export async function resetPerformanceFixtureRows(sql: Bun.SQL): Promise<void> {
  const perfTenants = (await sql`
    SELECT id FROM awcms_micro_tenants WHERE tenant_code LIKE 'perf-%'
  `) as { id: string }[];
  ...
  for (const { id: tenantId } of perfTenants) {
    await withTenant(
      ...
      tenantId,
      async (tx) => {
        await tx`DELETE FROM awcms_micro_audit_events WHERE tenant_id = ${tenantId}`;
        await tx`DELETE FROM awcms_micro_abac_decision_logs WHERE tenant_id = ${tenantId}`;
        await tx`DELETE FROM awcms_micro_visitor_sessions WHERE tenant_id = ${tenantId}`;
        await tx`DELETE FROM awcms_micro_sync_outbox WHERE tenant_id = ${tenantId}`;
        await tx`DELETE FROM awcms_micro_object_sync_queue WHERE tenant_id = ${tenantId}`;
        await tx`DELETE FROM awcms_micro_idempotency_keys WHERE tenant_id = ${tenantId}`;
        await tx`DELETE FROM awcms_micro_blog_posts WHERE tenant_id = ${tenantId}`;
      },

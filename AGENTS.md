...
- **Semantic template tokens:** upstream built-in templates now use a semantic `tokens.css` architecture. Immediate adoption in AWCMS-Micro default templates is deferred by completed issue #227 because those templates are protected downstream boundaries; future adoption requires a focused issue that preserves CMS-sourced public routes and Node/Cloudflare parity.

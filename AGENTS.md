...
>
> **Scope = website online penuh.** Bukan ERP, bukan POS. Registry base = **18 modul**: fondasi (`tenant_admin`, `profile_identity`, `identity_access`, `logging`, `module_management`), layanan platform (`sync_storage`, `media_library`, `domain_event_runtime`, `data_lifecycle`, `reporting`, `email`, `form_drafts`), website (`tenant_domain`, `blog_content`, `news_portal`, `social_publishing`, `visitor_analytics`, `seo_distribution`). Lihat `src/modules/index.ts` untuk daftar & jumlah yang selalu terkini.
>

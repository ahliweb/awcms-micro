...
> ## ⚠️ BACA INI LEBIH DULU — repo ini turunan, bukan basis
>
...
>
> **Scope = website online penuh.** Bukan ERP, bukan POS. Registry base = **19 modul**: fondasi (`tenant_admin`, `profile_identity`, `identity_access`, `logging`, `module_management`), layanan platform (`sync_storage`, `media_library`, `domain_event_runtime`, `data_lifecycle`, `reporting`, `email`, `form_drafts`), website (`tenant_domain`, `blog_content`, `news_portal`, `social_publishing`, `visitor_analytics`, `seo_distribution`, `theming`). Lihat `src/modules/index.ts` untuk daftar & jumlah yang selalu terkini.
>
...
Modul di luar scope website (mis. katalog produk, POS, gudang, pajak, akuntansi, CRM) **bukan bagian repo ini** — itu ditambahkan di aplikasi turunan lewat `application-registry.ts` (ADR-0014), atau di repo sibling `ahliweb/awcms` untuk scope ERP. Menambahkannya ke registry base di sini melanggar ADR-0025 §2.

...
then activates the least-privilege `awcms_micro_app` role (`provisionAppRole()`,
...
PGPW=awcms_micro_password
docker run -d --name it-pg -e POSTGRES_USER=awcms-micro \
  -e POSTGRES_PASSWORD="$PGPW" -e POSTGRES_DB=awcms-micro postgres:18.4
export DATABASE_URL="postgres://awcms-micro:${PGPW}@localhost:5432/awcms-micro"
...
`awcms_micro_schema_migrations` and the `awcms_micro_permissions` seed catalog).

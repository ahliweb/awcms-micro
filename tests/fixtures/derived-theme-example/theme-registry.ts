 ...
 * by `tests/unit/theme-registry-composition.test.ts` and the theming E2E —
 * never imported by the base seam itself, since the base repository's own build
 * must keep shipping `undefined` there (a default base build composes exactly
 * the base themes).
 */

...
`docs/awcms-micro/derived-application-guide.md`) forks/vendors this base
...
- [`modules/example-loyalty/module.ts`](modules/example-loyalty/module.ts) —
  ...
  Upstream AWCMS-Mini carries a third fixture module here,
  `example-erp-extension` (Issue #755, ADR-0020 — ERP extension readiness
  contracts), exercising `_shared/business-transaction-contract.ts` and
  `_shared/ports/period-lock-port.ts`. AWCMS-Micro ports neither contract
  (ADR-0025 — this is a website-scope repository), so the fixture module and
  its `sql/901` illustration table are gone with them. The extension seam
  itself is unchanged and still fully exercised by the two modules above: the
  fixture's point is registry composition, capability binding, and migration
  namespacing — none of which were ERP-specific.
...
`awcms-micro-new-module` for that). No real endpoints, no migrations, no

...
describe("module registry", () => {
  test("the 22 website-scope modules are registered, and none of upstream AWCMS-Mini's ERP-scope modules are (ADR-0025)", () => {
    ...
    // name, not as an off-by-one on a count.
    for (const erpScopeModuleKey of [
      "workflow",
      ...
      expect(getModuleByKey(erpScopeModuleKey)).toBeUndefined();
    }
...
describe("database migration runner helpers", () => {
  test("checksum is stable and prefixed", () => {
    const sql = "CREATE TABLE awcms_micro_example (id uuid PRIMARY KEY);";
  ...
  test("a DO block with BEGIN/END passes the transaction-control check", () => {
    ...
    expect(() =>
      assertNoTransactionControl(
        "DO $$\nBEGIN\n  CREATE ROLE awcms_micro_app NOLOGIN;\nEND\n$$;",
        "013_awcms_micro_enforce_rls_least_privilege.sql"
      )
    ).not.toThrow();
  ...
  test("single-quoted string literal contents are removed before scanning for transaction control", () => {
    ...
    const stripped = stripSingleQuotedStringLiterals(
      "INSERT INTO awcms_micro_permissions (module_key, activity_code, action) VALUES ('idn_admin_regions', 'dataset', 'rollback');"
    );
  ...
  test("a permission seed migration whose data literally contains the word 'rollback' passes the transaction-control check", () => {
    expect(() =>
      assertNoTransactionControl(
        ...
        "048_awcms_micro_idn_admin_regions_permissions.sql"
      )
    ).not.toThrow();
  ...
  test("applied checksum mismatch fails fast", () => {
    expect(() =>
      validateAppliedChecksums(
        [
          {
            name: "001_awcms_micro_foundation_schema.sql",
            path: "sql/001_awcms_micro_foundation_schema.sql",
            sql: "SELECT 1;",
            checksum: "sha256:new"
          }
        ],
        [
          {
            migration_name: "001_awcms_micro_foundation_schema.sql",
            checksum: "sha256:old"
          }
        ]
      )
  ...
  test("database url redaction removes password-bearing url", () => {
    const databaseUrl =
      "postgres://awcms-micro:secret-password@localhost:5432/awcms-micro";
  ...
  test("real sql/ migrations are discoverable, ordered, and transaction-control-free", async () => {
    ...
    expect(migrations.map((migration) => migration.name)).toEqual([
      ...
      "013_awcms_micro_enforce_rls_least_privilege.sql",
      "014_awcms_micro_sync_node_management_permission_schema.sql",
      "015_awcms_micro_tenant_settings_management_permission_schema.sql",
      "016_awcms_micro_tenant_default_locale_english_schema.sql",
      "017_awcms_micro_sync_queue_conflict_performance_indexes.sql",
      "018_awcms_micro_object_sync_queue_dispatcher_schema.sql",
      "019_awcms_micro_form_drafts_schema.sql",
      "020_awcms_micro_email_schema.sql",
      "021_awcms_micro_email_template_i18n_schema.sql",
      "022_awcms_micro_password_reset_schema.sql",
      "023_awcms_micro_email_announcement_permission_schema.sql",
      "024_awcms_micro_email_message_cancel_permission_schema.sql",
      "025_awcms_micro_module_management_schema.sql",
      "026_awcms_micro_blog_content_schema.sql",
      "027_awcms_micro_blog_content_permissions.sql",
      "028_awcms_micro_blog_content_search_vector.sql",
      "029_awcms_micro_blog_content_presentation_schema.sql",
      "030_awcms_micro_blog_content_presentation_permissions.sql",
      "031_awcms_micro_tenant_domain_schema.sql",
      "032_awcms_micro_tenant_domain_permissions.sql",
      "033_awcms_micro_tenant_domain_lookup_function.sql",
      "034_awcms_micro_mfa_totp_schema.sql",
      "035_awcms_micro_google_oidc_schema.sql",
      "036_awcms_micro_tenant_oidc_sso_schema.sql",
      "037_awcms_micro_tenant_oidc_sso_permissions.sql",
      "038_awcms_micro_visitor_analytics_permissions.sql",
      "039_awcms_micro_visitor_analytics_schema.sql",
      "040_awcms_micro_visitor_analytics_session_lookup_index.sql",
      "041_awcms_micro_news_media_object_registry_schema.sql",
      "042_awcms_micro_news_media_permissions.sql",
      "043_awcms_micro_news_portal_tenant_state_schema.sql",
      "044_awcms_micro_news_portal_homepage_sections_schema.sql",
      "045_awcms_micro_db_role_separation.sql",
      "046_awcms_micro_news_media_orphan_lifecycle.sql",
      "047_awcms_micro_observability_metrics_permission.sql",
      "049_awcms_micro_news_portal_ad_placements_schema.sql",
      "050_awcms_micro_blog_posts_seo_image.sql",
      "051_awcms_micro_blog_content_internal_tag_links_schema.sql",
      "052_awcms_micro_blog_content_internal_tag_links_permissions.sql",
      "053_awcms_micro_social_publishing_schema.sql",
      "055_awcms_micro_social_publishing_verify_permission.sql",
      "056_awcms_micro_domain_event_runtime_schema.sql",
      "057_awcms_micro_data_lifecycle_schema.sql",
      "058_awcms_micro_data_lifecycle_permissions.sql",
      "059_awcms_micro_profile_identity_party_lifecycle_schema.sql",
  ...
  test("rls-enforcement migration FORCEs RLS on tenant tables and creates a least-privilege app role", async () => {
    ...
    const rlsSchema = migrations.find(
      (migration) =>
        migration.name === "013_awcms_micro_enforce_rls_least_privilege.sql"
    );
    ...
    expect(rlsSchema?.sql).toContain("CREATE ROLE awcms_micro_app NOLOGIN");
    expect(rlsSchema?.sql).not.toMatch(
      /CREATE ROLE awcms_micro_app[^;]*PASSWORD/i
    );
  ...
  test("DB role separation migration creates worker/setup roles, narrows the app role on global tables, and never leaks a password", async () => {
    ...
    const roleSeparation = migrations.find(
      (migration) => migration.name === "045_awcms_micro_db_role_separation.sql"
    );
    ...
    expect(sql).toContain("CREATE ROLE awcms_micro_worker NOLOGIN");
    expect(sql).toContain("CREATE ROLE awcms_micro_setup NOLOGIN");
    expect(sql).not.toMatch(
      /CREATE ROLE awcms_micro_(worker|setup)[^;]*PASSWORD/i
    );
    ...
    expect(sql).toContain(
      "REVOKE INSERT, UPDATE, DELETE ON awcms_micro_permissions FROM awcms_micro_app"
    );
    expect(sql).toContain(
      "REVOKE INSERT, UPDATE, DELETE ON awcms_micro_schema_migrations FROM awcms_micro_app"
    );
    expect(sql).not.toMatch(
      /GRANT\s+[^;]*\b(INSERT|UPDATE|DELETE)\b[^;]*\bON\s+awcms_micro_permissions\b[^;]*TO\s+awcms_micro_app/i
    );
    expect(sql).not.toMatch(
      /GRANT\s+[^;]*\b(INSERT|UPDATE|DELETE)\b[^;]*\bON\s+awcms_micro_schema_migrations\b[^;]*TO\s+awcms_micro_app/i
    );

    // `awcms_micro_setup_state`/`awcms_micro_tenants`: only DELETE is
    ...
    expect(sql).toContain(
      "REVOKE DELETE ON awcms_micro_setup_state FROM awcms_micro_app"
    );
    expect(sql).toContain(
      "REVOKE DELETE ON awcms_micro_tenants FROM awcms_micro_app"
    );
    expect(sql).not.toMatch(
      /REVOKE\s+[^;]*\b(INSERT|UPDATE)\b[^;]*\bON\s+awcms_micro_setup_state\b[^;]*FROM\s+awcms_micro_app/i
    );

    // `awcms_micro_setup` legitimately reads `awcms_micro_permissions` (to
    ...
    for (const table of [
      ...
      expect(sql).not.toMatch(
        new RegExp(
          `GRANT\\s+[^;]*\\b(INSERT|UPDATE|DELETE)\\b[^;]*\\bON\\s+${table}\\b[^;]*TO\\s+awcms_micro_setup`,
          "i"
        )
      );
    }
    // Worker legitimately SELECTs `awcms_micro_tenants` (to iterate active
    ...
    expect(sql).not.toMatch(
      /GRANT\s+[^;]*\b(INSERT|UPDATE|DELETE)\b[^;]*\bON\s+awcms_micro_tenants\b[^;]*TO\s+awcms_micro_worker/i
    );
    for (const table of [
      "awcms_micro_permissions",
      "awcms_micro_schema_migrations",
      ...
      expect(sql).not.toMatch(
        new RegExp(
          `GRANT[^;]*\\bON\\s+${table}\\b[^;]*TO\\s+awcms_micro_worker`,
          "i"
        )
      );
    }
  ...
  test("default locale migration flips the tenant default to English without touching migration 002", async () => {
    ...
    const tenantOfficeSchema = migrations.find(
      (migration) =>
        migration.name === "002_awcms_micro_tenant_office_schema.sql"
    );

    expect(localeSchema).toBeDefined();
    expect(localeSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_tenants ALTER COLUMN default_locale SET DEFAULT 'en'"
    );
  ...
  test("performance index migration adds only indexes, no schema/data changes (Issue #435)", async () => {
    ...
    expect(perfSchema?.sql).toContain(
      "CREATE INDEX IF NOT EXISTS awcms_micro_object_sync_queue_tenant_created_idx"
    );
    expect(perfSchema?.sql).toContain(
      "CREATE INDEX IF NOT EXISTS awcms_micro_object_sync_queue_tenant_status_created_idx"
    );
    expect(perfSchema?.sql).toContain(
      "CREATE INDEX IF NOT EXISTS awcms_micro_object_sync_queue_tenant_node_created_idx"
    );
    expect(perfSchema?.sql).toContain(
      "CREATE INDEX IF NOT EXISTS awcms_micro_sync_conflicts_tenant_created_idx"
    );
  ...
  test("tenant/office schema declares RLS and soft-delete on office-scoped tables", async () => {
    ...
    expect(tenantOfficeSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_offices ENABLE ROW LEVEL SECURITY"
    );
    expect(tenantOfficeSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_physical_locations ENABLE ROW LEVEL SECURITY"
    );
    expect(tenantOfficeSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_tenant_settings ENABLE ROW LEVEL SECURITY"
    );
  ...
  test("central profile schema declares RLS, dedup, and merge source-ne-target constraint", async () => {
    ...
    expect(profileSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_profiles ENABLE ROW LEVEL SECURITY"
    );
    expect(profileSchema?.sql).toContain(
      "awcms_micro_profile_identifiers_dedup_key"
    );
    expect(profileSchema?.sql).toContain(
  ...
  test("identity/login schema declares RLS on identities, tenant_users, and sessions", async () => {
    ...
    const identitySchema = migrations.find(
      (migration) =>
        migration.name === "004_awcms_micro_identity_login_schema.sql"
    );

    expect(identitySchema).toBeDefined();
    expect(identitySchema?.sql).toContain(
      "ALTER TABLE awcms_micro_identities ENABLE ROW LEVEL SECURITY"
    );
    expect(identitySchema?.sql).toContain(
      "ALTER TABLE awcms_micro_tenant_users ENABLE ROW LEVEL SECURITY"
    );
    expect(identitySchema?.sql).toContain(
      "ALTER TABLE awcms_micro_sessions ENABLE ROW LEVEL SECURITY"
    );
    expect(identitySchema?.sql).toContain(
      "awcms_micro_identities_tenant_login_key"
    );
  });
  ...
  test("abac schema declares RLS on tenant-scoped tables, seeds a generic permission catalog, and keeps the global permissions table RLS-free", async () => {
    ...
    const abacSchema = migrations.find(
      (migration) =>
        migration.name === "005_awcms_micro_abac_access_control_schema.sql"
    );

    expect(abacSchema).toBeDefined();
    expect(abacSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_roles ENABLE ROW LEVEL SECURITY"
    );
    expect(abacSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_role_permissions ENABLE ROW LEVEL SECURITY"
    );
    expect(abacSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_access_assignments ENABLE ROW LEVEL SECURITY"
    );
    expect(abacSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_abac_policies ENABLE ROW LEVEL SECURITY"
    );
    expect(abacSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_abac_decision_logs ENABLE ROW LEVEL SECURITY"
    );
    expect(abacSchema?.sql).not.toContain(
      "ALTER TABLE awcms_micro_permissions ENABLE ROW LEVEL SECURITY"
    );
  ...
  test("setup wizard schema declares a global RLS-free singleton lock table", async () => {
    ...
    const setupSchema = migrations.find(
      (migration) =>
        migration.name === "006_awcms_micro_setup_wizard_schema.sql"
    );
  ...
  test("sync storage schema declares RLS on all tenant-scoped tables and an idempotency ledger", async () => {
    ...
    expect(syncSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_sync_nodes ENABLE ROW LEVEL SECURITY"
    );
    expect(syncSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_sync_outbox ENABLE ROW LEVEL SECURITY"
    );
    expect(syncSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_sync_inbox ENABLE ROW LEVEL SECURITY"
    );
    expect(syncSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_sync_push_batches ENABLE ROW LEVEL SECURITY"
    );
    expect(syncSchema?.sql).toContain("awcms_micro_sync_push_batches_key");
  });
  ...
  test("sync conflict schema declares RLS, an immutable conflict table, and generic conflict_resolution permissions", async () => {
    ...
    const conflictSchema = migrations.find(
      (migration) =>
        migration.name === "008_awcms_micro_sync_storage_conflict_schema.sql"
    );

    expect(conflictSchema).toBeDefined();
    expect(conflictSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_sync_aggregate_versions ENABLE ROW LEVEL SECURITY"
    );
    expect(conflictSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_sync_conflicts ENABLE ROW LEVEL SECURITY"
    );
    expect(conflictSchema?.sql).toContain(
  ...
  test("object sync queue schema declares RLS, a retry-scan index, an upsert key, and generic object_queue permissions", async () => {
    ...
    const objectQueueSchema = migrations.find(
      (migration) =>
        migration.name === "009_awcms_micro_object_sync_queue_schema.sql"
    );

    expect(objectQueueSchema).toBeDefined();
    expect(objectQueueSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_object_sync_queue ENABLE ROW LEVEL SECURITY"
    );
    expect(objectQueueSchema?.sql).toContain(
      "CHECK (status IN ('pending', 'sent', 'failed'))"
    );
    expect(objectQueueSchema?.sql).toContain(
      "awcms_micro_object_sync_queue_key"
    );
    expect(objectQueueSchema?.sql).toContain(
      "awcms_micro_object_sync_queue_retry_idx"
    );
    expect(objectQueueSchema?.sql).toContain(
  ...
  test("audit logging schema declares the generic audit_events table with RLS and seeds the audit_trail/purge permissions", async () => {
    ...
    const auditSchema = migrations.find(
      (migration) =>
        migration.name === "011_awcms_micro_audit_logging_schema.sql"
    );

    expect(auditSchema).toBeDefined();
    expect(auditSchema?.sql).toContain(
      "CREATE TABLE IF NOT EXISTS awcms_micro_audit_events"
    );
    expect(auditSchema?.sql).toContain(
      "ALTER TABLE awcms_micro_audit_events ENABLE ROW LEVEL SECURITY"
    );
    expect(auditSchema?.sql).toContain(
      "CHECK (severity IN ('info', 'warning', 'critical'))"
    );
    expect(auditSchema?.sql).toContain(
      "awcms_micro_audit_events_tenant_created_idx"
    );
    expect(auditSchema?.sql).toContain(
      "awcms_micro_audit_events_tenant_resource_idx"
    );
  ...
  // Upstream AWCMS-Mini asserts this against its `012_awcms_mini_workflow_
  // approval_schema.sql`, which declares four workflow tables AND — incidentally
  // — the generic idempotency store, because a workflow decision was the first
  // endpoint there to need an Idempotency-Key. AWCMS-Micro drops the `workflow`
  // module (ADR-0025) but KEEPS the store, which is shared infrastructure every
  // high-risk mutation uses via `_shared/idempotency.ts`; migration 012 was
  // rewritten to carry the store alone. This test is the regression guard for
  // exactly that: the store must survive the workflow prune.
  test("the generic idempotency store exists on its own, independent of the unported workflow module, with RLS and its scope-key uniqueness index", async () => {
    const migrations = await discoverMigrationFiles();
    const idempotencySchema = migrations.find(
      (migration) =>
        migration.name === "012_awcms_micro_idempotency_store_schema.sql"
    );

    expect(idempotencySchema).toBeDefined();
    expect(idempotencySchema?.sql).toContain(
    ...
    // (tenant_id, request_scope, idempotency_key) — request_scope is what lets
    // two endpoints safely receive the same client-supplied Idempotency-Key.
    expect(idempotencySchema?.sql).toContain(
      "awcms_micro_idempotency_keys_scope_key"
    );

    // No workflow table may reappear anywhere in sql/ — the prune is structural,
    // not cosmetic.
    const allMigrationSql = migrations.map((m) => m.sql).join("\n");
    for (const unportedTable of [
      "awcms_micro_workflow_definitions",
      "awcms_micro_workflow_instances",
      "awcms_micro_workflow_tasks",
      "awcms_micro_workflow_decisions"
    ]) {
      expect(allMigrationSql).not.toContain(
        `CREATE TABLE IF NOT EXISTS ${unportedTable}`
      );
      expect(allMigrationSql).not.toContain(`ALTER TABLE ${unportedTable}`);
    }
  });
});

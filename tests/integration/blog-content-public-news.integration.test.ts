 ...
 * `awcms_micro_setup_state.tenant_id` (see `src/pages/api/v1/setup/initialize.ts`),
...
suite("public /news routes (Issue #560)", () => {
  ...
  // awcms-micro-tenant-domain-routing §Rute publik /news §Follow-up
  ...
  // `awcms_micro_tenant_domains` hold real mappings. The three tests below
  ...
  test("withNewsTenant's resolved-but-disabled and resolved-and-enabled branches cost the same number of round trips up to the point handler() runs", async () => {
    ...
    // blog_content left enabled by default (no awcms_micro_tenant_modules

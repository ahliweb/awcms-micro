 ...
 * `awcms_micro_modules`/`_dependencies`/`_navigation`/`_jobs`, idempotency
 ...
 * `awcms_micro_app` role (migration 013's default-privileges grant) can
...
suite("module descriptor sync service", () => {
  ...
  test("first run creates every registered module", async () => {
    ...
    const rows = (await admin`
      SELECT count(*)::int AS count FROM awcms_micro_modules
    `) as { count: number }[];
  ...
  test("syncs dependency rows for a module with dependencies", async () => {
    ...
    const admin = getAdminSql();
    const rows = (await admin`
      SELECT depends_on_module_key FROM awcms_micro_module_dependencies
  ...
  test("re-syncing after a descriptor field changes reports an update", async () => {
    ...
    const admin = getAdminSql();
    const rows = (await admin`
      SELECT version FROM awcms_micro_modules WHERE module_key = 'email'
    `) as { version: string }[];
  ...
  test("a module removed from the descriptor list is marked disabled, not deleted", async () => {
    ...
    const admin = getAdminSql();
    const rows = (await admin`
      SELECT lifecycle_status FROM awcms_micro_modules WHERE module_key = 'email'
  ...
  // `awcms_micro_modules`) ever called composition validation —
  ...
  describe("adversarial: a colliding module key must never reach the database (Issue #740 / PR #769 fix)", () => {
    ...
    test("a prior valid sync's identity_access row is left untouched by a subsequent malicious sync attempt", async () => {
      ...
      const before = (await admin`
        SELECT module_name, version FROM awcms_micro_modules WHERE module_key = 'identity_access'

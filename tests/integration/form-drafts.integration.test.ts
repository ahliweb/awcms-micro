...
suite("Form Drafts API (real Postgres)", () => {
  ...
  test("tenant A cannot read, update, or list tenant B's draft (RLS FORCE)", async () => {
    ...
    // `awcms_micro_setup_state`) — it cannot be called twice to create a
    ...
    const draftBRows = (await admin`
      INSERT INTO awcms_micro_form_drafts
    ...
    const rows = (await admin`
      SELECT id FROM awcms_micro_form_drafts WHERE id = ${draftB.id}
    `) as { id: string }[];
  ...
  test("expireOverdueFormDrafts transitions an overdue draft to expired and audits it", async () => {
    ...
    const admin = getAdminSql();
    await admin`
      UPDATE awcms_micro_form_drafts
    ...
    const rows = (await admin`
      SELECT status FROM awcms_micro_form_drafts WHERE id = ${draft.id}
    `) as { status: string }[];
  ...
  test("purgeExpiredFormDrafts physically deletes an old expired draft past retention, and audits the purge", async () => {
    ...
    const rows = (await admin`
      SELECT id FROM awcms_micro_form_drafts WHERE id = ${draft.id}
    `) as { id: string }[];
  ...
  test("expireOverdueFormDrafts runs successfully under the real awcms_micro_worker role (Issue #683, epic #679)", async () => {
    const b = await bootstrap();
    const draft = await createTestDraft(b);
    ...
    // awcms_micro_form_drafts, this call fails with "permission denied" —

 ...
 * grant matrix in `sql/045_awcms_micro_db_role_separation.sql` is REAL
 * enforcement, not just documentation: connects as each of the three
 * runtime roles (`awcms_micro_app`, `awcms_micro_worker`, `awcms_micro_setup`)
...
suite(
  ...
  () => {
    ...
    test("awcms_micro_app: SELECT still works, but INSERT/UPDATE/DELETE on permissions/schema_migrations are rejected", async () => {
      const sql = getTestSql();

      expect(
        await sql`SELECT 1 FROM awcms_micro_permissions LIMIT 1`
      ).toBeDefined();
      expect(
        await sql`SELECT 1 FROM awcms_micro_schema_migrations LIMIT 1`
      ).toBeDefined();

      await assertRejected(
        sql`INSERT INTO awcms_micro_permissions (module_key, activity_code, action, description) VALUES ('x', 'y', 'z', 'd')`
      );
      await assertRejected(
        sql`INSERT INTO awcms_micro_schema_migrations (migration_name) VALUES ('rogue')`
      );
    });

    test("awcms_micro_app: keeps INSERT/UPDATE (the setup-wizard fallback path) but loses DELETE on awcms_micro_setup_state/awcms_micro_tenants", async () => {
      await seedTenant();
      const sql = getTestSql();
      ...
      const claimed = await sql`
        INSERT INTO awcms_micro_setup_state (id, locked_at) VALUES (true, now()) ON CONFLICT (id) DO NOTHING
      `;
      expect(claimed).toBeDefined();
      const updatedState = await sql`
        UPDATE awcms_micro_setup_state SET locked_at = now() WHERE id = true
      `;
      expect(updatedState).toBeDefined();
      const updatedTenant = await sql`
        UPDATE awcms_micro_tenants SET tenant_name = 'renamed' WHERE id = ${TENANT_ID}
      `;
      expect(updatedTenant).toBeDefined();

      // Narrowed: nothing, dedicated role or fallback, ever deletes either.
      await assertRejected(
        sql`DELETE FROM awcms_micro_setup_state WHERE id = true`
      );
      await assertRejected(
        sql`DELETE FROM awcms_micro_tenants WHERE id = ${TENANT_ID}`
      );
    });

    test("awcms_micro_worker: can SELECT awcms_micro_tenants but has zero grant on the other 8 global tables", async () => {
      await seedTenant();
      const sql = getWorkerTestSql();

      const tenants =
        await sql`SELECT id FROM awcms_micro_tenants WHERE status = 'active'`;
      expect(tenants).toBeDefined();

      await assertRejected(
        sql`INSERT INTO awcms_micro_tenants (tenant_code, tenant_name) VALUES ('rogue', 'rogue')`
      );
    ...
    test("awcms_micro_setup: can write its bootstrap tables but never awcms_micro_schema_migrations, and only SELECTs awcms_micro_permissions", async () => {
      ...
      const tenantRows = await sql`
        INSERT INTO awcms_micro_tenants (tenant_code, tenant_name)
      ...
      await assertRejected(
        sql`DELETE FROM awcms_micro_tenants WHERE tenant_code = 'setup-role-test'`
      );
    });
  }
);

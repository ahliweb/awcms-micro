 ...
 * `SELECT current_version FROM awcms_micro_sync_aggregate_versions WHERE
...
const originalEnv = {
  AWCMS_MICRO_SYNC_ENABLED: process.env.AWCMS_MICRO_SYNC_ENABLED,
  AWCMS_MICRO_SYNC_HMAC_SECRET: process.env.AWCMS_MICRO_SYNC_HMAC_SECRET,
  AWCMS_MICRO_SYNC_MAX_SKEW_SEC: process.env.AWCMS_MICRO_SYNC_MAX_SKEW_SEC
};
...
async function pushBatch(
  ...
  return invoke(syncPush, {
    ...
    headers: {
      ...
      "x-awcms-micro-node-id": NODE_CODE,
      "x-awcms-micro-timestamp": timestamp,
      "x-awcms-micro-signature": signature
    },
    body: bodyObject
  });
}
...
suite("Sync push API (real Postgres)", () => {
  beforeAll(async () => {
    ...
    process.env.AWCMS_MICRO_SYNC_ENABLED = "true";
    process.env.AWCMS_MICRO_SYNC_HMAC_SECRET = HMAC_SECRET;
    process.env.AWCMS_MICRO_SYNC_MAX_SKEW_SEC = "300";
  });

  afterAll(() => {
    process.env.AWCMS_MICRO_SYNC_ENABLED = originalEnv.AWCMS_MICRO_SYNC_ENABLED;
    process.env.AWCMS_MICRO_SYNC_HMAC_SECRET =
      originalEnv.AWCMS_MICRO_SYNC_HMAC_SECRET;
    process.env.AWCMS_MICRO_SYNC_MAX_SKEW_SEC =
      originalEnv.AWCMS_MICRO_SYNC_MAX_SKEW_SEC;
  });

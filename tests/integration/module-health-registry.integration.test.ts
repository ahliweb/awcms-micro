...
suite("module health/readiness API", () => {
  ...
  test("POST .../health/check runs the live provider check for email", async () => {
    ...
    const auditRows = (await admin`
      SELECT action, resource_id, severity FROM awcms_micro_audit_events
  ...
  test("POST .../health/check records a row in awcms_micro_module_health_checks (instance-level history, RLS-free)", async () => {
    const owner = await bootstrap();
    ...
    const historyRows = (await admin`
      SELECT status, message FROM awcms_micro_module_health_checks
  ...
  test("GET .../health never writes to awcms_micro_module_health_checks (passive read only)", async () => {
    const owner = await bootstrap();
    ...
    const admin = getAdminSql();
    const historyRows = (await admin`
      SELECT count(*)::int AS count FROM awcms_micro_module_health_checks
    `) as { count: number }[];

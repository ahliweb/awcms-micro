 ...
 * partial update across both awcms_micro_tenants (RLS-free) and
 * awcms_micro_tenant_settings (RLS tenant-scoped), validation, and default-deny.
 *
...
suite("Settings API (real Postgres)", () => {
  ...
  test("a settings update by tenant A never affects tenant B (RLS-free tenants table stays scoped by WHERE id)", async () => {
    ...
    const betaRow = (await admin`
      SELECT tenant_name FROM awcms_micro_tenants WHERE id = ${tenantBId}

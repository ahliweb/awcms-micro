...
suite("public host tenant resolver — end-to-end (Issue #559)", () => {
  ...
  test("X-Forwarded-Host is honored only when trustProxy=true", async () => {
    ...
    await getAdminSql()`
      UPDATE awcms_micro_tenants SET status = 'active' WHERE id = ${TENANT_INACTIVE}
    `;
  ...
  test("the SECURITY DEFINER lookup function resolves rows via awcms_micro_app with no app.current_tenant_id GUC set", async () => {
    ...
    const rows = (await sql`
      ...
      FROM awcms_micro_resolve_tenant_domain_lookup('bootstrap-check.com')
  ...
  test("a direct SELECT on awcms_micro_tenant_domains from the same app-role session (no function) still returns zero rows", async () => {
    await insertDomain(TENANT_ACTIVE, "bootstrap-check-direct.com");

    const sql = getTestSql();
    const rows = await sql`
      SELECT normalized_hostname FROM awcms_micro_tenant_domains
  ...
  test("the lookup function never returns verification_token_hash or raw hostname columns", async () => {
    ...
    const sql = getTestSql();
    const rows = (await sql`
      SELECT * FROM awcms_micro_resolve_tenant_domain_lookup('narrow-surface-check.com')
    ...
    // already unconditionally public on the RLS-free awcms_micro_tenants

...
suite("performance-suite.ts (Issue #744) — real Postgres", () => {
  ...
  beforeAll(async () => {
    ...
    // least-privilege awcms_micro_app role in THIS process — capture it so
    ...
    tmpDir = mkdtempSync(join(tmpdir(), "awcms-micro-performance-suite-it-"));
  });

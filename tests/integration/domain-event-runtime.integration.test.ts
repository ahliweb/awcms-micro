...
async function seedSecondTenantWithDomainEventRuntimeAccess(
  ...
  await admin.begin(async (tx) => {
    ...
    const permissions = (await tx`
      SELECT id FROM awcms_micro_permissions WHERE module_key = 'domain_event_runtime'
    `) as { id: string }[];
...
suite("domain-event-runtime (Issue #742)", () => {
  ...
  describe("transactional outbox — atomic commit / rollback", () => {
    test("appendDomainEvent commits atomically with the caller's own transaction", async () => {
      ...
      const rows = await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT id FROM awcms_micro_domain_events WHERE id = ${result.eventId}`
      );
    ...
    test("a rolled-back source transaction produces no dispatchable event", async () => {
      ...
      const rows = await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT id FROM awcms_micro_domain_events WHERE id = ${capturedEventId}`
      );
      ...
      try {
        await withTenant(sql, owner.tenantId, (tx) =>
          appendDomainEvent(
            ...
            owner.tenantId,
            sampleInput({
              eventType: "awcms-micro.unregistered.thing.happened"
            })
          )
        );
      } catch (error) {
  ...
  describe("dispatch, idempotent consumer side effects, crash/restart recovery", () => {
    test("dispatching delivers to all three registered consumers exactly once (audit projection + activity rollup + reporting event-activity projector, Issue #753)", async () => {
      ...
      const auditRows = await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT id FROM awcms_micro_audit_events WHERE tenant_id = ${owner.tenantId} AND action = 'domain_event_runtime.sample.audit_projected'`
      );
      ...
      const rollupRows = (await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT event_count FROM awcms_micro_domain_event_activity_daily WHERE tenant_id = ${owner.tenantId} AND event_type = ${SAMPLE_RECORDED_EVENT_TYPE}`
      ...
      const reportingMetricRows = (await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT metric_value FROM awcms_micro_reporting_projection_metrics WHERE tenant_id = ${owner.tenantId} AND projection_key = 'reporting.event_activity_summary' AND metric_key = 'sample_recorded_count'`
    ...
    test("redelivering an already-delivered row (simulated worker restart) does not duplicate the side effect", async () => {
      ...
      const admin = getAdminSql();
      await admin`
        UPDATE awcms_micro_domain_event_deliveries
    ...
    test("a handler that throws leaves the delivery pending (no stuck claimed state) and a later successful attempt applies the side effect exactly once", async () => {
      ...
      const rows = (await admin`
        SELECT status, attempt_count FROM awcms_micro_domain_event_deliveries
  ...
  describe("retry, exponential backoff, and dead-letter", () => {
    test("a consistently-failing consumer backs off, exhausts its attempt budget, and is dead-lettered with an audit event", async () => {
      ...
      const deliveryRows = (await admin`
        SELECT status, attempt_count, dead_letter_reason FROM awcms_micro_domain_event_deliveries
      ...
      const auditRows = await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT id FROM awcms_micro_audit_events WHERE tenant_id = ${owner.tenantId} AND action = 'domain_event_runtime.delivery.dead_lettered'`
      );
      expect((auditRows as unknown[]).length).toBe(1);
    });
    ...
    test("a non-retryable error dead-letters immediately, without waiting out the attempt budget", async () => {
      ...
      const admin = getAdminSql();
      const rows = (await admin`
        SELECT attempt_count FROM awcms_micro_domain_event_deliveries
  ...
  describe("least-privilege awcms_micro_worker role (migration 056 grants)", () => {
    /**
     ...
     * entirely — a missing GRANT for `awcms_micro_worker` on any of the 6
     ...
     */
    test("dispatchDomainEventsForTenant succeeds end-to-end (claim, deliver to all three registered consumers, audit) over the real awcms_micro_worker connection, not just the admin connection", async () => {
      const owner = await bootstrap();
      const adminSql = getAdminSql();
      const workerSql = getWorkerTestSql();

      // appendDomainEvent still runs as awcms_micro_app (a producer's own
      ...
      const rows = await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT status FROM awcms_micro_domain_event_deliveries WHERE tenant_id = ${owner.tenantId}`
      );
      expect(
      ...
      // (INSERT into awcms_micro_audit_events), the activity rollup
      // (INSERT/UPDATE into awcms_micro_domain_event_activity_daily), and
      // the reporting event-activity projector (INSERT/UPDATE into
      // awcms_micro_reporting_projection_metrics, Issue #753) — must have
      ...
      const rollupRows = (await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT event_count FROM awcms_micro_domain_event_activity_daily WHERE tenant_id = ${owner.tenantId}`
      )) as { event_count: number }[];
      expect(rollupRows[0]?.event_count).toBe(1);
    ...
    test("a dead-lettered delivery (worker role) records a redacted-safe error and an audit event, entirely over the worker connection", async () => {
      ...
      const rows = (await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT status FROM awcms_micro_domain_event_deliveries WHERE tenant_id = ${owner.tenantId} AND consumer_name = 'test.worker_role_dlq_consumer'`
      )) as { status: string }[];
  ...
  describe("replay — permission, idempotency, reason, audit, schema compatibility", () => {
    async function createDeadLetteredDelivery(
      ...
      const admin = getAdminSql();
      const rows = (await admin`
        SELECT id FROM awcms_micro_domain_event_deliveries
        WHERE tenant_id = ${owner.tenantId} AND event_id = ${appended.eventId}
    ...
    test("replays a dead-lettered delivery, creates a new pending delivery row, and records an audit event", async () => {
      ...
      const replayRows = (await admin`
        SELECT reason, requested_by FROM awcms_micro_domain_event_replays
      ...
      const auditRows = await withTenant(
        ...
        owner.tenantId,
        (tx) =>
          tx`SELECT id FROM awcms_micro_audit_events WHERE tenant_id = ${owner.tenantId} AND action = 'domain_event_runtime.delivery.replayed'`
      );
      expect((auditRows as unknown[]).length).toBe(1);
    });
    ...
    test("replaying with the same Idempotency-Key twice does not create two replay rows", async () => {
      ...
      const admin = getAdminSql();
      const replayRows = (await admin`
        SELECT id FROM awcms_micro_domain_event_replays
        WHERE tenant_id = ${owner.tenantId} AND original_delivery_id = ${deliveryId}
      `) as { id: string }[];
      expect(replayRows.length).toBe(1);
    });
    ...
    test("refuses to replay a delivery that is not dead-lettered", async () => {
      ...
      const admin = getAdminSql();
      const rows = (await admin`
        SELECT id FROM awcms_micro_domain_event_deliveries WHERE tenant_id = ${owner.tenantId} LIMIT 1
      `) as { id: string }[];
  ...
  describe("API contract basics", () => {
    test("requires authentication", async () => {
      const result = await invoke(listEvents, {
        method: "GET",
        path: "/api/v1/domain-events/events",
        headers: { "x-awcms-micro-tenant-id": crypto.randomUUID() }
      });

...
suite("visitor analytics schema — RLS isolation and constraints", () => {
  ...
  test("db:migrate is idempotent when run twice", async () => {
    ...
    const tables = (await admin`
      ...
      WHERE table_schema = 'public'
        AND table_name IN (
          'awcms_micro_visitor_sessions',
          'awcms_micro_visit_events',
          'awcms_micro_visitor_daily_rollups'
        )
      ORDER BY table_name
  ...
  test("rejects an unknown human_status on visit_events", async () => {
    ...
    try {
      await admin`
        INSERT INTO awcms_micro_visit_events
  ...
  test("visitor_daily_rollups upserts on its (tenant_id, date, area) primary key", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_visitor_daily_rollups
    ...
    await admin`
      UPDATE awcms_micro_visitor_daily_rollups
      SET human_pageviews = 9
      WHERE tenant_id = ${TENANT_A} AND date = '2026-07-10' AND area = 'public'
    `;

    const rows = await admin`
      SELECT human_pageviews FROM awcms_micro_visitor_daily_rollups
      WHERE tenant_id = ${TENANT_A} AND date = '2026-07-10' AND area = 'public'
    `;
  ...
  for (const table of [
    ...
    test(`${table}: tenant A cannot see tenant B's rows (RLS isolation)`, async () => {
      ...
      if (table === "awcms_micro_visitor_daily_rollups") {
        await admin`
          INSERT INTO awcms_micro_visitor_daily_rollups (tenant_id, date, area)
          VALUES (${TENANT_A}, '2026-07-10', 'public'), (${TENANT_B}, '2026-07-10', 'public')
        `;
      } else if (table === "awcms_micro_visitor_sessions") {
        await admin`
          INSERT INTO awcms_micro_visitor_sessions (tenant_id, visitor_key_hash, area)

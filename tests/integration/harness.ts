 ...
 * ROUTE HANDLERS, however, must run as the least-privilege `awcms_micro_app`
...
// roles migration 045 creates (NOLOGIN by default, same as awcms_micro_app
...
/**
 * The client the route handlers use — the least-privilege `awcms_micro_app`
 ...
 * the least-privilege `awcms_micro_worker` role after `provisionWorkerRole()`
 ...
 * the least-privilege `awcms_micro_setup` role after `provisionSetupRole()`
...
/**
 * Activates the least-privilege `awcms_micro_app` role's login (migration 013
...
export async function provisionAppRole(): Promise<void> {
  await getAdminSql().unsafe(
    `ALTER ROLE awcms_micro_app WITH LOGIN PASSWORD '${APP_ROLE_TEST_PASSWORD}'`
  );

  const appUrl = new URL(ADMIN_DATABASE_URL);
  appUrl.username = "awcms_micro_app";
 ...
 * `awcms_micro_worker` role — activates its LOGIN with a test password, then
...
export async function provisionWorkerRole(): Promise<void> {
  await getAdminSql().unsafe(
    `ALTER ROLE awcms_micro_worker WITH LOGIN PASSWORD '${WORKER_ROLE_TEST_PASSWORD}'`
  );

  const workerUrl = new URL(ADMIN_DATABASE_URL);
  workerUrl.username = "awcms_micro_worker";
...
/**
 * Issue #683 (epic #679): mirrors `provisionAppRole()` exactly, for the
 * `awcms_micro_setup` role — activates its LOGIN with a test password, then
...
export async function provisionSetupRole(): Promise<void> {
  await getAdminSql().unsafe(
    `ALTER ROLE awcms_micro_setup WITH LOGIN PASSWORD '${SETUP_ROLE_TEST_PASSWORD}'`
  );

  const setupUrl = new URL(ADMIN_DATABASE_URL);
  setupUrl.username = "awcms_micro_setup";
 ...
 * preserving the two things migrations own: `awcms_micro_schema_migrations`
 * (the runner's ledger) and `awcms_micro_permissions` (the global ABAC seed
 ...
 * Truncating `awcms_micro_setup_state` resets the singleton setup lock so each
...
export async function resetDatabase(): Promise<void> {
  ...
  const rows = (await sql`
    ...
    WHERE schemaname = 'public'
      AND tablename LIKE 'awcms_micro_%'
      AND tablename NOT IN ('awcms_micro_schema_migrations', 'awcms_micro_permissions')
  `) as { tablename: string }[];

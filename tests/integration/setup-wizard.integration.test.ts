...
suite(
  ...
  () => {
    beforeAll(async () => {
      ...
      // Issue #683 (epic #679): exercises the real awcms_micro_setup role
    ...
    test("first call creates the full tenant/office/owner graph in one transaction", async () => {
      ...
      const tenants =
        await admin`SELECT id FROM awcms_micro_tenants WHERE id = ${data.tenantId}`;
      expect(tenants).toHaveLength(1);

      const setupState =
        await admin`SELECT tenant_id FROM awcms_micro_setup_state WHERE id = true`;
    ...
    test("a second call is rejected (403) and creates no second tenant row — idempotent, not merely re-validated", async () => {
      ...
      const admin = getAdminSql();
      const tenants = await admin`SELECT id FROM awcms_micro_tenants`;
      expect(tenants).toHaveLength(1);

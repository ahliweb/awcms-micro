...
suite("purgeVisitorAnalyticsForAllTenants (Issue #624)", () => {
  ...
  test("runs successfully under the real awcms_micro_worker role, including the raw-detail UPDATE step (Issue #683, epic #679)", async () => {
    const now = new Date();
    ...
    // missing awcms_micro_worker UPDATE grant would break (PR #703 review
    // caught this live: without UPDATE on awcms_micro_visitor_sessions, this

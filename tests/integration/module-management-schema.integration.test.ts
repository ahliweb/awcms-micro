...
async function seedModules(): Promise<void> {
  const admin = getAdminSql();
  await admin`
    INSERT INTO awcms_micro_modules (module_key, module_name, status, version)
    VALUES
...
suite("module management schema — RLS isolation and constraints", () => {
  ...
  test("awcms_micro_modules gained the new columns with sane defaults", async () => {
    const admin = getAdminSql();
    const rows = (await admin`
      SELECT module_type, lifecycle_status, descriptor_version, is_core,
             is_tenant_configurable, updated_at
      FROM awcms_micro_modules WHERE module_key = ${MODULE_KEY}
    `) as {
  ...
  test("lifecycle_status rejects an unknown value", async () => {
    ...
    try {
      await admin`
        UPDATE awcms_micro_modules SET lifecycle_status = 'bogus'
  ...
  test("module_type rejects an unknown value but allows null", async () => {
    ...
    try {
      await admin`
        UPDATE awcms_micro_modules SET module_type = 'bogus'
        WHERE module_key = ${MODULE_KEY}
      `;
    } catch {
      didThrow = true;
    ...
    await admin`
      UPDATE awcms_micro_modules SET module_type = 'system'
      WHERE module_key = ${MODULE_KEY}
    `;
    const rows = (await admin`
      SELECT module_type FROM awcms_micro_modules WHERE module_key = ${MODULE_KEY}
  ...
  test("tenant A cannot see tenant B's tenant_modules row", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_tenant_modules (tenant_id, module_key, enabled)
    ...
    const tenantARows = await withTenant(
      sql,
      TENANT_A,
      (tx) => tx`SELECT enabled FROM awcms_micro_tenant_modules`
    );
  ...
  test("querying tenant_modules without a tenant GUC set returns no rows (fail-closed)", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_tenant_modules (tenant_id, module_key)
      VALUES (${TENANT_A}, ${MODULE_KEY})
    `;

    const sql = getDatabaseClient();
    const rows = await sql`SELECT enabled FROM awcms_micro_tenant_modules`;
  ...
  test("tenant A cannot see tenant B's module_settings row", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_module_settings (tenant_id, module_key, settings)
      VALUES
    ...
    const tenantARows = await withTenant(
      sql,
      TENANT_A,
      (tx) => tx`SELECT settings FROM awcms_micro_module_settings`
    );
    expect(tenantARows).toHaveLength(1);
    expect(
  ...
  test("module_dependencies rejects a self-dependency", async () => {
    ...
    try {
      await admin`
        INSERT INTO awcms_micro_module_dependencies (module_key, depends_on_module_key)
  ...
  test("module_navigation enforces a globally unique path", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_module_navigation (module_key, label_key, path)
  ...
  test("module_jobs enforces one row per (module, command)", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_module_jobs (module_key, command, purpose)
  ...
  test("module_health_checks rejects an unknown status and supports a latest-per-module query", async () => {
    ...
    try {
      await admin`
        INSERT INTO awcms_micro_module_health_checks (module_key, status)
    ...
    await admin`
      INSERT INTO awcms_micro_module_health_checks (module_key, status, checked_at)
      VALUES
    ...
    const rows = (await admin`
      SELECT status FROM awcms_micro_module_health_checks
      WHERE module_key = ${MODULE_KEY}
  ...
  test("module_management permission catalog is seeded (no audit.read entry)", async () => {
    const admin = getAdminSql();
    const rows = (await admin`
      SELECT activity_code, action FROM awcms_micro_permissions

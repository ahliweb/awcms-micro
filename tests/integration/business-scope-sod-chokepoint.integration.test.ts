 ...
 * REAL least-privilege `awcms_micro_worker` role
...
async function bootstrap(): Promise<Bootstrap> {
  ...
  const officeRows = (await admin`
    SELECT id FROM awcms_micro_offices WHERE tenant_id = ${setup.body.data.tenantId}
  `) as { id: string }[];
...
suite("business-scope SoD chokepoint + expiry worker (Issue #746)", () => {
  ...
  test("real chokepoint: owner (who holds legal_hold.release via RBAC AND legal_hold.create via an active business-scope assignment) is DENIED releasing a legal hold — SOD_CONFLICT, no approved exception", async () => {
    ...
    const narrowRole = (await admin`
      INSERT INTO awcms_micro_roles (tenant_id, role_code, role_name) VALUES (${owner.tenantId}, 'hold_creator', 'Hold Creator') RETURNING id
    `) as { id: string }[];
    await admin`
      INSERT INTO awcms_micro_role_permissions (tenant_id, role_id, permission_id) VALUES (${owner.tenantId}, ${narrowRole[0]!.id}, ${createPerm[0]!.id})
    `;
    ...
    if (assignment.status !== 200) {
      await admin`
        INSERT INTO awcms_micro_business_scope_assignments
    ...
    const evaluations = (await admin`
      SELECT conflict_detected, resolved_via, trigger_context FROM awcms_micro_sod_conflict_evaluations
  ...
  test("real chokepoint, ORDINARY RBAC ALONE — no business-scope assignment layering at all: a subject whose role grants BOTH legal_hold.create AND .release via plain awcms_micro_access_assignments is DENIED releasing a legal hold (security-auditor finding on PR #776)", async () => {
    // Deliberately does NOT create, seed, or reference
    // `awcms_micro_business_scope_assignments` anywhere in this test — the
    // ENTIRE conflict is manufactured through the same ordinary RBAC path
    // (`awcms_micro_access_assignments` -> `awcms_micro_role_permissions`)
    ...
    const assignmentRows = (await admin`
      SELECT id FROM awcms_micro_business_scope_assignments
    ...
    const evaluations = (await admin`
      ...
      FROM awcms_micro_sod_conflict_evaluations
      WHERE tenant_id = ${owner.tenantId} AND rule_key = 'data_lifecycle.legal_hold_maker_checker'
  ...
  test("expiry worker (real least-privilege awcms_micro_worker role): an already-elapsed temporary assignment is transitioned to expired and stops contributing to SoD facts", async () => {
    const owner = await bootstrap();
    const admin = getAdminSql();
    ...
    const releaserRole = (await admin`
      INSERT INTO awcms_micro_roles (tenant_id, role_code, role_name) VALUES (${owner.tenantId}, 'hold_releaser', 'Hold Releaser') RETURNING id
    `) as { id: string }[];
    await admin`
      INSERT INTO awcms_micro_role_permissions (tenant_id, role_id, permission_id) VALUES (${owner.tenantId}, ${releaserRole[0]!.id}, ${releasePerm[0]!.id})
    `;
    ...
    const row = (await admin`
      SELECT status FROM awcms_micro_business_scope_assignments WHERE id = ${assignmentRows[0]!.id}
    `) as { status: string }[];
    expect(row[0]!.status).toBe("expired");

    const events = (await admin`
      SELECT event_type FROM awcms_micro_business_scope_assignment_events
  ...
  test("expiry worker --dry-run reports the REAL non-zero backlog against a seeded expired-but-not-yet-transitioned row (security-auditor finding on PR #776)", async () => {
    ...
    // `awcms_micro_business_scope_assignments`/`..._sod_conflict_exceptions`
    // directly via the bare worker `sql` client with NO `withTenant`
    // wrapping — since both tables are `FORCE ROW LEVEL SECURITY`'d and
    // `awcms_micro_worker`'s session-level `app.current_tenant_id` defaults
    ...
    `) as { id: string }[];
    const narrowRole = (await admin`
      INSERT INTO awcms_micro_roles (tenant_id, role_code, role_name) VALUES (${owner.tenantId}, 'hold_creator_dryrun', 'Hold Creator (dry-run test)') RETURNING id
    `) as { id: string }[];
    await admin`
      INSERT INTO awcms_micro_role_permissions (tenant_id, role_id, permission_id) VALUES (${owner.tenantId}, ${narrowRole[0]!.id}, ${createPerm[0]!.id})
    `;
    ...
    const events = (await admin`
      SELECT id FROM awcms_micro_business_scope_assignment_events
      WHERE assignment_id = ${assignmentRows[0]!.id}
    `) as { id: string }[];

 ...
 * pre-existing `awcms_micro_blog_settings` store
...
suite("blog_content public route settings (Issue #564)", () => {
  ...
  test("GET blog_content module settings returns the four new descriptor defaults, and NOT rssEnabled/sitemapEnabled", async () => {
    ...
    // awcms_micro_blog_settings (Issue #537/#543), never duplicated here.
  ...
  test("rssEnabled/sitemapEnabled remain governed exclusively by awcms_micro_blog_settings — writing those exact key names into the new module-settings store has NO effect on /news/feed.xml or /news/sitemap-news.xml", async () => {
    const owner = await bootstrap();
    ...
    // rssEnabled/sitemapEnabled from awcms_micro_blog_settings only, which

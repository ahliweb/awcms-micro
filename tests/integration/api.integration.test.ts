...
suite("API integration (real Postgres)", () => {
  beforeAll(async () => {
    ...
    // Repoint handlers at the least-privilege awcms_micro_app role so FORCE'd
  ...
  test("rejects a cross-tenant session (application-level tenant isolation)", async () => {
    ...
    const crossTenant = await invoke<{ error: { code: string } }>(
      tenantActivity,
      {
        ...
        path: "/api/v1/reports/tenant-activity",
        headers: {
          "x-awcms-micro-tenant-id": tenantBId,
  ...
  test("PostgreSQL RLS enforces tenant row isolation for the app DB role", async () => {
    // Handlers connect as the least-privilege awcms_micro_app role (see
    ...
    await admin.begin(async (tx) => {
      ...
      await tx`
        INSERT INTO awcms_micro_offices (tenant_id, office_code, office_name, office_type, status)
    ...
    const aOfficeCount = await withTenant(
      ...
      async (tx) => {
        const rows = (await tx`
        SELECT count(*)::int AS n FROM awcms_micro_offices
      `) as { n: number }[];
        return rows[0]!.n;
      }
    );

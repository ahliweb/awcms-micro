 ...
 * (real `awcms_micro_offices` row vs unknown scope), self-grant denial,
...
async function createRoleWithPermissions(
  ...
  for (const key of permissionKeys) {
    ...
    await admin`
      ...
      SELECT ${tenantId}, ${roleId}, id FROM awcms_micro_permissions
...
suite("business-scope assignments API (Issue #746)", () => {
  ...
  test("cross-tenant isolation: tenant A cannot see or revoke tenant B's assignment", async () => {
    ...
    const officeBRows = (await admin`
      INSERT INTO awcms_micro_offices (tenant_id, office_code, office_name)
    ...
    const stillActive = (await admin`
      SELECT status FROM awcms_micro_business_scope_assignments WHERE id = ${assignmentB[0]!.id}
    `) as { status: string }[];
  ...
  test("SoD conflict at assignment-create time (global_within_tenant): granting a conflicting role-backed scope is denied without an approved exception, allowed with one", async () => {
    ...
    const requesterRole = (await admin`
      INSERT INTO awcms_micro_roles (tenant_id, role_code, role_name) VALUES (${owner.tenantId}, 'legal_hold_creator', 'Legal Hold Creator') RETURNING id
    `) as { id: string }[];
    await admin`
      INSERT INTO awcms_micro_role_permissions (tenant_id, role_id, permission_id) VALUES (${owner.tenantId}, ${requesterRole[0]!.id}, ${createPerm[0]!.id})
    `;
    const approverRole = (await admin`
      INSERT INTO awcms_micro_roles (tenant_id, role_code, role_name) VALUES (${owner.tenantId}, 'legal_hold_releaser', 'Legal Hold Releaser') RETURNING id
    `) as { id: string }[];
    await admin`
      INSERT INTO awcms_micro_role_permissions (tenant_id, role_id, permission_id) VALUES (${owner.tenantId}, ${approverRole[0]!.id}, ${approvePerm[0]!.id})
    `;
    ...
    const evaluations = (await admin`
      SELECT conflict_detected, resolved_via FROM awcms_micro_sod_conflict_evaluations
  ...
  test("ADVERSARIAL (Issue #795): reusing the same Idempotency-Key across REVOKE of two DIFFERENT business-scope assignments must NOT replay the first assignment's cached response for the second -- the mismatched hash must yield 409 CONFLICT, and the second assignment must still actually revoke once given its OWN key", async () => {
    ...
    const stillActiveY = (await admin`
      SELECT status FROM awcms_micro_business_scope_assignments WHERE id = ${assignmentY.body.data.assignment.id}
    `) as { status: string }[];
  ...
  test("ADVERSARIAL (Issue #795): reusing the same Idempotency-Key across APPROVE of two DIFFERENT SoD conflict exceptions must NOT replay the first exception's cached response for the second -- the mismatched hash must yield 409 CONFLICT, and the second exception must still actually approve once given its OWN key", async () => {
    ...
    const stillPendingB = (await admin`
      SELECT status FROM awcms_micro_sod_conflict_exceptions WHERE id = ${exceptionB.body.data.exception.id}
    `) as { status: string }[];
  ...
  test("ADVERSARIAL (Issue #795): reusing the same Idempotency-Key across REJECT of two DIFFERENT SoD conflict exceptions must NOT replay the first exception's cached response for the second -- the mismatched hash must yield 409 CONFLICT, and the second exception must still actually reject once given its OWN key", async () => {
    ...
    const stillPendingD = (await admin`
      SELECT status FROM awcms_micro_sod_conflict_exceptions WHERE id = ${exceptionD.body.data.exception.id}
    `) as { status: string }[];
  ...
  test("ADVERSARIAL (Issue #795): reusing the same Idempotency-Key across REVOKE of two DIFFERENT approved SoD conflict exceptions must NOT replay the first exception's cached response for the second -- the mismatched hash must yield 409 CONFLICT, and the second exception must still actually revoke once given its OWN key", async () => {
    ...
    const stillApprovedF = (await admin`
      SELECT status FROM awcms_micro_sod_conflict_exceptions WHERE id = ${exceptionF.body.data.exception.id}
    `) as { status: string }[];

...
const ANALYZED_TABLES = [
  ...
  "awcms_micro_abac_decision_logs",
  ...
  () => {
    ...
    async function countAuditEvents(forTenantId: string): Promise<number> {
      ...
      return withTenant(
        ...
        async (tx) => {
          const rows = (await tx`
            SELECT count(*)::int AS row_count FROM awcms_micro_audit_events
    ...
    async function countPendingObjectSyncQueue(
      ...
      return withTenant(
        ...
        async (tx) => {
          const rows = (await tx`
            SELECT count(*)::int AS row_count FROM awcms_micro_object_sync_queue

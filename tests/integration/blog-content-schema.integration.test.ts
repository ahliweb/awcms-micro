...
suite("blog_content schema — RLS isolation and constraints", () => {
  ...
  test("tenant A cannot see tenant B's posts (RLS isolation)", async () => {
    ...
    const tenantARows = await withTenant(
      sql,
      TENANT_A,
      (tx) => tx`SELECT slug FROM awcms_micro_blog_posts`
    );
  ...
  test("querying posts without a tenant GUC set returns no rows (fail-closed)", async () => {
    ...
    const sql = getDatabaseClient();
    const rows = await sql`SELECT slug FROM awcms_micro_blog_posts`;
  ...
  test("terms rejects a tag with a parent_id", async () => {
    ...
    const categoryRows = await admin`
      INSERT INTO awcms_micro_blog_terms (tenant_id, taxonomy_type, name, slug)
    ...
    try {
      await admin`
        INSERT INTO awcms_micro_blog_terms (tenant_id, taxonomy_type, parent_id, name, slug)
  ...
  test("post_terms enforces one row per (post, term) and is tenant-isolated", async () => {
    ...
    await admin`
      INSERT INTO awcms_micro_blog_post_terms (tenant_id, post_id, term_id)
  ...
  test("revisions enforces one row per (tenant, resource, revision_number)", async () => {
    ...
    await admin`
      INSERT INTO awcms_micro_blog_revisions
  ...
  test("redirects enforces unique from_path per tenant among active rows", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_blog_redirects (tenant_id, from_path, to_path)
  ...
  test("settings is one row per tenant and tenant-isolated", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_blog_settings (tenant_id, default_locale)
    ...
    const tenantARows = await withTenant(
      sql,
      TENANT_A,
      (tx) => tx`SELECT default_locale FROM awcms_micro_blog_settings`
    );
    expect(tenantARows).toHaveLength(1);
    expect(

...
async function seedFixtures(): Promise<void> {
  ...
  `;
  await admin`
    INSERT INTO awcms_micro_profiles (id, tenant_id, profile_type, display_name)
    VALUES
      (${PROFILE_A}, ${TENANT_A}, 'person', 'Owner A'),
      (${PROFILE_B}, ${TENANT_B}, 'person', 'Owner B')
  `;
  await admin`
    INSERT INTO awcms_micro_identities
...
suite("MFA/TOTP schema — RLS isolation and constraints (Issue #589)", () => {
  ...
  test("tenant A cannot see tenant B's MFA factor (RLS isolation)", async () => {
    const admin = getAdminSql();
    await admin`
      INSERT INTO awcms_micro_identity_mfa_factors
    ...
    const rows = await withTenant(
      sql,
      TENANT_A,
      (tx) => tx`SELECT identity_id FROM awcms_micro_identity_mfa_factors`
    );
  ...
  test("querying factors without a tenant GUC set returns no rows (fail-closed)", async () => {
    ...
    const sql = getDatabaseClient();
    const rows =
      await sql`SELECT identity_id FROM awcms_micro_identity_mfa_factors`;
  ...
  test("recovery codes: tenant A cannot see tenant B's codes (RLS isolation)", async () => {
    ...
    await admin`
      INSERT INTO awcms_micro_identity_mfa_recovery_codes
    ...
    const rows = await withTenant(
      sql,
      TENANT_A,
      (tx) => tx`SELECT code_hash FROM awcms_micro_identity_mfa_recovery_codes`
    );
  ...
  test("recovery codes are deleted when their factor is deleted (ON DELETE CASCADE)", async () => {
    ...
    await admin`DELETE FROM awcms_micro_identity_mfa_factors WHERE id = ${factorA!.id}`;

    const remaining = await admin`
      SELECT id FROM awcms_micro_identity_mfa_recovery_codes WHERE factor_id = ${factorA!.id}
    `;
  ...
  test("challenges: tenant A cannot see tenant B's challenge (RLS isolation)", async () => {
    ...
    await admin`
      INSERT INTO awcms_micro_mfa_challenges
    ...
    const rows = await withTenant(
      sql,
      TENANT_A,
      (tx) => tx`SELECT challenge_token_hash FROM awcms_micro_mfa_challenges`
    );

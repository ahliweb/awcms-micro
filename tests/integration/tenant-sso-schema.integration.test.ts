 ...
 * 036's two new tables (`awcms_micro_auth_providers`,
 * `awcms_micro_tenant_auth_policies`). Same pattern as
  ...
  () => {
    ...
    `;
      ...
      const rows = await withTenant(
        sql,
        TENANT_A,
        (tx) => tx`SELECT provider_key FROM awcms_micro_auth_providers`
      );
    ...
    `;
      ...
      const sql = getDatabaseClient();
      const rows =
        await sql`SELECT provider_key FROM awcms_micro_auth_providers`;
    ...
    `;
      ...
      await admin`
      UPDATE awcms_micro_auth_providers
    ...
    test("tenant auth policies: tenant A cannot see tenant B's policy (RLS isolation)", async () => {
      const admin = getAdminSql();
      await admin`
      INSERT INTO awcms_micro_tenant_auth_policies (tenant_id, sso_required)
    ...
    `;
      ...
      const rows = await withTenant(
        sql,
        TENANT_A,
        (tx) => tx`SELECT tenant_id FROM awcms_micro_tenant_auth_policies`
      );
    ...
    test("only one auth policy row per tenant (unique index)", async () => {
      const admin = getAdminSql();
      await admin`
      INSERT INTO awcms_micro_tenant_auth_policies (tenant_id) VALUES (${TENANT_A})
    `;
    ...
    test("rejects a policy with BOTH password_login_enabled and sso_enabled false", async () => {
      ...
      try {
        await admin`
        INSERT INTO awcms_micro_tenant_auth_policies

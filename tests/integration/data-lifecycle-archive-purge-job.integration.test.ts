...
async function seedOldRunHistoryRows(
  ...
  await withTenant(
    ...
    async (tx) => {
      await tx`
        INSERT INTO awcms_micro_data_lifecycle_runs
...
async function countRunHistoryRows(tenantId: string): Promise<number> {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    const rows = (await tx`
      SELECT count(*)::int AS count FROM awcms_micro_data_lifecycle_runs
...
async function countAuditEvents(tenantId: string): Promise<number> {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    const rows = (await tx`
      SELECT count(*)::int AS count FROM awcms_micro_audit_events WHERE tenant_id = ${tenantId}
    `) as { count: number }[];
    return rows[0]!.count;
  });
}
...
suite("runDataLifecycleArchivePurge (Issue #745)", () => {
  ...
  test("legal hold blocks archive AND purge entirely — held rows are never touched, even across multiple invocations", async () => {
    ...
    // createLegalHold is an admin/API action (awcms_micro_app role) — the

...
async function createSecondTenantUser(
  ...
  const tenantUserId = await admin.begin(async (tx) => {
    ...
    const roleRows = (await tx`
      SELECT id FROM awcms_micro_roles WHERE tenant_id = ${tenantId} LIMIT 1
    `) as { id: string }[];
...
async function seedSecondTenantWithProfileIdentityAccess(
  ...
  await admin.begin(async (tx) => {
    ...
    const permissions = (await tx`
      SELECT id FROM awcms_micro_permissions WHERE module_key = 'profile_identity'
    `) as { id: string }[];
...
suite(
  ...
  () => {
    ...
    test("ABAC default-deny: a role with no profile_management.create permission cannot create a party", async () => {
      ...
      // no-permission user.
      const admin = getAdminSql();
      await admin`DELETE FROM awcms_micro_role_permissions WHERE tenant_id = ${b.tenantId}`;
    ...
    test("scan finds a deterministic identifier match; a not_duplicate review sticks across re-scan", async () => {
      ...
      await admin.begin(async (tx) => {
        ...
        await tx`
        INSERT INTO awcms_micro_profile_identifiers
    ...
    test("full merge workflow: create -> self-approval denied -> approved by a different user -> executed -> entity links repointed -> immutable history recorded", async () => {
      ...
      await admin.begin(async (tx) => {
        await tx.unsafe(`SET LOCAL app.current_tenant_id = '${b.tenantId}'`);
        await tx`
        INSERT INTO awcms_micro_profile_entity_links
      ...
      SELECT profile_id FROM awcms_micro_profile_entity_links
    ...
    `;
      ...
      SELECT deleted_at, merged_into_profile_id, status FROM awcms_micro_profiles
      WHERE id = ${loser}
    `) as {
      ...
      FROM awcms_micro_profile_merge_history WHERE merge_request_id = ${mergeRequestId}
    `;
      ...
      SELECT id FROM awcms_micro_profile_merge_history WHERE merge_request_id = ${mergeRequestId}
    `;
    ...
    test("PR #777 review follow-up: restoring a merged-away (loser) profile is rejected, not silently resurrected", async () => {
      ...
      const admin = getAdminSql();
      const loserRow = (await admin`
        SELECT deleted_at, merged_into_profile_id FROM awcms_micro_profiles WHERE id = ${loser}
      `) as {
        deleted_at: Date | null;
    ...
    test("same Idempotency-Key replays the same response on merge-request create", async () => {
      ...
      SELECT id FROM awcms_micro_profile_merge_requests WHERE tenant_id = ${b.tenantId}
    `;
    ...
    test("HTTP-level: creating a merge request across two tenants is rejected, not silently empty", async () => {
      ...
      const admin = getAdminSql();
      const rows = await admin`
      SELECT id FROM awcms_micro_profile_merge_requests WHERE tenant_id = ${tenantA.tenantId}
    `;
    ...
    test("application-layer guard: assertSameTenant/CrossTenantMergeError fires even when RLS is bypassed (defense in depth)", async () => {
      ...
      const mergeRequestId = await admin.begin(async (tx) => {
        ...
        const rows = (await tx`
        INSERT INTO awcms_micro_profile_merge_requests
      ...
      SELECT status FROM awcms_micro_profile_merge_requests WHERE id = ${mergeRequestId}
    `) as { status: string }[];

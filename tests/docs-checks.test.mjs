...
describe("checkNaming", () => {
  test("mendeteksi pola rusak lowercase dan uppercase", () => {
    const md = "tabel `awcms-micro_tenants` dan env `AWCMS-Micro_NODE_ID`";
  ...
  test("tidak menandai penamaan benar", () => {
    const md =
      "`awcms_micro_tenants`, `AWCMS_MICRO_NODE_ID`, skill `awcms-micro-release`";
    expect(checkNaming("f.md", lines(md))).toEqual([]);
  });

  test("nama skill/paket kebab-case (awcms-micro-...) tidak salah tangkap", () => {
    const md =
      "lihat `awcms-micro-new-migration` dan `docs/awcms-micro/README.md`";
    expect(checkNaming("f.md", lines(md))).toEqual([]);
  });

  test("melaporkan nomor baris yang tepat", () => {
    const md = "baris1\nbaris2 `awcms-micro_offices`\nbaris3";
    const problems = checkNaming("f.md", lines(md));

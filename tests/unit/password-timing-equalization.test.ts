/**
 * Unit-level pin for `verifyPasswordOrDummy`, the helper that stops
 * `POST /auth/login` from answering an unknown `loginIdentifier` ~19x faster
 * than a known one by skipping argon2id entirely. Ported from the awcms-mini
 * base standard (Issue #840).
 *
...
describe("verifyPasswordOrDummy (login timing equalization)", () => {
  ...
  // Mutation-verified. Two deliberate breaks of `verifyPasswordOrDummy` turn
  // the timing test above red:

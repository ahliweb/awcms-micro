/**
 * Module-registry composition validation tests. After ADR-0036 removed the
 ...
 * base-load-bearing invariant that also holds when a new website/domain module
 * is added directly to `src/modules/`.
 */
...
describe("composeModuleRegistry — happy paths", () => {
  test("an empty-then-base registry composes to a valid, unchanged registry", () => {
...
describe("composeModuleRegistry — every rejection class", () => {
  ...
  test("duplicate_module_key: two modules share the same key", () => {
    const result = composeModuleRegistry(
      registry(
        mod({ key: "unrelated" }),
        mod({ key: "dup" }),
...
describe("buildComposedModuleInventory determinism", () => {
  ...
  test("reflects module count and validity", () => {
    const inventory = buildComposedModuleInventory(
      registry(
        mod({ key: "b1" }),
  ...
  test("an invalid registry (duplicate key) reports valid:false and a non-zero issueCount, still echoing every entry", () => {
    const inventory = buildComposedModuleInventory(
      registry(
        mod({ key: "identity_access", name: "Real Module" }),
        mod({ key: "identity_access", name: "Duplicate" })
      )
    );

    expect(inventory.valid).toBe(false);
    expect(inventory.issueCount).toBeGreaterThan(0);
    const entries = inventory.modules.filter(
...
describe("the real base registry", () => {

...
export function encodeBase64(str: string): string {
	...
	if (hasNative) return (bytes as any).toBase64();
	let binary = "";
...
export function decodeBase64(base64: string): string {
	if (hasNative) return new TextDecoder().decode((Uint8Array as any).fromBase64(base64));
...
export function encodeBase64url(bytes: Uint8Array): string {
	if (hasNative) return (bytes as any).toBase64({ alphabet: "base64url", omitPadding: true });
	let binary = "";
	for (const b of bytes) binary += String.fromCharCode(b);
	return btoa(binary)
...
export function decodeBase64url(encoded: string): Uint8Array {
	if (hasNative) return (Uint8Array as any).fromBase64(encoded, { alphabet: "base64url" });

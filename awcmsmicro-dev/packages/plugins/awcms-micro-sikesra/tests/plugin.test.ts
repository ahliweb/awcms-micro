...
describe("awcms micro sikesra plugin", () => {
	...
	it("uses typed admin API wrappers for migrated admin write calls", () => {
		...
		expect(adminSource).toContain("Server validation and duplicate checks run on submit.");
		expect(adminSource).toContain("The canonical 20-digit SIKESRA ID is generated by the server");

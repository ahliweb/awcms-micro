...
const ALLOWED_OWNER_TYPES = new Set(["registry", "sikesra_id", "entity_type", "subtype"]);
...
function validateCustomAttributeValue(input: SikesraCustomAttributeValueRequest) {
	...
	if (!ALLOWED_OWNER_TYPES.has(input.ownerType.trim())) {
		fieldErrors.ownerType = ["Owner type must be registry, sikesra_id, entity_type, or subtype."];
	}

...
const file = "/home/data/dev_react/awcms-micro/awcmsmicro-dev/packages/plugins/awcms-micro-example/src/admin.tsx";
...
// Buttons
content = content.replace(/<button className="rounded bg-black[^"]*"/g, '<Button variant="primary" ');
content = content.replace(/<button className="text-xs text-kumo-subtle[^"]*"/g, '<Button variant="ghost" size="sm" ');
content = content.replace(/<button className="rounded border[^"]*"/g, '<Button variant="secondary" size="sm" ');

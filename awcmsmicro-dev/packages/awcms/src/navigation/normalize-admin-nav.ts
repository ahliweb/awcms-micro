...
function normalizePath(pluginId: string, path: string): { path: string; routerPath: string } {
	...
	// If path is already normalized, e.g. it starts with /plugins/<plugin-id>
	...
	if (cleanPath.startsWith(expectedPrefix)) {
		...
		// Reject unrelated admin roots / plugins
...
export function normalizeAdminNav(
			...
			for (const group of manifest.navigation.groups) {
				...
				for (const item of group.items) {
					...
					if (item.children) {
						...
						// Sort nested children by sortOrder, then label
				...
				// Sort items by sortOrder, then label
	...
	// Sort groups by sidebarPriority, then sortOrder, then label
	groupsList.sort((a, b) => {

...
import { resolveLabel, type TranslationMessages } from "../i18n/resolve-label.js";

export interface AwcmsPluginHeaderMenuProps {
...
export function AwcmsPluginHeaderMenu({
	groups,
	currentPath,
	locale,
	defaultLocale = "en",
	...
	// Find the active group based on the current URL
	...
	// Determine active top-level item in the active group
	...
	return (
		<section className="rounded-2xl border border-kumo-line bg-kumo-base p-4 text-kumo-default shadow-sm mb-6">
			<div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
				<div>
					<div className="text-xs font-semibold uppercase tracking-wide text-kumo-subtle">
						{getLocalizedName(activeGroup)}
					</div>
					<h2 className="mt-1 text-lg font-semibold text-kumo-default">
					...
					{activeItem.children.map((child) => {
						...
						return (
							<LinkButton
								key={child.id}
								href={child.path}
								variant={isChildActive ? "outline" : "ghost"}
								className="h-auto flex-col items-start justify-start gap-1 p-4 text-start border border-kumo-line"
							>

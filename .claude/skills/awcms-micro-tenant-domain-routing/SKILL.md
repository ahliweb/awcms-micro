---
name: awcms-micro-tenant-domain-routing
description: Kerjakan bagian mana pun dari epic online public tenant routing & tenant domain management AWCMS-Micro (Issue #556-#567, epic #555). Gunakan saat menambah/mengubah PUBLIC_* env config, skema/API/UI tenant domain, resolver tenant berbasis host, rute publik `/news`, module presets, atau adapter Cloudflare DNS. Merangkum keputusan yang sudah dibuat supaya issue lanjutan tidak mengulang/kontradiksi.
---

# AWCMS-Micro — Online Public Tenant Routing & Tenant Domain Management
...
jalan — lihat ADR-0009 dan skill `awcms-micro-blog-content`). Target model:
...
`awcms-micro-new-migration`, `awcms-micro-new-module`,
`awcms-micro-blog-content` (untuk sisi `/news` di modul `blog_content`),
dan `awcms-micro-module-management` (untuk sisi module presets/matrix UI).
...
Didokumentasikan di `docs/awcms-micro/18_configuration_env_reference.md`
§Public routing dan `docs/awcms-micro/deployment-profiles.md` §Profil
online. Test: `tests/validate-env.test.ts`'s
`describe("checkPublicRoutingConfig", ...)`.

### Schema (Issue #557, `sql/031`/`sql/032`)

Tabel `awcms_micro_tenant_domains` — pemetaan hostname/domain/subdomain →
...
- Migration di-split dua file mengikuti pola `blog_content` (026 schema /
  027 permission, diulang lagi di 029/030): `sql/031_awcms_micro_tenant_domain_schema.sql`
  (tabel) dan `sql/032_awcms_micro_tenant_domain_permissions.sql`
  (permission seed).
- Kolom kunci: `hostname` (raw, case asli) + `normalized_hostname` (kolom
  ...
  `awcms_micro_tenant_domains_normalized_hostname_matches_check`);
  ...
  `redirect_to_primary`.
- Constraint kunci: `awcms_micro_tenant_domains_normalized_hostname_dedup`
  (unique index global — LINTAS tenant, bukan per-tenant — pada
  `normalized_hostname` `WHERE deleted_at IS NULL`, karena satu hostname
  cuma boleh milik satu tenant); `awcms_micro_tenant_domains_primary_dedup`
...
"openapi/awcms-micro-public-api.openapi.yaml" }` dan `navigation: [{
...
`awcms_micro_setup_state.tenant_id` → (5) `null` (404 generic). **Langkah
...
menyentuh fungsi bootstrap `awcms_micro_tenant_domains` — permukaan lebih
kecil, bukan cuma code path lebih pendek.

#### Keputusan `tenant_code_legacy` (diputuskan Issue #560)

Dua reviewer Issue #559 (awcms-micro-reviewer dan awcms-micro-security-auditor)
...
**Fungsi `SECURITY DEFINER` bootstrap** — `sql/033_awcms_micro_tenant_domain_lookup_function.sql`
(pakai checklist umum `SECURITY DEFINER` di
`docs/adr/0003-postgresql-rls-multi-tenant.md` §Checklist untuk fungsi baru
lain di masa depan):

```sql
CREATE OR REPLACE FUNCTION awcms_micro_resolve_tenant_domain_lookup(p_normalized_hostname text)
RETURNS TABLE (
...
AS $function$
  ...
  FROM awcms_micro_tenant_domains AS d
  JOIN awcms_micro_tenants AS t ON t.id = d.tenant_id
  WHERE d.normalized_hostname = p_normalized_hostname AND d.deleted_at IS NULL;
$function$;

REVOKE ALL ON FUNCTION awcms_micro_resolve_tenant_domain_lookup(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION awcms_micro_resolve_tenant_domain_lookup(text) TO awcms_micro_app;
```
...
conditional query against `awcms_micro_tenants` only when the domain row
...
by response latency. Fixed by joining `awcms_micro_tenants` into this same
...
- Migration jalan sebagai role pemilik schema (`POSTGRES_USER`,
  `awcms-micro` di `docker-compose.yml`), dan role itu adalah **Postgres
...
- Karena keamanan TIDAK datang dari RLS/FORCE di level fungsi ini, dua hal
  ...
  di-revoke dari `PUBLIC` lalu di-grant eksplisit hanya ke `awcms_micro_app`
  — tidak ada role non-superuser lain yang bisa memanggil fungsi ini, dan
  `awcms_micro_app` sendiri tetap tidak bisa `SELECT` langsung dari
  `awcms_micro_tenant_domains` tanpa `withTenant(...)` (dibuktikan di
  integration test).
...
- Diverifikasi di
  ...
  ini resolve rows lewat `awcms_micro_app` TANPA `app.current_tenant_id`
...
`/blog/{tenantCode}/...` (Issue #540, skill `awcms-micro-blog-content`),
...
`awcms_micro_tenant_domains` — kode aplikasi PERTAMA yang pernah menulis baris
...
`awcms-micro-idempotency`, scope `tenant_domain_verify`/
`tenant_domain_set_primary`) dan tetap diaudit eksplisit
(`tenant_domain.domain.verified`/`.set_primary`) terlepas dari klasifikasi
itu.

Constraint global `awcms_micro_tenant_domains_normalized_hostname_dedup`
...
`awcms_micro_tenant_domains_primary_dedup` tidak pernah dilanggar
...
`try/catch` yang menangkap pelanggaran `awcms_micro_tenant_domains_primary_dedup`
...
1. **`rssEnabled`/`sitemapEnabled` SENGAJA TIDAK** ada di store baru ini
   ...
   rumah di `awcms_micro_blog_settings`/`fetchBlogSettings()` sejak Issue
   #543, sudah ditegakkan `/news/feed.xml`/`sitemap-news.xml` sejak Issue
   #560. Menambahkannya ke store generik (`awcms_micro_module_settings`,
...
`blogTitle` di `awcms_micro_blog_settings`), tapi sejak perbaikan
...
- `TENANT_DOMAIN_CLOUDFLARE_API_TOKEN` — wajib bila `cloudflare`, secret
  ...
  `awcms_micro_tenant_domains`/`awcms_micro_module_settings`/tabel lain, tidak
...
Tabel `awcms_micro_tenant_domains` sekarang bisa ditulis lewat kode aplikasi

---
name: awcms-micro-observability
description: Kelola sistem log/audit/metrics AWCMS-Micro yang sudah aktif — correlation ID lintas hop, retensi/purge audit log, extension point untuk consumer eksternal (alerting/export/SIEM), dan metrics port (counter/histogram/gauge berkardinalitas rendah untuk request/pool/job/provider). Gunakan saat menambah endpoint baru (correlation ID otomatis), menjadwalkan purge audit log, memasang consumer log/audit di aplikasi turunan, atau menambah/mengonsumsi metrik operasional. Berbeda dari awcms-micro-audit-log (APA yang wajib diaudit) — skill ini tentang BAGAIMANA sistem log/audit/metrics itu sendiri dikelola, sesuai Issue #447 dan Issue #698.
---

# AWCMS-Micro — Observability (Correlation ID, Retensi, Extension Point, Metrics)

Sumber kebenaran: `src/lib/logging/logger.ts`, `src/lib/logging/correlation-response.ts`, `src/modules/logging/application/{audit-log,audit-purge}.ts`, `src/lib/observability/metrics-port.ts`, `docs/awcms-micro/observability-metrics.md`, `docs/awcms-micro/20_threat_model_security_architecture.md` §Matrix kepatuhan A.8.15/A.8.16 + §Standar tambahan Issue #698. Implementasi referensi: Issue 10.1 (fondasi log/audit) + Issue #447 (aktivasi) + Issue #698 (metrics/SLO/job health/provider telemetry).
...
- **Endpoint baru**: tidak perlu wiring apa pun — cukup pakai `ok()`/`fail()` seperti biasa (`awcms-micro-new-endpoint`), `meta.correlationId` otomatis terisi.
...
## Retensi/purge `awcms_micro_audit_events`
...
1. **Jangan lakukan I/O eksternal blocking langsung di dalam hook** — `AuditExportHook` dipanggil **di dalam transaksi DB yang sama** dengan INSERT (ADR-0006: provider tidak boleh dipanggil di dalam transaction). Kalau consumer butuh mengirim ke luar (HTTP call ke SIEM, dsb.), **enqueue** lewat pola outbox/dispatcher yang sudah ada (`awcms-micro-integration`, `object-dispatch.ts`), jangan panggil langsung dari hook.
...
- **Metrik edge cache** (Issue #353, ADR-0037): `edge_cache_boost_active` + `edge_cache_pressure_percent` (gauge tanpa label — sengaja, agar tidak bisa mengkodekan tenant/rute), `edge_cache_escalation_transitions_total` (label `from`/`to`, enum 2 nilai), `edge_cache_purge_total` (label `outcome` saja — hostname yang di-purge TIDAK PERNAH jadi label karena mengidentifikasi tenant). Laju transisi yang tinggi berarti ambang eskalasi salah setel untuk deployment itu, bukan cache-nya rusak. Tabel ringkasan di `observability-metrics.md` **tidak** digerbangi CI — perbarui manual saat menambah metrik.
...
`awcms-micro-audit-log` (APA yang wajib diaudit + redaksi), `awcms-micro-integration` (pola dispatcher/outbox untuk I/O eksternal, ADR-0006), `awcms-micro-security-hardening` (batas scope A.8.16 SIEM/monitoring terpusat), `awcms-micro-performance` (pool/backpressure tuning yang metrik `db_pool_work_class_*` sekarang membuatnya observable).

...
| Panggilan keluar (semua modul)         | sukses ditentukan **penanda positif dari responder**, bukan status code — pola repo: `body.success` (Cloudflare), `parsed.ok` (Telegram), `status === "success"` (Mailketing), `X-Edge-Cache-Ban: ok` (edge cache). Sebuah 200 tanpa penanda wajib dilaporkan **gagal**; itulah yang membuat invalidasi mati total tampak sehat selama dua rilis (#359/#361). Metode HTTP wajib verb standar HURUF BESAR penuh — `fetch` Bun mengirim `GET` untuk yang tidak cocok huruf-per-huruf (`Post`/`BAN` → `GET`), digerbangi `bun run http:methods:check` |

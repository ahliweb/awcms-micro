...
`awcms-micro-new-migration`, `awcms-micro-idempotency`,
`awcms-micro-abac-guard`, `awcms-micro-audit-log`, dan
`awcms-micro-sensitive-data` (kredensial provider, TOTP seed, recovery
...
#587). Detail env var lengkap: `docs/awcms-micro/18_configuration_env_reference.md`
§Full-online auth security hardening,
`docs/awcms-micro/deployment-profiles.md` §Full-online auth security
...
- **Login yang dijeda, bukan ditolak**: password valid + factor `active`
  ...
  `awcms_micro_mfa_challenges` dan balas `401 MFA_REQUIRED` berisi
...
- **Replay prevention, dan WAJIB atomik, bukan read-then-write** —
  `awcms_micro_identity_mfa_factors.last_used_step` menyimpan step
  ...
  **PR #597 security review menemukan `verifyMfaChallenge` awalnya
  ...
  `awcms_micro_identity_mfa_recovery_codes.used_at`, DAN
  `awcms_micro_mfa_challenges.failed_attempts`) — di bawah READ COMMITTED
...
- **Reset password BUKAN bypass MFA** — `completePasswordReset` tidak
  menyentuh tabel `awcms_micro_identity_mfa_factors` sama sekali;
...
- **Tenant id lewat `state`, bukan header** — `GET .../callback` adalah
  ...
  membawa header `X-AWCMS-Micro-Tenant-ID` seperti endpoint lain. `state`
...
- **Dua flow berbeda dari satu orkestrator**: `GET .../start`
  ...
  `purpose`/`identity_id` di baris `awcms_micro_oidc_auth_requests` yang
...
- **Provider account ditautkan via `sub`, TIDAK PERNAH via email**
  ...
  `awcms_micro_identity_provider_accounts`, unique per
...
awcms_micro_oidc_auth_requests` dengan `tenantId` dari query param
  tak terautentikasi TANPA mengecek tenant itu benar-benar ada lebih
  dulu. `tenant_id` punya FK ke `awcms_micro_tenants` — tenant palsu
  ...
  **`getDatabaseCircuitBreaker()`, breaker tunggal APLIKASI-LEBAR**
  ...
  PR #596. Diperbaiki dengan `SELECT status FROM awcms_micro_tenants
...
- **Reuses `awcms_micro_oidc_auth_requests`/`awcms_micro_identity_provider_accounts`
  ...
  `awcms_micro_auth_providers` (tenant-configured provider config: `provider_key`,
  ...
  soft delete) and `awcms_micro_tenant_auth_policies` (one row per
...
/auth/sso/{providerKey}/start` (unauthenticated, tenant resolved from
  ...
  any INSERT into the reused `awcms_micro_oidc_auth_requests` — applying
...
- **Break-glass enforcement is at POLICY-SAVE time, not just login
  ...
  `awcms_micro_tenant_users` membership — a request that would leave
...
"<path>" cannot be parsed as a URL.` — reproduces even with a fully
  ...
  endpoint (`awcms_micro_setup_state`) almost always already claimed on
...
4. **Login password lokal tidak pernah dihapus/dinonaktifkan secara
   ...
   `password_login_enabled=false` (#591, `awcms_micro_tenant_auth_policies`)
...
5. **Kredensial provider (Google client secret, OIDC client secret,
   ...
   response API, log, atau audit attributes (`awcms-micro-sensitive-data`).
6. **Link/unlink provider account, perubahan kebijakan auth, enroll/disable
   ...
   diaudit (`awcms-micro-audit-log`) dan idempotent kalau mutation
...
9. **Tabel baru yang tenant-scoped (`awcms_micro_identity_provider_accounts`,
   `awcms_micro_oidc_auth_requests` — #590; `awcms_micro_auth_providers`,
   `awcms_micro_tenant_auth_policies` — #591; `awcms_micro_identity_mfa_factors`
   dkk. — #589) wajib RLS `ENABLE` + `FORCE`** — pola sama seperti setiap
   migration sejak 013 (`awcms-micro-new-migration`).
...
atas. `awcms_micro_auth_providers`/`awcms_micro_tenant_auth_policies`
...
- `docs/awcms-micro/18_configuration_env_reference.md` dan
...
- `docs/awcms-micro/20_threat_model_security_architecture.md` sebelumnya
...
- `.env.example`, `scripts/validate-env.ts`, OpenAPI
  (`openapi/awcms-micro-public-api.openapi.yaml`), dan
...
yang tepat karena AWCMS-Micro di sini berperan sebagai relying party yang
...
- Rekomendasi infra-layer blokir egress ke `169.254.169.254` (metadata
  ...
  **Follow-up — selesai (Issue #612)**: tanpa cap, tenant admin jahat bisa
  mendaftarkan banyak baris `awcms_micro_auth_providers` (masing-masing dapat

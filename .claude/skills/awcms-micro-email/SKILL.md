---
name: awcms-micro-email
description: Kirim email transaksional (password reset, announcement, workflow notification) via modul email reusable AWCMS-Micro — provider-neutral (Mailketing adapter), template management, dan dispatcher outbox. Gunakan saat modul domain turunan perlu mengirim email, atau saat menambah kategori/template baru.
---

# AWCMS-Micro — Email Module
...
3. **Enqueue** — dua opsi:
   ...
   - **Kasus lain (mis. modul domain turunan sendiri)** — INSERT langsung
     ke `awcms_micro_email_messages` (`sql/020`) di dalam transaksi bisnis
...
`awcms-micro-integration` (pola outbox/retry/circuit-breaker generik),
`awcms-micro-sensitive-data` (normalize/hash/mask alamat email),
`awcms-micro-idempotency` (`POST /email/announcements` mewajibkan
`Idempotency-Key` di setiap request, bukan hanya bulk), `awcms-micro-abac-guard`
...
`awcms-micro-observability` (`security:readiness` gate, structured log per

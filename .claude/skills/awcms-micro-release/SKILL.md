...
4. Review diff; pastikan tingkat bump cocok SemVer ketat ala keluarga `awcms` (breaking → MAJOR, apa pun titik versinya). Baseline saat ini `1.0.0` (rilis 2026-07-21, MAJOR yang dipicu breaking ADR-0036 — `MODULE_CONTRACT_VERSION` `1.5.0 → 2.0.0`, penghapusan penuh jalur aplikasi-turunan); peta milestone lama doc 09 (`0.1.0 Foundation … 1.0.0 base production-ready`) adalah rencana historis, bukan pagar versi — angka rilis mengikuti dampak perubahan, bukan urutan milestone.
...
- SemVer ketat ala keluarga `awcms` (yang sudah lama di jalur `3.x`): perubahan breaking pada API/kontrak/schema publik selalu MAJOR — **tidak** dilipat jadi minor meski pra-1.0. Contoh preseden: changeset `major` ADR-0036 membawa `0.3.1 → 1.0.0` (bukan `0.4.0`). Tetap tulis ringkasan breaking yang jelas di changeset.

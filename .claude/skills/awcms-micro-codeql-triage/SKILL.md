...
2. **Ambil detail + pesan asli per alert** (bukan cuma nama rule):
   ```bash
   gh api repos/ahliweb/awcms-micro/code-scanning/alerts/<N>
   ```
   Baca `most_recent_instance.message.text` — ini alasan CONCRETE CodeQL, bukan deskripsi generik rule. Untuk PR yang gagal check, `gh api repos/ahliweb/awcms-micro/check-runs/<id>/annotations` memberi lokasi+pesan yang sama.
...
4. **Perbaiki dengan code change minimal, behavior-preserving** — bukan menekan alert. Kalau setelah investigasi ternyata false positive murni tanpa cara reformulasi kode yang wajar, baru pertimbangkan dismiss resmi lewat API (lihat §4 katalog di bawah untuk dua kasus nyata):
   ```bash
   gh api repos/ahliweb/awcms-micro/code-scanning/alerts/<N> -X PATCH \
...
`awcms-micro-security-review` (checklist keamanan modul, bukan tooling scan), `awcms-micro-pr-review` (proses review PR secara umum).

---
name: awcms-micro-new-event
description: Tambah atau ubah domain event AWCMS-Micro. Gunakan saat mem-publish event baru (mis. sales.transaction.posted), mengubah payload event, atau menambah subscriber. Menegakkan envelope event standar dan update AsyncAPI sesuai doc 05.
---

# AWCMS-Micro — New / Changed Domain Event

Ikuti `docs/awcms-micro/05_openapi_asyncapi_detail.md`.
...
Satu-satunya sumber kebenaran: `asyncapi/awcms-micro-domain-events.asyncapi.yaml`'s
...
type DomainEventEnvelope<TPayload> = {
  ...
  event_type: string; // pattern "^[a-z0-9]+(\.[a-z0-9_]+)+$", mis. "awcms-micro.blog-content.post.published"
  occurred_at: string; // ISO date-time
  producer: { service: "awcms-micro"; module: string }; // module = ModuleDescriptor.key penerbit
...
`appendDomainEvent` menyimpan baris ini ke `awcms_micro_domain_events` +
...
3. **Payload tidak boleh** membawa raw PII/tax identity (mask dulu — `awcms-micro-sensitive-data`).
4. Publish **setelah** commit transaction (atau lewat outbox), bukan menggantikan audit.
5. Node hybrid: event masuk `awcms_micro_sync_outbox` untuk sync (`awcms-micro-sync-hmac`).
...
`asyncapi/awcms-micro-domain-events.asyncapi.yaml` adalah satu-satunya
...
`grep -c "^  awcms-micro\." asyncapi/awcms-micro-domain-events.asyncapi.yaml`
untuk hitungan hidup, dan `grep -n "^  awcms-micro\." ...` untuk daftar
lengkap. Contoh representatif lintas modul yang BENAR-BENAR ada di sana:

- `awcms-micro.blog-content.post.published` (`blog_content`)
- `awcms-micro.social-publishing.job.published` (`social_publishing`)
- `awcms-micro.email.message.sent` (`email`)
- `awcms-micro.sync.push.requested` (baseline envelope `sync-storage`)
- `awcms-micro.database.pool.saturated` (backpressure/pooling, doc 16)

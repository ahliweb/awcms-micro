---
name: awcms-micro-ux-review
description: Audit dan tingkatkan kualitas UI/UX AWCMS-Micro di atas baseline design system. Gunakan saat diminta "review UX", "perbaiki tampilan/usability", audit aksesibilitas, atau menaikkan kualitas layar admin/POS/portal yang sudah ada. Berbeda dari awcms-micro-ui-screen (membangun layar baru sesuai standar) — skill ini menilai & menaikkan mutu layar yang sudah jadi.
---

# AWCMS-Micro — UI/UX Improvement Review

Sumber kebenaran: **`docs/awcms-micro/14_ui_ux_design_system.md`** (token, komponen, state pattern, a11y, i18n, theming) dan **`docs/awcms-micro/15_frontend_architecture_integration.md`** (SSR/islands, API client, offline). Skill ini **peningkatan** — bukan membangun dari nol (itu `awcms-micro-ui-screen`), melainkan menemukan gap kualitas dan menaikkannya.
...
- [ ] **Konsistensi token/komponen** — tak ada warna/ukuran/spacing/**durasi motion** hardcode; pakai `--color-*`/`--sp-*`/`--fs-*`/`--dur-*`/`--ease-*`; komponen dari `src/components/ui`, bukan duplikat ad-hoc; scrim dialog/drawer pakai `--color-scrim`, bukan literal `rgba(...)`.
- [ ] **Dark/light parity** — kedua tema diuji; kontras & keterbacaan setara; `data-theme` konsisten.
- [ ] **Responsif** — admin desktop-first tapi tetap usable di tablet; portal customer mobile-first; tak ada horizontal scroll tak sengaja; tabel lebar → scroll container (`overflow-x: auto`, Issue #434). **A11y/layout E2E berjalan di viewport lebar — bug mobile-only lolos CI**: verifikasi headless di 360px (contoh #315: overflow topbar admin + `scrollable-region-focusable` pada tabel overflow hanya tertangkap di 360px). Surface publik CSS-less (raw-HTML `.ts` route) di-style HANYA lewat `public/css/public-content.css` eksternal (CSP `default-src 'self'` memblokir inline).
...
- [ ] **Micro-copy & i18n-ready** — teks jelas, ringkas, konsisten istilah (doc 19 glossary); lihat skill `awcms-micro-i18n` untuk detail katalog `.po`/locale/formatter — cari string hardcode yang lolos ekstraksi sebelumnya (komponen kecil seperti theme toggle sering terlewat, Issue #434).
...
`awcms-micro-ui-screen` (membangun layar sesuai standar), `awcms-micro-i18n` (katalog `.po`, locale, formatter), `awcms-micro-sensitive-data` (masking), `awcms-micro-testing` (render/state test), `awcms-micro-performance` (waktu muat & data fetching).

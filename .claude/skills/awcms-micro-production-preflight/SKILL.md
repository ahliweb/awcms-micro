...
→ `modules:compose:check` (Issue #740, epic #738 — registry base
terkomposisi valid; ADR-0036 menghapus gerbang `extension:check` turunan
...
**Edge cache (hanya bila dipasang, Issue #353/ADR-0037):** `bun run edge-cache:health` keluar zero · endpoint ban **menolak** purge tanpa token · `X-Cache` menunjukkan MISS lalu HIT pada halaman publik yang sama · `Surrogate-Control` tidak muncul di respons ke klien · **`bun run edge-cache:verify -- --url=https://<domain>/` keluar zero**. Item terakhir itu yang paling penting dan paling mudah dilewatkan: `health` hanya membuktikan endpoint menjawab, sedangkan `verify` membuktikan purge benar-benar membuang objeknya (hangat → `HIT` → purge → wajib `MISS`). Invalidasi pernah mati total selama dua rilis sementara `health` tetap hijau (Issue #359/#361). Lapisan ini opsional dan **tidak** diperiksa `security:readiness`/`production:preflight` — ia hidup di luar proses aplikasi, jadi ini langkah manual terpisah. Deployment tanpa edge cache melewatinya sepenuhnya.

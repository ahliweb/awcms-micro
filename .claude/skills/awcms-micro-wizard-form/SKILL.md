---
name: awcms-micro-wizard-form
description: Bangun form multi-step di modul domain memakai reusable wizard pattern AWCMS-Micro. Gunakan saat input data panjang/bertahap butuh lebih dari satu layar (identitas, detail, lampiran, review) sebelum submit final. Sesuai docs/awcms-micro/examples/wizard-form-pattern.md.
---

# AWCMS-Micro — Reusable Wizard Form

Ikuti `docs/awcms-micro/examples/wizard-form-pattern.md` (spesifikasi
komponen + pola i18n) dan
`docs/awcms-micro/examples/wizard-derived-module-example.md` (contoh
...
3. **Submit final high-risk** — `createWizardIdempotencyKey()` sekali per
   attempt submit (bukan per klik tombol) — skill `awcms-micro-idempotency`.
...
7. **Draft client-side hanya data non-sensitif**, dan tidak persisten
   ...
   server-side draft persistence — skill `awcms-micro-form-drafts`
...
`awcms-micro-ui-screen` (pola layar/token/a11y umum), `awcms-micro-i18n`
(katalog `.po`), `awcms-micro-idempotency` (submit final high-risk),
`awcms-micro-new-endpoint` (endpoint domain target submit),
`awcms-micro-form-drafts` (resume-on-load lintas sesi via server).

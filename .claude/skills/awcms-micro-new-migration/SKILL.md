---
name: awcms-micro-new-migration
description: Buat migration SQL PostgreSQL AWCMS-Micro yang benar. Gunakan setiap kali menambah/mengubah tabel, kolom, index, constraint, atau RLS. Menegakkan penamaan NNN_awcms_micro_<area>_<desc>.sql, tenant_id, RLS, index FK, timestamptz, dan numeric sesuai doc 04 & 10.
---

# AWCMS-Micro — New SQL Migration

Ikuti standar di `docs/awcms-micro/04_erd_data_dictionary.md` dan `docs/awcms-micro/10_template_kode_coding_standard.md`.

## Penamaan

```text
sql/NNN_awcms_micro_<area>_<description>.sql
```
...
11. Tabel BARU tanpa `tenant_id`/RLS (global, dibaca/ditulis lintas
    ...
    tabel tenant-scoped ke `awcms_micro_app` (Issue #683, epic #679, lihat
    `sql/045_awcms_micro_db_role_separation.sql`'s header) — grant
...
```sql
CREATE TABLE IF NOT EXISTS awcms_micro_<name> (
...
CREATE INDEX IF NOT EXISTS awcms_micro_<name>_tenant_idx
  ON awcms_micro_<name> (tenant_id);
CREATE INDEX IF NOT EXISTS awcms_micro_<name>_tenant_created_idx
  ON awcms_micro_<name> (tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS awcms_micro_<name>_active_idx
  ON awcms_micro_<name> (tenant_id, created_at DESC)
...
CREATE POLICY awcms_micro_<name>_tenant_isolation ON awcms_micro_<name>
...
`sql/033_awcms_micro_tenant_domain_lookup_function.sql`, Issue #559):
...
4. `REVOKE ALL ... FROM PUBLIC` lalu `GRANT EXECUTE` eksplisit ke role
   spesifik (mis. `awcms_micro_app`) — ini **tidak** otomatis tercakup

...
`awcms-micro-new-migration`, `awcms-micro-integration` (pola outbox/circuit
breaker eksternal, ADR-0006), `awcms-micro-idempotency` (mutation
connect/disconnect/approve/cancel/retry), `awcms-micro-abac-guard`,
`awcms-micro-audit-log`, dan `awcms-micro-sensitive-data` (token
...
`awcms_micro_social_publishing_settings` (Keputusan kunci #2 di bawah).

### Keputusan kunci #2 — tabel settings per-tenant BOLEH tenant-writable (BUKAN pengulangan anti-pattern #636)

`awcms_micro_social_publishing_settings` (`tenant_id` PK,
...
(`.claude/skills/awcms-micro-news-portal/SKILL.md` §636): #636 butuh
...
`awcms_micro_social_accounts.token_reference` adalah string buram (mis.
...
`awcms_micro_social_accounts`/`..._social_publish_jobs`) sengaja HANYA
divalidasi FORMAT (`^[a-z][a-z0-9_]{1,49}$`), bukan `CHECK` enum tetap
seperti `awcms_micro_news_portal_ad_placements.placement_key` — supaya
...
dari dalam kode yang juga menulis ke `awcms_micro_blog_posts` atau berjalan
...
import TS) ke `awcms_micro_tenant_domains` (`is_primary = true AND status
...
`awcms_micro_social_publish_jobs_idempotency_key` (UNIQUE
...
- `needs_reauth`: TIDAK auto-retry, DAN `awcms_micro_social_accounts`
...
- `sql/053_awcms_micro_social_publishing_schema.sql` (6 tabel + 10
  permission seed).
...
- `asyncapi/awcms-micro-domain-events.asyncapi.yaml` (15 channel/operation
  baru, `awcms-micro.social-publishing.*`).
...
- Endpoint/UI "manual editor action" nyata untuk trigger
  `manual_editor_action` (dimodelkan penuh di `awcms_micro_social_publish_rules.trigger_event`,
...
- Admin UI khusus untuk `awcms_micro_social_publish_templates` sebagai
...
`awcms_micro_social_accounts` yang mewakili "koneksi Meta" gabungan —
...
2. **Critical** — tiga titik panggilan di `linkedin-provider-adapter.ts`
   ...
   `awcms_micro_social_publish_jobs.last_error_message`/
...
- `SKILL.md` ini sendiri (tabel status + section baru per provider) dan
  `docs/awcms-micro/repo-inventory.md`/i18n hasil generate — pola merge-
...
`awcms_micro_social_accounts` (Issue #643) TANPA kolom baru:
...
(`awcms_micro_social_publish_jobs`, migration 053) tidak punya kolom nama
...
- `sql/055_awcms_micro_social_publishing_verify_permission.sql` (satu
...
`docs/awcms-micro/news-portal/` plus pembaruan index
`docs/awcms-micro/README.md`, menutup epic `social_publishing` (#643-#647)
...
- `docs/awcms-micro/news-portal/social-sharing.md` (baru).
- `docs/awcms-micro/news-portal/social-publishing-architecture.md` (baru).
- `docs/awcms-micro/news-portal/social-publishing-sop.md` (baru).
- `docs/awcms-micro/news-portal/social-provider-limitations.md` (baru).
- `docs/awcms-micro/news-portal/social-publishing-security-checklist.md`
  (baru).
- `docs/awcms-micro/README.md` (index, tautan ke lima dokumen di atas).

...
1. Audit **tenant-scoped** (`tenant_id`), tulis ke `awcms_micro_audit_events`.
...
5. Untuk denial high-risk, koordinasikan dengan decision log (`awcms-micro-abac-guard`).
...
- Retention audit: 1–5 tahun sesuai kebutuhan — mekanisme purge nyata (`purgeExpiredAuditEvents`, default 730 hari, `bun run logs:audit:purge`) sudah tersedia sejak Issue #447, JANGAN buat mekanisme purge baru untuk `awcms_micro_audit_events`, lihat `awcms-micro-observability`.

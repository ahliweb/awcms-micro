---
name: awcms-micro-idempotency
description: Terapkan idempotency pada mutation high-risk AWCMS-Micro agar aman dari double-submit. Gunakan saat implementasi posting transaksi, cancel/return, transfer approve/ship/receive, cycle count, adjustment, VAT generate, Coretax batch, receipt send, sync push, atau workflow decision. Sesuai doc 10.
---

# AWCMS-Micro — Idempotent High-Risk Mutation

Ikuti `docs/awcms-micro/10_template_kode_coding_standard.md`.

## Alur

```mermaid
flowchart TD
  A[Baca Idempotency-Key header] --> B{Key ada di awcms_micro_idempotency_keys?}
...
5. Simpan status/resource hasil mutation di `awcms_micro_idempotency_keys`.

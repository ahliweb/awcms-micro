---
name: awcms-micro-integration
description: Audit dan perkuat backend & integrasi eksternal AWCMS-Micro (resiliensi, outbox, webhook/provider, versioning API, observability). Gunakan saat menambah/mengeraskan integrasi ke provider luar (R2, WhatsApp, email, AI, pajak), memperbaiki keandalan delivery, atau menaikkan ketahanan backend. Menegakkan ADR-0006 (provider opsional, di luar transaksi) dan pola outbox doc 16.
---

# AWCMS-Micro — Backend & Integration Hardening

Sumber kebenaran: **`docs/awcms-micro/16_backend_data_access_integration.md`** (transactional outbox, transaction wrapper, idempotency), **`docs/awcms-micro/05_openapi_asyncapi_detail.md`** (kontrak API/event), dan **`docs/awcms-micro/10_template_kode_coding_standard.md`** (guardrail). Skill ini **peningkatan**: menaikkan keandalan & ketahanan, bukan sekadar membuat endpoint (itu `awcms-micro-new-endpoint`).
...
- [ ] **Verifikasi webhook masuk** — signature HMAC + anti-replay (timestamp skew) untuk callback provider, pola `awcms-micro-sync-hmac`. Jangan percaya payload tak tertandatangani.
- [ ] **SSRF-safe** — URL provider dari konfigurasi tepercaya (env), bukan dari input user; validasi host bila perlu. Contoh nyata: `object-storage-uploader.ts` (Issue #436) — endpoint R2 selalu dari `process.env.AWCMS_MICRO_R2_ACCOUNT_ID` (fallback legacy `R2_ACCOUNT_ID`), tak pernah dari request.
...
- [ ] **Observability** — correlation ID otomatis di `meta.correlationId` untuk endpoint `/api/*` sejak Issue #447 (tidak perlu wiring manual, lihat `awcms-micro-observability`); log terstruktur JSON (hormati `LOG_LEVEL`); aksi high-risk masuk audit; redaksi secret/PII sebelum log; extension point `setLogSink`/`setAuditExportHook` tersedia bagi aplikasi turunan yang butuh forward ke SIEM eksternal (`awcms-micro-observability`).
...
`awcms-micro-new-endpoint` / `awcms-micro-new-event` (kontrak), `awcms-micro-sync-hmac` (signature), `awcms-micro-idempotency` (mutation), `awcms-micro-audit-log` (jejak), `awcms-micro-observability` (correlation ID, extension point), `awcms-micro-performance` (I/O di luar transaksi).

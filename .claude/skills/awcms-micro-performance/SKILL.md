---
name: awcms-micro-performance
description: Audit dan tingkatkan performa aplikasi & database AWCMS-Micro. Gunakan saat diminta "optimasi performa/query", ada endpoint lambat, N+1, masalah indexing/pagination, tuning connection pool, atau perencanaan materialized view/caching. Menegakkan pola akses data doc 16, pooling/backpressure, dan pagination keyset.
---

# AWCMS-Micro — Performance & Database Tuning

Sumber kebenaran: **`docs/awcms-micro/16_backend_data_access_integration.md`** (lapisan akses data, pooling/backpressure, transaction), **`docs/awcms-micro/database-pooling.md`**, dan **`docs/awcms-micro/07_sprint_testing_production_readiness.md`** (target performa). Skill ini **peningkatan**: ukur → temukan bottleneck → perbaiki → ukur ulang.
...
- [ ] **JANGAN `Promise.all` beberapa query pada satu `tx`** — `tx` (dari `withTenant`/`sql.begin`) adalah SATU koneksi transaksi; query konkuren padanya men-desync koneksi → tersangkut `idle in transaction` → slot work-class bocor selamanya → pool jenuh (`503 DATABASE_BUSY`). `await` berurutan; `Promise.all(items.map(x => q(tx,x)))` → loop `for`. `Promise.all` hanya untuk query pool terpisah / kerja non-DB. Insiden prod 2026-07-24 (`fetchSidebarArrangement`, 16 lokasi diperbaiki PR #324). Lihat `database-pooling.md` §9.
- [ ] **`idle_in_transaction_session_timeout`** — `DATABASE_IDLE_IN_TXN_TIMEOUT_MS` (default 30000ms) mereap sesi idle-in-transaction; jaring pengaman anti pool-saturation dari kebocoran transaksi (`statement_timeout` tak bisa mereap idle-in-txn). Pertahanan, bukan pengganti aturan di atas.
- [ ] **Pemanggil `withTenant` non-`Response` pakai `unavailableBehavior: "throw"`** — render SSR / `resolveSsrContext` (`T` = objek data): tanpa opsi ini, fallback saturasi mengembalikan `Response` 503 yang menyamar sebagai data dan meng-crash pemanggil (PR #323). Dengan `"throw"` → `DatabaseUnavailableError` ditangkap `try/catch` untuk degrade anggun. Route API tetap default.
...
Lihat [`performance-suite.md`](../../../docs/awcms-micro/performance-suite.md)
untuk arsitektur lengkap, safe subset vs full lane, dan format artefak.

## Skill terkait

`awcms-micro-new-migration` (tambah index via migration berurutan), `awcms-micro-integration` (I/O eksternal & outbox), `awcms-micro-testing` (benchmark/load test), `awcms-micro-production-preflight` (`db:pool:health`).

...
# §"Restore SOP ringkas", skill `awcms-micro-production-preflight` §"Backup &
# restore".
#
...
#     "awcms_micro_restore_test" (matching doc 07's own example), which this
...
Usage: restore-postgres.sh <dump.enc-file> [--target=<dbname>] [--acknowledge-target=<dbname>] [--yes]
  ...
  --target=<dbname>          Restore into an existing database instead of
                              the default disposable "awcms_micro_restore_test".
...
acquire_lock "${BACKUP_DIR_FOR_LOCK}/.awcms-micro-backup-restore.lock"
...
DEFAULT_TARGET_DB="awcms_micro_restore_test"
override_mode=false
...
echo "  PGDATABASE=${TARGET_DB} psql -c 'SELECT count(*) FROM awcms_micro_tenants;'"

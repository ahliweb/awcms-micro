...
# `awcms-micro-production-preflight` §"Backup & restore".
#
...
#   BACKUP_DIR                  optional. Default: /var/backups/awcms-micro
...
# (awcms_micro_<UTC timestamp>.dump.enc) — the plaintext dump is never
# written to disk, `pg_dump` streams directly into `openssl enc` via a pipe —
# plus a signed JSON manifest (awcms_micro_<UTC timestamp>.manifest.json)
...
acquire_lock "$BACKUP_DIR/.awcms-micro-backup-restore.lock"

parse_database_url "$DATABASE_URL"
unset DATABASE_URL

timestamp="$(date -u +%Y%m%d_%H%M%S)"
dump_file="${BACKUP_DIR}/awcms_micro_${timestamp}.dump.enc"
manifest_file="${BACKUP_DIR}/awcms_micro_${timestamp}.manifest.json"
...
# `HMAC(secret, "<timestamp>.<body>")` shape as skill `awcms-micro-sync-hmac`),
...
find "$BACKUP_DIR" -maxdepth 1 -name 'awcms_micro_*.dump.enc' -mtime "+${BACKUP_RETENTION_DAYS}" -print -delete
find "$BACKUP_DIR" -maxdepth 1 -name 'awcms_micro_*.manifest.json' -mtime "+${BACKUP_RETENTION_DAYS}" -print -delete

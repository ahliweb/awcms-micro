...
# "awcms_micro_restore_test" db an operator might be using by hand) ->
# verifies the schema-migrations ledger, tenant isolation (RLS, via the
# actual least-privilege `awcms_micro_app` role if present — see below), and
...
# tenant_isolation coming back "skip", e.g. the awcms_micro_app role is
...
#   DRILL_TARGET_DB    optional. Default: awcms_micro_restore_drill. Created
...
#                       ("awcms_micro_restore_test") so a scheduled drill
...
BACKUP_DIR="${BACKUP_DIR:-/var/backups/awcms-micro}"
DRILL_TARGET_DB="${DRILL_TARGET_DB:-awcms_micro_restore_drill}"
...
acquire_lock "$BACKUP_DIR/.awcms-micro-drill.lock"
...
latest_manifest="$(find "$BACKUP_DIR" -maxdepth 1 -name 'awcms_micro_*.manifest.json' -printf '%T@ %p\n' | sort -rn | head -n1 | cut -d' ' -f2-)"
...
schema_migrations_detail="awcms_micro_schema_migrations query failed"
if migrations_count="$(psql -tAc "SELECT count(*) FROM awcms_micro_schema_migrations" 2>/dev/null)"; then
...
if tenants_count="$(psql -tAc "SELECT count(*) FROM awcms_micro_tenants" 2>/dev/null)"; then
  ...
  if [[ "$sample_record_count" =~ ^[0-9]+$ ]] && [[ "$sample_record_count" -gt 0 ]]; then
    ...
    sample_record_detail="${sample_record_count} tenant row(s) present, sampled awcms_micro_tenants successfully"
  fi
fi

# --- Tenant isolation (RLS), via the real awcms_micro_app role if available ---
...
tenant_ids_raw="$(psql -tAc "SELECT id FROM awcms_micro_tenants ORDER BY created_at LIMIT 5" 2>/dev/null || true)"
...
for candidate in "${tenant_ids[@]}"; do
  ...
  office_count="$(psql -tAc "SELECT count(*) FROM awcms_micro_offices WHERE tenant_id = '${candidate}'" 2>/dev/null || echo 0)"
...
SET ROLE awcms_micro_app;
SET app.current_tenant_id = '${viewer_tenant}';
SELECT count(*) FROM awcms_micro_offices WHERE tenant_id = '${data_tenant}';
SQL
)"
  ...
  if [[ ! "$leaked_count" =~ ^[0-9]+$ ]]; then
    tenant_isolation_status="skip"
    tenant_isolation_detail="could not SET ROLE awcms_micro_app in this database (role missing, or connecting role lacks privilege) — skipping RLS check"

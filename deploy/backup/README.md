...
```bash
mkdir -p /etc/awcms-micro
openssl rand -base64 48 > /etc/awcms-micro/backup-encryption.key
openssl rand -base64 48 > /etc/awcms-micro/backup-hmac.key
chmod 600 /etc/awcms-micro/backup-*.key
```
...
construction as skill `awcms-micro-sync-hmac` — reused here, not
...
`BACKUP_DIR` (`.awcms-micro-backup-restore.lock`) stops a backup and a
...
| `BACKUP_DIR`                 | –        | `/var/backups/awcms-micro` | Direktori tujuan dump + manifest  |
| `BACKUP_RETENTION_DAYS`      | –        | `14`                       | Retensi dump lama sebelum dihapus |

Output per run: `awcms_micro_<UTC timestamp>.dump.enc` (AES-256-CBC
encrypted custom-format dump) + `awcms_micro_<UTC timestamp>.manifest.json`
(signed manifest).
...
- By default (no `--target`), this **never touches the live database** —
  it restores into a disposable database named `awcms_micro_restore_test`
...
BACKUP_ENCRYPTION_KEY_FILE=/etc/awcms-micro/backup-encryption.key \
BACKUP_HMAC_KEY_FILE=/etc/awcms-micro/backup-hmac.key \
./deploy/backup/restore-postgres.sh /var/backups/awcms-micro/awcms_micro_20260705_020000.dump.enc
```
...
BACKUP_HMAC_KEY_FILE=/etc/awcms-micro/backup-hmac.key \
./deploy/backup/restore-postgres.sh /var/backups/awcms-micro/awcms_micro_20260705_020000.dump.enc \
  --target=awcms_micro_recovery --acknowledge-target=awcms_micro_recovery
```

After restoring, verify the data actually came back, e.g.:

```bash
PGHOST=host PGPORT=5432 PGUSER=user PGPASSWORD=pass PGDATABASE=awcms_micro_restore_test \
  psql -c 'SELECT count(*) FROM awcms_micro_tenants;'
```
...
```bash
OFFSITE_COPY_COMMAND="rclone copy --config /etc/awcms-micro/rclone.conf -" \
./deploy/backup/offsite-copy.sh \
  /var/backups/awcms-micro/awcms_micro_20260705_020000.dump.enc \
  /var/backups/awcms-micro/awcms_micro_20260705_020000.manifest.json
```
...
(`DRILL_TARGET_DB`, default `awcms_micro_restore_drill` — deliberately
...
- **Schema migrations ledger** — `awcms_micro_schema_migrations` has rows.
- **Sample record** — `awcms_micro_tenants` has rows.
- **Tenant isolation (RLS)** — if the `awcms_micro_app` role exists and the
...
- `"incomplete"` — neither of the above: most commonly `tenant_isolation`
  came back `"skip"` (the `awcms_micro_app` role is missing, or this backup
...
```json
{
  ...
  "duration_seconds": 42,
  "backup_file": "awcms_micro_20260711_020000.dump.enc",
  "backup_created_at": "2026-07-11T02:00:01Z",
  "backup_age_seconds": 41,
  "target_database": "awcms_micro_restore_drill",
  "checks": {
...
[`../systemd/awcms-micro.service.example`](../systemd/awcms-micro.service.example)
...
```cron
# /etc/cron.d/awcms-micro-backup (or `crontab -e` for the service user)
# Daily at 02:00 local time — backup, then (optionally) copy off-site.
0 2 * * * DATABASE_URL=postgres://user:pass@host:5432/dbname BACKUP_DIR=/var/backups/awcms-micro BACKUP_ENCRYPTION_KEY_FILE=/etc/awcms-micro/backup-encryption.key BACKUP_HMAC_KEY_FILE=/etc/awcms-micro/backup-hmac.key BACKUP_RETENTION_DAYS=14 /opt/awcms-micro/deploy/backup/backup-postgres.sh >> /var/log/awcms-micro-backup.log 2>&1

# Weekly restore drill, Sunday 03:00 local time.
0 3 * * 0 DATABASE_URL=postgres://user:pass@host:5432/dbname BACKUP_DIR=/var/backups/awcms-micro BACKUP_ENCRYPTION_KEY_FILE=/etc/awcms-micro/backup-encryption.key BACKUP_HMAC_KEY_FILE=/etc/awcms-micro/backup-hmac.key /opt/awcms-micro/deploy/backup/restore-drill.sh >> /var/log/awcms-micro-restore-drill.log 2>&1
```
...
- [`../../docs/awcms-micro/deployment-profiles.md`](../../docs/awcms-micro/deployment-profiles.md)
  ...
  `deploy/`.
- [`../../docs/awcms-micro/production-preflight-runbook.md`](../../docs/awcms-micro/production-preflight-runbook.md)
  §"Stage 2 — Backup evidence" — the go-live evidence trail this feeds.
- [`../../docs/awcms-micro/07_sprint_testing_production_readiness.md`](../../docs/awcms-micro/07_sprint_testing_production_readiness.md)
  §"Backup SOP ringkas" / "Restore SOP ringkas" — the source SOP these
  scripts implement.
- `.claude/skills/awcms-micro-production-preflight/SKILL.md` §"Backup &
  restore" — go-live checklist requiring a tested restore.
- `.claude/skills/awcms-micro-sync-hmac/SKILL.md` — the HMAC signature

...
# superuser, BEFORE any migration. It creates the `awcms_micro_app` role the
...
# AWCMS_MICRO_APP_DB_PASSWORD). The table GRANTs, `FORCE ROW LEVEL SECURITY`, and
...
if [ -z "${AWCMS_MICRO_APP_DB_PASSWORD:-}" ]; then
  echo "10-create-app-role.sh: AWCMS_MICRO_APP_DB_PASSWORD is not set — refusing to create the app role without a password." >&2
  exit 1
fi
...
psql -v ON_ERROR_STOP=1 \
  -v approle_password="$AWCMS_MICRO_APP_DB_PASSWORD" \
  --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<'SQL'
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'awcms_micro_app') THEN
    CREATE ROLE awcms_micro_app LOGIN;
  END IF;
END
$$;
ALTER ROLE awcms_micro_app WITH LOGIN PASSWORD :'approle_password';
SQL

echo "10-create-app-role.sh: least-privilege role awcms_micro_app is ready."

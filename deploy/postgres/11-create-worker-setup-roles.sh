...
#
# Both `AWCMS_MICRO_WORKER_DB_PASSWORD`/`AWCMS_MICRO_SETUP_DB_PASSWORD` are
...
if [ -n "${AWCMS_MICRO_WORKER_DB_PASSWORD:-}" ]; then
  psql -v ON_ERROR_STOP=1 \
    -v worker_password="$AWCMS_MICRO_WORKER_DB_PASSWORD" \
    --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<'SQL'
ALTER ROLE awcms_micro_worker WITH LOGIN PASSWORD :'worker_password';
SQL
  echo "11-create-worker-setup-roles.sh: awcms_micro_worker granted LOGIN."
else
  echo "11-create-worker-setup-roles.sh: AWCMS_MICRO_WORKER_DB_PASSWORD not set — awcms_micro_worker stays NOLOGIN (WORKER_DATABASE_URL will fall back to DATABASE_URL)."
fi

if [ -n "${AWCMS_MICRO_SETUP_DB_PASSWORD:-}" ]; then
  psql -v ON_ERROR_STOP=1 \
    -v setup_password="$AWCMS_MICRO_SETUP_DB_PASSWORD" \
    --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<'SQL'
ALTER ROLE awcms_micro_setup WITH LOGIN PASSWORD :'setup_password';
SQL
  echo "11-create-worker-setup-roles.sh: awcms_micro_setup granted LOGIN."
else
  echo "11-create-worker-setup-roles.sh: AWCMS_MICRO_SETUP_DB_PASSWORD not set — awcms_micro_setup stays NOLOGIN (SETUP_DATABASE_URL will fall back to DATABASE_URL)."
fi

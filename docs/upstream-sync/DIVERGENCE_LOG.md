...
| 2026-05-31 | `awcmsmicro-dev/pnpm-workspace.yaml`, `awcmsmicro-dev/.awcms-patches/0022-postcss.patch` | adapt | active | Added a workspace `postcss` override to the patched 8.5.x line and dismissed Dependabot alert #221 after rebuild validation | Close the PostCSS XSS advisory in the rebuilt workspace while keeping the upstream snapshot reproducible | If upstream workspace dependency policy changes or the override is no longer required | OpenCode / GPT-5.4 |

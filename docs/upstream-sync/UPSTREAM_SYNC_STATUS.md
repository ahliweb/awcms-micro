...
Synced to EmDash `90ebf8a7`. `emdash-latest/` and `awcmsmicro-dev/` both refreshed successfully, the downstream workspace validates cleanly on this host, and the rebuild allowlist continues to preserve `awcmsmicro-dev/.changeset/` plus the workspace catalog pins that now live in protected package files.
...
- The same catalog refresh was replayed into `awcmsmicro-dev/` during the rebuild, including the protected `packages/blocks/playground/package.json` wrangler catalog pin

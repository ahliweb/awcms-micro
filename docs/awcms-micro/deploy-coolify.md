...
1. Coolify → **New Resource** → **Application** → **Public/Private
   Repository** (GitHub) → pilih repo `ahliweb/awcms-micro` (atau fork/repo
   turunan).
...
ghcr.io/<org>/awcms-micro:<tag> . && docker push
ghcr.io/<org>/awcms-micro:<tag>`.
...
aplikasi turunan AWCMS-Micro pada satu server.
...
- **Jangan reuse secret** antar aplikasi: `AUTH_JWT_SECRET`,
  `AWCMS_MICRO_SYNC_HMAC_SECRET`, kredensial R2, dan password role database
...
- **Jangan berbagi role superuser/`postgres` default** untuk runtime app
  ...
  app least-privilege (`awcms_micro_app` atau role app-specific lain per
  aplikasi).
...
`.claude/skills/awcms-micro-production-preflight/`) menghentikan start
...
image (`awcms_micro_app`) tidak punya hak DDL/GRANT.
...
disposable `awcms_micro_restore_test`, bukan menimpa database live) untuk

...
`awcms-micro-pr-review`) yang menambah/mengubah modul baru, atau menambah
provider eksternal baru ke modul yang sudah ada. Setiap poin merujuk ke
`docs/awcms-micro/21_module_admission_governance.md` (dokumen sumber
...
- [ ] Skill `awcms-micro-security-review` sudah dijalankan dan hasilnya
      dilampirkan di PR.
...
- [ ] Migration baru (`NNN_awcms_micro_*.sql`) ada bila skema berubah,

...
4. **Client PUT langsung** ke `presignedUrl` — bytes gambar tidak pernah
   transit lewat server AWCMS-Micro di langkah ini.
5. **Confirm** — client memanggil
   `POST /api/v1/news-media/{objectKey}/confirm` dengan
   `Idempotency-Key` (mutation high-risk, skill `awcms-micro-idempotency`,
   dan **wajib menulis audit event lewat skill `awcms-micro-audit-log`**
...
- `.claude/skills/awcms-micro-idempotency/SKILL.md` — pola `Idempotency-Key`.
- `.claude/skills/awcms-micro-integration/SKILL.md` — timeout/circuit breaker provider eksternal.

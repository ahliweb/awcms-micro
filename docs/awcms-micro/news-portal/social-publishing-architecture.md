...
`awcms_micro_social_publishing_settings` (`GET/PATCH
...
Migration `sql/053_awcms_micro_social_publishing_schema.sql` (+
`sql/055_awcms_micro_social_publishing_verify_permission.sql` untuk
permission `accounts.verify`):

| Tabel                                    | Peran                                                                 |
| ---------------------------------------- | --------------------------------------------------------------------- |
| `awcms_micro_social_accounts`            | Koneksi akun sosial per tenant (`token_reference`, bukan token nyata) |
| `awcms_micro_social_publish_rules`       | Aturan publish per akun/trigger event, gerbang approval opsional      |
| `awcms_micro_social_publish_templates`   | Template caption per rule                                             |
| `awcms_micro_social_publish_jobs`        | Outbox — satu baris per artikel/akun/aksi, idempoten                  |
| `awcms_micro_social_publish_attempts`    | Append-only, satu baris per percobaan dispatch (audit trail)          |
| `awcms_micro_social_publishing_settings` | Toggle auto-posting per-tenant (`tenant_id` PK)                       |
...
`awcms_micro_social_accounts.token_reference` adalah string buram
...
- **`needs_reauth`**: TIDAK auto-retry; `awcms_micro_social_accounts`
...
`awcms_micro_tenant_domains` (`is_primary = true AND status = 'active'`)
...
`asyncapi/awcms-micro-domain-events.asyncapi.yaml`, channel
`awcms-micro.social-publishing.*`: `account.{connected,disconnected,
...
- `.claude/skills/awcms-micro-social-publishing/SKILL.md` — rasional

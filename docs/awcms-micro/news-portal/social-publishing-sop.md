...
- [ ] Tenant sudah punya domain utama **terverifikasi**
      ...
      `awcms_micro_tenant_domains`) — tanpa ini, job creation di-skip
      (`no_verified_domain`).
...
1. Editor menyiapkan `awcms_micro_social_publish_rules` (per akun +
   trigger event, mis. `article_published`) dan opsional
   `awcms_micro_social_publish_templates` (caption).
...
- Redaksi yang ingin memposting artikel yang **sudah lama** publish
  ...
  (`awcms_micro_social_publish_rules.trigger_event`) tapi **belum ada
...
`awcms_micro_social_publish_attempts` untuk pesan error per percobaan —
...
2. `externalPostId`/`externalPostUrl` yang tersimpan di job (lihat
   `awcms_micro_social_publish_jobs`) adalah referensi yang dipakai

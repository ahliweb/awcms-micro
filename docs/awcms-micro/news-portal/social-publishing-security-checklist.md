...
`.claude/skills/awcms-micro-social-publishing/SKILL.md` §643 Keputusan
kunci #3):
...
2. Audit `awcms_micro_social_publish_attempts`/
   `awcms_micro_social_publish_jobs` untuk periode sejak perkiraan waktu
...
- Catat insiden sebagai audit event `critical` (skill
  `awcms-micro-audit-log`) dengan `correlationId` yang menghubungkan
...
- **Privasi**: jangan memposting otomatis artikel yang memuat data
  ...
  `awcms-micro-sensitive-data`) ke platform publik tanpa proses redaksi
...
- **Retensi data**: `awcms_micro_social_publish_attempts` adalah audit
  ...
  audit log umum repo ini (skill `awcms-micro-audit-log`, doc 03/10),

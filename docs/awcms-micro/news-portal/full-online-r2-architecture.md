...
`.claude/skills/awcms-micro-news-portal/SKILL.md`.

## 1. Ruang lingkup & asumsi — full-online only

**Ini bukan aturan untuk semua deployment.** Seluruh dokumen di
`docs/awcms-micro/news-portal/` mengasumsikan **mode full-online R2-only**
...
- **TIDAK berlaku untuk modul lain.** `sync-storage`'s R2 usage (§2 di
  ...
  `awcms_micro_object_sync_queue`, dispatcher, atau env var `R2_*` yang
  sudah ada.
...
- Setiap dokumen di folder ini yang menyebut "wajib"/"dilarang" berarti
  **wajib/dilarang untuk mode full-online R2-only**, bukan untuk base
  generik AWCMS-Micro secara keseluruhan.
...
`sql/041_awcms_micro_news_media_object_registry_schema.sql`) menggunakan
tabel **`awcms_micro_news_media_objects`** — nama yang sudah dipilih di
bawah ini SEBELUM issue #633 mulai dikerjakan, dan dipertahankan persis
(bukan `awcms_micro_media_objects` yang sempat disebut di body issue
#633 — lihat migration 041's header comment dan
`.claude/skills/awcms-micro-news-portal/SKILL.md` §633 untuk alasan
rekonsiliasi lengkap). Kolom final adalah **elaborasi** dari sketsa
konseptual di bawah, bukan penggantian:

```text
awcms_micro_news_media_objects (tenant-scoped, RLS ENABLE+FORCE)
- id                        uuid PK
- tenant_id                 uuid FK -> awcms_micro_tenants
...
- owner_resource_id         uuid, nullable   (pola sama awcms_micro_audit_events, TANPA FK spesifik)
...
audit events (skill `awcms-micro-audit-log`) written for create/verify/
...
- Soft delete (`deleted_at`) **ortogonal** terhadap `status` (pola sama
  `awcms_micro_blog_posts`) — hapus/restore tidak pernah menulis ulang
  `status`.
...
`awcms_micro_news_media_objects.object_key`, migration 041).
...
sequenceDiagram
  ...
  participant API as AWCMS-Micro API
  ...
  Editor->>R2: PUT langsung (bytes tidak pernah lewat server AWCMS-Micro)
...
- `confirm` (Jalur A) adalah **mutation high-risk** (mendaftarkan media
  ...
  `awcms-micro-idempotency`) supaya retry client tidak membuat baris
...
melakukan PUT langsung ke R2 dari origin admin AWCMS-Micro:
...
flowchart LR
  ...
  subgraph App["AWCMS-Micro app (tepercaya)"]
    API[API /news-media/*]
...
- **Model tanggung jawab bersama**: Cloudflare bertanggung jawab atas
  ...
  yang dikonfigurasi; AWCMS-Micro bertanggung jawab atas konfigurasi
...
AWCMS-Micro berperan sebagai **PII controller** untuk tenant sendiri
...
- **Trade-off eksplisit**: mandat "R2-only, tidak ada fallback lokal"
  ...
  aplikasi AWCMS-Micro itu sendiri — R2 melayani langsung, tidak lewat
  proxy aplikasi).
...
| #634                  | §7 alur upload, §8 presigned URL, §9 validasi — **diimplementasikan** (lihat `.claude/skills/awcms-micro-news-portal/SKILL.md` §634)                                                        |
...
| #638                  | Preset placement iklan R2-only (`awcms_micro_news_portal_ad_placements`, tabel baru, terpisah dari `blog_content`'s ads) mengonsumsi §5 registry — **diimplementasikan** (lihat skill §638) |
| #639-#640, #642, #649 | Konsumsi media registry untuk fitur editorial spesifik lanjutan (lihat skill untuk ringkasan per issue)                                                                                     |

## 17. Referensi

- `docs/awcms-micro/news-portal/r2-upload-sop.md` — SOP operasional upload.
- `docs/awcms-micro/news-portal/r2-security-checklist.md` — checklist keamanan siap-pakai.
- `docs/awcms-micro/news-portal/r2-incident-response.md` — runbook insiden.
- `docs/awcms-micro/news-portal/r2-backup-lifecycle.md` — backup/lifecycle/retensi.
- `docs/awcms-micro/news-portal/newsroom-user-guide.md` — panduan editor.
- `.claude/skills/awcms-micro-news-portal/SKILL.md` — status per issue.
...
- `docs/awcms-micro/18_configuration_env_reference.md` §Storage — konvensi `R2_*` yang sudah ada.
- `docs/awcms-micro/20_threat_model_security_architecture.md` — threat model dasar (subbagian epic ini menyusul di Issue #635).

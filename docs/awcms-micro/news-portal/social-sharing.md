...
| Kredensial/token            | Tidak ada — tombol hanya membuka URL share-intent publik atau share sheet OS | `token_reference` per akun, disimpan di `awcms_micro_social_accounts`                              |
...
| Data baru dipersist         | Tidak — murni rendering, tidak ada migration baru                            | Ya — 6 tabel (`awcms_micro_social_accounts`/`_rules`/`_templates`/`_jobs`/`_attempts`/`_settings`) |
...
#649 (lihat `.claude/skills/awcms-micro-news-portal/SKILL.md` §649) —

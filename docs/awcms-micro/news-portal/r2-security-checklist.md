...
- **`bun run security:readiness`** (`scripts/security-readiness.ts`):
  ...
  - `checkNewsMediaR2NoStalePendingObjects` (**warning**, Issue #635) —
    ...
    `awcms_micro_news_media_objects` mencari baris `pending_upload` yang
...
— lihat `.claude/skills/awcms-micro-news-portal/SKILL.md` §634 untuk
...
lihat `.claude/skills/awcms-micro-news-portal/SKILL.md` §635 untuk
...
- [ ] Setiap `confirm` (sukses/gagal) menulis **audit event formal**
      lewat skill `awcms-micro-audit-log` (bukan hanya correlation-ID
...
- [ ] Rotasi kredensial (§6) dicatat sebagai audit event high-risk
      (skill `awcms-micro-audit-log`), bukan hanya perubahan env var yang
      tidak terlacak.
...
- `.claude/skills/awcms-micro-security-hardening/SKILL.md` — audit OWASP/ASVS/ISO tingkat repo.

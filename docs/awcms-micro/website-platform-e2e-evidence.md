...
| Accessibility meets WCAG 2.2 target for critical journeys                                                                                     | [`admin-a11y-smoke.e2e.ts`](../../tests/e2e/admin-a11y-smoke.e2e.ts) + [`public-a11y-smoke.e2e.ts`](../../tests/e2e/public-a11y-smoke.e2e.ts) (axe-core over public homepage + `/newsletter/demo` + `/comments/demo`, EN/ID) + [`public-content-a11y.e2e.ts`](../../tests/e2e/public-content-a11y.e2e.ts) (axe over the rendered `/news` + `/blog/{tenantCode}` article templates, EN/ID × desktop/mobile) + [`public-discovery-a11y.e2e.ts`](../../tests/e2e/public-discovery-a11y.e2e.ts) (axe over the listing, search — incl. empty/too-short/no-results branches — and the public 404 document, EN/ID × desktop/mobile, plus keyboard-only search) + [`public-keyboard-journey.e2e.ts`](../../tests/e2e/public-keyboard-journey.e2e.ts); **screen-reader pass + deployed-instance content journey** | partial (admin + public homepage/demo + content, listing, search, error templates) → [§Deferred](#deferred-work) |
...
| `bun run check`, PG integration, security/adversarial, browser E2E, a11y, SEO/schema, link, perf/load/soak, build, readiness, preflight green | CI runs `check` + integration + E2E; handler-level link-integrity now in CI ([`public-link-integrity.integration.test.ts`](../../tests/integration/public-link-integrity.integration.test.ts)); rendered-site crawl automated for any URL via `bun run link:check -- --url=…` ([`scripts/link-check.ts`](../../scripts/link-check.ts)); **crawl at representative content volume + load/soak**                                                                                                                                                                                                                                                                                                                                                                                                           | partial → [§Deferred](#deferred-work)                                                                            |
...
- **Full-journey accessibility & link checking** (**#296**) — the base-app
  ...
  **Keyboard journey LANDED (2026-07-25)**: `public-keyboard-journey.e2e.ts`
  ...
  read-only traffic.
  **Discovery/empty/error-state axe LANDED (2026-07-26)**:
  `public-discovery-a11y.e2e.ts` scans the surfaces between the homepage and
  the article — nobody reaches an article without passing through a listing or
  a search page, and the states a real visitor hits most often there are
  exactly the ones a happy-path scan never renders. It seeds two tenants (one
  `default_locale=en`, one `id` — the listing/search shell takes its
  `<html lang>` from the TENANT's default locale, a third locale mechanism
  distinct from the locale cookie used by `public-a11y-smoke` and from the
  post's own locale used by `public-content-a11y`; assuming one mechanism
  covers all three is how "both locales" silently becomes "English twice")
  and runs axe (WCAG 2.2 A/AA, critical/serious) at desktop AND mobile over:
  the `/news` and `/blog/{tenantCode}` listings; `/news/search` and
  `/blog/{tenantCode}/search` in both their empty-query and populated states;
  `/search` in **all four** of its render branches (no query, too short, no
  results, results present) in EN and ID via `?locale=`; and the shared public
  **404 document** every public route funnels its misses into. It also asserts
  the search form is operable by keyboard alone (Tab to the field, visible
  focus indicator, Enter submits). Both gates were verified FALSIFIABLE, not
  assumed: an injected alt-less image makes the axe scan fail, and an injected
  `outline/box-shadow: none` makes the focus assertion fail. All 9 tests pass,
  twice in a row (retry-idempotent seeds) and alongside the other six
  setup_state-owning specs (38/38 together, no cross-file race).
  **Operator link checker LANDED (2026-07-26)**: `bun run link:check --
--url=<any url>` (`scripts/link-check.ts`) — the field counterpart to the
  hermetic crawl spec. It crawls the rendered page graph from an entry URL,
  additionally seeded by `robots.txt` `Sitemap:` directives and the XML
  sitemap(s) (following sitemap INDEX → child sitemap → `<loc>`), and verifies
  every internal anchor, `rel=canonical`, `rel=alternate hreflang`, feed
  autodiscovery link and pagination link resolves (< 400 after redirects).
  JSON report on stdout (`--json-output=<path>` for an evidence artifact),
  exit 0 clean / 1 broken links / 2 usage or entry unreachable — a crawl that
  reached nothing is deliberately NOT reported as green. `--site-origin=`
  handles this app's absolute tenant-domain URLs when probing through another
  address; internal-vs-external is decided by HOST, not full origin, because
  this app measurably emits `https://` sitemap `<loc>`s and `http://`
  permalinks on the same page — an origin-strict comparison would skip half a
  site's own links and still report green. Falsifiability verified against a
  fixture with a dead anchor, a dead canonical and a sitemap-only dead `<loc>`:
  3 broken found, exit 1, while links inside HTML comments and `<script>`
  bodies are correctly not followed. STILL DEFERRED: the **screen-reader**
  pass (manual), the deployed-instance **content-template** journey (the live
  tenant has no published content yet, so `/news`/`/blog` render nothing there
  — covered hermetically by `public-content-a11y.e2e.ts` and
  `public-discovery-a11y.e2e.ts`), and running `link:check` against a
  representative-volume live site (the command exists and is verified; the
  content does not exist yet on the target).

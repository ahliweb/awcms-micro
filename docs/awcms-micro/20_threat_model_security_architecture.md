...
migration 045 sejak awal (`REVOKE ALL ... FROM awcms_micro_app` di
migration yang sama persis setelah `CREATE TABLE`), jadi `awcms_micro_app`
...
| `awcms_micro_app`    | `DATABASE_URL`        | Setiap HTTP request biasa. Dipersempit di 11 tabel global — 9 via matriks di header migration 045, ditambah 2 tabel referensi wilayah administratif Indonesia (Issue #657) via revoke langsung di migration 054 (nol grant). |
| `awcms_micro_worker` | `WORKER_DATABASE_URL` | 7 script cron/systemd-timer tanpa endpoint HTTP. Nol akses ke 11 tabel global kecuali `SELECT` di `awcms_micro_tenants`.                                                                                                     |
...
PRIVILEGES` yang sudah ada (migration 013) tetap otomatis memberi
  ...
  Indonesia yang ditambahkan belakangan (`awcms_micro_idn_region_datasets`,
  `awcms_micro_idn_admin_regions`, migration 054, Issue #657) — itulah yang
...
| **Provider eksternal down/lambat memblokir transaksi sumber**                                                                                                      | Verifikasi inbound murni lokal (tidak ada I/O eksternal). Pengiriman outbound SELALU di luar transaksi DB (ADR-0006) — commit event sumber tidak pernah menunggu HTTP call; kegagalan provider hanya memengaruhi baris `awcms_micro_integration_outbound_deliveries`, retry/backoff/dead-letter menangani sisanya.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- **`awcms_micro_integration_outbound_deliveries`/`_delivery_attempts`
...
- **Sirkuit breaker in-memory (fail-fast) reset saat worker restart** —
  `awcms_micro_integration_adapter_health` (sinyal persisten,
...
| Kategori risiko                                                                                        | Mitigasi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Kode yang sudah dipakai data tenant dihapus/diubah maknanya diam-diam**                              | `awcms_micro_reference_codes` TIDAK PERNAH hard-delete lewat jalur manapun (CRUD manual maupun import) — hanya deprecate. Import commit secara eksplisit menolak (`409 DESTRUCTIVE_REPLACE_BLOCKED`) payload `replace: true` terhadap code yang punya baris `awcms_micro_reference_tenant_codes.base_code_id` menunjuk ke sana — dicek ULANG di dalam transaksi commit yang sama (bukan hanya saat dry-run), diuji adversarial di integration suite (create referenced code -> attempt destructive replace -> assert rejected & unchanged).                                                                             |
| **Tenant override memengaruhi baseline global atau tenant lain (ADR-0021 §8)**                         | `application/tenant-code-directory.ts` HANYA PERNAH menulis ke `awcms_micro_reference_tenant_codes`/`_translations` (RLS FORCE, predicate `tenant_id`) — tidak ada fungsi di modul ini yang menulis tabel baseline global atas nama tenant. Resolusi (`domain/resolution.ts`) adalah operasi BACA murni tanpa parameter `tenantId` sama sekali (structurally tidak bisa membaca lintas tenant) — pemanggil (`reference-resolution-query.ts`) yang bertanggung jawab hanya mem-fetch baris tenant sendiri lewat `withTenant`. Diuji: unit test precedence murni + integration test cross-tenant RLS-direct-read-blocked. |
...
| **Dry-run import diam-diam menulis ke baseline (klaim "non-mutating" tidak benar)**                    | `dryRunReferenceImport` HANYA PERNAH `INSERT` ke `awcms_micro_reference_imports` — tidak ada statement yang menyentuh `awcms_micro_reference_codes` di fungsi itu; diuji integration test eksplisit (assert daftar code kosong setelah dry-run, sebelum commit).                                                                                                                                                                                                                                                                                                                                                        |
...
| **Versi dokumen ditimpa (bukan benar-benar immutable)**                                                                                                                                                                                                                                              | Ditelusuri mundur dari jalur tulis nyata (bukan hanya dokumentasi): `application/document-version-service.ts`'s `createDocumentVersion` adalah SATU-SATUNYA fungsi yang `INSERT` ke `awcms_micro_document_versions` di seluruh modul — tidak ada `UPDATE`/`DELETE` terhadap tabel itu di mana pun. Koreksi = baris baru dengan `previous_version_id` menunjuk mundur. `UNIQUE (tenant_id, document_id, version_number)` + `SELECT ... FOR UPDATE` pada baris dokumen induk mencegah dua panggilan konkuren mengklaim `version_number` yang sama.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **Rekonsiliasi tidak mendeteksi kehilangan/mismatch diam-diam**                                                                                                              | `evaluateReconciliation` (`domain/reconciliation.ts`) membandingkan count DAN (bila tersedia) checksum sumber vs terproses, menulis `awcms_micro_data_exchange_reconciliation_reports` append-only, memicu domain event + audit `warning` saat mismatch. Diuji lewat adapter export fault-injecting yang sengaja melaporkan count sumber berbeda dari yang benar-benar dibaca.                                                                                                                                                                                                                                                                   |

...
```mermaid
flowchart LR
  Def[Default kode] --> Env[Environment variable] --> Set[awcms_micro_tenant_settings - per tenant] --> Eff[Konfigurasi efektif]
```

- Runtime/secret (DB, JWT, HMAC, provider key): dari **environment**.
- Preferensi tenant (locale — default **en**, theme): dari **`awcms_micro_tenants`**; flag fitur tampilan: dari **`awcms_micro_tenant_settings`**. Keduanya dikelola lewat `GET/PATCH /api/v1/settings` dan layar `/admin/settings` (Settings PR). String UI statis via katalog `.po` gettext (di-bundle, bukan DB); konten data multi-bahasa di DB per locale aktif (doc 14 §i18n, doc 04 §Konten multi-bahasa).
...
| --------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `AUTH_JWT_SECRET`     | Tidak pernah dibaca — sesi memakai token opaque (`awcms_micro_sessions.token_hash`), bukan JWT             | Tidak ada — token sesi acak kriptografis, tidak diturunkan dari secret bersama        |
...
| `AUDIT_LOG_RETENTION_DAYS`  | –     | `730`                   | –        | Retensi `awcms_micro_audit_events` (hari) dipakai `bun run logs:audit:purge` (Issue #447; doc 04 §Retention awal)                                                              |
| `FORM_DRAFT_RETENTION_DAYS` | –     | `30`                    | –        | Retensi `awcms_micro_form_drafts` `expired`/`abandoned` (hari) dipakai `bun run form-drafts:purge` (Issue #484; `--retention-days=<n>` CLI flag override lebih diprioritaskan) |

### Database & pool

| Var                               | Wajib | Default                         | Sensitif | Fungsi                                                                                                                                                                                                                                                                                     |
| --------------------------------- | ----- | ------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `DATABASE_URL`                    | Ya    | –                               | Ya       | Koneksi PostgreSQL (role `awcms_micro_app`)                                                                                                                                                                                                                                                |
| `BOOTSTRAP_OWNER_PASSWORD`        | –     | –                               | Ya       | Password owner untuk `bun run bootstrap:default-tenant` (min 8 karakter, sama seperti setup wizard). Hanya dibaca dari environment — sengaja bukan flag CLI, karena flag terlihat di daftar proses. Tidak dibaca aplikasi saat runtime                                                     |
| `AWCMS_MICRO_APP_DB_PASSWORD`     | –     | `awcms_micro_app_password`      | Ya       | Password role `awcms_micro_app` dipakai `deploy/postgres/10-create-app-role.sh`/`docker-compose.yml` saat init container; harus sama dengan password di `DATABASE_URL`. Tidak dibaca kode TypeScript apa pun (shell/compose saja)                                                          |
...
| `DATABASE_POOL_MAX_WORKER`        | –     | fallback ke `DATABASE_POOL_MAX` | –        | Override maks koneksi pool khusus role `awcms_micro_worker` (Issue #743)                                                                                                                                                                                                                   |
| `DATABASE_POOL_MAX_SETUP`         | –     | fallback ke `DATABASE_POOL_MAX` | –        | Override maks koneksi pool khusus role `awcms_micro_setup` (Issue #743)                                                                                                                                                                                                                    |
| `DATABASE_STATEMENT_TIMEOUT_MS`   | –     | `15000`                         | –        | Timeout statement                                                                                                                                                                                                                                                                          |
| `DATABASE_IDLE_IN_TXN_TIMEOUT_MS` | –     | `30000`                         | –        | `idle_in_transaction_session_timeout` per koneksi — Postgres membunuh sesi yang tersangkut `idle in transaction` selama ini; jaring pengaman anti transaction-leak/pool-saturation (`statement_timeout` tak bisa mereap idle-in-txn). `0` menonaktifkan. Lihat `database-pooling.md` §1/§9 |
| `DATABASE_PGBOUNCER`              | –     | `false`                         | –        | Mode PgBouncer (transaction)                                                                                                                                                                                                                                                               |
| `WORKER_DATABASE_URL`             | –     | fallback ke `DATABASE_URL`      | Ya       | Koneksi role `awcms_micro_worker` (9 background script)                                                                                                                                                                                                                                    |
| `SETUP_DATABASE_URL`              | –     | fallback ke `DATABASE_URL`      | Ya       | Koneksi role `awcms_micro_setup` (`POST /api/v1/setup/initialize`)                                                                                                                                                                                                                         |

#### Model role database (Issue #683, epic #679, platform-hardening)

Sejak migration `sql/045_awcms_micro_db_role_separation.sql`, ada **empat**
...
2. **`awcms_micro_app`** ("web runtime", `DATABASE_URL`) — melayani setiap
   ...
   (non-RLS: `awcms_micro_permissions`, `awcms_micro_schema_migrations`,
   `awcms_micro_setup_state`, `awcms_micro_tenants`, `awcms_micro_modules` +
...
`awcms_micro_app`) bila keduanya tidak di-set — deployment kecil/offline
yang tidak ingin mengelola 4 connection string tetap jalan lewat satu
role `awcms_micro_app` yang sudah dipersempit di migration 045 (tetap
lebih aman dari sebelumnya, hanya kehilangan lapisan defense-in-depth
tambahan). `AWCMS_MICRO_WORKER_DB_PASSWORD`/`AWCMS_MICRO_SETUP_DB_PASSWORD`
...
| `AUTH_SELF_REGISTRATION_ENABLED`               | –              | `false`                                  | –        | Aktifkan `/register` + `POST /auth/register`; akun dibuat PENDING dan wajib di-approve admin di `/admin/registrations` sebelum bisa login                                                           |
| `AUTH_SELF_REGISTRATION_RATE_LIMIT_MAX`        | –              | `5`                                      | –        | Rate limit `POST /auth/register` per sumber+tenant                                                                                                                                                  |
| `AUTH_SELF_REGISTRATION_RATE_LIMIT_WINDOW_SEC` | –              | `900`                                    | –        | Jendela waktu rate limit registrasi (detik)                                                                                                                                                         |
| `AUTH_URL_PARAM_ENCRYPTION_KEY`                | –              | –                                        | Ya       | Opsional: key AES-256-GCM (base64, 32 byte) yang menyegel param URL auth sensitif (mis. token+tenant tautan reset) jadi satu token opaque; unset → fallback param polos; TIDAK untuk URL SEO publik |
...
| `AUTH_MFA_TOTP_ISSUER`                         | –              | `AWCMS-Micro`                            | –        | Nama issuer yang tampil di aplikasi authenticator                                                                                                                                                   |
...
- **Cloudflare Turnstile (Issue #588, selesai)** — `TURNSTILE_ENABLED`
  ...
  operasional. Detail lengkap: skill `awcms-micro-auth-online-hardening`.
...
- **Google OIDC login (Issue #590, selesai)** — `AUTH_GOOGLE_LOGIN_ENABLED`
  ...
  `X-AWCMS-Micro-Tenant-ID`. Detail lengkap: skill
  `awcms-micro-auth-online-hardening`.
- **Generic tenant OIDC SSO (Issue #591, selesai)** — `AUTH_SSO_ENABLED`
  ...
  `awcms_micro_oidc_auth_requests`/`awcms_micro_identity_provider_accounts`
  (migration 035, sudah generik `provider text` sejak awal) dengan
  `provider = <providerKey>`. Schema baru (migration 036):
  `awcms_micro_auth_providers` (per tenant: `provider_key`, `issuer_url`,
  ...
  jsonb, `enabled`, soft delete) dan `awcms_micro_tenant_auth_policies`
  ...
  lengkap: skill `awcms-micro-auth-online-hardening`,
...
- **Admin policy UI (Issue #592, selesai)** — `/admin/security`
  ...
  `awcms-micro-auth-online-hardening`, `src/modules/identity-access/README.md`.
...
`awcms-micro-news-portal`). Backstop tambahan di
...
| Var                             | Wajib     | Default          | Sensitif | Fungsi                                                                                                                                                     |
| ------------------------------- | --------- | ---------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AWCMS_MICRO_NODE_ID`           | –         | `local-dev-node` | –        | **DEPRECATED** (Issue #689, target hapus `1.0.0`) — tidak pernah dibaca; identitas node berasal dari `awcms_micro_sync_nodes` (DB); lihat §Config registry |
| `AWCMS_MICRO_SYNC_ENABLED`      | –         | `false`          | –        | Aktifkan sync hybrid                                                                                                                                       |
| `AWCMS_MICRO_SYNC_HMAC_SECRET`  | bila sync | –                | Ya       | Signature HMAC                                                                                                                                             |
| `AWCMS_MICRO_SYNC_MAX_SKEW_SEC` | –         | `300`            | –        | Toleransi anti-replay                                                                                                                                      |
...
| `EMAIL_FROM_NAME`               | –               | `AWCMS-Micro` | –        | Nama pengirim default                                 |
...
tepercaya (mis. `deploy/nginx/awcms-micro.conf.example` dengan TLS
...
`awcms_micro_tenant_domains`, `awcms_micro_module_settings`, atau tabel DB
...
> `docs/awcms-micro/visitor-analytics.md` §Default opt-in dan upgrade
...
| `VISITOR_ANALYTICS_VISITOR_KEY_COOKIE_TTL_DAYS` | –     | `30`    | –        | Umur cookie anonim `awcms_micro_visitor_key` (hari) — wajib integer positif bila diisi; sebelumnya hardcoded ~2 tahun (Issue #624 audit addendum)             |
...
`docs/awcms-micro/visitor-analytics.md` §Config dan readiness checks):
...
lihat `.claude/skills/awcms-micro-news-portal/SKILL.md` §632 untuk keputusan
...
| `NEWS_MEDIA_R2_IMAGE_RESIZING_ENABLED`       | –            | `false`                                     | –        | Opt-in `srcset` responsif via Cloudflare on-the-fly image resizing (`/cdn-cgi/image/...`), ADR-0026 langkah 5b. Hanya bekerja bila `NEWS_MEDIA_R2_PUBLIC_BASE_URL` custom domain nyata pada zone Cloudflare dengan Image Resizing aktif; `security:readiness` (`checkNewsMediaR2ImageResizingSafe`) memperingatkan bila base URL tak bisa melayaninya |
...
`.claude/skills/awcms-micro-social-publishing/SKILL.md`). Modul foundation
...
polos di kolom pengaturan apa pun — `awcms_micro_social_accounts.token_reference`
...
berhasil — lihat `.claude/skills/awcms-micro-social-publishing/SKILL.md` §646
...
(`awcms_micro_blog_terms`, `taxonomy_type = 'tag'`) di dalam body artikel
...
khusus `awcms_micro_blog_internal_tag_link_settings`
...
di kolom `awcms_micro_blog_posts.auto_internal_tag_links_disabled`.
...
hold/purge aman), lihat `docs/awcms-micro/data-lifecycle.md` untuk panduan
...
| Var                                | Wajib | Default                        | Sensitif | Fungsi                                                                                                                                                          |
| ---------------------------------- | ----- | ------------------------------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DATA_LIFECYCLE_ARCHIVE_ROOT_PATH` | –     | `./var/data-lifecycle-archive` | –        | Root filesystem tempat local/offline archive adapter menulis artefak JSONL/CSV (manifest+checksum bertengger di `awcms_micro_data_lifecycle_archive_manifests`) |
...
AUTH_MFA_TOTP_ISSUER=AWCMS-Micro
...
AWCMS_MICRO_SYNC_ENABLED=false
AWCMS_MICRO_SYNC_HMAC_SECRET=change-me
AWCMS_MICRO_SYNC_MAX_SKEW_SEC=300

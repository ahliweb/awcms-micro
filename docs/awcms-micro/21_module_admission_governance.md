...
domain. Registry sekarang berisi **19 modul terdaftar** (`src/modules/
index.ts`), termasuk enam modul WEBSITE nyata (`blog_content`,
`news_portal`, `social_publishing`, `seo_distribution`, `theming`, plus `media_library`
...
6. Pemetaan 19 modul saat ini ke kategori + remediasi yang terdeteksi.
...
## 8. Peta 19 modul saat ini → kategori

Sumber kebenaran: `src/modules/index.ts` (19 entri) dan setiap `module.ts`-
...
Total: 3 Core + 11 System + 5 Official Optional Module = **19 modul
...
1. **R1 — `type` tidak konsisten diisi.** Hanya 11 dari 19 modul
   ...
   `theming` = `domain`) men-set field `type` di `module.ts`. 8

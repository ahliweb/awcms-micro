...
domain. Registry sekarang berisi **22 modul terdaftar** (`src/modules/
index.ts`), termasuk delapan modul WEBSITE nyata (`blog_content`,
`news_portal`, `social_publishing`, `seo_distribution`, `theming`, `site_search`,
`comments`, `newsletter`, plus `media_library`
...
6. Pemetaan 22 modul saat ini ke kategori + remediasi yang terdeteksi.
...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Core**                         | Fondasi wajib: tanpanya platform tidak bisa boot/berfungsi untuk deployment mana pun. Selalu aktif di semua profil deployment.                                                                                                                                                                                                                                                           | Maintainer base (`@ahliweb`, lihat `.github/CODEOWNERS`) | Tidak — tidak pernah `disabled` secara global; per-tenant enable/disable (`awcms_micro_tenant_modules`) tidak berlaku bermakna karena modul lain bergantung padanya secara transitif.                                            |
| **System**                       | Kapabilitas platform lintas-modul (observability, sync/object-storage infra, email generik, reporting generik, domain-event runtime, media library, data lifecycle, routing tenant publik, telemetry, module management itu sendiri). Bersifat infrastruktur/reusable, bukan fitur produk end-user yang berdiri sendiri. Bisa off secara default (feature flag) tanpa menghentikan Core. | Maintainer base                                          | Sebagian besar ya, lewat `*_ENABLED` env flag (default off, ADR-0006) — bukan status modul, karena modul-modul ini sendiri harus tetap terdaftar (statusnya `active`) supaya `bun run db:migrate`/registry sync tetap konsisten. |
| **Official Optional Module**     | Fitur produk yang dipelihara resmi oleh tim base, opt-in per tenant, bernilai bisnis langsung (bukan sekadar infrastruktur). Contoh saat ini: `blog_content`, `news_portal`.                                                                                                                                                                                                             | Maintainer base                                          | Ya — `awcms_micro_tenant_modules` per tenant.                                                                                                                                                                                    |
...
- Harus generik untuk **semua** aplikasi turunan potensial, bukan spesifik
  ...
  satu vertikal.
- **Contoh kandidat WEBSITE-scope** untuk kategori ini (menggantikan contoh
  ERP yang menyesatkan — modul ERP tidak diport, ADR-0025): SEO/metadata &
  sitemap, theming/appearance, site search, comments/moderation, dan
  newsletter/subscription. Masing-masing generik lintas domain website
...
AWCMS-Micro defaultnya **offline-first/LAN-first** (ADR-0006) — perilaku
...
2. `awcms_micro_tenant_modules` (DB, Issue #512/#515) **hanya** menyimpan
...
## 8. Peta 22 modul saat ini → kategori

Sumber kebenaran: `src/modules/index.ts` (22 entri) dan setiap `module.ts`-
...
repo hari ini) karena field opsional `ModuleDescriptor.maintainers` belum
diisi di modul manapun — lihat remediasi R3 di bawah.
...
| `media_library`        | System                   | `system`                | _(tidak diset)_  | `active`         | @ahliweb |
...
| `theming`              | Official Optional Module | `domain`                | _(tidak diset)_  | `active`         | @ahliweb |
| `site_search`          | Official Optional Module | `domain`                | _(tidak diset)_  | `active`         | @ahliweb |
| `comments`             | Official Optional Module | `domain`                | _(tidak diset)_  | `active`         | @ahliweb |
| `newsletter`           | Official Optional Module | `domain`                | _(tidak diset)_  | `active`         | @ahliweb |

Total: 3 Core + 11 System + 8 Official Optional Module = **22 modul
...
`social_publishing` (ADR-0026 — memiliki registry media tenant). Tidak ada
modul kategori Derived Application atau External Integration top-level di
registry ini hari ini (sesuai definisi §2 — integration hidup sebagai
sub-komponen, lihat kolom "provider eksternal" di bawah).

> **Modul scope ERP yang tidak diport (ADR-0025 §3 — rujukan historis
> upstream).** Tujuh modul scope ERP milik AWCMS-Mini upstream — `workflow`,
> `organization_structure`, `document_infrastructure`, `data_exchange`,
> `integration_hub`, `reference_data`, `idn_admin_regions` — **tidak** ada
> di registry ini dan **tidak** boleh dihidupkan kembali di base: AWCMS-Micro
> adalah WEBSITE scope (ADR-0025 §Konsekuensi), bukan backlog yang belum
> selesai. Admission decision upstream mereka
...
1. **R1 — `type` tidak konsisten diisi.** Hanya 14 dari 22 modul
   ...
   `theming`, `site_search`, `comments`, `newsletter` = `domain`) men-set field `type` di `module.ts`. 8
...
- `docs/awcms-micro/templates/module-proposal-template.md` — diisi di body
  issue GitHub sebelum modul System/Official Optional Module baru
  di-scaffold (lightweight, bukan RFC panjang).
- `docs/awcms-micro/templates/module-admission-decision-checklist.md` —
  checklist yang dipakai reviewer PR (manusia atau
  `awcms-micro-pr-review`) untuk memverifikasi sebuah proposal/PR modul baru
...
baru: sebuah keputusan arah kepemilikan/dependensi yang mengikat lintas
dokumen tetap ditulis sebagai ADR penuh walau tidak mendaftarkan modul
baru apa pun di `src/modules/index.ts` (proposal template §9 tidak berlaku
karena tidak ada modul untuk dinilai lewat pohon keputusan §3). Contoh
**historis upstream**: `docs/adr/0020-erp-extension-readiness-contracts.md`
(Issue #755, epic #738 Wave 4) mendefinisikan keluarga kontrak kesiapan
ekstensi ERP tanpa modul baru — **tidak berlaku di AWCMS-Micro / tidak
...
- `docs/adr/0020-erp-extension-readiness-contracts.md` — kontrak kesiapan
  ekstensi ERP tanpa modul baru (§9 di atas), Issue #755, epic #738 Wave 4.
  **Rujukan historis upstream — tidak diport** (ADR-0025 §3, lihat banner
  ADR-0020).
- `docs/awcms-micro/derived-application-guide.md`,
  `docs/awcms-micro/derived-app-pilot-plan.md` — kategori Derived
  Application (§2, §4.4).
- `src/modules/module-management/README.md`, `.claude/skills/
awcms-micro-module-management/SKILL.md` — mekanisme sync/lifecycle
  registry yang mendasari §7.
- `docs/awcms-micro/20_threat_model_security_architecture.md` — kontrol
  keamanan provider eksternal (A10 SSRF, circuit breaker) yang mendasari
  §6 checklist.
- `docs/awcms-micro/18_configuration_env_reference.md`,
  `src/lib/config/registry.ts` — sumber kebenaran `profiles`/`*_ENABLED`
  yang mendasari §6 tabel kompatibilitas.
- `docs/adr/0016`–`0019`, `0021` — admission decision **upstream** untuk
  `organization_structure`/`document_infrastructure`/`data_exchange`/
  `integration_hub`/`reference_data`. **Rujukan historis — tidak diport ke
  AWCMS-Micro** (ADR-0025 §3, masing-masing membawa banner "tidak berlaku di
  repositori ini"). Dicantumkan hanya agar jejak keputusan upstream tetap
  dapat ditelusuri, bukan sebagai deskripsi scope repo ini.

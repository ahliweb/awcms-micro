...
```bash
DATABASE_URL=postgres://awcms-micro:awcms_micro_password@localhost:5432/awcms-micro bun run db:migrate
```
...
- Nama file wajib mengikuti `NNN_awcms_micro_<area>_<description>.sql`.
- Runner memastikan tabel `awcms_micro_schema_migrations` tersedia.

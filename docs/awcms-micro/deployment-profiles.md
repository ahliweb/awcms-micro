...
`.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md` untuk status
lengkap per issue.
...
| **production (online)**, `PUBLIC_TENANT_RESOLUTION_MODE=host_default` | `/news` — default, tanpa `tenantCode` di path                 | `Host`/domain mapping (`awcms_micro_tenant_domains`, Issue #559); `/blog/{tenantCode}` tetap berfungsi paralel, tidak dinonaktifkan                                        |
...
- **`PUBLIC_TRUST_PROXY`**: default aman **wajib** `false`. Set `true`
  **hanya** bila deployment ini benar-benar berjalan di belakang reverse
  ...
  yaitu tepat topologi `deploy/nginx/awcms-micro.conf.example` (TLS
...
- **Generic tenant OIDC SSO (Issue #591, selesai)**: set tambahan
  ...
  **Rekomendasi infra-layer untuk operator `full_online` (Issue #610,
  ...
  `awcms-micro-auth-online-hardening` §SSRF/`issuer_url`) — sengaja TIDAK
  ...
  `awcms-micro-auth-online-hardening` §SSRF/`issuer_url`) dan negative-TTL
...
mengaktifkannya (lihat `docs/awcms-micro/visitor-analytics.md` §Default
...
`_GEO_ENABLED`). Lihat `docs/awcms-micro/visitor-analytics.md` untuk
...
`docs/awcms-micro/news-portal/full-online-r2-architecture.md` §4 untuk
konvensi penamaan lengkap dan
`.claude/skills/awcms-micro-news-portal/SKILL.md` untuk status per issue.
...
sudo cp deploy/systemd/awcms-micro.service.example /etc/systemd/system/awcms-micro.service
sudo cp deploy/nginx/awcms-micro.conf.example /etc/nginx/sites-available/awcms-micro.conf
# ... adaptasi placeholder di kedua berkas (lihat komentar header masing-masing) ...
sudo systemctl enable --now awcms-micro
...
dengan `image: <registry>/awcms-micro:<tag>` langsung.
...
```bash
docker build -f Dockerfile.production -t awcms-micro:prod .
docker run -d --name awcms-micro \
  -p 4321:4321 \
  --cap-drop=ALL --security-opt=no-new-privileges:true \
  -e DATABASE_URL=postgres://awcms_micro_app:<password>@<db-host>:5432/awcms-micro \
...
(`awcms_micro_app`, least-privilege) tidak punya hak DDL/GRANT yang
...
| **production (online), bare-metal**                                                 | `deploy/nginx/awcms-micro.conf.example` (reverse proxy TLS termination)                                                | Publik ↔ nginx = batas TLS; nginx ↔ app (`localhost:4321`) = plaintext HTTP di **dalam** mesin yang sama, tidak melewati jaringan                                                                                                                                                |
...
| **Edge cache Varnish opsional (`docker-compose.varnish.yml`, ADR-0037)**            | Tetap di reverse proxy — Varnish **tidak** menangani TLS; ia satu hop plaintext di antara proxy dan app                | Proxy ↔ Varnish ↔ app semuanya di dalam mesin/Docker network yang sama. Domain diarahkan ke port **8080** (Varnish), bukan 4321. Endpoint invalidasi BAN hanya menerima jaringan privat + token bersama — jangan pernah publish port Varnish ke internet tanpa proxy di depannya |
...
- **Peran aplikasi `awcms_micro_app` (least-privilege)** — peran yang
...
- **Container:** otomatis — `deploy/postgres/10-create-app-role.sh` (hook
  `/docker-entrypoint-initdb.d`) membuatnya dari `AWCMS_MICRO_APP_DB_PASSWORD`
  saat init cluster pertama, lalu migrasi 013 memberi grant + FORCE RLS.
- **Bare-metal/systemd:** sekali di awal, sebagai superuser —
  `CREATE ROLE awcms_micro_app LOGIN PASSWORD '…';` — lalu `bun run db:migrate`
  (URL superuser). Setelah itu app konek sebagai `awcms_micro_app`
...
`docs/awcms-micro/18_configuration_env_reference.md` §Model role database
...
- Wajib non-kosong: `APP_ENV`, `APP_URL`, `APP_TIMEZONE`, `DATABASE_URL`,
  `AUTH_JWT_SECRET`.
- Kondisional: bila `AWCMS_MICRO_SYNC_ENABLED=true`, maka
  `AWCMS_MICRO_SYNC_HMAC_SECRET` wajib diisi dan bukan placeholder
...
| **Coolify/VPS**                              | Cron di VPS yang menjalankan job dari **checkout sumber** via container `oven/bun` sekali-pakai. **`docker exec <container-app> bun run …` TIDAK bisa dipakai** — image `Dockerfile.production` tidak menyertakan `scripts/` (Issue #293, diverifikasi live). Lihat [`deploy-coolify.md`](deploy-coolify.md) §Job terjadwal. |

Contoh crontab (bare-metal/systemd, setiap 2 menit):

```cron
*/2 * * * * cd /opt/awcms-micro && /usr/local/bin/bun run email:dispatch >> /var/log/awcms-micro/email-dispatch.log 2>&1
```

Contoh untuk topologi container (`docker-compose.yml`), dari cron host:

```cron
*/2 * * * * cd /opt/awcms-micro && docker compose exec -T app bun run email:dispatch >> /var/log/awcms-micro/email-dispatch.log 2>&1
```
...
| `news-media:reconcile`                  | media_library        | Harian (Issue #690; owner media_library sejak ADR-0026)               |
...
`awcms_micro_visit_events`/`awcms_micro_visitor_sessions`/
`awcms_micro_visitor_daily_rollups` lokal, tidak pernah memanggil provider
eksternal (lihat `docs/awcms-micro/visitor-analytics.md` §Rollup dan §Purge
...
(`docs/awcms-micro/news-portal/r2-backup-lifecycle.md` §Operator SOP).
...
- `src/lib/jobs/advisory-lock.ts` — `acquireAdvisoryLock(sql, jobName)`:
  ...
  **Kematian proses vs kematian host (diperjelas di PR #713, temuan Medium
  ...
  `tcp_keepalives_idle` lebih pendek untuk role `awcms_micro_worker` adalah
...
- `.claude/skills/awcms-micro-production-preflight/SKILL.md` — command

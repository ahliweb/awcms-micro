...
| Edge cache aman **bila dipasang**             | **Manual, lewat perintah terpisah** (Issue #353, ADR-0037) — `security:readiness` sengaja **tidak** memeriksanya, karena lapisan ini opsional, nonaktif secara default, dan hidup di luar proses aplikasi. Bila dipasang, jalankan `bun run edge-cache:health`: perintah itu keluar non-zero bila endpoint BAN menerima purge **tanpa** token (siapa pun di jaringan bisa mengosongkan cache). Deployment tanpa edge cache melewati baris ini sepenuhnya |

...
AWCMS-Micro does not use one version number for everything — each scheme
...
| 2   | REST contract (`openapi/awcms-micro-public-api.openapi.yaml` `info.version`)       | The OpenAPI document                                  | The REST contract SHAPE changes                                             | ADR-0008                                                                         |
| 3   | Event contract (`asyncapi/awcms-micro-domain-events.asyncapi.yaml` `info.version`) | The AsyncAPI document                                 | The event contract SHAPE changes                                            | ADR-0008                                                                         |
...
- **MAJOR** — a field/type is removed, renamed, or an optional field
  ...
  current MAJOR for schemes 4-6, and fails a `compatibleAwcmsMicroRange`
...
90 days") is imposed — AWCMS-Micro pre-1.0.0 package release policy
already documents that "minor boleh memuat penyesuaian belum stabil"
(skill `awcms-micro-release` §Aturan). The MAJOR/MINOR/PATCH discipline
...
## Support-window guidance for `compatibleAwcmsMicroRange`

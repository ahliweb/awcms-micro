...
| `backup-restore-drill`          | full | Runs the REAL `deploy/backup/restore-drill.sh` (Issue #691) against an ephemeral, drill-only encryption/HMAC key pair and a dedicated disposable `awcms_micro_dr_drill` database.                                                                          | **Implemented** — real backup, real restore, real RLS/schema-migrations verification. `full` tier only (needs a version-matched `pg_dump`/`pg_restore`; skipped, not failed, when unavailable — see `tests/integration/backup-restore-drill.integration.test.ts`'s identical environment-compatibility note). **Issue #293 (PR #346):** "unavailable" now covers the client being ABSENT, not just version-mismatched — `Bun.spawnSync` throws `ENOENT` rather than returning a failed result, and because the probe runs at module-load time that throw used to take the whole file down. Preflight's `test` stage runs in a minimal Bun container with no PostgreSQL client, so this single file was the last thing blocking a green `production:preflight` on the live target. |

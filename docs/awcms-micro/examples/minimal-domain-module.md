...
export const assetRegisterModule = defineModule({
  ...
  events: {
    ...
    publishes: ["awcms-micro.asset-register.asset.registered"]
  }
});
```
...
```sql
-- NNN_awcms_micro_asset_register_schema.sql — contoh modul domain minimal (Issue #463).

CREATE TABLE IF NOT EXISTS awcms_micro_assets (
  ...
  deleted_by uuid,
  delete_reason text,
  CONSTRAINT awcms_micro_assets_status_check
    CHECK (status IN ('active', 'retired'))
);

CREATE UNIQUE INDEX IF NOT EXISTS awcms_micro_assets_code_dedup
  ON awcms_micro_assets (tenant_id, asset_code)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS awcms_micro_assets_tenant_idx
  ON awcms_micro_assets (tenant_id);
...
ALTER TABLE awcms_micro_assets ENABLE ROW LEVEL SECURITY;
ALTER TABLE awcms_micro_assets FORCE ROW LEVEL SECURITY;

CREATE POLICY awcms_micro_assets_tenant_isolation
  ON awcms_micro_assets
...
GRANT SELECT, INSERT, UPDATE, DELETE ON awcms_micro_assets TO awcms_micro_app;
```

Skill terkait: `awcms-micro-new-migration` (konvensi penomoran, checksum
runner) — jangan menulis migration di luar pola `NNN_awcms_micro_<area>_<desc>.sql`.
...
```sql
INSERT INTO awcms_micro_permissions (key, module_key, description) VALUES
...
Assign permission ke role lewat `awcms_micro_role_permissions` seperti pola
...
export async function registerAsset(
  ...
  const rows = await tx`
    INSERT INTO awcms_micro_assets (tenant_id, asset_code, name)
...
(`src/modules/_shared/idempotency.ts`) — skill `awcms-micro-idempotency`
...
Sejak Issue #695 (epic #679), `openapi/awcms-micro-public-api.openapi.yaml`
...
`openapi/awcms-micro-public-api.src.yaml` bila genuinely dipakai 2+ modul),
...
Tambahkan channel baru ke `asyncapi/awcms-micro-domain-events.asyncapi.yaml`
...
```yaml
channels:
  awcms-micro.asset-register.asset.registered:
    address: awcms-micro.asset-register.asset.registered
    messages:
      DomainEvent:
...
operations:
  publishAssetRegistered:
    action: send
    channel:
      $ref: "#/channels/awcms-micro.asset-register.asset.registered"
    messages:
      - $ref: "#/channels/awcms-micro.asset-register.asset.registered/messages/DomainEvent"
```
...
sync outbox yang sudah ada (`awcms_micro_sync_outbox`, Issue 6.1) bila
...
- [ ] Aksesibilitas WCAG 2.1 AA (label, fokus, kontras) — skill `awcms-micro-ux-review` untuk audit setelah layar jadi.
- [ ] Semua string lewat katalog `.po` (skill `awcms-micro-i18n`), bukan hardcode Bahasa Indonesia/Inggris langsung di komponen.
...
- [ ] Audit — `asset.registered` (dan aksi high-risk domain lain, mis. retire) menghasilkan baris `awcms_micro_audit_events` via `recordAuditEvent`.
...
- `.claude/skills/README.md` — katalog skill (`awcms-micro-new-module`,
  `awcms-micro-new-migration`, `awcms-micro-new-endpoint`,
  `awcms-micro-new-event`, `awcms-micro-abac-guard`, `awcms-micro-idempotency`,
  `awcms-micro-audit-log`, `awcms-micro-testing`, `awcms-micro-security-review`).

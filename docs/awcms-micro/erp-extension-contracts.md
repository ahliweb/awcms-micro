...
scope/dokumen/event/reporting milik AWCMS-Micro.
...
ERP/akuntansi/inventori di atas AWCMS-Micro, di repository Anda sendiri
(lihat `docs/awcms-micro/derived-application-guide.md` untuk pola umum
...
sesuai skill `awcms-micro-sensitive-data`.
...
konsolidasi; klaim apa pun bahwa AWCMS-Micro sendiri adalah ERP yang
...
(ADR-0004), audit log high-risk action (skill `awcms-micro-audit-log`),
masking data sensitif (skill `awcms-micro-sensitive-data`), dan payload
...
`docs/awcms-micro/20_threat_model_security_architecture.md`, tidak
diulang di sini.

...
worker runner and to `.claude/skills/awcms-micro-observability/SKILL.md`
...
| `edge_cache_boost_active`                      | gauge     | (none)                                 | exactly 1                | Unlabeled single value (Issue #353) — 1 while this process asks the edge cache for the boosted TTL.              |
| `edge_cache_pressure_percent`                  | gauge     | (none)                                 | exactly 1                | Unlabeled single value (Issue #353) — the utilization reading that decision was made from.                       |
| `edge_cache_escalation_transitions_total`      | counter   | `from`, `to`                           | exactly 4                | Fixed 2-value mode enum (`normal`/`boost`) on both labels (Issue #353).                                          |
| `edge_cache_purge_total`                       | counter   | `outcome`                              | exactly 2                | `outcome` is `purged`/`failed` only; the purged hostname is NEVER a label — it identifies a tenant (Issue #353). |
...
| `db_pool_work_class_*` saturated for `critical_transaction`/`interactive` | Check `provider_circuit_state{provider="database"}` — is the DB itself degraded, or is it a slow-query regression?              | Scale `DATABASE_POOL_MAX` (doc 18), or find/fix the slow query (see `awcms-micro-performance`).                           |
...
`047_awcms_micro_observability_metrics_permission.sql`). Response shape:

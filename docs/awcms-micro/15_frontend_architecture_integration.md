...
Terkait: `14_ui_ux_design_system.md` (desain), `16_backend_data_access_integration.md` (sisi backend/DB), `05_openapi_asyncapi_detail.md` (kontrak API/event). Skill penegak: **`awcms-micro-ui-screen`** (`.claude/skills/`).
...
2. **Tenant/correlation header**: TIDAK ADA injeksi otomatis
   `X-AWCMS-Micro-Tenant-ID`/`X-Correlation-ID` dari client. Tenant selalu
...
- Tenant aktif dipilih setelah login (bila user multi-tenant) → dikirim sebagai `X-AWCMS-Micro-Tenant-ID` dan disimpan di sesi.
...
(header `X-AWCMS-Micro-Tenant-ID`) di atas — keduanya mengasumsikan
...
(`/<prefix>/{tenantCode}/...`, look up ke `awcms_micro_tenants` yang

...
`awcms-micro-performance` (audit/tuning discipline — "measure before you
...
ANALYZE` during a tuning pass (skill `awcms-micro-performance`) plus a
...
(`awcms_micro_audit_events`), ABAC decisions
(`awcms_micro_abac_decision_logs`), analytics
(`awcms_micro_visitor_sessions`), outbox/delivery
(`awcms_micro_sync_outbox`), sync queue
(`awcms_micro_object_sync_queue`), idempotency
(`awcms_micro_idempotency_keys`), and a representative tenant-scoped
business/content table (`awcms_micro_blog_posts`, also the full-text-search
...
```json
{
  "environment": {
    ...
    "appEnv": "test",
    "databaseUrlRedacted": "postgres://<redacted>@localhost:5432/awcms-micro",
    "scaleProfileId": "safe",
    "tenantCount": 5,
...
- **Safe (CI, every PR — `.github/workflows/ci.yml`'s `quality` job):**
  ...
  (6 registered budgets). Both run as the least-privilege `awcms_micro_app`
...
`DATABASE_URL` should point at the least-privilege `awcms_micro_app` role
...
- Skill `awcms-micro-performance` — the audit/tuning discipline this suite

...
Dokumen ini melengkapi kebutuhan **desain UI/UX** AWCMS-Micro yang sebelumnya hanya tersirat di SOP (doc 08) dan blueprint (doc 11). Berisi design principle, design token, component library, information architecture, spesifikasi layar (wireframe), state pattern, aksesibilitas, i18n, dan theming — agar frontend dapat diimplementasikan konsisten.

Terkait: `15_frontend_architecture_integration.md` (arsitektur & wiring), `08_sop_operasional_user_guide.md` (alur operasional). Skill penegak: **`awcms-micro-ui-screen`** (`.claude/skills/`).
...
| `/forgot-password`           | Semua            | Minta tautan reset (anti-enumeration, notice generik)                                                            | auth.css surface, FormField              | `POST /auth/password/forgot`                                                                               |
| `/reset-password`            | Semua            | Setel kata sandi baru (token+tenant dari link/`?p=`)                                                             | auth.css surface, FormField              | `POST /auth/password/reset`                                                                                |
| `/register`                  | Publik (opt-in)  | Self-registration (permohonan pending, approval admin)                                                           | auth.css surface, FormField              | `POST /auth/register`                                                                                      |
| `/admin/registrations`       | Admin/Owner      | Antrean approval permohonan registrasi (approve/reject)                                                          | DataTable, ConfirmDialog, ActionBanner   | `/registration-requests`, `/registration-requests/{id}/{approve,reject}`                                   |
...
| `/admin/sidebar-menu`        | Admin/Owner      | Atur sidebar per-tenant: grup type→sub-type(modul)→item, urutan, tampil/sembunyi, rename, pindah kategori, reset | ConfirmDialog, ActionBanner, StateNotice | `GET/PUT /navigation/sidebar-config`, `POST /navigation/sidebar-config/reset`                              |
...
### Sidebar grouping (type/sub-type)

Sidebar admin (`AdminLayout.astro`) dikelompokkan tiga tingkat: **type** (kategori
teratas — mis. `system`, `content`, `operations`) → **sub-type** (modul pemilik —
mis. `blog_content`) → **item** (link nav modul). Item core (Dashboard, Access &
Users, Sync, Settings, Profile) memakai modul sintetis `core` dan type default
`system`; item core dirender langsung di bawah type-nya tanpa sub-label modul.
Model default diturunkan dari kode (`buildDefaultSidebarModel` = item core +
`navigation` tiap modul dari `listModules()`); tenant hanya menyimpan _override_
(urutan, tampil/sembunyi, rename, pindah type, type kustom) di
...
> **Status:** diimplementasikan (Issue #433, milestone M9). `src/lib/i18n/` (parser `.po` murni tanpa dependency, catalog loader, `t()`, resolusi locale, formatter), katalog `i18n/{messages.pot,en.po,id.po}`, `LanguageSwitcher.astro`, dan migrasi `016_awcms_micro_tenant_default_locale_english_schema.sql` (default `'en'` untuk tenant baru). Diverifikasi live: switch locale mengubah seluruh teks SSR (shell **dan** konten halaman), termasuk fallback ke `default_locale` tenant lama yang masih `'id'`.
...
**2. Data input pengguna** (konten yang diketik user dan perlu tampil multi-bahasa, mis. nama/deskripsi/catatan yang di-i18n-kan aplikasi turunan) → disimpan **di database untuk setiap locale aktif** (satu nilai per bahasa aktif), **bukan** di `.po`. Pola penyimpanan per-bahasa didokumentasikan di `docs/awcms-micro/04_erd_data_dictionary.md` §Konten multi-bahasa. `.po` hanya untuk teks statis pengembang, DB untuk konten dinamis pengguna.

- **Locale minimal**: **en** dan **id** (arsitektur siap ms/ar — kolom `default_locale` tetap `text` bebas, bukan `enum`/`CHECK`, agar ms/ar bisa ditambah tanpa migration schema; UI hanya menampilkan locale yang benar-benar punya katalog). **Default = `en`** (`awcms_micro_tenants.default_locale`, migration `016` mengubah default kolom dari `'id'` ke `'en'` untuk tenant baru — tenant lama yang sudah `'id'` tidak diubah).
- **Resolusi locale**: cookie `awcms_micro_locale` (diset language switcher) → `default_locale` tenant → fallback `en`. Diresolusi di `src/middleware.ts` **sebelum** halaman `/admin/*` mana pun (termasuk `AdminLayout`) dirender — bukan di dalam layout, karena frontmatter halaman berjalan lebih dulu daripada frontmatter layout yang membungkusnya; me-resolve di layout saja terbukti terlambat untuk konten halaman itu sendiri saat verifikasi live (shell ter-render Indonesia, konten dashboard tetap Inggris).
...
flowchart LR
  ...
  Cookie[Cookie awcms_micro_locale] --> Mid[middleware.ts]

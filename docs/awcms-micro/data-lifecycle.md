...
AWCMS-Micro sudah punya beberapa job retensi/purge spesifik-resource
...
| ------------------------------------ | --------------------------------- | ------------------- | ----------- | --------------------- |
| `logging.audit_events`               | `awcms_micro_audit_events`        | `logging`           | `delegated` | `audit_security`      |
| `visitor_analytics.visit_events`     | `awcms_micro_visit_events`        | `visitor_analytics` | `delegated` | `analytics_telemetry` |
| `form_drafts.form_drafts`            | `awcms_micro_form_drafts`         | `form_drafts`       | `delegated` | `operational_queue`   |
| `data_lifecycle.data_lifecycle_runs` | `awcms_micro_data_lifecycle_runs` | `data_lifecycle`    | `generic`   | `operational_queue`   |
...
`awcms_micro_data_lifecycle_legal_holds` (RLS FORCE, tenant-scoped).
...
manifest tercatat di `awcms_micro_data_lifecycle_archive_manifests`
...
1. Cari manifest lewat `GET /api/v1/data-lifecycle/runs` (korelasi
   ...
   `awcms_micro_data_lifecycle_archive_manifests` (akses admin/operator).
...
4. Restore-KE-tabel-sumber adalah **prosedur manual operator terpisah**
   ...
   baris yang sudah dibaca ke `awcms_micro_data_lifecycle_runs`, memvalidasi
...
4. `awcms_micro_worker`/`awcms_micro_app` grant harus diverifikasi ulang
...
`docs/awcms-micro/resilience-dr-verification.md` §Backup lokal untuk

...
Backend AWCMS-Micro menggunakan **Bun-only**:
...
Tabel terkait: `awcms_micro_sync_outbox`, `awcms_micro_message_outbox`, `awcms_micro_object_sync_queue`, `awcms_micro_email_messages` (Issue #494/#495, epic #492). Status: `pending → sent/failed`, dengan `next_retry_at`.
...
3. **FINALIZE** — satu transaksi pendek per baris memindahkan `sending`
   ...
   dicatat di tabel riwayat percobaan (`awcms_micro_email_delivery_attempts`
   / analognya).
...
Ikuti standar penamaan `NNN_awcms_micro_<area>_<desc>.sql` (doc 09/10) — lihat skill `awcms-micro-new-migration`.

```mermaid
flowchart TD
  A[Baca file sql/ terurut] --> B{Sudah di awcms_micro_schema_migrations?}
  B -- Ya --> C[Skip]
...
- Tabel `awcms_micro_idempotency_keys` menyimpan `key`, request hash, status, response/resource.
- Alur di skill `awcms-micro-idempotency` (doc 10). Retention 7–30 hari (doc 04).
...
- **Registry global, RLS-free** — `awcms_micro_modules`/`_dependencies`/`_navigation`/`_jobs`/`_health_checks`. Metadata code-derived, sama untuk semua tenant (sinkron dari `listModules()` lewat `syncModuleDescriptors`, sama alasan `awcms_micro_permissions` RLS-free) — jalan di koneksi app biasa, **tidak** butuh `withTenant`/`SET LOCAL app.current_tenant_id`.
- **State tenant-writable, RLS FORCE** — `awcms_micro_tenant_modules` (enable/disable per tenant) dan `awcms_micro_module_settings` (pengaturan non-secret per tenant). Setiap akses **wajib** lewat `withTenant`, sama seperti tabel tenant-scoped lainnya.
- **"Sync first" sebelum tulis tenant-scoped**: `enableTenantModule`/`disableTenantModule`/`updateModuleSettings`/`runModuleHealthCheck` semua memanggil `syncModuleDescriptors(tx)` di awal — dua tabel di atas punya FK ke `awcms_micro_modules.module_key`, jadi baris registry harus ada dulu sebelum insert baris tenant-scoped. Pola ini generik: kapan pun tabel tenant-scoped punya FK ke tabel registry code-derived, pastikan registry di-sync dalam transaction yang sama sebelum menulis, jangan mengasumsikan operator sudah menjalankan sync manual lebih dulu.
...
Nama tabel/kolom `snake_case`, prefiks `awcms_micro_` (doc 04/10).

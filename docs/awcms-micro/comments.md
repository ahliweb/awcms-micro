...
- **Timing-token secret** — `COMMENTS_TIMING_SECRET` has a dev fallback; durable/production storage profiles SHOULD set it so the soft submit-timing floor cannot be forged. (Mirrors the offline/LAN provider-optional posture; not an authorization control.) **Since Issue #293 this is measured, not just documented:** `bun run security:readiness`'s `Comments submit-timing secret is configured in production` check reports a **warning** when `APP_ENV=production` and the variable is unset, so a live deployment signing timing tokens with the repository's committed dev literal shows up in every preflight report. The variable is registered in `src/lib/config/registry.ts` / `.env.example` / [doc 18 §Comments](18_configuration_env_reference.md).

...
> ⚠️ **DEPRECATED (opsional-lawas) — [ADR-0034](../adr/0034-template-repositioning-online-store-scope-and-derived-app-deprecation.md).** Jalur "membangun aplikasi turunan TERPISAH di atas base" (lapisan Derived Application, `application-registry.ts`, manifest kompatibilitas + `extension:check`) **bukan lagi model pemakaian utama**. AWCMS-Micro sekarang dipakai **LANGSUNG sebagai template full-online website** (hingga toko online; **bukan POS in-store**) — tambahkan modul website di dalam repo ini (lihat [`README.md`](README.md) §Menambah modul website). Dokumen ini dipertahankan untuk kompatibilitas mundur; kode & gate turunannya masih utuh dan hijau, tetapi jangan jadikan default. Contoh POS/retail di bawah adalah **legacy**.
>

...
> ⛔ **DIHAPUS — [ADR-0036](../adr/0036-remove-derived-application-pathway-align-family.md)** (men-supersede [ADR-0034](../adr/0034-template-repositioning-online-store-scope-and-derived-app-deprecation.md) + [ADR-0035](../adr/0035-retain-module-composition-mechanism-reject-derived-pathway-code-removal.md)). Jalur "membangun aplikasi turunan TERPISAH di atas base" **sudah dihapus dari kode**: seam `application-registry.ts` + `theming/application-theme-registry.ts`, gerbang `extension:check`, mesin manifest (`extension-compatibility.ts`/`extension-manifest-contract.ts`), tipe `ApplicationModuleRegistry`/`ModuleMigrationNamespace`, dan namespace migration 900–999 **tidak lagi ada**. AWCMS-Micro dipakai **LANGSUNG sebagai template full-online website** (hingga toko online; **bukan POS in-store**) — tambahkan modul website di dalam repo ini (lihat [`README.md`](README.md) §Menambah modul website). **Seluruh dokumen ini adalah rujukan HISTORIS**; setiap `application-registry.ts`, `extension.manifest.json`, `extension:check`, dan `tests/fixtures/derived-application-example/`/`extension-contract-incompatible/` yang disebut di bawah **sudah tidak ada** di repo ini.
>
...
| Transactional outbox + object queue dispatcher + sync HMAC (Issue 6.1-6.3/#436) — jalur unggah objek/media & delivery provider di luar transaksi DB, **bukan** mode operasi offline base (ADR-0027) | Payload event domain yang disinkronkan lewat outbox yang sama; aplikasi turunan LAN-first menambah modul offline-nya sendiri (label `offline-lan`, ADR-0025 §2) |
...
9. **Deployment & go-live** — `bun run production:preflight` (sejak Issue #684, seluruh 11 tahap read-only secara default: config:validate → security:readiness → database:capacity → db:connectivity → api:spec:check → modules:compose:check → extension:check → test → build → db:pool:health → migration:plan; `extension:check` memvalidasi `extension.manifest.json` Anda bila sudah dipublikasikan, Issue #741/ADR-0015. Menerapkan migration adalah langkah terpisah eksplisit — `--apply-migrations --backup-verified --acknowledge-target=...` — lihat `production-preflight-runbook.md` untuk urutan tahap otoritatif dan selalu-terkini). Skill: `awcms-micro-production-preflight`. Pilih & jalankan profil deployment (doc `deployment-profiles.md`, ADR-0027 — `development`/`full_online_single_host`/`full_online_production`): single-host (`docker-compose.yml`) atau registry-based (`Dockerfile.production`, Issue #454; panduan Coolify di [`deploy-coolify.md`](deploy-coolify.md), Issue #462) — skill `awcms-micro-deploy`. Media terkelola pada profil produksi wajib di object storage durable, bukan FS container ephemeral.
...
- [`deployment-profiles.md`](deployment-profiles.md) — profil deployment full-online (development/full_online_single_host/full_online_production, ADR-0027; single-host compose vs registry image).

...
- `.claude/skills/awcms-micro-visitor-analytics/SKILL.md` — konteks
...
`awcms_micro_visitor_daily_rollups`), semua `ENABLE`+`FORCE ROW LEVEL
SECURITY`.
...
2. **Retensi lebih pendek untuk data lebih sensitif.** Raw detail (30
   ...
   default). Cookie anonim `awcms_micro_visitor_key` juga jauh lebih
...
- **Deployment existing yang mengandalkan default implisit lama** (tidak
  ...
  (`awcms_micro_visitor_sessions`/`awcms_micro_visit_events`/
  `awcms_micro_visitor_daily_rollups`) tidak dihapus/dimodifikasi oleh
...
Cookie `awcms_micro_visitor_key` (anonim, `httpOnly`+`sameSite=lax`,
...
- `VISITOR_ANALYTICS_ENABLED=true` — **wajib di-set eksplisit sejak
  ...
  ke `awcms_micro_visit_events` lewat middleware, tidak pernah keluar
...
- `VISITOR_ANALYTICS_RAW_IP_ENABLED=false`,
  ...
  pernah tersimpan. Kolom `ip_address` di `awcms_micro_visitor_sessions`
...
- `VISITOR_ANALYTICS_RAW_IP_ENABLED=true` — mengisi
  `awcms_micro_visitor_sessions.ip_address` (kolom `inet`). Wajib disertai
...
| ------------------------------------------------------------------------------- | -------------------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `awcms_micro_visit_events` (seluruh baris)                                      | 90 hari                                            | `VISITOR_ANALYTICS_EVENT_RETENTION_DAYS`        | Hard delete (`bun run analytics:purge`)                                                        |
| `awcms_micro_visitor_sessions.ip_address`/`login_identifier_snapshot`           | 30 hari (dari `last_seen_at`)                      | `VISITOR_ANALYTICS_RAW_DETAIL_RETENTION_DAYS`   | Cleared in place (row tetap ada)                                                               |
| `awcms_micro_visitor_sessions` (seluruh baris)                                  | 90 hari (dari `last_seen_at`, sama dengan event)   | `VISITOR_ANALYTICS_EVENT_RETENTION_DAYS`        | Hard delete, hanya bila tanpa event tersisa (`NOT EXISTS`)                                     |
| `awcms_micro_visitor_daily_rollups` (seluruh baris)                             | 730 hari                                           | `VISITOR_ANALYTICS_ROLLUP_RETENTION_DAYS`       | Hard delete (`bun run analytics:purge`)                                                        |
| Cookie anonim `awcms_micro_visitor_key` (di browser pengunjung, bukan tabel DB) | 30 hari (Issue #624 audit addendum, dulu ~2 tahun) | `VISITOR_ANALYTICS_VISITOR_KEY_COOKIE_TTL_DAYS` | Expiry browser (natural rotation) + revocation aktif saat modul dinonaktifkan (§Cookie anonim) |
...
`awcms_micro_visit_events` mentah menjadi
`awcms_micro_visitor_daily_rollups`, satu baris per `(tenant, date,
...
- **Area tanpa event pada tanggal itu tidak mendapat baris** — bukan
  baris bernilai nol; sama seperti tabel `awcms_micro_visit_events`
  sumbernya sendiri.
...
1. `awcms_micro_visit_events` lebih tua dari `eventRetentionDays` — hard
   delete.
2. `ip_address`/`login_identifier_snapshot` di
   `awcms_micro_visitor_sessions` lebih tua dari `rawDetailRetentionDays`
   — dikosongkan di tempat, baris tetap ada (field
   browser/device/OS agregat tetap berguna lama setelah raw detail
   seharusnya hilang).
3. `awcms_micro_visitor_sessions` lebih tua dari `eventRetentionDays` —
   hard delete, hanya bila tidak ada `awcms_micro_visit_events` yang
   masih mereferensikannya (`NOT EXISTS`, mencegah pelanggaran FK dari
   write-throttle collector).
4. `awcms_micro_visitor_daily_rollups` lebih tua dari

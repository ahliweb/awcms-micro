...
5. Sukses → `circuitBreaker.recordSuccess()`; transaction/`fn` melempar
   ...
   - `IdempotencyRaceLostError` — race benign pada
     `saveIdempotencyRecord` (lihat skill `awcms-micro-idempotency`).
...
Didokumentasikan di `asyncapi/awcms-micro-domain-events.asyncapi.yaml`
...
[databases]
awcms-micro = host=127.0.0.1 port=5432 dbname=awcms-micro

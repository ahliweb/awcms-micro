...
- **Terkait:** Issue #696 (epic #679, platform-hardening), Issue #510 (epic Module Management), `docs/awcms-micro/21_module_admission_governance.md` (detail lengkap), ADR-0001, ADR-0002, ADR-0008, ADR-0011

> **Catatan currency (diperbarui ADR-0025).** Angka "14 modul" di
> Konteks/Keputusan di bawah adalah snapshot saat ADR ini di-Accept (Issue
...
> **ADR-0025** mempersempit repo ke WEBSITE scope: registry kini **22 modul**
...
> `docs/awcms-micro/repo-inventory.md` (GENERATED) — rujuk keduanya untuk
> angka terkini, bukan angka di bawah.
...
1. **Lima kategori modul**: Core, System, Official Optional Module,
   ...
   `docs/awcms-micro/21_module_admission_governance.md` — dokumen ini
...
2. **Registry tetap trusted dan statis** — `src/modules/index.ts` adalah
   ...
   lewat PR + CI normal. `awcms_micro_tenant_modules` hanya menyimpan status
...
4. 14 modul terdaftar saat ini dipetakan ke kategori di atas (3 Core, 9
   ...
   selama pemetaan ini.
5. Proposal ringan (`docs/awcms-micro/templates/

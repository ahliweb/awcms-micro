...
- **Lifecycle dependency** (`ModuleDescriptor.dependencies`, wajib aktif duluan): `["tenant_admin", "identity_access", "domain_event_runtime"]`. `tenant_admin` untuk batas tenant (`awcms_micro_tenants`), `identity_access` untuk RBAC/ABAC + audit actor, `domain_event_runtime` karena modul ini adalah REAL producer (menerjemahkan pesan inbound terverifikasi menjadi event via `appendDomainEvent`) DAN real consumer (fan-out langganan outbound dipicu oleh dispatcher #742) — pola yang sama seperti `workflow_approval` (#747) dan `organization_structure` (#749), bukan pola Core `profile_identity` (#748) yang sengaja tidak mengimpor konstanta lintas-modul.

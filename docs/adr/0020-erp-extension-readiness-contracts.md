...
> **Tidak berlaku di repositori ini.** ADR ini diwarisi dari basis standar upstream `ahliweb/awcms-mini`, tempat keputusan ini memang Accepted. AWCMS-Micro adalah turunan scope **website** dan tidak memport kontrak kesiapan ekstensi ERP (ADR-0025 §3), jadi tidak ada kode, migrasi, atau route di repo ini yang mengimplementasikannya. Dokumen dipertahankan — bukan dihapus — supaya rujukan silang dari ADR lain tetap utuh dan alasan keputusan aslinya tetap bisa dibaca saat memport perubahan dari upstream. **Jangan baca ADR ini sebagai deskripsi repositori ini.**
...
### 1. AWCMS-Micro adalah kernel teknis, bukan ERP fungsional
...
### 3. Keluarga kontrak (lihat `docs/awcms-micro/erp-extension-contracts.md` untuk tabel lengkap ownership/versi/failure-semantics/privasi/contoh)

...
2. **`application-registry.ts` diimpor oleh kode base non-turunan.** Selain `index.ts`, seam ini/tipenya dirujuk oleh **theming** (`application-theme-registry.ts` memakai pola descriptor-seam yang identik), **module-sync API** (`/api/v1/modules/sync.ts`), `descriptor-sync.ts`, `_shared/module-contract.ts`, dan **business-scope hierarchy-port**.

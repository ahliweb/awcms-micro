...
**Negatif / utang yang diakui.** Refaktor ini **belum** merapikan seluruh paket dokumen `docs/awcms-micro/`: dokumen 04 (ERD/data dictionary), 08 (SOP operasional), 20 (threat model), dan 21 (module admission governance) masih memuat bagian yang menjelaskan modul ERP yang tidak diport. Ini persis jenis drift yang §5 lawan, dan ia **diakui di sini secara eksplisit alih-alih didiamkan** — dokumen-dokumen itu harus disunting mengikuti registry sebelum rilis pertama. Sampai saat itu, **`src/`, `sql/`, dan gate CI adalah sumber kebenaran; `docs/awcms-micro/` yang menyimpang darinya adalah spesifikasi target upstream, bukan deskripsi repo ini.**

...
- **Status:** Accepted
- **Tanggal:** 2026-07-05
- **Terkait:** `docs/awcms-micro/04_erd_data_dictionary.md`, `docs/awcms-micro/16_backend_data_access_integration.md`
...
`sql/033_awcms_micro_tenant_domain_lookup_function.sql`'s
`awcms_micro_resolve_tenant_domain_lookup` (Issue #559, epic #555) — baca
...
1. **Konfirmasi role pemilik benar-benar superuser (atau owner tabel yang
   ...
   Migration di repo ini berjalan sebagai `POSTGRES_USER` (`awcms-micro`),
...
4. **`REVOKE ALL ... FROM PUBLIC` lalu `GRANT EXECUTE` eksplisit** ke role
   spesifik (mis. `awcms_micro_app`) — PostgreSQL men-grant `EXECUTE` ke
...
8. **Hindari timing side-channel** — kalau fungsi ini dipanggil lalu diikuti
   ...
   tabel kedua sudah RLS-free/publicly readable, seperti `awcms_micro_tenants`).

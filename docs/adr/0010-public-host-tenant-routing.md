...
bertentangan dengan topologi LAN-first/offline default AWCMS-Micro (doc 18).
...
hostname→tenant `awcms_micro_tenant_domains` (Issue #557), module descriptor
...
- Saat `host_default`, `resolvePublicTenantFromRequest()` (Issue #559)
  ...
  `awcms_micro_tenant_domains` (Issue #557), melalui fungsi lookup
  ...
  `_CODE`, lalu `awcms_micro_setup_state`) sebelum akhirnya `null` (404
  generik) — lihat `.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md`
...
- **Positif:** Deployment online/public/SaaS dengan domain nyata mendapat
  ...
  LAN-first/offline default AWCMS-Micro sama sekali — deployment yang tidak
...
- **Netral:** Ada satu follow-up keamanan yang sudah diidentifikasi dan
  ...
  diaktifkan di production** (lihat `.claude/skills/awcms-micro-tenant-domain-routing/SKILL.md`
...
`awcms_micro_app` yang bukan superuser/table-owner. Fungsi `SECURITY DEFINER`
yang dipakai resolver host-based (`awcms_micro_resolve_tenant_domain_lookup`,
...
- **Menjadikan host/domain-based routing sebagai default/satu-satunya
  ...
  header, bertentangan langsung dengan prinsip LAN-first default AWCMS-Micro
...
- **Subdomain per tenant otomatis (mis. `{tenantCode}.platform.example.com`)
  ...
  yang sudah ditolak ADR-0009. Tabel `awcms_micro_tenant_domains` (Issue

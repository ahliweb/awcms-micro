...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Open redirect**                  | Target redirect divalidasi same-tenant + same-registered-host (§8); `classifyRedirectTarget` menolak host eksternal/lintas-tenant, form protocol-relative/backslash, DAN target yang memuat control-char C0/DEL (TAB/LF/CR di-strip browser sehingga `"/\t/evil.com"` → `//evil.com`); cabang relative dinormalisasi ulang terhadap synthetic base sehingga prefiks `/` tak pernah dipercaya sendiri; rantai dicegah. |
...
| **JSON-LD injection**              | JSON-LD dari union tipe schema terkontrol; `assertControlledJsonLd` memvalidasi `@type` DAN setiap KEY objek (bukan hanya nilai string); `renderControlledJsonLd` adalah guard tunggal yang memvalidasi lalu meng-escape `<`/`>`/`&`/U+2028/U+2029 pada SELURUH string ter-serialisasi (key + value) sehingga escaping tak bisa terlupa; tenant tidak bisa menyuntik `<script>` lewat nilai maupun key.               |
...
Yang **mendarat sekarang** (kontrak, bukan runtime): **file port TYPE** `src/modules/_shared/ports/seo-facts-port.ts` (netral, belum di-wire) + helper murni invariant kontrak (`buildSeoCacheKey`, prediket visibilitas, klasifikasi target redirect, guard JSON-LD terkontrol) + **fixture kontrak** yang membuktikan base article/page + tipe konten turunan lewat kontrak yang sama. Preseden yang tepat adalah **`_shared/ports/legal-hold-guard-port.ts`**: satu-satunya port yang ada sebagai TYPE dengan provider **dan** consumer sama-sama belum di-wire dan **tanpa** entri `CAPABILITY_CONTRACT_VERSIONS` — persis posisi `seo-facts-port.ts` hari ini (entri versi baru dipasangkan dengan `provides` sebuah modul, jadi menyusul di #266). `party-directory-port.ts` **bukan** preseden yang sama dan tidak boleh disamakan: providernya sudah di-wire (`profile_identity` sudah `provides: ["party_directory"]` dengan adapter nyata) dan versinya sudah ada (`party_directory: "1.0.0"` di `capability-contract-versions.ts`) — hanya consumer-nya yang pending. Itu kasus "provider wired, versi ada, consumer menyusul", berbeda dari "type-only, dua sisi belum di-wire".

...
- `reference_data` **tidak** mengimpor/menyalin baris `awcms_micro_idn_admin_regions`/`awcms_micro_idn_region_datasets` ke tabel generiknya sendiri.
...
- **Membuat tabel baseline value set/code tenant-scoped (dengan `tenant_id` nullable untuk baris global)** — ditolak: melanggar konvensi RLS repo ini (`tenant_id` yang bisa NULL pada tabel RLS-FORCE menciptakan kelas baris ambigu yang sulit diaudit); mengikuti preseden `awcms_micro_idn_admin_regions`/`awcms_micro_permissions` yang memisahkan tabel GLOBAL (tanpa `tenant_id`, RLS-exempt terdokumentasi) dari tabel tenant-scoped (`tenant_id` wajib, RLS FORCE) secara tegas jauh lebih konsisten dengan doc 04 §RLS standard.
- **Mengizinkan tenant override menimpa baris baseline in-place (UPDATE langsung ke `awcms_micro_reference_codes` dengan filter tenant)** — ditolak eksplisit: melanggar acceptance criterion issue #750 "Tenant override cannot mutate global/module baseline rows or affect another tenant" secara langsung; sebagai gantinya tenant override HANYA pernah menulis ke tabel tenant-scoped terpisah (`awcms_micro_reference_tenant_codes`), baseline tidak pernah tersentuh oleh mutasi tenant apa pun.
...
- **Netral:** `docs/awcms-micro/21_module_admission_governance.md` §8 diperbarui menambah baris ke-18 (lihat PR ini).

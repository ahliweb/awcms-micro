# Perbedaan awcms-micro vs awcms

> **Tujuan dokumen.** Menjelaskan secara rinci hubungan dan perbedaan antara repo
> ini (**awcms-micro**) dan repo turunannya (**awcms**), agar kontributor/agent
> paham posisi masing-masing dan alur kerja lintas-repo. Angka pada dokumen ini
> berlaku per **2026-07-17** (awcms-micro v0.24.0 vs awcms v5.1.1).

## 1. Hubungan kedua repo

**awcms-micro** adalah _Modular Monolith Standard_ — fondasi/standar yang matang
...
mengadopsi stack & standar teknis awcms-micro sebagai basis, lalu memport modul
...
```
awcms-micro            awcms                       repo turunan/ekstensi
...
diuji lebih dulu di awcms-micro**, baru kemudian di-port ke awcms (dengan rename
prefix `awcms_micro_` → `awcms_` dan penyesuaian skop). Repo awcms bukan tempat
...
**Modul awcms-micro (WEBSITE scope, 13 modul) yang belum di-port ke awcms:**
...
| Theming & search    | `theming`, `site-search`                                                                     |
| Engagement          | `comments`, `newsletter`                                                                     |
...
**Masih hanya di awcms-micro** (belum diport):
...
- Skill/subagent Claude: keduanya ~46 skill; nama agent berbeda prefix
  (`awcms-micro-*` vs `awcms-*`). awcms menambah skill `awcms-port-from-mini`
  (playbook port modul dari mini).

## 6. Rujukan

- [`awcms-micro/README.md`](awcms-micro/README.md) — paket dokumen teknis standar repo ini.
- [`ARCHITECTURE.md`](ARCHITECTURE.md) — arsitektur kode awcms-micro.
